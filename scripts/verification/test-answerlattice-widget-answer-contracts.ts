...
assert(feedbackRoute.includes('const transactionResult = await db.runTransaction'), 'widget feedback must derive replay state from the committed transaction attempt');
...
assert(escalationServer.includes('const transactionResult = await db.runTransaction'), 'widget escalation must derive its response from the committed transaction attempt');
