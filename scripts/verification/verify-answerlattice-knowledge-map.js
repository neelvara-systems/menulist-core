...
const canonicalAnswerEditor = read('src/components/templates/answerlattice/governance/CanonicalAnswerEditor.tsx');
...
includes(dashboard, "return result.graph[requestedEntityId] ? requestedEntityId : ''", 'Knowledge Map exact inbound entity focus');
includes(dashboard, 'if (requestedEntityMissing && !selectedId) return;', 'Knowledge Map missing inbound entity fallback prevention');
includes(dashboard, 'No substitute topic is shown.', 'Knowledge Map missing inbound entity warning');
