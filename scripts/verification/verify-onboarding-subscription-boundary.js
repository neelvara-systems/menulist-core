...
const ownerSidebar = read('src/components/organisms/sidebar/index.tsx');
...
requireText(ownerSidebar, 'const hasRecoveryOnlyAccess = hasRecoveryOnlyWorkspaceAccess(storeDetails, hasPaidAccess);', 'unpaid owner recovery-only navigation state');
requireText(ownerSidebar, 'hasRecoveryOnlyAccess && !isStarterRecoveryRoute(nav.route)', 'unpaid owner recovery-only navigation filter');
requireText(ownerLayout, 'const hasRecoveryOnlyAccess = hasRecoveryOnlyWorkspaceAccess(storeDetails, hasPaidAccess);', 'unpaid owner direct-route recovery state');
requireText(ownerLayout, 'const isResolvingOwnerWorkspaceAccess = !shouldRenderMobileShell', 'owner workspace subscription-resolution render gate');
requireText(ownerLayout, 'const isRedirectingOwnerWorkspace = !shouldRenderMobileShell && isOwnerWorkspaceRoute', 'owner workspace redirect render gate');
requireText(ownerLayout, 'return <BrandedPageLoader page="Checking Plan Access" brand="menulist" />;', 'owner workspace controls remain unmounted while access resolves or redirects');
requireText(ownerLayout, 'hasRecoveryOnlyAccess && !isStarterRecoveryRoute(pathname)', 'unpaid owner direct-route recovery redirect');
requireText(ownerLayout, "router.replace('/billing');", 'unpaid owner canonical billing recovery redirect');
