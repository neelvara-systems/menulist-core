...
function verifyEnvironmentTargets() {
  ...
  for (const [label, content] of [
    ...
    assertNotIncludes(content, '\nMENULIST_GEMINI_AI_KEY_4=', `${label} must not restore retired MenuList shared Gemini slot 4`);
    assertNotIncludes(content, '\nMENULIST_GEMINI_TEXT_AI_KEY=', `${label} must keep the extraction-only Gemini secret out of root env`);
  ...
  assertIncludes(functionsProductionEnv, 'ENABLE_MESSAGING_ONBOARDING=false', 'Production messaging onboarding disabled boundary');
  ...
  assertIncludes(menulistProductionProviderSetup, '- [x] `PROD-D04`', 'MenuList production provider ledger Resend preparation');
  assertIncludes(menulistProductionProviderSetup, '`MENULIST_RESEND_API_KEY` and', 'MenuList production provider ledger product-scoped Resend API key');
  assertIncludes(menulistProductionProviderSetup, '`MENULIST_RESEND_WEBHOOK_SECRET`', 'MenuList production provider ledger product-scoped Resend webhook secret');
  ...
  assertIncludes(deploymentReadme, 'production revalidation secret is rotated consistently', 'Deployment hub current production revalidation state');
  assertIncludes(menulistProductionProviderSetup, '- [x] `PROD-D03`', 'MenuList production provider ledger Sentry completion');
  assertIncludes(menulistProductionProviderSetup, 'Google Workspace apex MX records were not', 'MenuList production Resend DNS isolation boundary');
  assertIncludes(menulistProductionProviderSetup, 'No SMTP credential, provider send,', 'MenuList production email activation boundary');
  ...
  assertIncludes(menulistProductionProviderSetup, 'Razorpay activation is blocked while any production binding starts with', 'MenuList production Razorpay Live release gate');
  ...
  assertIncludes(menulistProductionProviderSetup, 'deliberately parked production WhatsApp', 'MenuList production WhatsApp parking decision');
  assertIncludes(menulistProductionProviderSetup, 'Blocked behind `PROD-D09`', 'MenuList production WhatsApp ownership blocker');
  assertIncludes(menulistProductionProviderSetup, 'Do not use the founder\'s personal profile to\n    create a temporary production boundary', 'MenuList production WhatsApp personal-profile prohibition');
  ...
  assertIncludes(menulistProductionProviderSetup, 'WhatsApp is deliberately parked until Meta has a business-owned verified', 'MenuList production ownership audit blocker');
  ...
  assertIncludes(menulistProductionProviderSetup, 'messaging onboarding, WhatsAppOS, owner WhatsApp delivery, and platform\n    WhatsApp alerts explicitly disabled', 'MenuList production Functions messaging fail-closed boundary');
  assertIncludes(menulistProductionProviderSetup, 'no dummy Meta secret is needed for the parked\n    target', 'MenuList production Functions secret placeholder prohibition');
  assertIncludes(menulistProductionProviderSetup, '- [x] `PROD-E07`', 'MenuList production Functions secret manifest completion');
  assertIncludes(menulistProductionProviderSetup, '`GCP_BUDGET_WEBHOOK_SECRET` version 1', 'MenuList production distinct budget webhook secret evidence');
  ...
  assertIncludes(menulistProductionProviderSetup, 'Follow-up metadata review confirms the temporary Razorpay', 'MenuList production temporary Razorpay managed-environment evidence');
  ...
  assertIncludes(productionCertificationRunbook, 'Historical authenticated evidence remains relevant but is not the current', 'Production certification runbook historical Functions evidence boundary');
  assertIncludes(productionCertificationRunbook, 'operator state: the default package-local scoped set was retried on July 9 with', 'Production certification runbook historical Functions retry date');
  assertIncludes(productionCertificationRunbook, '`npm --prefix functions run deploy:menulist-qa`', 'Production certification runbook scoped MenuList QA Functions command');
  assertIncludes(productionCertificationRunbook, 'The July 11 shared', 'Production certification runbook historical AI-gateway subset date');
  assertIncludes(productionCertificationRunbook, 'AI-gateway 13-target subset above reached the same pre-upload blocker after', 'Production certification runbook historical AI-gateway subset blocker');
  ...
  assertIncludes(launchPrerequisites, 'Current operator evidence refreshed August 13, 2026: Firebase CLI is', 'Launch prerequisites current Firebase operator evidence date');
  assertIncludes(launchPrerequisites, 'authenticated as `admin@neelvara.com`', 'Launch prerequisites current Firebase authentication evidence');
  ...
  assertIncludes(launchPrerequisites, 'August 13 CLI authentication/list access pass', 'Launch prerequisites summary current Functions operator evidence');
  assertIncludes(launchPrerequisites, 'the changed `processMenuImagesJob` revision deployed successfully with the isolated extraction secret', 'Launch prerequisites summary extraction worker deploy evidence');
  assertIncludes(launchPrerequisites, 'Other changed targets still need their own scoped deploy and live evidence.', 'Launch prerequisites summary remaining scoped Functions deploy boundary');
  ...
  assertIncludes(productionCertificationRunbook, 'Current operator boundary refreshed August 13, 2026: Firebase CLI is', 'Production certification runbook current Functions operator evidence');
  assertIncludes(productionCertificationRunbook, 'Current changed-revision evidence: the scoped August 13 deploy of', 'Production certification runbook current changed-revision deploy evidence');
  assertIncludes(productionCertificationRunbook, '`functions:processMenuImagesJob` completed with source hash', 'Production certification runbook extraction worker deploy evidence');
  ...
  assertIncludes(productionReadinessAudit, 'Firebase CLI was reauthenticated as', 'Production readiness audit current Firebase authentication summary');
  assertIncludes(productionReadinessAudit, 'Firebase CLI authentication and QA function-list access now pass.', 'Production readiness audit current remaining-risk Functions boundary');
  assertIncludes(productionReadinessAudit, 'The August 13 scoped deploy activated changed revisions for', 'Production readiness audit current changed Functions deployment evidence');
  assertIncludes(productionReadinessAudit, 'The July 9 Cloud Resource Manager 403 and August 1 missing-auth attempts remain historical evidence, not the current operator state', 'Production readiness audit historical Functions blocker boundary');
  ...
  assertIncludes(ownerActionItems, 'Fund `menulist-qa` Gemini prepay and certify one hosted synthetic extraction', 'Owner action items current Gemini provider blocker');
  assertIncludes(ownerActionItems, 'gcloud secrets versions list MENULIST_GEMINI_TEXT_AI_KEY --project=menulist-qa', 'Owner action items extraction secret metadata command');
  assertIncludes(ownerActionItems, 'Redeploy only if source or a bound', 'Owner action items bounded redeploy instruction');
  ...
  assertNotIncludes(ownerActionItems, 'firebase functions:secrets:set GEMINI_AI_KEY_4 --project menulist-qa', 'Owner action items retired scoped Gemini slot 4 command');
  assertNotIncludes(ownerActionItems, '# 4. Redeploy CF + Vercel', 'Owner action items stale broad deploy instruction');
  assertIncludes(launchPrerequisites, 'firebase functions:secrets:set MENULIST_GEMINI_TEXT_AI_KEY --project menulist-qa', 'Launch prerequisites extraction secret command');
  assertNotIncludes(launchPrerequisites, 'firebase functions:secrets:set GEMINI_AI_KEY_4 --project menulist-qa', 'Launch prerequisites retired MenuList slot 4 command');
  assertIncludes(initialAccountSetupGuide, '`MENULIST_GEMINI_TEXT_AI_KEY` is bound only to menu extraction', 'Initial setup guide extraction credential boundary');
  assertNotIncludes(initialAccountSetupGuide, '\n- `GEMINI_AI_KEY_4`\n', 'Initial setup guide retired MenuList slot 4 list item');
  ...
  assertIncludes(aiDataExtractionFinalAudit, '**Current credential contract (August 13, 2026):**', 'AI data extraction final audit current credential warning');
  ...
  assertIncludes(aiDataExtractionProductionAudit, '**Current credential contract (August 13, 2026):**', 'AI data extraction production audit current credential warning');
  ...
  assertIncludes(aiDataExtractionCfExecutionAudit, '**Current credential contract (August 13, 2026):**', 'AI data extraction CF audit current credential warning');
  ...
  assertIncludes(envProductionExample, 'NEXT_PUBLIC_MENULIST_RAZORPAY_KEY_ID=rzp_test_<temporary-shared-id>', 'Production env template pre-live Razorpay Test Mode exception');
  assertIncludes(envProductionExample, 'Keep the webhook secret endpoint-specific', 'Production env template Razorpay webhook isolation');
