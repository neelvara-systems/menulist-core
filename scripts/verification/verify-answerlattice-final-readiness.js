...
const widgetConversation = read('src/lib/answerlattice/widgetConversationServer.ts');
...
assertIncludes(widgetSearch, 'persistAnswerlatticeWidgetConversation({', 'Widget conversation persistence wiring');
assertIncludes(widgetConversation, "db.collection(DB_COLLECTIONS.CHAT_SESSIONS).doc(conversationId)", 'Widget conversation persistence target');
assertIncludes(widgetConversation, "`${tId}:${sId}:${sessionId}`", 'Widget conversation tenant/workspace document isolation');
assertIncludes(proxy, "'x-answerlattice-request-path',", 'Answerlattice callback-path controlled-header registration');
assertIncludes(proxy, 'CONTROLLED_PRODUCT_REQUEST_HEADERS.forEach((header) => requestHeaders.delete(header))', 'Answerlattice callback-path inbound header spoofing guard');
assertIncludes(proxy, "requestHeaders.set('x-answerlattice-request-path', `${pathname}${request.nextUrl.search}`)", 'Answerlattice callback-path propagation');
assertIncludes(answerlatticeLayout, "redirect(`/signin?callbackUrl=${encodeURIComponent(callbackPath)}`)", 'Answerlattice exact signed-out destination recovery');
