...
assert((paymentHook.match(/if \(checkoutInFlightRef\.current\)/g) || []).length === 5, 'Subscription, pending-subscription continuation, upgrade, top-up, and onboarding checkout entry points must reject same-tick duplicates');
assert((paymentHook.match(/checkoutInFlightRef\.current = false;/g) || []).length === 5, 'Every guarded checkout path must release its duplicate-operation guard after settlement');
