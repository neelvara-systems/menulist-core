...
assert(contentFeedbackDal.includes('acquireAnswerlatticePendingMutation('), 'content feedback clients must acquire attempt-owned idempotency claims');
assert(contentFeedbackDal.includes('settleAnswerlatticePendingMutation('), 'content feedback clients must settle only the exact acknowledged idempotency claim');
assert(!contentFeedbackDal.includes('pendingContentFeedbackRequests.delete(requestKey)'), 'content feedback clients must not let an older response delete a newer request identity');
...
assert(feedbackDal.includes('resolveCurrentSessionUserDocumentId(session)'), 'general feedback must bind retry identity to the canonical session actor');
assert(feedbackDal.includes('acquireAnswerlatticePendingMutation('), 'general feedback must acquire attempt-owned idempotency claims');
assert(feedbackDal.includes('settleAnswerlatticePendingMutation('), 'general feedback must settle only the exact acknowledged idempotency claim');
assert(!feedbackDal.includes('pendingFeedbackRequests.delete(requestKey)'), 'general feedback must not let an older response delete a newer request identity');
assert(feedbackDal.includes('normalizeAnswerlatticeScopeDocumentId(persisted.tId) !== tId'), 'general feedback surface mutation must require canonical persisted tenant scope');
assert(feedbackDal.includes('normalizeAnswerlatticeScopeDocumentId(persisted.sId) !== sId'), 'general feedback surface mutation must require canonical persisted workspace scope');
assert(!feedbackDal.includes('Number(persisted.tId)'), 'general feedback surface mutation must not coerce persisted tenant scope');
assert(!feedbackDal.includes('Number(persisted.sId)'), 'general feedback surface mutation must not coerce persisted workspace scope');
assert(feedbackDal.includes('const actorId = resolveCurrentSessionUserDocumentId(session)'), 'general feedback surface mutation must use the canonical session actor');
assert(feedbackDal.includes('surfaceAssignedAt: hasSurface ? serverTimestamp() : null'), 'general feedback surface mutation must persist the commit timestamp');
assert(feedbackDal.includes('modifiedOn: serverTimestamp()'), 'general feedback surface mutation must persist the commit modification timestamp');
assert(rules.includes('request.resource.data.surfaceAssignedBy == request.auth.uid'), 'feedback rules must bind surface attribution to the authenticated actor');
assert(rules.includes('request.resource.data.surfaceAssignedAt == request.time'), 'feedback rules must bind surface assignment time to the commit time');
assert(rules.includes('request.resource.data.modifiedBy == request.auth.uid'), 'feedback rules must bind modification attribution to the authenticated actor');
assert(rules.includes('request.resource.data.modifiedOn == request.time'), 'feedback rules must bind modification time to the commit time');
