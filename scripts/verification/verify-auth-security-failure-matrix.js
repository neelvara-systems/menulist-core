...
const loginIdentifiers = read('src/lib/auth/loginIdentifiers.ts');
...
assertIncludes(loginIdentifiers, 'normalizeCredentialLoginIdentifier', 'Credentials login must keep shared identifier normalization.');
assertIncludes(loginIdentifiers, 'normalizeCredentialLoginPassword', 'Credentials login must keep shared password normalization.');
assertIncludes(loginPage, 'const loginIdentifier = normalizeCredentialLoginIdentifier(values?.email);', 'Login page must normalize the identifier before both auth handoffs.');
assertIncludes(loginPage, 'const password = normalizeCredentialLoginPassword(values?.password);', 'Login page must normalize the password before both auth handoffs.');
assert((loginPage.match(/password,/g) || []).length >= 2, 'Login page must reuse the normalized password for NextAuth and Firebase Auth.');
assertIncludes(authIndex, 'normalizeCredentialLoginIdentifier((credentials as any)?.email)', 'NextAuth credentials authorization must normalize the identifier server-side.');
assertIncludes(authIndex, 'normalizeCredentialLoginPassword((credentials as any)?.password)', 'NextAuth credentials authorization must normalize the password server-side.');
