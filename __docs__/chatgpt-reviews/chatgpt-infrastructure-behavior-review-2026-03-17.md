...
| 9 | **Canonical menu URL per restaurant** | ✅ FULLY BUILT. `{subdomain}.menulist.online/menu` (reserved slug under the current production domain contract). Custom domains supported. 301 redirects from subdomain to custom domain for SEO consolidation. Slug permanence with `previousSlugs` chain-redirect. | `src/app/client/[[...slug]]/page.tsx` |
