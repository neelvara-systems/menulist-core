...
Default summaries carry a bounded, per-item price/availability contribution list for the nightly drift task. Every browser ledger write uses Firestore server time and the rules require the resolved event timestamp to equal `request.time`; open-ended browser readers and both nightly consumers also cap their source window at one run-specific current timestamp so a malformed legacy/Admin future row cannot remain in every rolling window. The Functions mirror validates the same contract, scans each store's 30-day ledger once with stable timestamp/document pagination, partitions events by active project, and writes metrics in bounded batches. Derived item documents whose events leave the rolling window are deleted, and 180-day price staleness is marked unavailable when the 30-day source window cannot prove it. If a single revision exceeds the compact contribution cap, only overflow price/availability events fall back to the existing detailed event path.

Authenticated browser readers runtime-project each persisted event against the exact collection tenant/store scope and canonical bounded project/item/category/user identities before returning it. Missing, malformed, or foreign embedded scope is omitted with a bounded diagnostic rather than entering internal change history or drift counts.

The supported pricing-integrity legacy event shape remains private and append-only, but it is not a free-form compatibility bypass. Client rules bind `actorUserId` to the authenticated Firebase UID/current `uId`, require an allowlisted event-type/entity-type pair, require project-bound events to reference an existing project in the current tenant/store path, and require store-bound hours/GBP/POS events to use the current store ID for both legacy `projectId` and `entityId`. Canonical browser events cannot claim `SYSTEM` provenance; genuine server/Admin events bypass client rules through the Admin SDK.
