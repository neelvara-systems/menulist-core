...
| P0 | Blocked | Align live production host before Search Console. | Founder | Live curl check on June 23, 2026 | `menulist.online` serves the app but emits `menulist.ai` canonical/sitemap URLs; `menulist.ai` currently serves a `/lander` shell, not the MenuList app. Do not submit Search Console until hosting/DNS matches the chosen canonical host. |
