...
The payload stores the size-limited neutral `CreativeEditorDocument`. List calls do not read this payload. Open calls first require an ownership-safe versioned path for the exact platform category/template or tenant/store/template, check the Storage blob size against `MAX_DOCUMENT_BYTES`, decode JSON, and apply the runtime editor-document schema. Oversized, malformed, structurally invalid, or cross-scope references fail before the editor receives a document.
