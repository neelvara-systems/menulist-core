...
| Dynamic icon/splash/screenshot routes fail closed on limiter outage, never shared-cache transient fallbacks, bound rendered text, and reject untrusted server-fetched image URLs | ✅ Done | `src/lib/pwa/customerAppAssets.tsx`, three dynamic asset routes, `npm run verify:customer-app-pwa` |
