...
| `/middleware/publicApi.ts` | ✅ Done | Public Turnstile verification logs provider HTTP, parse, and request failures with stable security codes plus bounded path/status metadata; provider JSON is capped, redirects are not followed, requests use an 8-second abort deadline, and timers clear on every outcome |
| `/app/api/public/contact/route.ts` | ✅ Done | MenuList public contact failures use bounded security diagnostics with a normalized contact failure code, topic/phone/message/source-path/referrer length context, and source error name/code/status only; limiter infrastructure failures fail closed and persisted attribution excludes URL queries/fragments |
...
| `/lib/apiHelper/apiCallComposerServer.ts` | ✅ Done | Server DAL wrapper failures use normalized `secureError` diagnostics, require an authenticated session for every wrapped call, and rethrow failures instead of returning a query-shaped sentinel |
