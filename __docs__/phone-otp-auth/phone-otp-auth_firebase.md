...
July 28 persisted-auth boundary hardening adds no reads or writes for valid challenge/token documents. Every transaction now admits exact server-owned persisted shapes before attempt, expiry, reservation, user or email identity is trusted. Malformed challenge/token records fail closed and remain available for bounded diagnostics/TTL cleanup rather than being coerced into valid auth state.

Both server-only collections carry a real Firestore `expiresAt` timestamp and are included in `scripts/setup-firestore-ttl.sh`. The guarded operator command must be applied separately to `menulist-qa` and `menulist`; TTL deletion is asynchronous and billable. Source presence is not proof that either remote policy is active, so operators must confirm the field policy in each project before claiming retention closure.
