...
July 28 persisted-auth boundary hardening: challenge and login-token documents are runtime-projected before identity or attempt state is used. Attempts must be exact safe integers from zero through five; timestamps must be real Admin Firestore `Timestamp` values; hashes, operation IDs, user IDs, canonical non-empty token email, phone fields, status and reservation fields must satisfy their closed shapes. Negative, string, fractional or otherwise malformed counters cannot expand the guess budget. Malformed token records cannot consume against a user with missing email. Raw login tokens must have the exact 43-character base64url shape generated by the server. Invalid persisted records fail with the existing fixed invalid-code/token paths and do not mutate user identity.
