...
- Create/Edit dialogs and the public-name/start/end inputs carry stable programmatic names. Mounted guards restore their active state on every effect setup, including React Strict Mode replay. A form opened before project hydration adopts the default base menu once available without resetting a schedule the owner already entered.
- A successful create upserts the authoritative new project summary before selecting it. Edit merges the authoritative returned lifecycle status and current draft into the same cache. Ending or cancelling the currently selected special updates its terminal state and returns the owner to the regular base menu immediately.
