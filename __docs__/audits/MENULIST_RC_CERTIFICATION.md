...
**Current tested product commit:** `06fe2ecb00d3ecd4c4c78d77c9777a3a9df484cd`
...
The current generator discovered 8,475 source candidates across the shared
...
| User-control candidates | 7,492 | 5,291 MenuList controls page-mapped; 155 MenuList controls explicitly classified as non-shipped source |
...
Current product classification yielded 5,957 MenuList candidates and 2,518
...
| Route/control inventory | App Router, controls, flags, Functions | PASS (discovery, private-route classification/access, API classification, and static reachability) | `npm run verify:menulist-rc-inventory`; 8,475 unique rows; all 58 MenuList private pages explicitly classified and signed-out access-tested; all 136 MenuList route handlers have resolved methods/access classes; 5,291/5,446 MenuList control candidates map to page routes and 155 are explicitly classified as non-shipped source |
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity and bounded authenticated recovery interaction) | `/api/version` returned exact product commit `04a736fed09a888af91635150fb4942caf85799a` with verified provenance. Connected Chrome retained the approved Google owner identity and yearly Starter `Payment pending` Billing state across three hard reloads. Vercel's exact request log showed no `/api/auth/set-claims` request after the build became active; the latest app-host request remained the pre-fix 17:40:21 entry. Direct `/users` again returned the unpaid fixture to Billing without adding a claim-sync request. Billing History, Help recovery, blank ticket validation, and View Plans had already passed. Live Razorpay checkout was not started. |
...
- Hosted QA exposed the exact verified tested product commit
  `04a736fed09a888af91635150fb4942caf85799a`. The signed-in
