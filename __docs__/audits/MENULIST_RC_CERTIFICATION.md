...
**Current tested product commit:** `96f0c0425a65593410877ef7583169915c8568fe`
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity and bounded recovery interaction) | At `2026-08-25T16:15:18+05:30`, `/api/version` returned exact product commit `96f0c0425a65593410877ef7583169915c8568fe` with verified provenance. Signed-in pending-owner Billing/history/Help/recovery evidence remains valid through ancestor `801f87f…`; exact-current evidence includes the complete Arabic document-locale route sweep and corrected access-denied copy. |
...
| Global signed-out recovery routes | `/403`, `/404`, `/unauthorized`, `/offline`, invalid `/invite`, `/product`, disabled `/creative-editor-smoke`, protected `/test-sentry`, invalid `/msg-preview/[sessionId]` | PASS (behavior and copy) / mobile layout and home-exit fixes pending hosted retest | Exact hosted `96f0c04…` Chrome at 320×568 rendered readable apostrophes on `/403` and `/unauthorized`, exposed both recovery actions, and leaked no protected content. The same pass found and traced the 328px horizontal width and owner-app-relative “Go Home” destination recorded as MLRC-036/037. |
