...
**Current tested MenuList product commit:** `c15d0a1fab052d7c126565cac01cc10cbbeef40e`
...
readback proved exact MenuList staging commit `c15d0a1fab052d7c126565cac01cc10cbbeef40e`
...
`c15d0a1fab052d7c126565cac01cc10cbbeef40e`
are included. `main`
remains untouched.
...
| Hosted QA | Exact candidate on QA owner app and tenant subdomain | PASS (deployment identity, fresh entitled owner, Billing, direct-route entitlement bootstrap, menu-link failure recovery, and temporary-status public propagation) / IN PROGRESS (successful menu upload/item mutation and publish propagation) | `/api/version` returned exact verified build `c15d0a1fab052d7c126565cac01cc10cbbeef40e` (`menulist-core-gik49l0xm-neelvara-systems.vercel.app`). A fresh 72-hour provider-free fixture created by the shared onboarding transaction passed Firebase Web Auth, exact owner claims, full tenant/store reads, the production active-subscription query, and the shared entitlement predicate without data repair. Browser sign-in with deliberate leading/trailing password whitespace succeeded. Billing rendered `MenuList Pro — QA Certification`, `No payment — QA only`, 75/75 credits, and the explicit no-payment statement. Projects recovered from one transient Firestore-unavailable write and created its default project once. Exact hosted `8688fbb…` retained `/dashboard` and `/projects` across independent direct hard loads without a Billing redirect. Exact hosted `c15d0a1…` rejected an owned non-menu URL, retained persistent owner-safe failure copy as an accessibility alert, re-enabled the form, and cleared the old error on URL correction. The same owner then published `Closed today`; the tenant Official Business Page rendered the banner immediately. Clearing it removed the banner on both a warm reload and a cache-busted request. The disposable fixture remains in use for the menu upload/item mutation/publish pass. Live Razorpay checkout was not started. |
...
request. Custom-domain, screen, successful menu upload/item mutation, and
physical-device propagation remain unverified and are not represented as
