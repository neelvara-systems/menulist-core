...
| MLRC-057 | Medium | Desktop Business Settings save completion | An acknowledged short-descriptor write persisted and propagated immediately to the Official Business Page, but desktop Business Settings showed no visible success confirmation. The owner could not distinguish a completed write from an inert Save Changes action without refreshing or opening the public page. | Announce `Business settings saved` only after the existing store/create path completes and the initiating tenant/store scope remains active. Preserve validation returns, rejected-write behavior, DAL authority, cache invalidation, mobile feedback, and operation count. | `verify:public-business-truth`; focused ESLint; strict TypeScript; exact hosted owner/public retest | Defect reproduced on exact hosted `cde4708…`; source correction and hosted corrected-build retest pending. | FIXED — RETEST PENDING |
