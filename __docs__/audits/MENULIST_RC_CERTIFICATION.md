...
**Current tested product commit:** `6c9e6eca2d8c8918b8849f196be3ae417a7b70c3`
...
proved exact staging commit `6c9e6eca2d8c8918b8849f196be3ae417a7b70c3` on
QA. The hosted MLRC-045 interaction remains attributable to its included
runtime commit `32440eca8e171212fb77983218d3a071e0db5981`; all shared Admin
Timestamp corrections from `320bd3b0b59ea83f89dbfe460bbc14262743f4b2` are
also included and passed the full hosted anonymous route-module boundary.
MLRC-047 is a newer local candidate change and is not represented as hosted.
...
| MLRC-047 | Medium | Anonymous/internal batch-image worker admission and Firebase cost | The route checked SAFE_MODE before authenticating the Cloud Tasks project header and shared secret. Each malformed external request therefore attempted an `ops_config/system` Firestore read before it could return `403`; two local certification probes reached the full 30-second client deadline while that unnecessary read failed. | Move project/secret admission ahead of SAFE_MODE. Keep the admitted path unchanged: SAFE_MODE still precedes body parsing, rate limiting, job reads, provider work, Storage, accounting, and mutations. Extend both worker-admission source-order verifiers and the feature cost/implementation docs. | `verify-system-strengthening-boundary`; `verify-menulist-api-tenant-safety`; full 136-handler/153-method anonymous boundary; focused ESLint; strict TypeScript | Direct invalid-worker POST now returns `403` in 0.029 seconds with zero Firebase operations. The complete local anonymous boundary passes with 403×1, no timeouts, and no failures. | CLOSED (local; hosted descendant pending) |
...
| `npm run test:menulist-api-anonymous-boundary` (MLRC-047 retest) | PASS local — 136 handlers / 153 methods; 200×5, 301×1, 400×15, 401×128, 403×1, 404×3; zero timeout and zero failure. Two earlier correctly configured runs reproduced one 30-second batch-worker timeout before the fix; the direct corrected worker probe returns 403 in 0.029 seconds. |
...
| `npm run certify:menulist-local` | PASS WITH EXTERNAL BLOCKER — fresh uninterrupted current-candidate run completed 160/161 checks; all executable checks passed, 42 sibling-product scripts were explicitly excluded, and only `verify:upstash-readiness` was `BLOCKED_EXTERNAL`. |
...
| `npm run docs:check-links` | PASS — 3,053 files and 5,338 internal links scanned; zero broken links. The command reported 64 non-failing filename-policy warnings, including the task-mandated `MENULIST_RC_CERTIFICATION.md` name and pre-existing uppercase video working documents. |
...
- MLRC-041 is closed on exact hosted QA commit `194f39a…` at 320×568. MLRC-042
  ...
  `04a736f…`; the failure-screen Retry/Sign out interaction was not deliberately
  re-induced after the rate-limit stress and remains source-regression evidence.
...
- Hosted QA exposed and refreshed onto the exact verified tested product commit
  `6c9e6eca2d8c8918b8849f196be3ae417a7b70c3`. The signed-in
