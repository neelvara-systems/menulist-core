...
**Current staging commit:** `4592516b8bb22f07da5f48b714b2493c7e0ba566`
...
| Route-handler anonymous boundary | All 136 MenuList handlers and 153 exported HTTP methods | PASS local and exact hosted candidate | Exact hosted commit `6854cf4e7881f5a5d5fe3ef939245cd1d1b88e50` returned 200×5, 301×1, 400×15, 401×126, 403×1, and 404×5 with zero timeout/5xx; the former batch-image trace crash now rejects anonymously with 401. Live Razorpay execution remains excluded. |
...
| Pending owner direct-route gate | Billing recovery versus direct `/locations` access | FAIL hosted / PASS source | Hosted pending owner could deep-link to Locations and view mutation controls; the shared owner layout now redirects every store-backed recovery-only owner to Billing before rendering non-recovery routes |
...
| Mobile Billing recovery | Direct `/billing`, pending-owner summary, history, support handoff, touch targets, overflow | PASS layout/history / FAIL support handoff | Exact hosted `4592516…` rendered native `MobileBillingScreen` at 390×844 with zero horizontal overflow and all visible actions at least 44px. Billing History opened a truthful empty sheet. Both support actions were dead because they crossed restricted `/dashboard`; MLRC-029 corrects them to `/help-center/ticket`, with hosted retest pending. |
...
| `npm run test:menulist-api-anonymous-boundary` | PASS local and hosted — exact hosted `6854cf4…` exercised 136 handlers / 153 methods with status counts 200×5, 301×1, 400×15, 401×126, 403×1, 404×5 and zero failures/5xx. |
...
- MLRC-028 is closed on exact hosted QA. MLRC-029 is corrected in source but
  still requires an exact hosted mobile click-through retest.
