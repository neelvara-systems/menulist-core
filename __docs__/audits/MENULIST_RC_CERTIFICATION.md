...
**Current tested product commit:** `2b149b025c8c8737c8e8e7b8aa64fd4cfa12cc95`
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity and bounded recovery interaction) | At `2026-08-25T16:41:33+05:30`, `/api/version` returned exact product commit `2b149b025c8c8737c8e8e7b8aa64fd4cfa12cc95` with verified provenance. Signed-in pending-owner Billing/history/Help/recovery evidence remains valid through ancestor `801f87f…`; exact-current evidence includes public-feedback/global recovery corrections and 44px access-denied actions. |
...
| Global signed-out recovery routes | `/403`, `/404`, `/unauthorized`, `/offline`, invalid `/invite`, `/product`, disabled `/creative-editor-smoke`, protected `/test-sentry`, invalid `/msg-preview/[sessionId]` | PASS (behavior, copy, width, destination, and 44px target) / initial-viewport action visibility fix pending hosted retest | Exact hosted `2b149b0…` Chrome at 320×568 measured both `/403` and `/unauthorized` actions at 44px with a 320px document width. The actions began below the initial 568px viewport because the shared result retained redundant vertical padding; MLRC-041 removes that source-level obstruction. |
| Digital screen invalid-token recovery | `/screen/[token]` with deterministic invalid token; generic Back/Home recovery | PASS (truthful invalid state and Back) / home, touch, and initial-viewport fixes pending hosted retest | Exact hosted `756f247…` at 320×568 rendered the noindex generic not-found boundary with zero overflow and no screen/store disclosure. Go Back restored the previous pricing route. Go Home entered protected `/dashboard`; both controls measured 40px and began just below the initial viewport. MLRC-039 through MLRC-041 correct those shared recovery defects. Active/expired screen rendering still requires an isolated valid fixture. |
...
| MLRC-039 | Medium | Generic not-found “Go Home” from app-hosted public routes, including invalid `/screen/[token]` | The shared action pushed relative `/`. On `app.menulist.digital`, host routing converted it to protected `/dashboard`, so a public screen viewer was routed into owner authentication instead of the MenuList public website. | Send the home action to environment-governed `PLATFORM_URL`; keep history-based Go Back as the first recovery option. | `verify:global-accessibility-boundary`; focused ESLint; TypeScript | Exact hosted `756f247…` invalid-screen click entered `/dashboard`; fixed hosted-build retest pending | CLOSED (source) |
| MLRC-040 | Medium | Generic not-found Back/Home actions at small-mobile width | Both shared recovery buttons measured 40px high at 320×568, below the repository’s 44px mobile interaction minimum. | Apply a 44px minimum height to both existing actions without changing their labels, order, or behavior. | `verify:global-accessibility-boundary`; focused ESLint; TypeScript | Exact hosted `756f247…` measured both at 40px; fixed hosted-build retest pending | CLOSED (source) |
| MLRC-041 | Medium | Access-denied and generic not-found recovery action visibility at 320×568 | The outer page already supplied safe-area padding, while Ant Design `Result` added another 48px block padding. At the smallest required viewport, access-denied actions began at 572px and not-found actions at 575px, leaving the initial screen without any visible next action. | Remove redundant Result padding while retaining the outer safe page spacing, 44px action targets, existing illustration, wrapping, and desktop max width. | `verify:global-accessibility-boundary`; focused ESLint; TypeScript | Exact hosted `2b149b0…` measured access actions below the fold; exact `756f247…` invalid-screen recovery reproduced the same issue; fixed hosted-build retest pending | CLOSED (source) |
