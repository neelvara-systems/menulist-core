...
**Current tested product commit:** `b1750e0b149adb99f822667ddde5ed5ca1af1073`
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity and bounded recovery interaction) | At `2026-08-25T15:27:05+05:30`, `/api/version` returned exact product commit `b1750e0b149adb99f822667ddde5ed5ca1af1073` with verified provenance. Signed-in pending-owner Billing/history/Help/recovery evidence remains valid through ancestor `801f87f…`; exact-current 320px evidence adds create-menu country flags and pricing comparison disclosure. |
...
| Current hosted mobile website routes | Static marketing, feature, industry, legal, trust, tools, English resources, and all 112 locale-prefixed resource variants | PASS (render/responsive) / one accessibility fix pending hosted retest | Exact `b1750e0…` QA at 320×568 rendered every static website route and every reviewed localized resource variant with a non-empty heading, no application/not-found state, and no horizontal overflow. Arabic content computed RTL, but the document root retained the cookie locale; MLRC-033 corrects the assistive-technology boundary. |
...
- MLRC-033 is closed in source and awaits exact hosted readback across direct
  Arabic entry, Arabic-to-English navigation, and browser back/forward before
  its runtime status can be closed.
