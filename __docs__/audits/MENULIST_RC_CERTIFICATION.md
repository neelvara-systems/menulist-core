...
| `npm run certify:menulist-local` | PASS WITH EXTERNAL BLOCKER — current full run passed 5 checks then correctly stopped on the one owner-PWA AssetOS fingerprint made stale by MLRC-043; after MLRC-044, `--start-at verify:asset-factory` passed 155/156 remaining checks. Combined unique result: 160/161 checks passed; 42 sibling-product scripts explicitly excluded; only `verify:upstash-readiness` was `BLOCKED_EXTERNAL`. |
