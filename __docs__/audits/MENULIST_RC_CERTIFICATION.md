...
**Current tested product commit:** `2d4d370c4baf0dd04a2d34be63dc3f5f10672c00`
...
readback proved exact staging commit `2d4d370c4baf0dd04a2d34be63dc3f5f10672c00`
...
and truthful MenuList QA lease presentation from that same commit are included. `main`
remains untouched.
...
- Initial fixture classes: unauthenticated visitor, new/no-tenant user,
  ...
  QA owner is now active at tenant `3`, store `3`, with a labelled 72-hour
...
The current generator discovered 8,501 source candidates across the shared
...
| User-control candidates | 7,518 | 5,291 MenuList controls page-mapped; 155 MenuList controls explicitly classified as non-shipped source |
...
| Local aggregate | MenuList-filtered repository production-readiness gate | PASS WITH EXTERNAL BLOCKER | A fresh uninterrupted `npm run certify:menulist-local` pass on the current candidate completed 160/161 checks. All 160 executable checks passed, including all 42 Firestore Rules predeploy suites, strict TypeScript, lint, documentation links, MobileShell regression coverage, cache/public-truth boundaries, Functions build/preflight, and AssetOS. The sole non-pass is the unavailable Upstash target credential gate. |
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity, fresh entitled owner, Billing, and client-navigation recovery) / OPEN (hard-load entitlement gate) | `/api/version` returned exact verified build `2d4d370c4baf0dd04a2d34be63dc3f5f10672c00` (`menulist-core-9w1zjz1od-neelvara-systems.vercel.app`). A fresh 72-hour provider-free fixture created by the shared onboarding transaction passed Firebase Web Auth, exact owner claims, full tenant/store reads, the production active-subscription query, and the shared entitlement predicate without data repair. Browser sign-in with deliberate leading/trailing password whitespace succeeded. Billing rendered `MenuList Pro — QA Certification`, `No payment — QA only`, 75/75 credits, and the explicit no-payment statement. Projects recovered from one transient Firestore-unavailable write, created its default project once, and Dashboard rendered entitled owner controls through client navigation. A direct hard load could still redirect to Billing during the new-scope subscription bootstrap; MLRC-052 fixes that race and awaits exact hosted retest. Live Razorpay checkout was not started. |
...
| MLRC-052 | High | Fresh or hard-loaded entitled owner routes | A scope change reset subscription state and exposed `activeSubscriptionLoading=false` before the new scope's entitlement query began. Dashboard and Projects could therefore misclassify a valid paid/QA-entitled owner as unpaid and redirect to Billing during bootstrap; client navigation worked after Billing had allowed the query to settle. | Keep subscription gates loading immediately after resetting a signed-in MenuList store scope, then let the existing scoped request lifecycle settle success or failure. Preserve Answerlattice separation and stale-request guards. | `test-session-store-context-boundary`; `verify:auth-onboarding-flow`; exact QA Firebase Web client; strict TypeScript; lint; production build | Root-cause reproduction and all local/shared-auth checks pass. Exact staging deployment and direct hard-load browser retest are pending. | OPEN — HOSTED SOURCE RETEST |
...
| `npm run certify:menulist-local` | PASS WITH EXTERNAL BLOCKER — fresh uninterrupted post-MLRC-049 run completed 160/161 checks; all 160 executable checks passed, 42 sibling-product scripts were explicitly excluded, and only `verify:upstash-readiness` was `BLOCKED_EXTERNAL`. The external target was separately proven through the connected QA console and hosted contact limiter without exposing its secret URL/token. |
...
- Hosted QA exposed and refreshed onto exact verified tested product commit
  `2d4d370c4baf0dd04a2d34be63dc3f5f10672c00`. A fresh provider-free
  ...
  project creation, transient-write recovery, and client-navigation Dashboard
  access. MLRC-052 remains open until its subscription-bootstrap correction is
  deployed and direct hard loads of Projects and Dashboard pass. Public-menu
  mutation/propagation, multi-location mutation, installed PWA, physical
  entitled-owner/public-menu, and television-screen journeys remain unverified.
