...
**Current tested product commit:** `1d952f0872d5002aac5d3a1dda72277567d4d4c1`
...
readback proved exact staging commit `1d952f0872d5002aac5d3a1dda72277567d4d4c1`
...
truthful MenuList QA lease presentation from that same commit, and the first
MLRC-052 loading-state correction from `1d952f0872d5002aac5d3a1dda72277567d4d4c1`
are included. `main`
remains untouched.
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity, fresh entitled owner, Billing, and client-navigation recovery) / OPEN (hard-load entitlement gate) | `/api/version` returned exact verified build `1d952f0872d5002aac5d3a1dda72277567d4d4c1` (`menulist-core-58whq5q8e-neelvara-systems.vercel.app`). A fresh 72-hour provider-free fixture created by the shared onboarding transaction passed Firebase Web Auth, exact owner claims, full tenant/store reads, the production active-subscription query, and the shared entitlement predicate without data repair. Browser sign-in with deliberate leading/trailing password whitespace succeeded. Billing rendered `MenuList Pro — QA Certification`, `No payment — QA only`, 75/75 credits, and the explicit no-payment statement. Projects recovered from one transient Firestore-unavailable write, created its default project once, and Dashboard rendered entitled owner controls through client navigation. Exact hosted `1d952f0…` proved that retaining the mutable loading flag across the scope-reset effect alone was insufficient: direct hard loads of both Dashboard and Projects still redirected to Billing. The follow-up now requires a settled subscription scope before owner children render and presents retry recovery on query failure; hosted retest remains pending. Live Razorpay checkout was not started. |
