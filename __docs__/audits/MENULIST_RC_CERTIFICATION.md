...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity and bounded recovery interaction) | At `2026-08-25T16:50:00+05:30`, `/api/version` returned exact product commit `f0508560931d80d4f6dc9fd9c56a7638a9813994` with verified provenance. Signed-in pending-owner Billing/history/Help/recovery evidence remains valid through ancestor `801f87f…`; exact-current evidence includes corrected invalid-screen Back/Home behavior and 44px actions. The same pass kept MLRC-041 open because access-denied Home remained below the first 320×568 viewport. |
...
| Global signed-out recovery routes | `/403`, `/404`, `/unauthorized`, `/offline`, invalid `/invite`, `/product`, disabled `/creative-editor-smoke`, protected `/test-sentry`, invalid `/msg-preview/[sessionId]` | PASS (behavior, copy, width, destination, and 44px target) / access-denied first-viewport refinement pending hosted retest | Exact hosted `f050856…` Chrome at 320×568 proved generic not-found Back/Home at 44px, visible in the initial viewport, and routed Home to `https://menulist.digital/`. `/403` and `/unauthorized` stayed 320px wide with 44px actions, but Home remained below the first viewport; MLRC-041 now also scales the illustration responsively and compacts the action gap. |
...
| MLRC-041 | Medium | Access-denied and generic not-found recovery action visibility at 320×568 | The outer page already supplied safe-area padding, while Ant Design `Result` added another 48px block padding. At the smallest required viewport, access-denied actions began at 572px and not-found actions at 575px, leaving the initial screen without any visible next action. | Remove redundant Result padding, make the contextual illustration responsive on narrow/short screens, and compact the wrapped action gap while retaining safe outer spacing, 44px targets, and the desktop maximum. | `verify:global-accessibility-boundary`; focused ESLint; TypeScript | Exact hosted `f050856…` closed the generic not-found case but still placed access-denied Home at 580–624px; responsive illustration/action refinement pending hosted retest | CLOSED (source) |
