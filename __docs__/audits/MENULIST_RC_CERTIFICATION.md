...
| MLRC-041 | Medium | Access-denied and generic not-found recovery action visibility at 320×568 | The outer page supplied 24px safe padding outside a `100vh` minimum while Ant Design `Result` added another 48px block padding. At the smallest required viewport, content remained taller than the viewport even after the illustration/action compaction. | Remove redundant Result padding, make the illustration responsive, compact the wrapped action gap, and include safe padding inside a border-box `100dvh` shell while retaining 44px targets and the desktop maximum. | `verify:global-accessibility-boundary`; focused ESLint; TypeScript | Exact hosted `98bf966…` put both actions on one row at 524–568px but retained a 592px shell; dynamic-viewport border-box refinement pending hosted retest | CLOSED (source) |
