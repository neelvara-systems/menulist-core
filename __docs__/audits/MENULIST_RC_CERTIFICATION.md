...
**Current tested MenuList product commit:** `8688fbb20d486f98fef56fda3c154ce0ffa44a5e`
...
readback proved exact MenuList staging commit `8688fbb20d486f98fef56fda3c154ce0ffa44a5e`
on QA; the current `staging` branch is its Answerlattice-only descendant
`7736c569ec426257347f62a6ead968b4858366c1`. The hosted MLRC-045 interaction remains attributable to its included
...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity, fresh entitled owner, Billing, and direct-route entitlement bootstrap) / IN PROGRESS (menu mutation and propagation) | `/api/version` returned exact verified build `8688fbb20d486f98fef56fda3c154ce0ffa44a5e` (`menulist-core-lhm3tjrln-neelvara-systems.vercel.app`). A fresh 72-hour provider-free fixture created by the shared onboarding transaction passed Firebase Web Auth, exact owner claims, full tenant/store reads, the production active-subscription query, and the shared entitlement predicate without data repair. Browser sign-in with deliberate leading/trailing password whitespace succeeded. Billing rendered `MenuList Pro — QA Certification`, `No payment — QA only`, 75/75 credits, and the explicit no-payment statement. Projects recovered from one transient Firestore-unavailable write and created its default project once. Exact hosted `8688fbb…` then retained `/dashboard` and `/projects` across independent direct hard loads, rendered the entitled dashboard controls, and rendered the first-use upload/link-import Projects state without a Billing redirect. The disposable fixture is now being used for the remaining menu mutation/public-propagation pass. Live Razorpay checkout was not started. |
...
- Hosted QA exposed and refreshed onto exact verified tested MenuList product
  commit `8688fbb20d486f98fef56fda3c154ce0ffa44a5e`. A fresh provider-free
  ...
  project creation, transient-write recovery, and direct hard-load Dashboard
  and Projects access. MLRC-052 is closed. Public-menu
