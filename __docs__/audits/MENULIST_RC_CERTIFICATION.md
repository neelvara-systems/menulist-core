...
| Hosted QA | Exact candidate on QA owner app | PASS (deployment identity, fresh entitled owner, Billing, direct-route entitlement bootstrap, and menu-link failure recovery) / IN PROGRESS (successful menu mutation and propagation) | `/api/version` returned exact verified build `c15d0a1fab052d7c126565cac01cc10cbbeef40e` (`menulist-core-gik49l0xm-neelvara-systems.vercel.app`). A fresh 72-hour provider-free fixture created by the shared onboarding transaction passed Firebase Web Auth, exact owner claims, full tenant/store reads, the production active-subscription query, and the shared entitlement predicate without data repair. Browser sign-in with deliberate leading/trailing password whitespace succeeded. Billing rendered `MenuList Pro — QA Certification`, `No payment — QA only`, 75/75 credits, and the explicit no-payment statement. Projects recovered from one transient Firestore-unavailable write and created its default project once. Exact hosted `8688fbb…` retained `/dashboard` and `/projects` across independent direct hard loads without a Billing redirect. Exact hosted `c15d0a1…` rejected an owned non-menu URL, retained persistent owner-safe failure copy as an accessibility alert, re-enabled the form, and cleared the old error on URL correction. The disposable fixture is now being used for the remaining successful menu mutation/public-propagation pass. Live Razorpay checkout was not started. |
