...
| Owner mobile source boundary | MobileShell route map, Billing and Help recovery, PWA lifecycle, locale coverage | PASS (source) / FAIL hosted / BLOCKED (true device) | Connected Chrome at 390×844 reproduced the subscription card replacing `/help-center`; MLRC-022 adds all four Help sub-screens to the recovery bypass and the Help/PWA/subscription gates pass. Updated hosted build and physical-device evidence remain pending. |
...
- MLRC-028 is closed in source with deterministic adjacent gates, but its
  corrected 390px hosted visual retest is pending the next exact staging build.
