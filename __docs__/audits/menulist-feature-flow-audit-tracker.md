...
| 28 | MenuList Help Center and Answerlattice support boundary | Complete | MenuList now owns a zero-backend Home/FAQ/email-contact surface on desktop and MobileShell; legacy MenuList help deep links recover to FAQ or Contact without importing Answerlattice DAL/API/Firebase/provider contracts. The full ticket/search/feedback/attachment application remains governed only on Answerlattice routes. Existing Answerlattice support index exemptions and scoped rule contracts remain unchanged. |
...
| 28 | MenuList Help Center and Answerlattice support boundary | Medium | Local source complete | MenuList FAQ/contact navigation and legacy recovery; strict separation from Answerlattice search, tickets, feedback, attachments, DAL/API/Firebase/provider paths; desktop/MobileShell parity |
...
The current MenuList Help Center pass covers owner navigation, FAQ disclosure,
email contact, truthful legacy deep-link recovery, desktop and MobileShell
parity, and strict product isolation. MenuList Help performs zero DAL, API,
Firebase, listener, AI, or provider work. Authenticated search, canonical/RAG
responses, KB/changelog, feedback, tickets, replies, status, satisfaction,
attachments, and notifications belong to the separate Answerlattice Help
Center and remain governed on Answerlattice routes only.
...
Historical item-30 evidence covered FAQ, Tools, WhatsApp, private Invite and reviewed locale-prefixed Resources on the then-active `menulist.digital/ml` alias. That hosted alias is superseded: `menulist.digital` now belongs to the MenuList QA website/app/customer domain family, while `/ml` remains local/internal only. Header active state and the language switcher inspect the public pathname after removing a permitted local alias; language changes reapply it. The language menu has explicit button/menu state and Escape focus recovery. Terms and Refund pages no longer promise universal publishing, all-plan access, fixed 30-day deletion, absolute generated-output ownership or an unverified Razorpay certification.

The legal cross-check exposed a real billing mismatch: owner UI and cancellation terms promised paid access through `cycleEndDate`, but the store/platform plan mirror dropped cancelled/paused rows immediately. Current-cycle cancelled/paused rows now retain that mirror; a bounded hourly task in the existing leased maintenance scheduler processes at most 500 due exact-dual-`ML` rows, changes them to `expired`, repairs entitlement, and leaves a durable retry marker on partial failure. The original due index is now product-prefixed as `subscriptions(pId ASC, productId ASC, status ASC, cycleEndDate ASC)`. Root type/lint, Functions lint/build/preflight, website copy/locale/discovery/contact/tools, billing unit/source, pricing, tenant-safety, MobileShell, dependency and Firestore emulator gates pass. Both smallest-scope QA deploys were attempted: the index stopped at the Firebase Rules test endpoint HTTP 403, and the Function passed predeploy lint/build then stopped at Cloud Resource Manager HTTP 403; neither uploaded. Approved app/Vercel release, canonical/alias browser/device/accessibility smoke, mutable Razorpay sandbox evidence, analytics/contact provider evidence, Search Console/discovery observation, legal approval and production-host smoke remain owner/release pending.
