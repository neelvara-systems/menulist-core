...
| Desktop theme library journey | Change brand look opens at full viewport size with the complete catalog width available. Theme cards flow from the existing 190px minimum without a persistent right pane. Selecting any current or alternate theme opens a right-to-left preview drawer with the six governed assets in a two-column, orientation-aware bento. The catalog remains mounted behind it, so Back, drawer Close, mask dismissal, or Escape clears the unapplied preview selection and its `Previewing` badge while preserving search, the Recommended/All filter, catalog scroll position, keyboard focus, and already-rendered visible card previews without regeneration. Current versus Previewing remains explicit while the drawer is open; Apply actions appear only for an unapplied drawer theme. Close exits the complete theme flow. |
| Preview fidelity and load | Desktop and mobile catalog cards, Brand Kit bentos, theme-comparison bentos, and selected-asset previews render the canonical PNG from current business/menu/runtime input. The screen copy is capped at a 1200 px long edge while downloadable output stays full resolution. Portrait, landscape, square, two-face, and full-page files remain fully visible with contained fitting and no synthetic QR/text/artwork. Off-screen catalog cards do not render until near the viewport and unload after leaving that window; no more than two jobs render concurrently; repeated identical inputs reuse the bounded 12-result cache; Blob URLs are revoked on off-screen unload, replacement, or unmount. Complete Menu Kit is labelled as a set, not shown as a fake single output. |
...
| Theme library | Change brand look opens a full-viewport searchable library with only the modal title as its heading. The fixed header/footer frame stays visible around the independently scrollable full-width catalog, and underlying dashboard controls cannot cover it. Recommended shows only the active/recommended eligible subset; All themes shows the full business-eligible set. Clicking a theme opens a higher-layer right-side drawer using the same six representative assets as the Brand Kit home, without unmounting or hiding the catalog. The drawer bento gives Print Menu and Entrance Poster the taller cells and supports contained portrait, landscape, square, and paired output. Every theme preview occupies the full card width and its centered name sits below without horizontal competition. The applied family keeps a green Current badge, border, and state tint through hover; a different pending card keeps a light-blue border and tint through hover; neutral cards use a restrained light hover border and surface shade. A pending theme cannot impersonate applied state. The drawer footer identifies the real menu in its primary Apply action, exposes Apply to all menus, and keeps Back to themes independently available; the modal footer retains menu context, Close, and return-to-business behavior. Back, drawer Close, and Escape close only the drawer and restore focus to the originating theme card without regenerating the visible catalog previews. While a preference write is pending, drawer and modal dismissal are unavailable; they recover together after success or failure. |
