...
| Check | Expected |
| --- | --- |
| `node scripts/verification/verify-printable-asset-templates.js` | Verifies 9 families, required asset support, no hardcoded sample output text, QR safety constants, and nav route contract. |
...
| Template count | 9 template families appear for supported asset types. |
| Preview | Preview uses real store/logo/color/URL. |
...
- Do not add Firestore writes for template selection.
