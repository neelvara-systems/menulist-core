...
`MobileOfficialPageScreen` uses the same `updateStore()`, `uploadOBPCover()`, `uploadOBPPhoto()`, and `deleteOBPPhotos()` paths as desktop-backed OBP settings. Standalone mobile, desktop, and the embedded B2C editor use the shared coordinate and public-link validators before persistence. Saves must require `assertStoreUpdateSucceeded()` before photo cleanup, saved baselines, or success copy. New uploads enter the cleanup-candidate queue immediately; cleanup receives the just-saved `publicPresence` references and excludes retained cover/gallery URLs, including duplicate/re-added prepared-media URLs. Reset/normal unmount removes abandoned uploads, while failed deletes remain queued. An abrupt close is recovered by a bounded browser-local tenant/store cleanup journal: the next exact-scope mount retries only media absent from committed cover/gallery references. Failed saves restore both optimistic `publicPresence` and `businessCopyMeta`, log `mobile_official_page_save_failed` with bounded store, tenant, localized-language count, photo count, delete-queue count, cover presence, and special-note presence metadata, and show fixed owner-facing copy. Mobile Basic Settings likewise rolls back the complete optimistic phone tuple (`countryCode`, `dialCode`, canonical `phone`, and local `phoneNumber`) instead of leaving partially updated contact state after a rejected store write.

`MobileAdvancedSettingsScreen` validates each edited social link and the complete social map through `ownerSocialMediaBoundary.ts` before calling its existing `updateStore()` path. Invalid or reserved entries remain in the local draft for correction and show fixed owner copy; they do not write. Successful social changes preserve the existing one-store-write behavior, acknowledged settlement, optimistic screen state, and retryable failure handling.
...
- **Abrupt-close recovery:** Upload one unsaved cover and one unsaved gallery photo, close the tab, then reopen the same tenant/store editor. Both unreferenced Storage objects are removed. A saved cover survives close/reopen, and Remove + acknowledged Save deletes it. Other tenant/store journals remain untouched.
