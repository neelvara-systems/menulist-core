...
`deleteOBPPhotos()` keeps cleanup best-effort after the related store save succeeds. New uploads and replaced/removed objects enter the same set of retryable cleanup candidates. Every desktop/mobile/embedded caller passes final saved `publicPresence` references; the helper deduplicates the queue and excludes any URL still used by `businessCover` or `photos[]`. Reset, store switch, or normal unmount can remove abandoned immediate uploads without deleting committed media. Because an abrupt tab close cannot reliably finish asynchronous unmount work, Mobile Official Page also journals returned immutable media URLs synchronously in a bounded tenant/store-scoped browser entry before applying them to draft state. The next mount for that exact scope retries only URLs not referenced by committed `publicPresence`, acknowledges failures back to the journal, and removes the entry after successful cleanup. The helper returns only failed URLs so callers retain them for retry. Failed cleanup uses bounded Storage diagnostics with counts and URL length metadata only; raw Storage URLs and provider errors are not direct-console logged.
...
Owner social-link write admission uses `src/lib/obp/ownerSocialMediaBoundary.ts` before the existing store update on both desktop Business Settings and Mobile More > Social Media. Known Facebook, Instagram, X/Twitter, LinkedIn, and YouTube fields use the same HTTPS and matching-host contract as public rendering; owner-entered handles are canonicalized to admitted HTTPS profile URLs. Custom platform links accept generic public HTTPS URLs. Blank entries are omitted so the nested-difference writer can remove prior values, while malformed, insecure, credentialed, oversized, wrong-host, duplicate-normalized, reserved WhatsApp, or invalid-key entries stop the complete save before any write. Desktop default/custom inputs, clear actions, and custom removal are named; custom rows retain stable internal render identity while their owner-facing label changes so keyboard focus survives each character. Desktop draft changes use functional state updates so rapid multi-field edits and custom-row removal cannot restore stale link values before the single parent save. Mobile open, edit, remove, editor-close, name, and link controls are named, and icon actions retain 44px minimum targets. Source gate: `npm run verify:official-business-page-boundary`.
...
Desktop Business Settings uses a component-bound Ant message context and shows
`Business settings saved` only after the existing acknowledged store/create
flow completes and the initiating tenant/store scope is still active. The same
mounted context carries validation feedback; validation returns and rejected
writes do not show success. This confirmation adds no Firebase operation or
cache path.
...
Mobile Official Page additionally persists a browser-local cleanup journal under `menulist:obp-media-cleanup:v1:{tenantId}:{storeId}`. Scope identifiers must match the bounded document-ID-safe character contract, entries are deduplicated, URLs are length-bounded, and at most 48 candidates are retained. A mount reads only its exact scope, filters candidates against the current committed cover/gallery references, and retries Storage deletion. An acknowledged save or cleanup replaces the journal with only failed candidates; an empty result removes it. This recovery adds no Firestore document, listener, Function, rule, index, or cache path and never treats a different tenant/store journal as eligible work.
