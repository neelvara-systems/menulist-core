...
**Public-page icon style:** Desktop registers the existing `publicPresence.iconVariant` value with the parent Business Settings form, and mobile persists the same value through its existing OBP save. Switching between `icons` and `emoji` therefore uses the existing store update only. Accessible naming and the public render projection add no read, write, listener, index, rule, Storage operation, Cloud Function, or deploy requirement.

**Action and policy visibility:** The seven customer-action flags and three policy-link flags remain leaves of the existing `publicPresence` map. Desktop can save the complete set in one existing store update; mobile uses the same values and save path. Accessible names add no read, write, listener, index, rule, Storage operation, Cloud Function, or deploy requirement.
...
| --------- | ---------- | ------- | --------- | ------------ | --------- | ---------------------------------------------------------------------- |
| Delete replaced or abandoned OBP cover/gallery object | Firebase Storage | Owner saves after removing/replacing media, resets/navigates, or next opens the exact tenant/store editor after an abrupt close | Rare | 1 object per unreferenced URL | Hard | Store update succeeds first for committed replacements/removals. Mobile immediate-upload URLs are also stored in a bounded browser-local tenant/store cleanup journal so a later exact-scope mount can recover from an interrupted tab close. Candidates are deduplicated and filtered against committed `businessCover` and `photos[]`; a still-referenced prepared-media URL is never deleted. Failed object cleanup is retained for retry and does not roll back saved `publicPresence`. |
...
- **Interrupted-upload recovery:** Mobile Official Page journals at most 48 immutable media URLs in tenant/store-scoped browser storage before draft admission. A later exact-scope mount deletes only candidates absent from committed cover/gallery references. The journal adds zero Firestore reads/writes, Function invocations, listeners, or cloud records; it trades one eventual Storage delete per genuine orphan for bounded retained bytes instead of indefinite orphan accumulation.
...
Failed best-effort cleanup deletes in `deleteOBPPhotos()` are diagnostics-only. Every immediate upload and replaced/removed URL is a cleanup candidate until acknowledged store truth retains it. Every caller supplies final saved cover/gallery references, so duplicate or re-added URLs are filtered before deletion; reset/navigation can remove abandoned uploads. Mobile additionally persists returned immediate-upload URLs in a browser-local tenant/store journal before draft state is applied, because asynchronous unmount cleanup cannot be guaranteed during abrupt tab termination. The next exact-scope mount retries only unreferenced candidates and clears or rewrites the journal from the deletion result. The helper returns failed URLs for bounded retry instead of forgetting them. Failures use `storage_obp_photo_batch_delete_failed` with bounded delete counts and URL length metadata, and they do not roll back the saved store update.
