...
**OBP dashboard summary-read diagnostics:** The Owner Dashboard OBP overview reads daily OBP docs for visible status and uses the optional `_obp_overall_summary` doc only for `viewsChange`. If that optional summary read fails, the dashboard keeps the existing daily-doc overview, returns `viewsChange: null`, and logs bounded `owner_dashboard_obp_summary_read_failed` diagnostics only. This adds no fallback Firestore read, write, delete, Storage operation, Cloud Function, API route, cache invalidation, rule, index, or deploy requirement.
