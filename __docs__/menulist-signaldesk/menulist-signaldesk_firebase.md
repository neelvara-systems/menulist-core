...
| --- | --- | --- |
| Default seed | Preview-CTA identity convergence runs first. The foundation transaction then reads 54 documents (current CTA plus 53 unique business defaults) and writes at most 53 business rows plus one audit, one deterministic timeline, and one daily-cost merge. A clean replay performs no foundation write or side effect. The separate default-content-source transaction reads source, distribution pause, and current pod, then creates at most one source plus its cost merge. | Admin-only action; 18 account/use provider rows produce 17 deduplicated provider budgets. Missing rows use transaction `create`; valid existing operational truth is preserved; malformed or wrong-product collisions fail closed; only exact historical route/pod/Offer shapes migrate. No sender-domain sentinel is created, and content-source activation requires strict current founder-approved pod authority. |
