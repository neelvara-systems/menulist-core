...
| AI assist | Checks AI-worker pause, writes AI worker run, decision snapshot, AI operation ledger, model eval, run timeline, provider spend, audit event, and cost summary only after provider output parses as object-shaped JSON. | AI route must be active, Gemini-backed, and within provider/budget policy; no AI calls from dashboard/list/mobile views. Malformed or non-object provider output logs bounded parse/shape diagnostics and does not write AI result ledgers. |
