...
| SD-I016 | Implement assisted channel and provider-send plumbing | Codex | Done | Email-approved drafts can create email handoffs; WhatsApp/Instagram contact and window plumbing remains gated until same-channel draft/approval creation exists, Messenger outbound is unsupported, and provider send remains disabled until send readiness is approved |
...
| SD-I054 | Implement and harden bounded Revenue Operating Layer | Codex | Done | Added `/signaldesk/revenue`, then revalidated layered flags, current source/contact authority, activation-only wins, strict document projection, founder-only envelopes, duplicate rejection, role-bounded reads, exact-retry no-op behavior, isolated E2E, and current docs; provider send remains disabled |
...
| SD-I058 | Run social-evidence-informed Bengaluru activation trial | Founder + Codex | In progress | Exact zero-external-spend plan approved: all 25 public evidence-only candidate reviews and 12 held documentation-level evidence packets are prepared; all 12 packet sources were rechecked on August 10, remain held with no route, and expire after September 9; runtime entry, permissioned introductions, 5 private previews, 3 activations, 1 proof asset, and the partner referral test remain pending; no business has been contacted |
...
| SD-I066 | Reconcile published authority and natural proof expiry | Codex | Done locally | Added the hourly leased proof-permission lifecycle, product-scoped bounded queries, durable retry/backoff/failure evidence, strict dependency/marker validation, published-truth removal-review incidents, app/scheduler token ownership, exact counters, three composite indexes, and focused emulator regressions. Functions/index deploy remains pending authorized SignalDesk Firebase access. |
| SD-I067 | Close final local authority, replay, and owner-control regressions | Codex | Done locally | Full authenticated emulator E2E now passes after exact CTA/sender replay binding, bounded concurrent AI admission, terminal proof/content lifecycle enforcement, published-authority incident resolution, pending-mission critical-reply priority, exact non-secret coordination-token classification, and stale fixture correction. Static verifier, semantic rules, route/API smoke, TypeScript, emulator parse, and diff hygiene all pass; no deploy, real send, paid API call, public route, or MenuList runtime change occurred. |
| SD-I068 | Revalidate and harden AI Intelligence end to end | Codex | Done locally | Prevented mixed-run workspace starvation with separate bounded indexed queries; constrained UI admission to eligible targets and active Gemini routes; added explicit prompt-injection boundaries and output caps; made exact shadow-review replay write-free; rebuilt all seven AI Intelligence docs; and passed focused runtime, accounting, workspace, type, lint, link, and diff gates. The required QA index deploy remains blocked by Firebase CLI authentication. |
| SD-I069 | Revalidate and harden Evidence Packets end to end | Codex | Done locally | Added suppression-sensitive packet identity, truthful hold progression for non-draftable authority, policy-aware desktop admission, and independent paired expiry for historical packets after target refresh; rebuilt all seven docs and added focused E2E/lifecycle coverage. SignalDesk QA index/Function deployment remains pending authenticated Firebase CLI access. |
| SD-I070 | Revalidate and harden Draft Control end to end | Codex | Done locally | Enforced current evidence identity, email-only templates, supported declared variables, deterministic prohibited-claim rejection, exact template fingerprints, approval-time template revalidation, typed API admission, and policy/evidence/template/sender/CTA-aware desktop controls; rebuilt all seven docs and added a focused emulator gate. Provider send remains disabled. |
| SD-I071 | Revalidate and harden Approval Queue end to end | Codex | Done locally | Added pending-first bounded reads, exact same-actor terminal replay, strict permission and packet-readiness UI admission, typed API payloads, current authority documentation, and focused emulator coverage. Provider send remains disabled. |
| SD-I072 | Implement the daily activation operator loop | Codex | Done locally | Added activation-first Today ordering, safe resolved-work filtering and target dedupe, read-only journey, anonymous observer-only MenuList setup handoff, permission-aware Content prefill, seven-day outcome measurement, docs parity, and a focused permanent gate. No provider send, route-token activation, auto-publish, MenuList truth write, or Firebase infrastructure change. |
