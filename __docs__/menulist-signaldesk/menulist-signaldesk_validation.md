...
| Model cascade | Score, evidence, draft, and reply-classification use `gemini-3.5-flash-lite`; an independent critic checks every child; only exceptions may escalate through the same executable Gemini adapter to `gemini-3.6-flash`. Exact untouched retired routes migrate, while unsupported owner-modified Gemini routes stop for review before provider work. OpenAI and Anthropic remain non-executable policy records. |
...
| Local SignalDesk Firebase mode/env | Local ignored MenuList env files are sanitized to the current `menulist-qa` project contract and must never point to a retired Firebase project. SignalDesk still needs its own `SIGNALDESK_FIREBASE_MODE` / project override before cloud smoke. Local emulator data-flow smoke now runs through `scripts/verification/smoke-signaldesk-workflow.js` with `FIRESTORE_EMULATOR_HOST`; browser-auth and role-negative coverage still require a seeded SignalDesk session/Auth emulator. |
