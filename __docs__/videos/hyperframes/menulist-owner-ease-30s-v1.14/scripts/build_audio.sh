...
SOURCE="$PROJECT_DIR/assets/music/source/menulist-one-link-motion-v2-seed-260719.wav"
STING="$PROJECT_DIR/../../brand-audio/sfx/menulist-approval-sting-original-v1.wav"
WORK_DIR="$PROJECT_DIR/renders/work-audio"
BED="$WORK_DIR/menulist-one-link-motion-v2-production-bed.wav"
DUCKED="$WORK_DIR/menulist-one-link-motion-v2-ducked.wav"
PREMASTER="$WORK_DIR/owner-ease-30s-v1.14-premaster.wav"
PASS1="$WORK_DIR/owner-ease-30s-v1.14-loudnorm-pass1.json"
PASS1_LOG="$WORK_DIR/owner-ease-30s-v1.14-loudnorm-pass1.log"
MASTER="$PROJECT_DIR/assets/mix/owner-ease-30s-v1.14-master.wav"
...
# Preserve the source's gradual build while clearing narration frequencies.
# The final ramp restores musical energy after Tara finishes speaking.
ffmpeg -hide_banner -loglevel error -y \
  -i "$SOURCE" \
  -filter_complex "[0:a]atrim=0:30,asetpts=PTS-STARTPTS,highpass=f=55,lowpass=f=15000,equalizer=f=1500:t=q:w=0.9:g=-3.0,equalizer=f=3400:t=q:w=1.1:g=-1.4,equalizer=f=7200:t=q:w=0.8:g=-1.3,volume='0.50+0.18*(t/30)+1.00*max(0,min(1,(t-26.7)/2.3))':eval=frame,afade=t=in:st=0:d=0.55,afade=t=out:st=29.68:d=0.32,alimiter=limit=0.84:attack=5:release=100,atrim=0:30[bed]" \
  -map "[bed]" -ar 48000 -ac 2 -c:a pcm_s24le "$BED"

ffmpeg -hide_banner -loglevel error -y \
  -i "$BED" \
  -i "$VOICE" \
  -filter_complex "[1:a]adelay=220|220,apad,atrim=0:30[voice];[0:a][voice]sidechaincompress=threshold=0.037:ratio=3.0:attack=30:release=390[ducked]" \
...
printf 'Created v1.14 audio master: %s\n' "$MASTER"
