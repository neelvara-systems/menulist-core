...
Desktop Assets adds template-family actions on top of readiness, print-shop handoff, preview, and reprint guidance. Clicking a template opens a modal for that exact template, shows the generated output preview inside the modal, and offers separate PDF/image download actions. The dashboard and modal use `businessProfile.ts` plus the shared `AssetBusinessProfileEditor` to expose applicable-field readiness and edit canonical store/tenant settings in place. General assets use the core identity set; Business Card and the dashboard additionally cover public contact fields. A successful save updates shared context and rebuilds the active preview from the acknowledged values without a Settings detour or asset-local copy. Table Tent, Single Card, Counter Sticker, Entrance Poster, Feedback QR, Flyer, Gift Certificate, Business Card, ID Card, Invitation, Postcard, Product Tag, and Campaign Poster use editor-backed `CreativeEditorDocument` templates for preview/download and can open **Customize in editor** as a fullscreen governed editor. Business Card PDF renders front and back faces together for print handoff; Business Card image download renders two separate PNG files, one front and one back. Print Menu renders the generated menu PDF first page as the image preview and inherits category icon data, visibility, Lucide theme colour, native emoji appearance, and source-hash freshness from Menu Card Export. Preview, edited documents, and downloads create temporary browser blobs without uploading output.
...
`MobileMoreScreen` renders `MobilePrintAssetsScreen`, which reuses `MobileShareScreen` in focused `printAssets` mode. This preserves existing mobile project selection and download handlers. Template rows open an in-shell bottom sheet with the preview already visible and separate PDF/image download actions. The same shared profile editor opens as a nested in-shell sheet, preserves permission and dirty-close boundaries, writes canonical settings, and immediately refreshes an open preview. Individual file downloads use the shared printable renderer, matching desktop output without generating the full ZIP first unless the owner chooses Complete Menu Kit. Mobile does not expose drag/resize editing, but it uses the same editor-backed document renderer for supported single assets, including campaign flyers, gift certificates, front/back business cards, ID cards, invitations, postcards, product tags, and campaign posters. Business Card image download also downloads both front and back PNG files on mobile.
