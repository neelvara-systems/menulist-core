...
|------------|--------|-------|
| `canonica_entity_search_index` | READ (existing) | Already loaded for query matching; context matching reuses same data |
| `canonica_canonical_answers` | READ (existing) | Already fetched for top entities; context doesn't add reads |
| `canonica_releases` | READ (existing) | Already fetched for version; unchanged |
...
| Context tokenization | <0.5ms | Reuses `canonicaTokenize()` |
