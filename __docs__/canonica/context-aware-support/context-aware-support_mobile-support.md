...
|------|----------|--------|-------|
| **Frequency** | Daily/multiple times per day? | No — context-aware support is an infrastructure enhancement to the retrieval pipeline. End-users interact via the widget (which is embedded in the CLIENT's product, not in Canonica's dashboard). SaaS founders configure context in product code, not via mobile UI. | ❌ |
...
The only "UI" for this feature is the **widget context integration code** that Canonica clients write in their own products — which is a developer activity, not a mobile task.
