...
- `pId` = 2-char uppercase product code (ML, CN, SF, GR, VM)
...
## 4. Canonica Integration (CCT Pattern)

Any product using Canonica follows the same flow:

1. Register as Canonica client (get `clientId`, `secretKey`, `tId`, `sId`)
2. Generate signed JWT (CCT) containing: `clientId`, `traceId`, `requestId`, user identity, optional source scope
3. Canonica decodes CCT → builds `CanonicaPlatformContext` → writes documents
...
9. Register as Canonica client if using Canonica
