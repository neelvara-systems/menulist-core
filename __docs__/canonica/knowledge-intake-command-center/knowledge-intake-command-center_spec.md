...
Build Canonica's long-term intake experience for solo founders, first-time SaaS owners, AI-native builders, and small product teams who need a support layer before they have a support team.

The owner should experience this as:

1. Pay for a Canonica workspace.
...
4. Canonica shows what it found and what it trusts.
5. Canonica builds a product map and asks only important launch decisions.
6. Canonica generates source-backed drafts.
7. Owner approves what becomes official.
8. Canonica publishes KB, FAQ, approved answers, product page help, and readiness summaries.
...
- studios shipping multiple small SaaS products, one Canonica workspace per product
...
Current Canonica import is mounted through `/canonica/kb-generation`, which reuses the shared `platform/KBGeneration` component. It supports file upload, pasted URL text, starter text templates, AI-generated articles, article review, duplicate reconciliation, publishing, generated FAQs, and embeddings.
...
- app URL is not distinguished from a public website/docs URL; Canonica must not attempt credentialed app crawling
...
3. Canonica runs a cheap discovery pass using sitemap, robots, canonical links, llms.txt, and selected public pages.
4. Canonica shows candidate pages grouped as product overview, pricing, docs, FAQ, changelog, legal/security, API, and low-value pages.
...
- if unchanged, Canonica updates freshness metadata only and skips AI/provider work

The first screen should say:

**Paste your product link. Canonica finds the support-worthy pages. You choose what becomes source material.**
...
If sources conflict, Canonica creates a review item instead of guessing.

Example:

Pricing page says API access is Enterprise-only. Old sales deck says API access is included in Pro. Canonica recommends the pricing page as higher authority and asks the owner to confirm.
...
- user has access to the Canonica workspace
...
Before processing, Canonica shows:
...
Before any provider prompt is created, normalized source material must pass a secret/private-data classification and redaction step. If risky content remains, Canonica creates a review item instead of sending that material to drafting prompts by default.

### 9.4 Source Audit

After normalization, Canonica shows:
...
**Canonica found 28 product concepts, 6 workflows, 4 plans, and 9 launch decisions.**
...
After audit and product map, Canonica generates:
...
Publishing is not complete when draft documents are written. Publishing is complete only when the approved output is visible to the existing Canonica runtime paths that already serve owners and end users.
...
- No generic CMS/importer detached from Canonica support outputs.
...
- publishing uses existing Canonica destination systems instead of duplicate content models
