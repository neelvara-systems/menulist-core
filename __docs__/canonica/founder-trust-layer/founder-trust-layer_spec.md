...
SaaS founders who deploy Canonica need to know **one thing**: Is the AI answering users correctly?
...
- **Source:** `aiSearchHistory.confidence` + `canonica_signalEvents` (type = 'escalation')
- **Healthy:** ≥ 85%
- **Attention:** 70-84%
- **Critical:** < 70%
...
- **Source:** `canonica_canonicalAnswers` where `governance.driftFlag === true`
- **Healthy:** ≤ 5%
- **Attention:** 6-15%
- **Critical:** > 15%
...
**Data source:** Derived from `aiSearchHistory.matchedEntityIds` + `confidence` + `canonica_signalEvents`
