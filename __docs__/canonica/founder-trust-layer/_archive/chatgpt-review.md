...
| "5 event types (QUERY_RECEIVED, ANSWER_RENDERED, FOLLOWUP_QUERY, ESCALATION_TRIGGERED, RESOLUTION_CONFIRMED)" | ❌ Over-engineering — Canonica doesn't need lifecycle events per query. `aiSearchHistory` + `signalEvents` capture everything | **REJECT** |
...
| "SHA256 hash for privacy" | ❌ Wrong for Canonica — raw queries needed for debugging, entity resolution improvement | **REJECT** |
...
| ChatGPT Claim | Cascade Assessment | Verdict |
|---|---|---|
| "6-hourly Cloud Function" | ❌ Over-engineering — Canonica runs nightly batch (3 AM UTC). 6-hourly adds complexity for minimal benefit | **REJECT** |
...
| "Convert high-volume events into small deterministic metrics" | ✅ Correct principle — matches Canonica's nightly batch philosophy | **ACCEPT** |
...
**ChatGPT accuracy for 10.2: ~40%** — Core principle correct, implementation details wrong for Canonica.
...
| "Failure signals: escalation, entityMismatch, lowConfidence, followUp" | ✅ Valid — maps to existing Canonica signals + retrieval results | **ACCEPT** |
...
| "30-day rolling window" | ❌ Different — Canonica uses 14-day signal window (doctrine). Trust metrics use 24h for freshness | **ADAPT** |
...
| ChatGPT Claim | Cascade Assessment | Verdict |
|---|---|---|
| "5 categories (KNOWLEDGE_GAP, LOW_CONFIDENCE, ENTITY_MISMATCH, RETRIEVAL_FAILURE, USER_REQUESTED_HUMAN)" | ✅ Valid and implementable — maps cleanly to existing Canonica data | **ACCEPT** |
| "Deterministic classification logic" | ✅ Matches Canonica philosophy (rule-driven, not ML) | **ACCEPT** |
...
| ChatGPT Claim | Cascade Assessment | Verdict |
|---|---|---|
| "Raw events TTL 7-10 days" | ❌ Wrong for Canonica — `aiSearchHistory` is valuable long-term. `signalEvents` has 12-month TTL | **REJECT** |
| "Aggregated metrics stored permanently" | ✅ Valid — `platformSummary/trustMetrics_*` is permanent | **ACCEPT** |
| "BigQuery export for archival" | ❌ Overkill — not needed at Canonica's scale | **REJECT** |
| "Dashboard reads only aggregated data" | ✅ Correct — 1 read per dashboard view | **ACCEPT** |
| "System health self-monitoring" | ✅ Valid concept — already logged in `CanonicaNightlyResult` | **ALREADY EXISTS** |
| "Cost projection $20-50/month at 1M queries" | ❌ Irrelevant — Canonica's implementation costs ~$0.09/month at 1000 tenants | **WRONG SCALE** |
...
3. **Escalation classification:** 5 categories map cleanly to Canonica's retrieval pipeline
...
1. **6 new collections** → 0 new collections needed (Canonica already has the infrastructure)
2. **`support_events` parallel stream** → `aiSearchHistory` already captures this
3. **6-hourly aggregation** → Canonica is nightly-batch architecture
4. **Query hashing** → Loses diagnostic value, not needed for privacy at Canonica's scale
5. **BigQuery archival** → Overkill
6. **`workspaceId` terminology** → Canonica uses `tId`/`sId`
...
| Source | Key Finding | Canonica Alignment |
|---|---|---|
| **Intercom Fin** | 3 KPIs: Automation Rate, Involvement Rate, Resolution Rate | ✅ Canonica has 4 (adds Drift + Entity Health) |
