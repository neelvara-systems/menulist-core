...
### 1.1 — System Position Inside Canonica
...
```typescript
// Additive to src/types/canonica/index.ts
...
export type CanonicaTriggerActionType =
  (typeof CANONICA_TRIGGER_ACTION_TYPES)[keyof typeof CANONICA_TRIGGER_ACTION_TYPES];
...
export type CanonicaTriggerStatus =
  (typeof CANONICA_TRIGGER_STATUS)[keyof typeof CANONICA_TRIGGER_STATUS];
...
export type CanonicaTriggerSource =
  (typeof CANONICA_TRIGGER_SOURCE)[keyof typeof CANONICA_TRIGGER_SOURCE];
...
export interface CanonicaPredictiveSuggestion {
  ...
  procedure?: CanonicaProcedure; // If workflow_guide type
...
Extend existing `CANONICA_SIGNAL_TYPE` (additive, freeze-compliant):

```typescript
// Add to CANONICA_SIGNAL_TYPE
...
metadata: {
    ...
    page: string;
    entityId?: string;
    actionType: CanonicaTriggerActionType;
}
```
...
**Collection:** `canonica_predictiveTriggers`
...
**Location:** Existing `canonica_signalEvents` collection
...
```
src/types/canonica/index.ts                          — +3 interfaces, +3 const objects (additive)
src/lib/canonica/predictiveEngine.ts                 — Core evaluation engine (~250 lines)
src/database/canonica/predictiveTriggers.ts           — DAL for trigger CRUD (~150 lines)
src/app/api/canonica/predictive-help/route.ts        — API route (~120 lines)
src/hooks/canonica/usePredictiveTriggers.ts          — Admin hook for trigger management (~100 lines)
src/components/templates/canonica/governance/PredictiveTriggerManager.tsx  — Admin UI (~200 lines)
functions-canonica/src/canonica/predictiveTriggerSync.ts  — Nightly: auto-gen + cache rebuild (~200 lines)
```
...
functions-canonica/src/constants/features.ts          — +1 feature flag
src/constants/database.ts                            — +1 collection constant
functions-canonica/src/constants/database.ts          — +1 collection constant
src/types/canonica/index.ts                          — +signal types, +trigger types
src/lib/canonica/signalEmitter.ts                    — +emitSuggestionSignal helper
functions-canonica/src/canonica/canonicaNightly.ts   — +Step 16 (trigger sync + auto-gen)
src/components/templates/canonica/governance/index.tsx — +Triggers tab
```
...
### 6.1 — POST /api/canonica/predictive-help
...
Added to `canonicaNightly.ts` after existing 15 steps.
...
1. POST to `/api/canonica/predictive-help` with context
...
**Decision:** Reuse existing `canonica_signalEvents` with new signal types.
