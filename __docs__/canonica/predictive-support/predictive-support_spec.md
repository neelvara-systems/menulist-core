...
| **SaaS founder** (Canonica tenant) | Reduced support volume. Better product experience. Self-improving help. |
...
**Already exists:** `CanonicaFrictionSnapshot` computed nightly with:
...
```
1. Founder opens Canonica Governance Dashboard → Trigger Rules tab
...
|-------|-------|-------------|
| Phase 1 | Manual trigger rules + API + widget rendering | `ENABLE_CANONICA_PREDICTIVE_SUPPORT` |
| Phase 2 | Auto-trigger generation from friction patterns | Same flag + nightly batch extension |
| Phase 3 | Learning loop (effectiveness scoring + auto-adjust) | Same flag + `ENABLE_CANONICA_PREDICTION_LEARNING` |
