...
Predictive Support is the **proactive help layer** for Canonica. Instead of waiting for users to ask questions, the system detects where the user is in the product, evaluates friction patterns for that location, and surfaces contextual help *before* confusion turns into a support ticket.
...
**Key design choice:** Reuses existing Canonica infrastructure (context payload, friction stats, entity index, canonical answers, Upstash Redis). Only **1 new collection** + **1 platformSummary doc** + **1 API route** + **1 lib module** needed.
...
| [predictive-support_helpdoc.md](./predictive-support_helpdoc.md) | Customers | Help documentation for SaaS founders using Canonica |
...
| Architecture (Pub/Sub + Cloud Run) | 20% | Massive over-engineering for Canonica's scale |
...
- **Depends on #1** (Context-Aware) — CanonicaContextPayload provides page/feature/workflow. ✅ DONE
