...
SaaS founders using Canonica receive support signals (tickets, negative chat feedback, escalations) but have **no way to see WHERE their product is failing users**. They can see individual tickets and mutation proposals, but cannot answer:
...
**Input:** `canonica_signalEvents` (existing)
**Output:** `canonica_frictionDailyStats` (new collection)
...
**Input:** `canonica_frictionDailyStats` (last 7 days + previous 7 days)
...
- Page context correlation (requires `ENABLE_CANONICA_CONTEXT_AWARE` + sufficient data)
