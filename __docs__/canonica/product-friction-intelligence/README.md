...
Product Friction Intelligence converts Canonica's existing support signals into actionable product friction insights for SaaS founders. It answers: **"Where is my product confusing users?"**
...
|----------|--------|-----------|
| Topic taxonomy | Reuse entity graph (entities ARE topics) | Canonica already has 7 entity types. No separate clustering needed. |
| New collections | 1 only: `canonica_frictionDailyStats` | Insights stored in existing `platformSummary`. Minimal Firestore footprint. |
| Clustering method | Entity-based (NOT embedding-based) | Canonica doctrine: deterministic > LLM. Entity graph is the taxonomy. |
| Processing cadence | Nightly batch (extends canonicaNightly.ts) | Industry standard: Intercom uses weekly, we use nightly for faster signals. |
...
- **Required:** `ENABLE_CANONICA_SIGNAL_MUTATION: true` (signal events must be flowing)
...
Per Canonica doctrine and this feature's scope:
