...
- With `ENABLE_CANONICA_SUPPORT_BOARD_SOURCE_SYNC=false`, ticket/signal sync CTAs are hidden and hook sync calls no-op.
- With `ENABLE_CANONICA_SUPPORT_BOARD_SOURCE_SYNC=true`, source sync CTAs are visible.
...
- With `ENABLE_CANONICA_SUPPORT_BOARD_SYNC=false`, nightly Support Board sync returns without source reads or writes.
- With `ENABLE_CANONICA_SUPPORT_BOARD_NIGHTLY_SUMMARY=false`, board refresh does not read `supportBoardSummary_*`.
- With `ENABLE_CANONICA_SUPPORT_BOARD_SYNC=true`, nightly Support Board sync runs inside the existing scheduler only.
- Existing Canonica nightly scheduler calls Support Board sync; no standalone scheduler exists.
