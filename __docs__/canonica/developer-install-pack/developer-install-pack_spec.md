...
- No supported SDK or public npm package promise. Canonica client installs use the v1 script and `window.CanonicaWidget` browser contract directly.
...
- Generated prompts must not ask owners to manually maintain `ALLOWED_ORIGINS` or `BLOCKED_ROUTES` variables. Canonica dashboard owns those values.
...
- Allowed origins and blocked routes are configured in Canonica dashboard UI, not duplicated as client-product settings.
