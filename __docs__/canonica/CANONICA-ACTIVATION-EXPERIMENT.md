# Canonica — Activation Experiment Framework

> **Purpose:** Define hard success/failure criteria, measurement plan, and activation sequence for Canonica's controlled experiment phase.
> **Created:** 2026-03-15
> **Prerequisite:** All infrastructure audited and cleared (see `canonica-activation-clearance.md`)
...
**Not the goal:** Full Canonica rollout, UI, API, or marketing. This is measurement only.
...
**Flag enabled:** `ENABLE_CANONICA_SIGNAL_MUTATION` only
...
**Flag enabled:** `ENABLE_CANONICA_ONTOLOGY`
...
**Flags enabled:** `ENABLE_CANONICA_CANONICAL_ANSWERS` + `ENABLE_CANONICA_DRIFT_DETECTION`
...
| Signal events         | `canonica_signalEvents` collection      | type, entityId, timestamp, metadata                             |
| Audit logs            | `canonica_auditLogs` collection         | action, entityType, entityId, timestamps                        |
| Mutation proposals    | `canonica_mutationProposals` collection | status transitions (pending → approved/rejected)                |
...
```
canonica_signalEvents WHERE timestamp >= [week_start]
→ Count by type
...
From canonica_canonicalAnswers: count WHERE governance.driftFlag=true
```
...
| ---- | ---------------------------------------------------------------------------------------------- | --------------------------------------------- |
| 1    | Deploy current codebase to production (all flags OFF)                                          | Verify zero Canonica impact on existing flows |
...
| Step | Action                                      | Verification                                                |
| ---- | ------------------------------------------- | ----------------------------------------------------------- |
| 3    | Set `ENABLE_CANONICA_SIGNAL_MUTATION: true` | Verify signals appear in `canonica_signalEvents` within 24h |
...
| Step | Action                                               | Verification                                |
| ---- | ---------------------------------------------------- | ------------------------------------------- |
| 6    | Set `ENABLE_CANONICA_ONTOLOGY: true`                 | No visible change to users                  |
...
| 11   | Create initial release (v1.0.0) for version binding  | Verify release in `canonica_releases`       |

### Week 3: Canonical Retrieval Live

| Step | Action                                        | Verification                                |
| ---- | --------------------------------------------- | ------------------------------------------- |
| 12   | Set `ENABLE_CANONICA_CANONICAL_ANSWERS: true` | Check `CANONICAL_HIT` logs appear within 1h |
| 13   | Set `ENABLE_CANONICA_DRIFT_DETECTION: true`   | Verify nightly job runs at 2:30 AM UTC      |
...
**Kill switch:** Set `ENABLE_CANONICA_CANONICAL_ANSWERS: false` → instant revert to RAG-only.
...
**Conclusion:** Even worst-case, Canonica's Firestore costs are negligible compared to the existing RAG pipeline's embedding API costs.
...
4. **Documentation:** Update `canonica-activation-clearance.md` with experiment results
...
1. **Disable:** Set `ENABLE_CANONICA_CANONICAL_ANSWERS: false`
2. **Keep Signals:** Leave `ENABLE_CANONICA_SIGNAL_MUTATION: true` — signal data is still valuable
3. **Root Cause:** Analyze `CANONICAL_MISS` reasons to understand failure mode
4. **Decide:** Fix and retry OR pivot Canonica to signal-only mode (governance without retrieval)

---

_This framework turns Canonica from "elegant engineering" into a measurable experiment. The infrastructure is ready. The metrics will prove whether it deserves to be a product._
...
**Rule:** Governance must remain active. Without this weekly cycle, Canonica becomes static documentation.
...
**Current tracking:** `aggregateCoverageKPI()` runs nightly, stores in `platformSummary/canonica_{sId}`.
