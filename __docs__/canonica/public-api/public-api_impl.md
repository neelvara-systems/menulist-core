...
| File | Role |
| --- | --- |
| `src/lib/canonica/publicApi.ts` | Shared API-key authentication, rate limiting, schema version, timestamp helpers |
...
4. `validatePublicApiKey()` resolves the hash-only key from `stores.publicApi.apiKeyHash`.
5. The resolved key must be a Canonica key (`productId: "CN"` when present, `purpose` starts with `canonica` when present).
