...
`functions-canonica/src/logic/publishApprovedJob.ts` reads article docs before writes, then:

- publishes approved articles,
- creates or updates `canonica_faqs` docs for reviewed `generatedFaqs`,
...
`/canonica/faqs` uses `CanonicaFaqManagement`.
