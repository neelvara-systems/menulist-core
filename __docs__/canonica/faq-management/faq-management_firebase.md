...
- Browser calls `/api/canonica/public-content?type=faqs`.
...
- FAQ save/archive and article-driven FAQ status changes invalidate `canonica-public-faqs-{tId}-{sId}` and shared Canonica public tags.
- No realtime listener.
...
- One Canonica public cache tag revalidation for FAQ/KB/context output.
...
Canonica FAQ indexes exist in both shared and Canonica-specific index files for:
