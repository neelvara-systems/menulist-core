...
- **Types:** Extended `CanonicaMutationProposal` type — shared
- **Feature flags:** `ENABLE_CANONICA_AUTO_KNOWLEDGE` — shared
- **Signal emitter:** `emitCanonicaSignal()` — already works on mobile (signals generated from mobile chat/feedback feed into this pipeline)
