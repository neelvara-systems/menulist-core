...
Canonica monitors your support signals — tickets, chat feedback, and escalations — to detect topics where your documentation is missing. When a knowledge gap is confirmed, Canonica generates a draft canonical answer for you to review and publish.
...
Every time a user submits a ticket, gives negative feedback on a chat answer, or escalates to a human agent, Canonica records a signal. These signals are grouped by product entity (the specific feature, workflow, or concept involved).

### 2. Knowledge Gaps Are Detected
Each night, Canonica checks whether any product entity has accumulated support signals but has no canonical answer. If an entity has 5 or more signals without documentation, it's flagged as a knowledge gap.

### 3. A Draft Is Generated
For each confirmed knowledge gap, Canonica generates a structured draft answer using:
...
1. Go to your **Canonica Dashboard**
...
Yes. If you prefer to write all documentation manually, you can disable Automatic Knowledge Creation in your feature settings. Canonica will still detect knowledge gaps — it just won't generate drafts.

### What if I disagree with the draft?
Click "Reject." The proposal is dismissed. Canonica will not regenerate a draft for the same entity unless new signals accumulate.

### Does this cost extra?
No. Automatic Knowledge Creation is included in your Canonica plan. AI generation costs are negligible (<$0.01 per draft).

### How does this relate to my existing articles?
Drafts are for topics where NO canonical answer exists. If you already have documentation covering a topic, Canonica won't generate a duplicate draft. The system checks existing coverage before creating proposals.
