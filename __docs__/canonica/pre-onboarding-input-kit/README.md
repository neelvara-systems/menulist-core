# Canonica Pre-Onboarding Input Kit

> **Feature:** Client-side preparation system for Canonica onboarding inputs
> **Status:** DOCUMENTED + PUBLIC WEBSITE INPUT CREATED
> **Date:** 2026-05-31
> **Audience:** Product, Website, Support, Customer Success, AI coding-agent users
> **Doctrine fit:** Strengthens Knowledge Intake, Product Surfaces, Canonical Answers, and Drift Governance without turning Canonica into a helpdesk or generic documentation CMS.

---

## What This Is

The Pre-Onboarding Input Kit is a reusable preparation stage before a client starts Canonica onboarding.

It gives a SaaS founder or product owner a master prompt they can run inside Codex, Cursor, Windsurf, Antigravity, Claude Code, or another AI coding tool. The agent inspects whatever source bundle the client has: repo/code when available, docs, website links, help content, owner notes, policies, routes, support flows, screenshots, and risk boundaries. It then creates a structured upload folder for Canonica Knowledge Intake.
...
- Not a replacement for Canonica Knowledge Intake.
...
Canonica works best when source input is already organized around product truth:
...
Most SaaS teams already have that information scattered across code, docs, websites, release notes, help centers, and support replies. The pre-onboarding kit turns scattered material into a Canonica-ready source package.
...
When a client gives the AI agent a repo or workspace that contains more than one product, the agent must not prepare one blended Canonica package. It must:
...
The public prompt also accounts for common product categories adjacent to Canonica:
...
Canonica should support these input expectations without copying their publishing model. Pre-onboarding output remains review-ready source material, not an approved public demo, website page, FAQ, or legal answer.
...
Before a client enables live user support in Canonica, account-specific and production-only facts still need review:
...
The Pre-Onboarding Input Kit prepares cleaner input for Canonica Knowledge Intake. It does not bypass Canonica.

Expected flow:

1. Client runs the prompt in their AI IDE.
2. Agent creates a `*-canonica-pre-onboarding-inputs/` folder.
3. Client reviews the folder for private data and correctness.
4. Client uploads selected sources through Canonica Knowledge Intake.
5. Canonica creates review drafts.
...
| Date | Change |
| --- | --- |
| 2026-05-31 | Initial reusable process extracted from the MenuList-on-Canonica onboarding package. |
