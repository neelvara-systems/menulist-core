...
```text
https://canonica.app/pre-onboarding.md
```
...
This internal file exists so the prompt can be reviewed alongside the feature docs. Keep it aligned with `src/lib/canonica/preOnboardingPrompt.ts`.
...
- generate a structured `*-canonica-pre-onboarding-inputs/` folder;
- create 26 source families for Canonica Knowledge Intake, marking unavailable or not-applicable families instead of inventing content;
