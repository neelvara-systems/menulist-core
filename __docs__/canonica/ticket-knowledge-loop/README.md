...
```
Existing Canonica Architecture:
┌──────────────────────────────────────────────────────────┐
...
| Decision                                    | Rationale                                                                                       |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| **Zero new Firestore collections**          | Reuse existing `canonica_mutationProposals` + `canonica_signalEvents` + `canonica_auditLogs`    |
...
| **Suggestion-only, never auto-publish**     | Canonica doctrine: signals propose, humans approve                                              |
...
| Component design         | ⚠️ 40%   | Proposed 9+ new collections, per-ticket processing — both wrong for Canonica       |
| Infrastructure hardening | ⚠️ 30%   | Most "missing" components already exist in Canonica (idempotency, queue, dedup)    |
...
| -------------------------- | ----------------------------------------------------------------------- | ----------- |
| Resolution extractor       | `functions-canonica/src/canonica/resolutionExtractor.ts`                | ✅ NEW      |
| Ticket knowledge prompt    | `functions-canonica/src/canonica/ticketKnowledgePrompt.ts`              | ✅ NEW      |
| Feature flag (frontend)    | `src/config/features.ts`                                                | ✅ MODIFIED |
| Feature flag (CF)          | `functions-canonica/src/constants/features.ts`                          | ✅ MODIFIED |
| Types (additive fields)    | `src/types/canonica/index.ts`                                           | ✅ MODIFIED |
| Signal emitter enhancement | `src/lib/canonica/signalEmitter.ts`                                     | ✅ MODIFIED |
| Nightly batch step         | `functions-canonica/src/canonica/canonicaNightly.ts`                    | ✅ MODIFIED |
| UI wiring                  | `src/components/templates/platform/supportTickets/TicketDetailView.tsx` | ✅ MODIFIED |

## Cross-References

- **Item #4 (Auto Knowledge Creation):** `__docs__/canonica/automatic-knowledge-creation/`
- **Ticket System:** `__docs__/canonica/ticket-system/`
- **AI Escalation (Item #8):** `__docs__/canonica/ai-failure-escalation/`
- **Canonica Doctrine:** `__docs__/canonica/doctrine/`
- **Expansion Tracker:** `__docs__/canonica/canonica-expansion-tracker.md`
