...
> **Date:** 2026-03-09
> **Source:** ChatGPT conversation on System #3 of Canonica ICP Coverage Index
...
ChatGPT provided extensive architectural guidance on building an "Instant Response Infrastructure" for Canonica. The conversation covered 5 capability blocks (cache layer, key strategy, invalidation, pre-cache, widget optimization) with detailed production-scale designs.
...
1. Canonica's existing entity-first retrieval architecture
2. Existing Upstash Redis in the project
3. Existing `aiSearchHistory` and `queryEmbeddings` caching layers
4. Canonica's deterministic tokenizer and search index
5. The `coreSearch()` unified pipeline architecture

This led to proposing redundant systems (Intent Engine, NLP normalization) that Canonica already has in different form.
...
| Tenant-isolated cache | ✅ AGREE | tId + sId in key. Mandatory for Canonica. |
...
| Use intent-based canonicalization | ❌ DISAGREE | Canonica uses ENTITY-based resolution, not intents. Entity resolution already normalizes queries deterministically. |
...
| Question normalization pipeline | ⚠️ PARTIAL | Canonica already has `canonicaTokenize()` in `tokenizer.ts`. No additional NLP needed. |
...
| Response streaming for cached answers | ⚠️ PARTIAL | Streaming was REMOVED from Canonica (see memory: streaming fully removed). Not applicable. |
...
1. **Entity-based keys instead of Intent Engine** — Canonica already resolves queries to entities. No parallel system needed.
...
5. **No streaming** — Was already removed from Canonica. Not relevant.
