...
1. **Help Center Feedback Flow** — selectable category form -> `addFeedback()` -> Canonica `feedback` collection -> optional `feedback` signal event
2. **Owner Feedback Review** — `/canonica/feedback` -> `getFeedbackForWorkspace()` + `getProductSurfacesForSession()` -> scoped review list/detail with optional Product Surface assignment
...
| `src/app/(canonica)/canonica/feedback/page.tsx` | — | Authenticated Canonica owner route for feedback review |
| `src/components/templates/canonica/feedback/CanonicaFeedbackReview.tsx` | — | Workspace-scoped wrapper around the reusable feedback review template |
...
|----------|:-----:|:------:|-------|
| `addFeedback(data)` | 0 | 1-2 | `canonicaRequestBodyComposer` + `addDoc` to Canonica `feedback`; emits one non-blocking `feedback` signal when `ENABLE_CANONICA_SIGNAL_MUTATION` is true |
...
interface Feedback {
    id?: string;
    pId?: string;   // from canonicaRequestBodyComposer
...
ShareFeedbackView → Submit selected category
  ...
  → canonicaRequestBodyComposer (adds pId="CN", tId, sId, uId, sourceContext, traceId, createdOn)
    → addDoc to feedback collection
    → emitCanonicaSignal(type='feedback', entityId='unresolved') [non-blocking]
...
```
/canonica/feedback
  → CanonicaFeedbackReview reads session tId/sId
  ...
  → Optional "Add to Support Board" writes canonica_supportBoardCards(sourceType='feedback', relatedSurfaceId, relatedContextKeys)
```

### 3.3 Feedback To Support Board / Answer Proposal
```
addFeedback()
  → canonica_signalEvents(type='feedback', metadata.feedbackId)
  → Owner can add the feedback row directly to Support Board from /canonica/feedback
...
| 3 | No owner view for reviewing submitted feedback | Medium | — | Resolved by `/canonica/feedback`; `/platform/feedback-admin` remains platform-wide |
...
| 6 | Unresolved feedback signals could become automatic mutation proposals | Medium | `signalMutation.ts`, `canonicaNightly.ts` | Resolved by skipping `entityId='unresolved'` in mutation clustering |
