...
| Public API key auth | `publicApi`, `widget/search` | Store key lookup after format/rate guard | MenuList `stores` | Public API/widget calls | Exact auth | No | Low | Existing malformed-key short-circuit retained; no unsafe key cache added. |
