...
Provide an AI-powered conversational support assistant that answers user questions by searching a knowledge base using semantic vector search and generating contextual answers via Gemini AI — reducing support ticket volume and giving owners instant self-service answers.
...
- **Architecture:** Excellent. RAG pipeline is well-structured with Canonica canonical-first retrieval integrated. Caching at 3 levels (response, embedding, SWR).
...
- **Canonica Integration:** `attemptCanonicalRetrieval` properly called before RAG fallback. `emitCanonicaSignal` fires on negative feedback.
