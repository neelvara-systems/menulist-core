# Canonica — Guided Workflows: Firebase Cost Tracking
...
This feature adds ZERO new Firestore collections and ZERO additional reads per query. All procedure data is embedded in the existing `canonica_canonical_answers` document.
...
|------------|-------|------|-------------------|
| `canonica_canonical_answers` | Stores procedure data in `content.procedure` field | ❌ Existing | Zero additional reads. Document size increases ~1-2KB per procedure answer |
| `canonica_audit_logs` | Logs procedure answer creation/updates | ❌ Existing | Same write pattern as text answers |
| `canonica_entity_search_index` | Used during retrieval (unchanged) | ❌ Existing | Zero additional reads |
...
```
canonica_canonical_answers: tId ASC + sId ASC + answerType ASC + status ASC
```
...
| **Canonica approach: Embedded** | **2 (unchanged)** | **0 new** | **$0.00 additional** |
