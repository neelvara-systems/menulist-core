...
When Canonica's AI can't confidently answer a user's question, it automatically detects the failure and offers a smooth path to human support. Instead of a dead end, users see a "Still need help?" option that creates a pre-filled support ticket with full context.
...
Canonica uses five independent signals to detect when escalation is needed:
...
1. Escalation creates an **ESCALATION signal** in Canonica's mutation engine
...
It's enabled automatically when you activate Canonica's canonical answer engine. No separate setup required.
...
Canonica creates an asynchronous ticket — there's no live chat queue. Users are notified when their ticket is answered, just like regular support.
