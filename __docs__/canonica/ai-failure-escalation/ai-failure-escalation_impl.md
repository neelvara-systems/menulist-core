...
│                                                             │
│  Fire-and-forget:                                           │
│    + emitCanonicaSignal(type: ESCALATION)                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
New file: `src/lib/canonica/escalationEvaluator.ts`
...
if (FEATURE_FLAGS.ENABLE_CANONICA_AI_ESCALATION) {
  try {
    const { evaluateEscalation } =
      await import("@lib/canonica/escalationEvaluator");
...
if (data.source === "ai_escalation" && data.escalationContext) {
  emitCanonicaSignal({
    type: CANONICA_SIGNAL_TYPE.ESCALATION,
    tId: submitData.tId,
    sId: submitData.sId,
    metadata: {
      ticketId: docRef.id,
...
| ---------------------------------------------------------------------- | ------------------------------------------ | ------------ |
| `src/lib/canonica/escalationEvaluator.ts`                              | Escalation trigger logic + context builder | ~150         |
| `src/lib/canonica/escalationTypes.ts`                                  | Type definitions for escalation            | ~80          |
...
| `src/lib/canonica/canonicalRetrieval.ts`    | Return `entityDebug` in `CanonicalRetrievalResult`                                         | Additive     |
...
| `src/config/features.ts`                    | Add `ENABLE_CANONICA_AI_ESCALATION` flag                                                   | Additive     |

### Files NOT Modified

- `src/lib/canonica/signalEmitter.ts` — Already supports ESCALATION type
- `src/lib/canonica/signalMutation.ts` — Already clusters ESCALATION signals with 3x weight
- `functions-canonica/src/canonica/canonicaNightly.ts` — Already processes escalation signals
- `src/database/canonica/signalEvents.ts` — Already stores escalation events
...
8. **Feature flag** — Add `ENABLE_CANONICA_AI_ESCALATION` to `features.ts`
...
**Rationale:** At Canonica scale (~5-50 escalations/day), total escalation data is ~44KB/day. Firestore document limit is 1MB. Cloud Storage adds unnecessary complexity (bucket management, lifecycle policies, URL generation).
...
**Rationale:** Canonica has no GCP Pub/Sub infrastructure. All async processing uses Firestore triggers or nightly batch. Adding Pub/Sub for escalation events is over-engineering at current scale.
...
**Rationale:** Canonica's ICP is SaaS founders/developers — rational, technical users. Sentiment analysis adds complexity with low ROI for this audience. The 5 existing triggers cover all critical escalation scenarios.
