...
All 9 Canonica collections remain unchanged. No new collections created.

| Collection | DB Constant | Firebase Project | Purpose |
|------------|-------------|-----------------|---------|
| entities | `CANONICA_ENTITIES` | canonica | Product ontology entities |
| entityRelations | `CANONICA_ENTITY_RELATIONS` | canonica | Entity relationships |
| entitySearchIndex | `CANONICA_ENTITY_SEARCH_INDEX` | canonica | Deterministic retrieval index |
| entityCandidates | `CANONICA_ENTITY_CANDIDATES` | canonica | AI extraction staging |
| canonicalAnswers | `CANONICA_CANONICAL_ANSWERS` | canonica | Governed answer assets |
| signalEvents | `CANONICA_SIGNAL_EVENTS` | canonica | Raw signal log |
| mutationProposals | `CANONICA_MUTATION_PROPOSALS` | canonica | Mutation queue |
| auditLogs | `CANONICA_AUDIT_LOGS` | canonica | Audit trail |
| releases | `CANONICA_RELEASES` | canonica | Version timeline |
...
All existing Canonica indexes remain unchanged:
...
No changes to Firestore security rules needed. All Canonica collections already have admin-only rules. The `kb_articles` collection uses existing rules that allow read/write with proper auth.

---

## 7. Backup & Recovery

- All entity operations produce audit logs (`CANONICA_AUDIT_LOGS`)
