...
> **Audience:** Canonica customers (SaaS founders/support teams)
...
Canonica automatically detects entities in your articles and uses them to find the right documentation when users ask questions.
...
2. Canonica detects the product concepts (entities) in your article
3. When a user asks a question, Canonica identifies which product concept they're asking about
4. Canonica retrieves articles that cover that concept
...
- Canonica detects: Webhooks entity
...
When Canonica detects a new product concept it hasn't seen before, it creates a **candidate entity** for your review.
...
Aliases help Canonica understand different ways users refer to the same concept.

**Example:**
- Entity: **API Keys**
- Aliases: "token", "access token", "auth key"

When a user asks about "token not working," Canonica knows they mean API Keys.
...
No. Canonica detects entities automatically when you create or update articles. You only need to review suggestions for new concepts.

**What if Canonica suggests a wrong entity?**
