# Canonica — Entity System (Product Ontology Layer)
...
> **Feature Flag:** `ENABLE_CANONICA_ONTOLOGY` (existing, OFF by default)
...
The Entity System is **Pillar 1** of Canonica — the foundation layer that everything else depends on. Entities represent stable product concepts (features, plans, roles, workflows, states, integrations, error codes) that exist in a SaaS product's architecture. They are the semantic index layer above raw documentation.

**Without entities:** Canonica is semantic search over documents (a help center).
**With entities:** Canonica understands product structure and retrieves knowledge deterministically (a knowledge control plane).
...
|-----------|--------|----------|
| Entity Types (7 types, 3 statuses) | ✅ COMPLETE | `src/types/canonica/index.ts` |
| Entity DAL (12 functions) | ✅ COMPLETE | `src/database/canonica/entities.ts` |
| Entity Relations (6 types + DAL) | ✅ COMPLETE | `src/database/canonica/entities.ts` |
| Entity Search Index (types + DAL) | ✅ COMPLETE | `src/database/canonica/entities.ts` |
| Entity Candidates (7 functions + promote) | ✅ COMPLETE | `src/database/canonica/entityCandidates.ts` |
| Entity Extraction Pipeline | ✅ COMPLETE | `src/lib/canonica/entityExtraction.ts` |
| Canonical Retrieval (3-layer, entity-first) | ✅ COMPLETE | `src/lib/canonica/canonicalRetrieval.ts` |
| Drift Detection (4 classes) | ✅ COMPLETE | `src/lib/canonica/driftDetection.ts` |
| Signal Mutation Engine | ✅ COMPLETE | `src/lib/canonica/signalMutation.ts` |
| useEntities Hook | ✅ COMPLETE | `src/hooks/canonica/useEntities.ts` |
| Governance UI Components | ✅ COMPLETE | `src/components/templates/canonica/governance/` |
| Entity Health Scores | ✅ COMPLETE | `src/components/templates/canonica/governance/EntityHealthScore.tsx` |
...
|---|-------------|--------|------------|
| E1 | Add `aliases[]` to CanonicaEntity | Governance + query detection | Low |
...
## ChatGPT Discussion vs Canonica Reality

| ChatGPT Layer | Canonica Status | Decision |
|---------------|----------------|----------|
...
| Entity Coverage Index | ✅ CanonicaCoverageKPI exists | Keep as-is |
...
| Product Surface Awareness | ✅ CanonicaContextPayload | Keep as-is |
...
| [entity-system_helpdoc.md](./entity-system_helpdoc.md) | Customers | How to manage entities in Canonica |
