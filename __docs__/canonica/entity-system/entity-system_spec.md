...
### E1 — Add `aliases[]` to CanonicaEntity

**What:** Add an `aliases: string[]` field to the `CanonicaEntity` interface and entity documents.

**Why:** Currently aliases only exist on `CanonicaEntitySearchIndex.synonyms`. Entity documents themselves don't carry aliases, making governance harder — owners can't see or edit aliases when managing entities.
...
- Feature-flagged: `ENABLE_CANONICA_ONTOLOGY` must be true
...
| Product Surface Awareness (new collection) | Yes | `CanonicaContextPayload` already handles this. |
