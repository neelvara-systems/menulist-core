...
|---|----------------|---------|--------|
| 1 | Entity Registry (entities collection) | ✅ AGREE — ALREADY BUILT | `CANONICA_ENTITIES` collection with full CRUD DAL. 7 entity types, 3 statuses. |
...
| 4 | Entity Search Index | ✅ AGREE — ALREADY BUILT | `CanonicaEntitySearchIndex` with canonicalName, synonyms, normalizedTokens, weight. |
...
| 10 | Entity Coverage Index (ECI) | ✅ AGREE — ALREADY BUILT DIFFERENTLY | `CanonicaCoverageKPI` component exists with hit/miss ratio. Different implementation, same goal. |
...
| 18 | Entity Description for AI context | ✅ AGREE — ALREADY BUILT | CanonicaEntity has `description: string` field. |
...
| 24 | Product Surface Awareness (new collection) | ❌ REJECT — ALREADY HANDLED | `CanonicaContextPayload` (page, feature, workflow, entityHints) already provides this. No new collection needed. |
...
1. **ChatGPT doesn't know Canonica's codebase.** It proposed building ~12 layers from scratch. Canonica already has 8 of them built and operational.
...
3. **ChatGPT's v1 recommendation (final section) aligns well** with Canonica's current state — keep it simple, entityIds on articles, aliases, candidates pipeline. Canonica has all of this except entityIds on KB articles.

4. **ChatGPT's advanced layers (reasoning engine, trust scoring, cross-tenant, etc.) are premature** for Canonica's current stage. Correctly deferred.

5. **ChatGPT's data model suggestions are partially wrong** for Canonica:
...
6. **Canonica's existing architecture is MORE sophisticated** than ChatGPT's suggestions in several areas:
