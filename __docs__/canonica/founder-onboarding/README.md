...
Reduce Canonica's time-to-value from signup to working AI support from ~30 minutes (manual KB upload → review → approve entities → create answers) to **<5 minutes** (upload docs → AI bootstraps everything → system starts answering immediately → founder reviews gradually).
...
Canonica ALREADY has "auto activation" via RAG. When KB articles are published and embedded, they're immediately searchable. The gap is bootstrapping the **canonical layer** (entities + governed answers) faster — so the deterministic retrieval path starts working alongside RAG from day one.
...
| Decision | Rationale |
|----------|-----------|
| **No new Firestore collections** | Uses existing: `canonica_entities`, `canonica_entityCandidates`, `canonica_canonicalAnswers`, `canonica_mutationProposals`, `canonica_auditLogs`, `kb_generation_jobs` |
...
| `orgId` model | ❌ WRONG | Canonica uses `tId`/`sId`/`pId` |

**Overall ChatGPT accuracy: ~55%** — Valid problem identification, but solution design is disconnected from existing Canonica infrastructure.
...
- `src/lib/canonica/entityExtraction.ts` (entity extraction)
- `functions-canonica/src/canonica/draftGenerator.ts` (answer draft generation)
- `ENABLE_CANONICA_ONTOLOGY` flag (entity layer)
- `ENABLE_CANONICA_CANONICAL_ANSWERS` flag (answer layer)
