# Canonica — Founder Onboarding — ChatGPT Conversation Review
...
ChatGPT proposed a 5-component "Founder Onboarding System" (capabilities #32-36) with additional infrastructure layers. The problem identification is valid — empty knowledge base is the #1 adoption killer. However, the solution design is ~55% accurate because ChatGPT doesn't know the existing Canonica infrastructure.
...
| Chunk to 800-1200 tokens | Yes | Article content is stored as TipTap JSON, embeddings handle relevance | ⚠️ PARTIAL — chunking isn't needed for Canonica's model |
...
| Entity types: FEATURE, WORKFLOW, SETTING, INTEGRATION, ERROR, PLAN, ROLE | Close | Exactly these 7 types in `CANONICA_ENTITY_TYPES` | ✅ MATCHES |
| Extract only on import/update, never per query | Yes | E4 auto-extract on article save, batch on demand | ✅ MATCHES |
| Need `entities` collection | Yes | `canonica_entities` collection exists with full schema | ✅ EXISTS |
...
| Structured output (steps, warnings) | Yes | `CanonicaProcedure` type exists with steps[], warnings[], prerequisites[] | ✅ EXISTS (type) |
| `canonicalAnswers` collection | Yes | `canonica_canonicalAnswers` exists with full governed schema | ✅ EXISTS |
...
**Verdict: 40% accurate.** The core insight (immediate answers) is valid but already solved by RAG. The "auto mode" concept that serves unreviewed drafts as canonical violates Canonica's governance doctrine.
...
| `entities` | ✅ | Already exists as `canonica_entities` |
| `canonicalAnswers` | ✅ | Already exists as `canonica_canonicalAnswers` |
| `reviewQueue` | ❌ | `canonica_mutationProposals` + `canonica_entityCandidates` serve this purpose |
...
6. **Feature flag** `ENABLE_CANONICA_FOUNDER_ONBOARDING`
