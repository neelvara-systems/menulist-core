# Canonica Source File Inventory

> **Status:** Current as of 2026-05-24
> **Purpose:** Record the Canonica source/doc surface reviewed for the system inventory.  
> **Rule:** Code paths are primary. Existing docs are secondary and must be reconciled to runtime behavior.

---

## Canonica Documentation Roots Reviewed

All files under these folders are Canonica-related and were included in the inventory scope:

- `__docs__/canonica/_archive/`
- `__docs__/canonica/ai-failure-escalation/`
- `__docs__/canonica/ai-qna-chatbot/`
- `__docs__/canonica/automatic-knowledge-creation/`
- `__docs__/canonica/canonica-website/`
- `__docs__/canonica/changelog-system/`
- `__docs__/canonica/chat-monitoring/`
- `__docs__/canonica/client-activation-command-center/`
- `__docs__/canonica/client-onboarding/`
- `__docs__/canonica/compiled-context-distribution/`
- `__docs__/canonica/context-aware-support/`
- `__docs__/canonica/deployment/`
- `__docs__/canonica/doctrine/`
- `__docs__/canonica/email-notifications/`
- `__docs__/canonica/entity-system/`
- `__docs__/canonica/feedback-system/`
- `__docs__/canonica/founder-onboarding/`
- `__docs__/canonica/founder-trust-layer/`
- `__docs__/canonica/guided-workflows/`
- `__docs__/canonica/help-center/`
- `__docs__/canonica/help-widget/`
- `__docs__/canonica/instant-response-infrastructure/`
- `__docs__/canonica/kb-generation-pipeline/`
- `__docs__/canonica/knowledge-base/`
- `__docs__/canonica/product-friction-intelligence/`
- `__docs__/canonica/product-surface-contexts/`
- `__docs__/canonica/public-api/`
- `__docs__/canonica/system-inventory/`
- `__docs__/canonica/ticket-knowledge-loop/`
- `__docs__/canonica/ticket-system/`

Top-level Canonica docs included:

- `__docs__/canonica/CANONICA-ACTIVATION-CLEARANCE.md`
- `__docs__/canonica/CANONICA-ACTIVATION-EXPERIMENT.md`
- `__docs__/canonica/CANONICA-BUILD-PRIORITY-ROADMAP.md`
- `__docs__/canonica/README.md`
- `__docs__/canonica/canonica-expansion-tracker.md`
- `__docs__/canonica/canonica-forensic-audit-2026-03-07.md`
- `__docs__/canonica/canonica-production-certification.md`
- `__docs__/canonica/canonica-strategic-improvements.md`
- `__docs__/canonica/firebase-cost-optimization-audit.md`
- `__docs__/canonica/self-sellable-product-strategy.md`

---

## Canonica Website Files

- `src/app/sites/canonica/about/page.tsx`
- `src/app/sites/canonica/components/CTASection.tsx`
- `src/app/sites/canonica/components/CanonicaLink.tsx`
- `src/app/sites/canonica/components/ComparisonSection.tsx`
- `src/app/sites/canonica/components/Footer.tsx`
- `src/app/sites/canonica/components/Header.tsx`
- `src/app/sites/canonica/components/HeroSection.tsx`
- `src/app/sites/canonica/components/HowItWorksSection.tsx`
- `src/app/sites/canonica/components/PillarsSection.tsx`
- `src/app/sites/canonica/components/ProductPreviewSection.tsx`
- `src/app/sites/canonica/components/StructuredData.tsx`
- `src/app/sites/canonica/components/SystemCoverageSection.tsx`
- `src/app/sites/canonica/contact/page.tsx`
- `src/app/sites/canonica/demo/CanonicaPublicDemo.tsx`
- `src/app/sites/canonica/demo/page.tsx`
- `src/app/sites/canonica/enginePillars.ts`
- `src/app/sites/canonica/faq/page.tsx`
- `src/app/sites/canonica/get-started/OnboardingForm.tsx`
- `src/app/sites/canonica/get-started/page.tsx`
- `src/app/sites/canonica/install/page.tsx`
- `src/app/sites/canonica/integrations/page.tsx`
- `src/app/sites/canonica/layout.tsx`
- `src/app/sites/canonica/not-found.tsx`
- `src/app/sites/canonica/page.tsx`
- `src/app/sites/canonica/pricing/page.tsx`
- `src/app/sites/canonica/privacy-policy/page.tsx`
- `src/app/sites/canonica/product/page.tsx`
- `src/app/sites/canonica/resources/page.tsx`
- `src/app/sites/canonica/robots.txt/route.ts`
- `src/app/sites/canonica/security/page.tsx`
- `src/app/sites/canonica/siteConfig.ts`
- `src/app/sites/canonica/sitemap.xml/route.ts`
- `src/app/sites/canonica/styles.css`
- `src/app/sites/canonica/systemCoverage.ts`
- `src/app/sites/canonica/terms-of-service/page.tsx`
- `src/app/sites/canonica/updates/page.tsx`
- `src/app/sites/canonica/use-cases/page.tsx`

---

## Canonica Dashboard Routes

- `src/app/(canonica)/layout.tsx`
- `src/app/(canonica)/canonica/page.tsx`
- `src/app/(canonica)/canonica/activation/page.tsx`
- `src/app/(canonica)/canonica/changelog/page.tsx`
- `src/app/(canonica)/canonica/conversations/page.tsx`
- `src/app/(canonica)/canonica/dashboard/page.tsx`
- `src/app/(canonica)/canonica/docs/page.tsx`
- `src/app/(canonica)/canonica/governance/page.tsx`
- `src/app/(canonica)/canonica/help/page.tsx`
- `src/app/(canonica)/canonica/kb-generation/page.tsx`
- `src/app/(canonica)/canonica/knowledge-base/page.tsx`
- `src/app/(canonica)/canonica/product-surfaces/page.tsx`
- `src/app/(canonica)/canonica/release-notes/page.tsx`
- `src/app/(canonica)/canonica/settings/page.tsx`
- `src/app/(canonica)/canonica/support/page.tsx`
- `src/app/(canonica)/canonica/tickets/page.tsx`
- `src/app/(canonica)/canonica/weekly-digest/page.tsx`
- `src/app/(canonica)/canonica/widget/page.tsx`

---

## Canonica APIs and Widget Runtime Routes

- `src/app/api/canonica/activation/summary/route.ts`
- `src/app/api/canonica/bundles/public/[...path]/route.ts`
- `src/app/api/canonica/bundles/rebuild/route.ts`
- `src/app/api/canonica/mcp/route.ts`
- `src/app/api/canonica/mcp/session/route.ts`
- `src/app/api/canonica/onboard/route.ts`
- `src/app/api/canonica/operations/status/route.ts`
- `src/app/api/canonica/product-surfaces/rebuild-summary/route.ts`
- `src/app/api/canonica/public/v1/answers/route.ts`
- `src/app/api/canonica/public/v1/entities/route.ts`
- `src/app/api/canonica/public/v1/signals/route.ts`
- `src/app/api/canonica/tenant-summary/route.ts`
- `src/app/api/canonica/translate/route.ts`
- `src/app/api/canonica/widget-config/route.ts`
- `src/app/api/canonica/widget-key/route.ts`
- `src/app/api/canonica/workspace-profile/route.ts`
- `src/app/api/widget/config/route.ts`
- `src/app/api/widget/feedback/route.ts`
- `src/app/api/widget/search/route.ts`
- `src/app/widget/[apiKey]/WidgetClient.tsx`
- `src/app/widget/[apiKey]/page.tsx`
- `public/widget/canonica-widget.js`

---

## Canonica Dashboard Components

- `src/components/canonica/CanonicaDashboardLayout.tsx`
- `src/components/canonica/CanonicaHeader.tsx`
- `src/components/canonica/CanonicaSidebar.tsx`
- `src/components/mobile/screens/MobileCanonicaClientScreen.tsx`
- `src/components/templates/canonica/CanonicaCoverageKPI.tsx`
- `src/components/templates/canonica/CanonicaSettings.tsx`
- `src/components/templates/canonica/EntityCandidateReview.tsx`
- `src/components/templates/canonica/MutationProposalReview.tsx`
- `src/components/templates/canonica/activation/CanonicaActivationCommandCenter.tsx`
- `src/components/templates/canonica/activation/CanonicaOperationsPanel.tsx`
- `src/components/templates/canonica/clientPortal/CanonicaClientHome.tsx`
- `src/components/templates/canonica/content/CanonicaContentWorkbench.tsx`
- `src/components/templates/canonica/content/CanonicaCustomerFlowChecklist.tsx`
- `src/components/templates/canonica/content/CanonicaSurfaceReadinessMatrix.tsx`
- `src/components/templates/canonica/governance/AnswerUsageAnalytics.tsx`
- `src/components/templates/canonica/governance/AnswerVersionHistory.tsx`
- `src/components/templates/canonica/governance/CanonicalAnswerEditor.tsx`
- `src/components/templates/canonica/governance/DriftDashboard.tsx`
- `src/components/templates/canonica/governance/EntityHealthScore.tsx`
- `src/components/templates/canonica/governance/EntityManagementDashboard.tsx`
- `src/components/templates/canonica/governance/FounderTrustDashboard.tsx`
- `src/components/templates/canonica/governance/FrictionTab.tsx`
- `src/components/templates/canonica/governance/MultiLanguageArticles.tsx`
- `src/components/templates/canonica/governance/WhiteLabelBranding.tsx`
- `src/components/templates/canonica/governance/index.tsx`
- `src/components/templates/canonica/productSurfaces/CanonicaProductSurfaces.tsx`
- `src/components/templates/canonica/weeklyDigest/CanonicaWeeklyDigest.tsx`
- `src/components/templates/canonica/widgetManagement/CanonicaWidgetManagement.tsx`

---

## Shared Help Center Components Used by Canonica

- `src/components/templates/main-app/helpCenter/ChangelogView.tsx`
- `src/components/templates/main-app/helpCenter/ContactUsView.tsx`
- `src/components/templates/main-app/helpCenter/FaqView.tsx`
- `src/components/templates/main-app/helpCenter/FeatureRequests.tsx`
- `src/components/templates/main-app/helpCenter/FeatureUsage.tsx`
- `src/components/templates/main-app/helpCenter/GeneralFeedback.tsx`
- `src/components/templates/main-app/helpCenter/HeroSearchBar.tsx`
- `src/components/templates/main-app/helpCenter/MainSectionTabs.tsx`
- `src/components/templates/main-app/helpCenter/ShareFeedbackView.tsx`
- `src/components/templates/main-app/helpCenter/TicketHistoryView.tsx`
- `src/components/templates/main-app/helpCenter/TicketItem.tsx`
- `src/components/templates/main-app/helpCenter/TicketView.tsx`
- `src/components/templates/main-app/helpCenter/events.ts`
- `src/components/templates/main-app/helpCenter/index.tsx`
- `src/components/templates/main-app/helpCenter/landing/Breadcrumbs.tsx`
- `src/components/templates/main-app/helpCenter/landing/BrowseCategories.tsx`
- `src/components/templates/main-app/helpCenter/landing/LandingFooter.tsx`
- `src/components/templates/main-app/helpCenter/landing/RecentlyViewed.tsx`
- `src/components/templates/main-app/helpCenter/landing/RunningTickets.tsx`
- `src/components/templates/main-app/helpCenter/landing/TrendingTopics.tsx`
- `src/components/templates/main-app/helpCenter/landing/WhatsNew.tsx`
- `src/components/templates/main-app/helpCenter/landing/index.tsx`
- `src/components/templates/main-app/helpCenter/onboarding/GettingStarted.tsx`
- `src/components/templates/main-app/helpCenter/tabsConfig.tsx`
- `src/components/templates/platform/supportTickets/TicketDetailView.tsx`

---

## Canonica Data Layer, Hooks, Lib, Constants, and Types

- `src/constants/canonica/database.ts`
- `src/constants/canonica/domains.ts`
- `src/constants/canonica/navigations.ts`
- `src/data/canonica/plans.ts`
- `src/types/canonica/index.ts`
- `src/database/canonica/auditLogs.ts`
- `src/database/canonica/branding.ts`
- `src/database/canonica/canonicalAnswers.ts`
- `src/database/canonica/coverageKPI.ts`
- `src/database/canonica/entities.ts`
- `src/database/canonica/entityCandidates.ts`
- `src/database/canonica/frictionStats.ts`
- `src/database/canonica/mutationProposals.ts`
- `src/database/canonica/productSurfaces.ts`
- `src/database/canonica/releases.ts`
- `src/database/canonica/signalEvents.ts`
- `src/database/canonica/trustMetrics.ts`
- `src/hooks/canonica/useCanonicalAnswers.ts`
- `src/hooks/canonica/useEntities.ts`
- `src/hooks/canonica/useEntityCandidates.ts`
- `src/hooks/canonica/useFrictionInsights.ts`
- `src/hooks/canonica/useMutationProposals.ts`
- `src/lib/canonica/activationSummary.ts`
- `src/lib/canonica/cacheFreshness.ts`
- `src/lib/canonica/cacheVersionAdmin.ts`
- `src/lib/canonica/cacheVersionClient.ts`
- `src/lib/canonica/cacheVersionManifest.ts`
- `src/lib/canonica/cacheVersionServer.ts`
- `src/lib/canonica/canonicalRetrieval.ts`
- `src/lib/canonica/compiledContext.ts`
- `src/lib/canonica/compiledSourceVersionsAdmin.ts`
- `src/lib/canonica/compiledSourceVersionsClient.ts`
- `src/lib/canonica/contextBundleBuilderServer.ts`
- `src/lib/canonica/documentComposer.ts`
- `src/lib/canonica/draftGenerator.ts`
- `src/lib/canonica/draftPrompt.ts`
- `src/lib/canonica/driftDetection.ts`
- `src/lib/canonica/entityExtraction.ts`
- `src/lib/canonica/escalationEvaluator.ts`
- `src/lib/canonica/escalationTypes.ts`
- `src/lib/canonica/instantCache.ts`
- `src/lib/canonica/instantCache.types.ts`
- `src/lib/canonica/mcpSession.ts`
- `src/lib/canonica/procedureValidation.ts`
- `src/lib/canonica/productSurfaceContent.ts`
- `src/lib/canonica/productSurfaceContentServer.ts`
- `src/lib/canonica/publicApi.ts`
- `src/lib/canonica/sessionScope.ts`
- `src/lib/canonica/signalEmitter.ts`
- `src/lib/canonica/signalMutation.ts`
- `src/lib/canonica/schedulerSettings.ts`
- `src/lib/canonica/tenantSummaryAdmin.ts`
- `src/lib/canonica/tenantSummaryClient.ts`
- `src/lib/canonica/tokenizer.ts`
- `src/lib/canonica/widgetConfig.ts`
- `src/lib/canonica/widgetRuntimeStatus.ts`

---

## Canonica Cloud Functions Package

- `functions-canonica/src/callableAuth.ts`
- `functions-canonica/src/firebaseAdmin.ts`
- `functions-canonica/src/index.ts`
- `functions-canonica/src/config/secrets.ts`
- `functions-canonica/src/constants/database.ts`
- `functions-canonica/src/constants/features.ts`
- `functions-canonica/src/types/constants.ts`
- `functions-canonica/src/types/index.ts`
- `functions-canonica/src/types/knowledgeBase.types.ts`
- `functions-canonica/src/utils/aiUtils.ts`
- `functions-canonica/src/utils/tiptapUtils.ts`
- `functions-canonica/src/canonica/cacheVersionManifest.ts`
- `functions-canonica/src/canonica/canonicaMasterScheduler.ts`
- `functions-canonica/src/canonica/canonicaNightly.ts`
- `functions-canonica/src/canonica/compiledContextVersions.ts`
- `functions-canonica/src/canonica/contextBundleBuilder.ts`
- `functions-canonica/src/canonica/draftGenerator.ts`
- `functions-canonica/src/canonica/frictionAggregation.ts`
- `functions-canonica/src/canonica/frictionInsight.ts`
- `functions-canonica/src/canonica/onboardingBootstrap.ts`
- `functions-canonica/src/canonica/resolutionExtractor.ts`
- `functions-canonica/src/canonica/schedulerTime.ts`
- `functions-canonica/src/canonica/tenantSummary.ts`
- `functions-canonica/src/canonica/ticketKnowledgePrompt.ts`
- `functions-canonica/src/logic/embedArticleWorker.ts`
- `functions-canonica/src/logic/publishApprovedJob.ts`
- `functions-canonica/src/logic/regenerateEmbedding.ts`

---

## Canonica Firebase Config Files

- `firebase-canonica.json`
- `firestore-canonica.rules`
- `firestore-canonica.indexes.json`
- `storage-canonica.rules`

---

## Inventory Notes

- `functions-canonica` is intentionally separate from MenuList Cloud Functions.
- Canonica dashboard and website routes are intentionally separate from the MenuList owner dashboard.
- MenuList Help Center components are shared support surfaces, not Canonica management screens.
- Public API, MCP, multi-language, advanced white-label, and AI escalation are present but rollout-gated or disabled by default.
- Predictive support, workflow integrations, and graph traversal are active runtime paths. Keep them bounded through Redis cooldowns, event caps, sanitized workflow delivery, and summary-backed graph/index reads.
- Compiled context bundles are the approved read distribution layer for widget, public API, and MCP; Storage bundles must never include drafts, tickets, chats, raw signals, audit logs, API keys, or billing data.
- Notifications, workflow integrations, ticket knowledge extraction, and product friction intelligence are enabled with rate limits, nightly caps, and summary-backed UI reads.
- Website copy must treat rollout-gated code as controlled capability, not as a default promise.
