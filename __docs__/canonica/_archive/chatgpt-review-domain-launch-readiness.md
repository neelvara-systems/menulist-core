# Canonica — ChatGPT Conversation Review: Domain, Launch Readiness & Failure Modes

> **Date:** 2026-03-06
> **Source:** ChatGPT conversation post-domain purchase (canonica.app)
...
The conversation covers 6 topics after purchasing `canonica.app`:
...
| ------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| Subdomain structure | `canonica.app` (marketing), `app.canonica.app` (app), `docs.canonica.app` (docs), `api.canonica.app` (API) | ✅ SOUND — Standard SaaS pattern (Stripe, Linear, Vercel all do this) |
| HTTPS               | `.app` enforces automatically                                                                              | ✅ CORRECT — .app TLD requires HTTPS                                  |
| www redirect        | `www.canonica.app` → `canonica.app`                                                                        | ✅ CORRECT — Non-www canonical is standard                            |
...
| Social handles      | @canonica / @canonicaapp on X, LinkedIn, GitHub, YouTube                                                   | ✅ SOUND — Reserve early                                              |
| Brand spelling      | "Canonica" (consistent capitalization)                                                                     | ✅ ALREADY LOCKED — See doctrine/04-canonica-identity.md              |
...
- "Canonica should remain the knowledge control plane, not a helpdesk replacement" → ✅ EXACTLY what doctrine says
...
ChatGPT's 6-layer framework is a genuinely useful way to evaluate Canonica's completeness. The assessment is largely correct. Minor gap: ChatGPT doesn't know signal entity resolution already exists.
...
- `aggregateCoverageKPI()` in `functions/src/canonica/canonicaNightly.ts` (lines 446-490)
- Reads 24h of search history, counts `canonical: true` vs `canonical: false`
- Stores in `platformSummary/canonica_{sId}` with date, hits, misses, rate, total
- DAL: `getCanonicaCoverage()` in `src/database/canonica/coverageKPI.ts`
- Listed as completed item #5 in `canonica-strategic-improvements.md`
...
- `resolveUnresolvedSignals()` in `functions/src/canonica/canonicaNightly.ts` (lines 354-433)
...
- Listed as completed item #2 in `canonica-strategic-improvements.md`
...
- `runCanonicaNightly()` orchestrator in `functions/src/canonica/canonicaNightly.ts` (lines 729-809)
- **7-step batch job:**
...
- Feature flag: `ENABLE_CANONICA_NIGHTLY` in `functions/src/constants/features.ts`
- Wired into unified nightly scheduler (`decisionBlocksScoring.ts`)
- Listed as completed item #1 in `canonica-strategic-improvements.md`

### Additional Features ChatGPT Didn't Know Exist

| Feature                                   | File                                               | Status   |
| ----------------------------------------- | -------------------------------------------------- | -------- |
| Recurring fallback → auto proposals       | `detectRecurringFallbacks()` in canonicaNightly.ts | ✅ Built |
| Post-mutation impact tracking (14-day)    | `trackMutationImpact()` in canonicaNightly.ts      | ✅ Built |
| Confidence auto-adjustment                | `autoAdjustConfidence()` in canonicaNightly.ts     | ✅ Built |
...
| ------------------------------------ | --------------------------------- | --------------------------------------------------------------------------------- |
| Internal activation (14-30 days)     | Run with one real tenant first    | ✅ ALIGNS — `canonica-activation-experiment.md` already defines 4-week experiment |
...
ChatGPT's launch strategy perfectly aligns with the existing `canonica-activation-experiment.md`. The activation experiment framework already defines hard success/failure criteria, measurement plan, activation sequence, risk mitigations, and go/no-go decision framework.
...
| Item                                                                      | Assessment                                                                                                            |
| ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| MenuList as Tenant #1                                                     | ✅ ALREADY DOCUMENTED in doctrine — "MenuList is first Canonica client (tenant=MenuList, store=Default)"              |
...
- "Canonica should remain knowledge control plane, not helpdesk" is correct
...
| 4 failure mode warnings     | Add to activation experiment doc                              | `canonica-activation-experiment.md` |
| Entity category suggestions | Add to activation experiment doc                              | `canonica-activation-experiment.md` |
| Weekly governance cycle     | Add to activation experiment doc                              | `canonica-activation-experiment.md` |
