# Canonica Cost Read-Model Guardrails

> **Status:** Active Architecture Guardrail  
> **Last Updated:** 2026-05-31  
> **Scope:** Canonica dashboard, widget/runtime APIs, nightly scheduler summaries, owner review surfaces, and platform monitoring.

## Purpose

Canonica must stay predictable as tenants, support questions, tickets, signals, intake jobs, and governance records grow. The default architecture is not "make every screen realtime." The default is:
...
This keeps Canonica usable for small SaaS founders while protecting Firebase spend.
...
| Product surfaces | `CANONICA_PRODUCT_SURFACE_LIMIT` |
...
| Knowledge intake jobs/sources/review items | bounded by `CANONICA_KNOWLEDGE_INTAKE_CONSTRAINTS` |

Do not introduce a new list without adding the cap to the feature's Firebase doc.

### 3. Realtime Is Exceptional

Canonica dashboard and widget configuration surfaces should not use Firestore `onSnapshot` by default.
...
Presence, typing indicators, live cursors, and ephemeral collaboration state should not be stored in Firestore. If Canonica later needs those features, use a dedicated ephemeral realtime layer after a separate architecture decision.
...
- `src/app/api/canonica/widget-activity/route.ts` uses the indexed tenant/widget query first. If that fails, the fallback now remains tenant-scoped before filtering widget rows.
- `src/database/canonica/signalEvents.ts` clamps caller-provided window and result limits.
- `src/database/canonica/auditLogs.ts` clamps audit loads to 200.
- `src/database/canonica/supportBoard.ts` clamps board loads to the configured max and protects against invalid limits.
- `functions-canonica/src/canonica/canonicaNightly.ts` and related scheduler modules own recurring summary preparation. Do not create separate scheduled functions for new Canonica summary maintenance unless doctrine changes.

## Review Checklist

Before merging Canonica code that reads Firestore:
