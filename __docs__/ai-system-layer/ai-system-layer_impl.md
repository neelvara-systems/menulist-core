...
`src/lib/ai/capacityCheck.ts` now reserves paid subscription credits in the existing subscription plus `menulistAiOperations/{tId}/{sId}/{operationId}` transaction before provider work. Settlement promotes the hidden reservation shell to the normal operation row; failure refunds the exact charged recurring/top-up buckets once. Stale interactive reservations are recovered by the consolidated maintenance scheduler, while deterministic batch reservations remain durable only while staged work is retryable. The `CREDITS_EXHAUSTED` lifecycle message remains fire-and-forget after settlement reaches zero; failed imports/sends use bounded `ai_capacity_credits_exhausted_lifecycle_message_*` diagnostics.
...
- Answerlattice RAG uses the shared registry. Its first launch contract is `gemini-embedding-2`, 768 dimensions, Firestore field `embedding`, and cache key `gemini-embedding-2:768:v1`; there is no retired-model call, second vector field, dual-write, or corpus backfill.
...
July 5 weekly narrative output boundary: `/api/analytics/weekly-narrative/generate-local` now normalizes generated weekly narrative output before persistence. Parsed provider JSON still accepts the same `narrative`, `highlights`, and `recommendations` shape, but each generated string is stripped of control/template characters, whitespace-normalized, capped, and fallback-protected before being written to `insights/{tId}/stores/{sId}/ai/weekly` or counted in the success response. This preserves the authenticated route, rate limit, permission gate, SAFE_MODE check, Gemini call, deterministic fallback copy, Firestore write path, and best-effort AI operation accounting.
...
July 5 text AI operation response summaries follow-up: text/design AI operation input now keeps generated owner-facing text objects out of `clientResponse` rows by passing pre-summarized response payloads into the shared accounting finalizer. `/api/descriptions`, `/api/translations`, `/api/new-item-metadata`, `/api/business-copy`, `/api/seo`, `/api/reviews/suggest`, `/api/campaigns/caption`, and `/api/menu-card-export/design-advisor` now use route-local response summary helpers with `responseSummaryKind` markers. `src/lib/ai/operationLog.ts` preserves those pre-summarized payloads in `accounting_only` mode. Valid owner responses still return generated descriptions, translations, metadata, copy, review replies, captions, and design advice; only the operation-row payload shape changes.

July 12 text-output contract audit: business-copy output is projected through `src/lib/ai/businessCopyOutput.ts`, including the required `specialNote` field used by desktop/mobile consumers. Description output is projected to requested item/language IDs, and payload-visible existing descriptions upgrade a requested free first-pass operation to the rewrite action. Translation input must contain canonical menu keys plus canonical project/file scope; the server derives the minimum safe billing action from target-language and entity scope, drops unknown response keys, and clients revalidate the exact requested map before merge. Description and translation project scope is mandatory and the project-policy loader is the tenant/store authority; the former session-to-itself checks were removed. Business-copy and translation JSON retries aggregate token usage, provider-call count, latency, and internal provider-cost estimates across every successful provider response while charging the owner operation once.

July 12 new-item metadata output audit: `src/lib/ai/newItemMetadataOutput.ts` is the shared server/client runtime contract. It accepts only multilingual name/description maps, input-matched attribute IDs, and business-applicable low-risk metadata. Item identity, category, active state, owner source-language name/description, attribute IDs/order, and prices cannot be changed by provider output. Attribute translations merge by ID rather than array position. The request now requires canonical project/file scope, non-empty item identity/name, unique attribute IDs, unique target languages, and bounded arrays. The route verifies project/outlet policy before provider work and both the route and browser service reject malformed successful responses.

July 5 follow-up: `/api/campaigns/caption` now records unrecoverable provider-response parse failures through capped `campaign_caption_provider_response_parse_failed` diagnostics with fixed `return_caption_generation_failed` policy. The route strips JSON fences, accepts extractable object-fragment JSON before failure, returns the generic caption failure when the provider response is unusable, and fail-closes non-object output before phrase-guarding or accounting. Diagnostic metadata is limited to action/request/model/project/store/tenant/user/campaign/surface/language presence-length fields plus response length, trimmed length, candidate length, parse stage, fenced-response flag, and object-fragment flag. AI accounting input carries bounded prompt summaries and caption response summaries instead of raw prompt item/business fields or generated caption objects. Raw provider response text, prompt item/business copy, generated captions, response preview text, raw prompt item/business fields, and exception text are not logged.

July 5 follow-up: `/api/new-item-metadata` now records unrecoverable provider-response parse failures through capped `new_item_metadata_provider_response_parse_failed` diagnostics with fixed `return_metadata_generation_failed` policy. The route strips JSON fences, accepts extractable object-fragment JSON before failure, and returns the existing generic metadata failure without writing a usable operation row or consuming credits when the provider response is unusable. Diagnostic metadata is limited to action/request/model/project/file/store/tenant/user/source/target/tone/business/content/attribute presence-length/count fields plus response length, trimmed length, candidate length, parse stage, fenced-response flag, and object-fragment flag. Local API response and success/error logs record response and transaction summaries only, and accounting input carries bounded item/language summaries instead of raw prompt item/language payloads. Raw provider response text, prompt item/language copy, generated metadata, response preview text, full provider response objects, raw prompt item/language payloads, and exception text are not logged. The July 12 exact projection supersedes the earlier forbidden-field deletion boundary for successful output.

July 13 accounting follow-up: `/api/image-generation`, `/api/image-editing`, and `/api/image-generation/batch-generation` reserve paid units before provider work and settle the exact reservation after valid output. Batch reservations use deterministic operation IDs, survive staged accounting/append retries, and are refunded on terminal, cancelled, or max-attempt acknowledgement when still unsettled. Batch max-attempt claims now persist failed item/job state instead of leaving expired processing executions. Local route logs and operation input payloads remain bounded; raw prompts, item details, reference URLs, base64 images, full provider responses, and full transaction objects are not written.

July 26 generated-image output follow-up: the shared Gemini generation runner and the independently implemented image-editing runner project provider `inlineData` through `src/lib/ai/generatedImageOutput.ts` before single generation, editing, or the batch worker receives bytes. The boundary admits only canonical base64 JPEG, PNG, or WebP content whose declared MIME matches magic bytes, caps each image at 15 MB, caps each response at four images, and records decoded size. Malformed, mismatched, unsupported, or oversized image parts are excluded; an all-invalid response follows each route's existing no-image/refund failure path. Generation and editing drafts therefore no longer deliver raw unvalidated provider bytes to the browser, while batch media still passes through the stricter `menuItem` preparation/upload profile before persistence.

July 6 follow-up: Batch image project/job ID boundary. `src/lib/ai/imageBatchIdBoundary.ts` now source-gates batch project IDs, tenant/store scope, and Firestore auto-ID shaped batch job IDs before `/api/image-generation/batch-trigger`, `/api/image-generation/batch-generation`, `src/database/imageBatchProcessing/index.tsx`, or `src/database/imageBatchProcessing/server.ts` compose image-batch Firestore refs. Valid Cloud Tasks worker, prompt-cache, capacity, accounting, Storage upload, and owner review flows are unchanged.
...
July 5 transaction DB local error follow-up: `/api/business-copy`, `/api/descriptions`, `/api/image-editing`, `/api/new-item-metadata`, `/api/seo`, and `/api/translations` now write bounded `getAIErrorDiagnostics(transactionError)` source-error metadata to local `TRANSACTION_DB_ERROR` logs instead of raw accounting exception objects. The existing `logAIRouteFailure(...)` call remains the structured failure diagnostic, the routes still rethrow the accounting failure, and valid provider calls, generated output, owner review/save flows, successful accounting finalizer behavior, and credit consumption are unchanged.
...
July 13 browser response-contract follow-up: `src/lib/ai/operationHistoryClientContract.ts` is the shared runtime and TypeScript contract for MenuList and Answerlattice history pages. Both DALs validate every returned row, preserve the canonical Firestore document ID, visible platform transaction ID and ISO timestamp, require finite numeric accounting fields and actual enum strings, normalize bounded file/item/language structures, and admit nested response detail only when it is JSON-safe. Unknown row fields are omitted. Continuation cursors contain exactly one valid document ID, `hasMore` requires a cursor, and MenuList manual continuation requires an empty capped page. Desktop, mobile, detail views, and Answerlattice billing state consume the projected DTO and use safe nested-object readers instead of broad `any` contracts. A malformed successful response now follows the existing bounded failure path before UI state changes.

July 13 transaction-history authorization/cache follow-up: after syntactic query/date admission and the shared read limiter, `/api/ai-operations` re-reads the current store through `requireAnyStorePermission()` and requires `canAccessBilling` before cursor or page reads. Platform sessions retain the helper's explicit platform bypass. The MenuList browser DAL now matches the Answerlattice transport policy with `cache: 'no-store'`, same-origin credentials, and manual redirects before bounded response parsing. Unused `getAllAiOperations`, `getAiOperationsByStoreId`, and `getAiOperationById` exports were removed because they had no first-party callers, requested 100 rows from a route capped at 50, and did not implement their names' scope semantics.

July 5 transaction-history query boundary: `/api/ai-operations` and `/api/answerlattice/ai-operations` now validate operation-history query cursors and date filters through `src/lib/ai/operationHistoryQuery.ts` before Firestore cursor/query work. The routes accept only simple Firestore document ID cursor values that also pass the shared Firestore reserved/path guard, and strict `YYYY-MM-DD` or browser ISO `...Z` date filters. The helper rejects malformed date values, reserved/path-shaped cursor IDs, and reversed or wider-than-366-day date ranges. Existing auth, tenant/store scope, rate limiting, owner/platform field filtering, capped pagination, and bounded diagnostics remain unchanged.

AI Operations History Scope Document ID Boundary: `/api/ai-operations` validates tenant/store transaction-history scope with the shared Firestore document ID guard plus exact positive numeric admission before rate-limit key hashing, cursor reads, or `menulistAiOperations/{tId}/{sId}` queries. Valid numeric scope keeps the same owner/platform field filtering, capped pagination, cursor/date filters, and bounded diagnostics. Malformed scope returns the existing not-onboarded response before Firestore refs are built.

AI Operations Persisted Cursor Admission: after the existing scoped cursor-document read, `/api/ai-operations` requires the requested document to exist, requires its `createdOn` value to be a valid Firestore timestamp or `Date`, and requires that timestamp to remain inside the active inclusive date filter. Missing, corrupt, or filter-incompatible cursors return fixed `400 Invalid cursor` before `startAfter()` or the bounded action scan. Omitted cursors and valid current cursors retain the existing behavior.

AI Operations Filtered-History Continuation Boundary: action filters retain the bounded 500-document server scan because each store is a dynamic nested collection and cannot share one action/created-time composite index. A scan window with no matching rows is not proof that older matching history is absent. `resolveAiOperationActionScanBoundary()` now distinguishes a buffered matching row from a capped scan, advances the cursor past every fully inspected document, and returns `requiresManualContinuation` when an empty capped window has older history. Desktop keeps the page/cursor available. Mobile stops `InfiniteScroll` and presents an explicit translated 44px continuation action, preventing silent multi-window read chaining while keeping older matching history reachable.

AI Operations Desktop Accessibility Boundary: the desktop Transactions action filter now exposes its existing localized `Filter by action` text as the combobox accessible name instead of relying on a visual placeholder. Every icon-only row details action also exposes the localized Details label plus that row's localized operation name. This changes no history query, filter, cursor, project enrichment, owner/platform field projection, Firebase operation, provider call, or payment behavior; Mobile Transactions already names its filter and refresh actions and retains its existing touch-native detail rows.

AI Operations Canonical Response Projection Boundary: `/api/ai-operations` now delegates owner and platform rows to `projectAiOperationHistoryFields()`. The helper reads and serializes only fields admitted by the applicable role allowlist, rather than recursively serializing the complete Firestore document and filtering afterward. It ignores any persisted `id` field and appends the Firestore document path ID last, keeping operation identity authoritative for desktop/mobile keys and detail selection. Existing timestamps still cross the server/client boundary as ISO strings. Hidden raw provider fields remain unavailable and can no longer fail owner serialization merely by existing on a legacy row.

July 14 accounting-scope and owner-history correction: a paid operation remains under the selected outlet `sId`, while its hidden reservation persists `accountingBillingStoreId` for the effective subscription that was actually debited. Settlement, explicit refund, legacy settlement, and stale-reservation recovery validate the same tenant and use that billing store; pre-change direct-store shells fall back to operation `sId`. Returned balance events include `billingStoreId`, and the browser applies them only to a matching active subscription. Successful/partial authenticated menu extraction atomically writes its existing detailed `MENULIST_AI_OPERATIONS` platform audit plus a compact zero-credit row in the selected outlet's nested owner history. Desktop/mobile action filters now use the MenuList-only owner action allowlist, and desktop exposes manual Previous/Next continuation for empty capped filtered windows just as mobile does.
