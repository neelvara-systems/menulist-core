...
July 5 follow-up: `/api/image-generation`, `/api/image-editing`, `/api/image-generation/batch-trigger`, and `/api/image-generation/batch-generation` keep the same provider calls, owner preview/upload behavior, batch worker flow, operation finalizer, and credit consumption order, but their local route logs and operation input payloads are bounded. Local logs carry request/response/transaction/task summaries only, batch-trigger prompt-block responses return counts only, and operation rows carry `itemSummary` plus `generationConfigSummary` instead of raw item details or generation config payloads. Raw prompts, item descriptions/categories/attributes, reference image URLs, generated base64 images, raw item ID arrays, full provider responses, full transaction objects, and raw image accounting input payloads are not written by these routes.
