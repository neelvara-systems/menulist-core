...
- Desktop Business Settings writes changed `workingHours` leaves through `updateStore()` and requires `assertStoreUpdateSucceeded()`.
- `MobileWorkingHoursEditScreen` and the Today quick-hours sheet use optimistic local state but require `assertStoreUpdateSucceeded()` before treating the save as confirmed. Their `updateStore()` payloads contain only `storeId`, `tenantId`, changed `workingHours` leaves, and `hoursLastUpdatedAt`; Today owns only the current day, while the full editor derives a deep difference from the current store model. They must never spread the loaded store snapshot or replay unchanged days because an hours change must not overwrite unrelated concurrent store truth.
