...
- `MobileWorkingHoursEditScreen` uses optimistic local updates, then persists a minimal store/tenant/changed-day/timestamp patch through `updateStore()`; the Today sheet owns only its current-day leaf, and the full editor derives changed days against current store truth.
...
- Neither mobile hours path may spread `storeDetails` into the write; unrelated profile, publication, billing, cache, or owner settings must remain outside an hours mutation.
