...
**Last Source Gate Update:** July 28, 2026
...
- `POST /api/store/public-api-key` is dynamic, authenticated with `withAuth()`, feature-gated by `ENABLE_PUBLIC_API`, derives one exact tenant/store scope from every supplied session alias, requires one exact actor identity, is permission-gated by `MANAGE_INTEGRATIONS`, and is fail-closed rate-limited by hashed actor and store segments before a strict 1KB JSON body is parsed. The request includes the exact tenant/store selected when the owner initiated it; both identifiers must normalize and equal the admitted session scope before Firestore work. Tenant/store lifecycle, ownership, current store-role permission, and the key write are then re-read/applied in one Firestore transaction, so a concurrent block, delete, ownership change, role-definition change, or browser store switch cannot redirect the mutation.
- Key generation creates an `ml_` key and stores only the non-secret credential projection: `publicApi.apiKeyHash`, `keyPrefix`, `createdAt`, `productId: ML`, `purpose: menulist_public_api`, and `scopes: [public:read]`. It returns the raw key once with exact tenant/store acknowledgement and logs bounded diagnostics. Every key-management response, including bounded-body and permission failures, receives non-overridable `private, no-store, max-age=0`, `Pragma: no-cache`, and `nosniff` headers. Revoke transactionally deletes `publicApi` and returns the same exact-scope acknowledgement.
...
9. Key-management rate-limit keys combine `hashPublicRateLimitValue(actorId)` and `hashPublicRateLimitValue(storeId)`, preventing raw identity storage and preventing one manager from consuming every other manager's allowance for the store. Generate/revoke diagnostics use bounded store/tenant/user presence-length metadata only.
...
14. Every key-management route response uses a route-owned protected response helper. Caller/helper headers cannot override `private, no-store, max-age=0`, `Pragma: no-cache`, or `X-Content-Type-Options: nosniff`, including errors returned by the bounded JSON parser or permission helper.
