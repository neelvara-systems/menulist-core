...
- Answerlattice management access scope boundary: `src/lib/answerlattice/accessControl.ts` now treats the shared session scope as the only route-session source, then normalizes persisted `stores`, `users`, `stores[]`, and `answerlatticeRoles` tenant/store fields with `normalizeAnswerlatticeScopeDocumentId()` before granting access or role permissions. Malformed persisted scope fails closed before management context is returned or role backfill can write.
