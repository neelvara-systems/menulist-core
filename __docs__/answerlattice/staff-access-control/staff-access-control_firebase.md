...
Bridge erasure remains one default-Firebase user transaction read plus, only after exact scope/emptiness proof, one update. Every present nested tenant alias must agree; any direct store alias, non-empty `storeIds`, or malformed non-array membership list blocks erasure. The sync path also distinguishes absent root scope from conflicting scope aliases before deciding whether legacy root fields may be written. This adds no read, write, collection, index, rule, Function, provider call, cache mutation or deploy target.
