...
| -------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------ |
| Signal emitter             | `src/lib/answerlattice/signalEmitter.ts`                  | ✅ Fires ticket signals on creation                                |
...
| Nightly batch orchestrator | `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts` | ✅ 13+ steps, idempotent                                           |
...
| File                                                                    | Change                                                            | Purpose                                 |
| ----------------------------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------- |
| `src/lib/answerlattice/signalEmitter.ts`                                     | Emit resolution metadata on ticket status change                  | Rich signal with resolution content     |
| `src/types/answerlattice/index.ts`                                           | Add `sourceTicketIds[]`, `resolutionContext` to mutation proposal | Lineage tracking                        |
| `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts`                    | Add Step 14                                                       | Orchestrate ticket knowledge extraction |
| `functions-answerlattice/src/constants/features.ts`                          | Add `ENABLE_ANSWERLATTICE_TICKET_KNOWLEDGE`                            | CF feature flag                         |
| `src/config/features.ts`                                                | Add `ENABLE_ANSWERLATTICE_TICKET_KNOWLEDGE`                            | Frontend feature flag                   |
...
| `functions-answerlattice/src/answerlattice/draftGenerator.ts`                     | Handle `ticket_resolution` draft source                           | Generate drafts from resolution context |
...
```typescript
// Existing AnswerlatticeSignalEvent.metadata — additive fields
metadata: {
...
```typescript
// Existing AnswerlatticeMutationProposal.suggestedChange — additive fields
suggestedChange: {
...
- `answerlattice_signalEvents` — enriched metadata on ticket resolution signals
- `answerlattice_mutationProposals` — proposals with `draftSource: 'ticket_resolution'`
- Ticket-resolution proposal writes include Answerlattice `pId`, `createdBy` / `modifiedBy`, and `modifiedOn`; merges into existing proposals also update modification metadata.
- `answerlattice_auditLogs` — provenance tracking
...
**File:** `src/lib/answerlattice/signalEmitter.ts`
...
**File:** `functions-answerlattice/src/answerlattice/resolutionExtractor.ts`
...
**File:** `functions-answerlattice/src/answerlattice/ticketKnowledgePrompt.ts`
...
```
Query answerlattice_canonicalAnswers where:
  - tId == tId
  - sId == sId
...
```
Query answerlattice_mutationProposals where:
  - tId == tId
  - sId == sId
...
**File:** `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts`

Add Step 14 to the `runAnswerlatticeNightly()` orchestrator:
...
if (FUNCTION_FLAGS.ENABLE_ANSWERLATTICE_TICKET_KNOWLEDGE) {
  try {
    ...
    console.error(
      `[Answerlattice Nightly] Ticket knowledge extraction failed for ${tId}/${sId}:`,
      error,
    );
...
**Decision:** Reuse existing `answerlattice_mutationProposals` with additive fields.

**Rationale:**

- Firebase cost discipline (core Answerlattice principle)
...
- Entities are the structural key in Answerlattice (doctrine principle)
- Vector similarity requires external DB or expensive embedding calls
- At Answerlattice's scale, entity grouping is sufficient
...
- Ticket system is a separate concern (not Answerlattice-owned)
...
### Cloud Function (`functions-answerlattice/src/constants/features.ts`)
...
- Add additive fields to `AnswerlatticeMutationProposal.suggestedChange` type
...
- Wire into `answerlatticeNightly.ts` as Step 14
