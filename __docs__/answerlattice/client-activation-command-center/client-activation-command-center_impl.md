...
The shared sidebar applies permissions and feature flags first, then projects direct grouped links under Get Live, Improve answers, Run Support, Customer help, and Workspace. A single synthetic **All tools** item in the Advanced section toggles every remaining authorized route in component-local state. It does not navigate or close the mobile drawer. Selecting a real destination uses the existing router and drawer-close behavior. If the current route is advanced, that route remains visible while active so the operator never loses their location. Governance's in-page tab bar keeps its existing contract: Trusted Answers stays primary and its existing dropdown exposes all other permitted tabs.
