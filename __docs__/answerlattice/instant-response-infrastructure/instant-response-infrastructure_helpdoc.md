...
When your users ask a question that has been answered before, Answerlattice returns the answer instantly — in under 20 milliseconds. No waiting, no loading spinner.
...
2. Answerlattice checks if this question matches a known answer
3. If yes → the answer is returned instantly from cache
4. If no → Answerlattice runs the full search pipeline and stores the answer for next time
...
When you update a canonical answer in the Answerlattice dashboard, the cached version is automatically replaced on the next query. There is no manual "clear cache" step needed.
...
A: No. Instant caching is enabled automatically when your Answerlattice instance has canonical answers configured.
