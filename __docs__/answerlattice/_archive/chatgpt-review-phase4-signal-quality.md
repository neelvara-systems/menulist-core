# ChatGPT Review — Answerlattice Phase 4 (Signal Quality + White-Label + Multi-Language)
...
| File | Change |
|------|--------|
| `src/lib/answerlattice/canonicalRetrieval.ts` | Added `ENTITY_MATCH_MIN_SCORE` threshold — bypasses canonical when entity match too weak |
| `src/database/answerlattice/entityCandidates.ts` | Added `ONTOLOGY_AUTHORITY_RULES` guard to `promoteCandidate()` |
