...
### Target: Governed Answer Infrastructure (Entity-Canonical Core)
...
- Nightly drift audit job (Answerlattice Cloud Function, exported from `functions-answerlattice/src/index.ts`)
- Release-triggered drift evaluation

**Cascade recommendation:** Reuse the scheduler reliability pattern, but keep the runtime in `functions-answerlattice/`. Add drift evaluation to the Answerlattice scheduler. Feature flag: `ENABLE_ANSWERLATTICE_DRIFT_DETECTION`.
...
```typescript
// Collection: entities (via DB_COLLECTIONS.ANSWERLATTICE_ENTITIES)
interface AnswerlatticeEntity {
  id: string;
  tenantId: string;
...
// Collection: entityRelations (via DB_COLLECTIONS.ANSWERLATTICE_ENTITY_RELATIONS)
interface AnswerlatticeEntityRelation {
  id: string;
  tenantId: string;
  fromEntityId: string;
  toEntityId: string;
...
// Collection: releases (via DB_COLLECTIONS.ANSWERLATTICE_RELEASES)
interface AnswerlatticeRelease {
  id: string;
  tenantId: string;
...
Answerlattice App Release ID Boundary: browser release reads normalize the document ID; create/activate actions cross the bounded authenticated release route. The Admin server lifecycle revalidates exact release, entity and canonical-answer product/tenant/store/version contracts and owns the transaction/lease, drift audit and compiled-context source marking.
...
- DAL files in `src/database/answerlattice/` (new folder)
- Feature flags in `src/config/features.ts`
- Cloud Functions follow existing scheduler pattern
- Types in `src/types/answerlattice.ts` (new file)
