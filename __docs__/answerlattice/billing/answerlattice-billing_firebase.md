...
- Replacing an unpaid retired-plan checkout reuses the same bounded pending query and provider read. When the provider checkout is still safely replaceable, it adds at most one provider cancellation and one Firestore transaction read/write to expire the exact unchanged pending document before the normal new-checkout writes. Identical intent still reuses the existing checkout with no Answerlattice write; a checkout under payment confirmation fails closed and creates no second subscription.
...
- Subscription projection is read-only and cost-neutral. Client/Admin active readers, paid Knowledge Intake, AI-credit transaction rechecks, direct-ID server reads, and grace-expiry transaction rechecks share an exact runtime boundary for critical status, interval, currency, amount, quantity, credit balances, payment counts, bounded history, and lifecycle timestamps; coercible or accessor-backed persisted values fail closed without another Firestore operation.
...
- Answerlattice entitlement sync requires exact `AL` product aliases and agreeing numeric tenant/store aliases on both its entry object and transaction-current subscription before constructing workspace references, selecting the active subscription, or updating `stores/{sId}` and the subscription doc. Contradictory entry identity performs no read/write effect. It does not fan out to MenuList public cache tags.
...
- Support search debits before provider work in the same transaction that creates its protected `reserved` operation and root recovery pointer. Monthly reset, debit, both new/resumed reservations, and settlement revalidate transaction-current exact product/workspace shape plus active status and the earliest unexpired end boundary before any write or provider admission. Refund uses the same exact shape without requiring active status. Settlement or request failure deletes the pointer; the existing hourly master scheduler bounds stale recovery to 50 pointers per run and restores credits only after exact product/scope/subscription/unit/debit/period validation. A malformed pointer remains for repair and makes the recovery task fail with a fixed bounded code after other pointers are processed, rather than publishing a successful scheduler outcome. This adds no scheduled Function export and needs no composite index.
...
The active-subscription direct-doc read path avoids the old general billing-page composite query. Its capped fallback query includes exact `pId + productId + tenantId + storeId + tId + sId`. Entitlement synchronization intentionally applies the same six identity predicates before its bounded current-active Admin query, so contradictory duplicate aliases cannot consume the ten-row window or let an old/foreign subscription clear a concurrently active replacement; `firestore-answerlattice.indexes.json` includes `subscriptions` on `pId + productId + status + storeId + tenantId + tId + sId + cycleEndDate desc`. Paid billing history uses `payment_transactions` on `pId + productId + event + storeId + tenantId + created_at desc`; the same product-scoped history index exists in the shared MenuList and dedicated Answerlattice index files.
...
| Date | Version | Change |
|------|---------|--------|
| 2026-07-29 | 2.0.5 | Documented all duplicate workspace predicates before the bounded entitlement query |
| 2026-07-29 | 2.0.4 | Documented cost-neutral paid-intake and transaction-current AI subscription authority |
| 2026-07-29 | 2.0.3 | Documented exact cost-neutral active-subscription runtime projection for client and Admin readers |
