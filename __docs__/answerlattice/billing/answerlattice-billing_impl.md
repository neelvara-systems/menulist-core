...
- If a new or retired-plan recovery checkout has no frozen tax snapshot, the authenticated Billing screen collects and normalizes the required billing profile before calling the existing subscription API. It does not bypass tax admission, infer a legal address, or persist profile data outside the pending subscription created by the server.
