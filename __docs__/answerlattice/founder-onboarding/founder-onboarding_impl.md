# Answerlattice — Founder Onboarding (Knowledge Bootstrap Engine) — Implementation
...
The Founder Onboarding Engine is a **post-publish extension** to the existing KB generation pipeline. It hooks into the moment KB articles are published and embedded, then bootstraps the Answerlattice canonical layer automatically.
...
```
functions-answerlattice/src/answerlattice/onboardingBootstrap.ts    # Core bootstrap engine (CF-side)
...
```
functions-answerlattice/src/answerlattice/answerlatticeNightly.ts         # Add Step 12: onboarding bootstrap
functions-answerlattice/src/constants/features.ts               # Add ENABLE_ANSWERLATTICE_FOUNDER_ONBOARDING
functions-answerlattice/src/constants/database.ts               # Add KB_GENERATION_JOBS + KB_ARTICLES constants
src/config/features.ts                                     # Add ENABLE_ANSWERLATTICE_FOUNDER_ONBOARDING
src/types/knowledgeBase.ts                                 # Add onboardingBootstrap field to IngestionJob
src/types/answerlattice/index.ts                                # Add 'onboarding_bootstrap' to draftSource union
src/components/templates/answerlattice/governance/index.tsx      # Add bootstrap review filter
```

### Existing Files Referenced (Logic Patterns — NOT directly importable from CF)

```
src/lib/answerlattice/entityExtraction.ts                       # extractEntitiesFromArticles() — PATTERN REFERENCE ONLY
src/database/answerlattice/entityCandidates.ts                  # addEntityCandidate(), promoteCandidate() — PATTERN REFERENCE ONLY
src/database/answerlattice/entities.ts                          # addEntity(), upsertEntitySearchIndex() — PATTERN REFERENCE ONLY
src/database/answerlattice/canonicalAnswers.ts                  # addCanonicalAnswer() — PATTERN REFERENCE ONLY
src/database/answerlattice/mutationProposals.ts                 # addMutationProposal() — PATTERN REFERENCE ONLY
src/database/answerlattice/auditLogs.ts                         # addAuditLog() — PATTERN REFERENCE ONLY
functions-answerlattice/src/answerlattice/draftGenerator.ts          # Gemini prompt patterns (DIRECTLY reusable — same CF project)
```

> **CRITICAL ARCHITECTURE NOTE:** The client-side DAL files (`src/database/answerlattice/*.ts`) use
> `answerlatticeFirebaseClient` (Firebase client SDK). The bootstrap engine runs in Cloud Functions
> (`functions-answerlattice/`) which uses `firestoreAdmin` (Firebase Admin SDK). These are **incompatible** —
...
> `DB_COLLECTIONS` constants, exactly as `draftGenerator.ts` and `answerlatticeNightly.ts` already do.
> The client-side DAL functions serve as **logic pattern references** — the CF mirrors their behavior
> using the admin SDK. This is the established Answerlattice CF pattern.
...
**On `AnswerlatticeMutationProposal.suggestedChange`** (already has `draftSource`):
...
**Alternative:** As a step in the Answerlattice nightly batch (simpler, less latency-sensitive).

**Decision:** Run as **nightly batch step** (Step 12 in `answerlatticeNightly.ts`). Rationale:

- Bootstrap is not latency-sensitive (RAG works immediately)
- Avoids coupling to MenuList's KB publish CF
- Runs in Answerlattice's own Firebase project
...
Output: Entity candidates in answerlattice_entityCandidates
```
...
- Requires `ENABLE_ANSWERLATTICE_ONTOLOGY: true` (entity layer)
- Requires `ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS: true` (answer layer)
...
**In `answerlatticeNightly.ts`, add Step 12:**
...
>
> The existing `discoverActiveTenants()` queries `answerlattice_entities` to find tenants.
...
**Decision:** Nightly batch (Step 12 in answerlatticeNightly)
**Rationale:**

- Bootstrap is not latency-sensitive (RAG provides immediate answers)
- Avoids coupling Answerlattice CF to MenuList's KB publish CF
- Naturally cost-controlled (runs once/day)
- Idempotent by design
- Consistent with existing Answerlattice architecture
...
- `answerlattice_entityCandidates` for extracted entities
- `answerlattice_mutationProposals` for answer drafts
- `kb_generation_jobs` for progress tracking (additive field)
- Reduces maintenance, indexing, and cost

### ADR-5: Single Feature Flag

**Decision:** One flag `ENABLE_ANSWERLATTICE_FOUNDER_ONBOARDING` controls entire bootstrap
**Rationale:**

- Bootstrap is atomic — all steps or none
- Individual step flags would create confusing partial states
- Depends on `ENABLE_ANSWERLATTICE_ONTOLOGY` + `ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS` (validated at runtime)
...
3. **Engine:** Create `onboardingBootstrap.ts` in `functions-answerlattice`
4. **Integration:** Add Step 12 to `answerlatticeNightly.ts`
...
4. **Bootstrap for Existing Tenants:** Current design runs on new KB imports. Existing Answerlattice tenants (MenuList itself) may want to bootstrap retroactively. Config flag `SKIP_IF_ENTITIES_EXIST` controls this.

### Architectural Considerations (Updated Post-Audit 2026-03-09)

1. **Cross-Project Read:** Bootstrap engine (in `functions-answerlattice`) needs to read `kb_articles` from the Answerlattice Firestore. This is already the case — KB articles live in Answerlattice Firestore alongside entities. No cross-project reads needed.
...
3. **CF vs Client DAL (CRITICAL — found in audit):** All `src/database/answerlattice/*.ts` DAL files use `answerlatticeFirebaseClient` (client SDK). The bootstrap CF uses `firestoreAdmin` (admin SDK). These are incompatible — CF must mirror DAL logic using admin SDK directly, same as `draftGenerator.ts`. See §2 architecture note.

4. **Tenant Discovery Gap (CRITICAL — found in audit):** `discoverActiveTenants()` in `answerlatticeNightly.ts` queries `answerlattice_entities`. New tenants with zero entities (the exact bootstrap target) would NOT be discovered. Step 12 must have its own discovery query against `kb_generation_jobs`. See §7 architecture note.

5. **Missing DB Constants (found in audit):** `functions-answerlattice/src/constants/database.ts` lacks `KB_GENERATION_JOBS` and `KB_ARTICLES` constants. Must be added during implementation. See §2 modified files list.
