...
- `answerlattice_entityCandidates`: `tId + sId + status` (exists)
- `answerlattice_entities`: `tId + sId` (exists)
- `answerlattice_mutationProposals`: `tId + sId + mutationType + status` (exists)
...
|----------|------|-----------|
| `extractEntitiesFromArticles()` | `src/lib/answerlattice/entityExtraction.ts` | Entity extraction |
| `addEntityCandidate()` | `src/database/answerlattice/entityCandidates.ts` | Store candidates |
| `addEntity()` | `src/database/answerlattice/entities.ts` | Promote entity |
| `upsertEntitySearchIndex()` | `src/database/answerlattice/entities.ts` | Build search index |
| `addMutationProposal()` | `src/database/answerlattice/mutationProposals.ts` | Store draft proposals |
| `addAuditLog()` | `src/database/answerlattice/auditLogs.ts` | Audit trail |
| `buildSearchIndexEntry()` | `src/lib/answerlattice/entityExtraction.ts` | Index construction |
| `getEntities()` | `src/database/answerlattice/entities.ts` | Dedup check |

Note: Bootstrap engine runs in Cloud Functions (`functions-answerlattice`), so it uses `firebase-admin` SDK directly, not the client-side DAL. The DAL functions listed above are the equivalent patterns — CF code will mirror them using `firestoreAdmin`.
