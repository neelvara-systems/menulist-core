...
August 22 QA browser verification exposed the first-write form of this transaction: Firestore must read a missing `answerlattice_cacheVersions/kb_{tId}_{sId}` or `canonical_{tId}_{sId}` document before it can create version 1. Dedicated and shared rules now admit only that missing-document `get`, only for the authenticated active workspace and a user with knowledge-control or widget-management permission. Existing cache-version documents remain data-bound, cross-workspace and unsupported IDs remain denied, and write validation is unchanged. The dedicated/shared platform-summary rule suites cover the admitted and denied cases.
