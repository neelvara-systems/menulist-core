...
The route uses `runtimeStatus`, `allowedOrigins`, `blockedRoutes`, and widget key metadata already returned by `/api/answerlattice/widget-config`. It optionally reads `/api/answerlattice/activation/summary` for workspace name/readiness. The widget-config browser request uses no-store cache, same-origin credentials, and manual redirect handling before response parsing. Widget-config responses are parsed through a 64 KB bounded JSON reader and must match the install setup shape before local setup state is replaced; optional activation-summary responses reuse the shared activation dashboard response guard and are skipped with bounded diagnostics if unavailable. Failed install-state loading uses fixed dashboard copy instead of route or browser exception text. Download-kit and machine-readable-docs links open in a new tab with `noopener,noreferrer`, check the returned browser window, and log `answerlattice_install_link_open_failed` with bounded link href/key/label presence-length metadata only before showing fixed `Could not open install link` copy. AI packet, agent-file, script-snippet, and setup-snapshot copy actions use the shared Answerlattice support clipboard helper, wait for Clipboard API success or acknowledged textarea fallback success, and log `answerlattice_install_copy_failed` with bounded copy-value length/support metadata only before fixed failure copy. The widget management Install tab now links to `/answerlattice/install-center` instead of duplicating the full agent packet. Existing `/answerlattice/widget` remains the widget configuration route and must keep working. The protected packet/kit endpoints rate-limit before permission and store reads, perform one authenticated store read, log unexpected failures with fixed-code bounded tenant/store metadata, and never return the raw key by default.
