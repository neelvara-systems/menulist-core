...
Answerlattice supports a constrained first-party browser SDK through the v1 loader and `window.AnswerlatticeWidget`. It does not support a separately installed npm package or broad general-purpose public SDK. Existing internal helper source may stay in the repo for experiments, but public pages, dashboard snippets, Markdown mirrors, agent files, and ZIP packets must not offer it to end users. Generated docs and packets point agents to `/widget/v1/answerlattice-widget.js`.
...
The route uses `runtimeStatus`, `allowedOrigins`, `blockedRoutes`, and widget key metadata already returned by `/api/answerlattice/widget-config`. It optionally reads `/api/answerlattice/activation/summary` for workspace name/readiness. The widget-config browser request uses no-store cache, same-origin credentials, and manual redirect handling before response parsing. Widget-config responses are parsed through a 64 KB bounded JSON reader; arrays, per-item strings, widget-key prefix, errors, runtime timestamps, and the non-negative safe-integer seen count must match bounded shapes before local setup state is replaced. Optional activation-summary responses reuse the shared activation dashboard response guard and are skipped with bounded diagnostics if unavailable. Failed install-state loading uses fixed dashboard copy instead of route or browser exception text.

The workspace ZIP is fetched with same-origin credentials, no-store cache, and manual redirect handling. The browser requires a successful `application/zip` response, rejects an invalid or over-2-MiB declared length, then rejects an empty or over-2-MiB blob before creating the local download. Failure logs use the fixed `answerlattice_install_kit_download_failed` code and fixed customer copy. Machine-readable-docs links still open in a new tab with `noopener,noreferrer`, check the returned browser window, and log `answerlattice_install_link_open_failed` with bounded link metadata only. AI packet, agent-file, script-snippet, and setup-snapshot copy actions use the shared Answerlattice support clipboard helper and fixed failure copy. Every Install Center command action keeps a 44px minimum target and wrap-safe height.

The widget management Install tab links to `/answerlattice/install-center` instead of duplicating the full agent packet. Existing `/answerlattice/widget` remains the widget configuration route. The protected packet/kit endpoints rate-limit on actor + tenant + workspace before permission and store reads, require `MANAGE_WIDGET`, perform one authenticated exact-scope store read, set private `no-store` and `nosniff` headers on success/failure, log unexpected failures with fixed-code bounded tenant/store metadata, and never return the raw key by default. ZIP generation rejects empty or over-2-MiB output.
...
`buildAnswerlatticeWidgetEmbedSnippet()` HTML-escapes the script URL, widget key, and comma-joined blocked routes before placing them in attributes. Dashboard-controlled values therefore cannot terminate an attribute in copied install markup.
