...
- Dashboard `/answerlattice/install-center` reuses the existing widget-config GET response and existing `runtimeStatus`; it may also read the existing activation summary for workspace name/readiness. No new collection scan, listener, scheduled job, or telemetry collection was added.
- Dashboard `/api/answerlattice/widget-agent-packet` and `/api/answerlattice/widget-agent-kit` each perform one authenticated `stores/{sId}` read when the owner requests or downloads the workspace-specific packet/kit.
