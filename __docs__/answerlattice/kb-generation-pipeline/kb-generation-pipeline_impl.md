...
|------|---------|---------|
| `functions-answerlattice/src/logic/embedArticleWorker.ts` | Task queue | Dedicated Answerlattice worker. Verifies product/workspace/job/run ownership, reuses or generates a validated embedding, and records durable completion. It does not make the article public; finalization owns visibility. |
| `functions-answerlattice/src/logic/regenerateEmbedding.ts` | HTTPS callable | Re-generates one article embedding inside the isolated Answerlattice project. Uses `FieldValue.vector()` for storage. |
| `functions/src/triggers/shared` | Compatibility exports | The historical MenuList `embedArticleWorker`, `publishApprovedJobFn`, and `regenerateEmbedding` names perform no Answerlattice data/provider work and return or log the fixed runtime-moved boundary. `processMenuImages` remains an independently retired MenuList extraction callable. |
