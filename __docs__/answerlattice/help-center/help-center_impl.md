...
The Help Center home follows the same truth boundary. Failed category, popular-article, or changelog requests render persistent translated error alerts with scoped retry controls; they do not render `No categories available`, `No articles available`, or a blank What's New panel as if the current approved source loaded successfully. Retry reuses the existing protected public-content transport and adds no fallback source or alternate support backend.
