...
- Canonical answer creation from drafts exists through the protected governance action and atomic Admin Firestore transaction (`governanceClient.ts`, governance route, `governanceServer.ts`)
...
| 4   | Automatic Knowledge Creation  | ✅ IMPLEMENTED | 2026-03-09; hardened 2026-07-11 | Draft generator/prompt in app and Functions, Nightly Step 9, additive proposal fields, server governance contracts/client/route/transaction, and rules denying direct canonical/proposal-decision browser writes. Docs: `__docs__/answerlattice/automatic-knowledge-creation/` (8 docs + archive). | Flag ON, capped, human-reviewed. Zero new collections. Approval atomically commits canonical truth, proposal state, audit snapshot, and runtime invalidation. |
