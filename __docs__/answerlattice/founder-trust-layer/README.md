...
> **Expansion Item:** #10 (Answerlattice ICP Coverage Index)
> **Tracker:** `__docs__/answerlattice/answerlattice-expansion-tracker.md`
...
│                                                                      │
│  NIGHTLY STEP (extends answerlatticeNightly.ts)                          │
...
| Aggregation frequency | **Nightly**                              | Matches Answerlattice's batch architecture                         |
...
| **5 infrastructure layers**    | ❌ Mostly redundant — 3 of 5 already exist in Answerlattice                      |
| **6 new collections**          | ❌ Wrong — 0 new collections needed                                         |
| **Event instrumentation**      | ❌ Redundant — `aiSearchHistory` already captures events                    |
| **6-hourly aggregation**       | ❌ Over-engineering — nightly is correct for Answerlattice                       |
| **Entity reliability scoring** | ✅ Valid concept — but already built as `EntityHealthScore.tsx`             |
| **Escalation classification**  | ✅ Valid — maps cleanly to existing Answerlattice concepts                       |
| **Retention governance**       | ✅ Valid concept — but already implemented (12-month TTL)                   |
| **Query hashing**              | ❌ Wrong — loses diagnostic value, Answerlattice needs raw queries for debugging |
| **BigQuery archival**          | ❌ Overkill — not needed at Answerlattice's scale                                |
...
- **Requires:** `ENABLE_ANSWERLATTICE_ONTOLOGY` + `ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS` + `ENABLE_ANSWERLATTICE_DRIFT_DETECTION`
- **Benefits from:** `ENABLE_ANSWERLATTICE_FRICTION_INTELLIGENCE` (enriches friction data)
...
1. `src/components/templates/answerlattice/governance/FounderTrustDashboard.tsx` — Trust dashboard UI
2. `src/database/answerlattice/trustMetrics.ts` — One-read DAL for `platformSummary/trustMetrics_{tId}_{sId}`
3. `src/types/answerlattice/index.ts` — `AnswerlatticeTrustMetrics` types
4. `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts` — Trust metrics aggregation step
5. `src/config/features.ts` — Frontend `ENABLE_ANSWERLATTICE_TRUST_METRICS` flag
6. `functions-answerlattice/src/constants/features.ts` — Cloud Functions `ENABLE_ANSWERLATTICE_TRUST_METRICS` flag
7. `__docs__/answerlattice/founder-trust-layer/` — Documentation set
