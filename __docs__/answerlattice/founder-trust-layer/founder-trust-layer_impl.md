...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                         NIGHTLY BATCH (answerlatticeNightly.ts)               │
│                                                                          │
...
**Location:** `platformSummary/trustMetrics_{tId}_{sId}` (Answerlattice Firestore)
**Written by:** `answerlatticeNightly.ts` (nightly batch)
**Read by:** `FounderTrustDashboard.tsx` (1 read)

```typescript
interface AnswerlatticeTrustMetrics {
...
**Location:** `src/components/templates/answerlattice/governance/FounderTrustDashboard.tsx`
...
**Location:** Add to existing `src/database/answerlattice/coverageKPI.ts` (or create `trustMetrics.ts`)

```typescript
export interface AnswerlatticeTrustMetrics { ... }  // Matches §2.1

export const getTrustMetrics = async (
    tId: number, sId: number
): Promise<AnswerlatticeTrustMetrics | null> => {
    return await apiCallComposer(
        async () => {
            const docRef = doc(answerlatticeFirebaseClient,
...
```typescript
// Nightly batch
if (!FUNCTION_FLAGS.ENABLE_ANSWERLATTICE_TRUST_METRICS) {
  console.log("[Answerlattice Trust] Metrics disabled. Skipping.");
  return;
}
...
| ------------------------------------------------------------------------ | ----------------------------- | -------------- |
| `src/components/templates/answerlattice/governance/FounderTrustDashboard.tsx` | Trust dashboard UI            | ~200           |
| `src/database/answerlattice/trustMetrics.ts`                                  | DAL for trust metrics read    | ~30            |
| `src/types/answerlattice/index.ts`                                            | Add AnswerlatticeTrustMetrics type | +60 (additive) |

### Modified Files (4)

| File                                                     | Change                                                         | Lines (est.) |
| -------------------------------------------------------- | -------------------------------------------------------------- | ------------ |
| `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts`     | Add trust metrics aggregation step + pass data between steps   | +80          |
| `src/config/features.ts`                                 | Add `ENABLE_ANSWERLATTICE_TRUST_METRICS` flag                       | +15          |
| `functions-answerlattice/src/constants/features.ts`           | Add `ENABLE_ANSWERLATTICE_TRUST_METRICS` flag (in `FUNCTION_FLAGS`) | +1           |
| `src/components/templates/answerlattice/governance/index.tsx` | Add Trust tab (same pattern as Friction/Branding tabs)         | +15          |
...
**Rationale:** ChatGPT proposed 6 new collections. Answerlattice's architecture already captures all needed data across `aiSearchHistory`, `answerlattice_signalEvents`, and `answerlattice_canonicalAnswers`. Adding parallel collections would duplicate data and increase Firestore cost.
...
**Rationale:** Answerlattice's architecture is batch-oriented. Real-time metrics would require onChange triggers and counters, adding complexity. Founders check trust weekly, not per-minute.
