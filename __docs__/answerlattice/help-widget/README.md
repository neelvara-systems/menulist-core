...
MenuList is wired as a normal external client through `src/components/answerlattice/MenuListAnswerlatticeWidgetEmbed.tsx`. The owner app layout loads the widget only when `NEXT_PUBLIC_MENULIST_ANSWERLATTICE_WIDGET_KEY` is configured. No widget key is hardcoded in source, and the embed does not read MenuList Firebase to resolve Answerlattice scope. Local development uses `http://localhost:3000/widget/answerlattice-widget.js`; QA/Preview MenuList uses `https://answerlattice.menulist.online/widget/answerlattice-widget.js`; production MenuList uses `https://answerlattice.com/widget/v1/answerlattice-widget.js`. `NEXT_PUBLIC_MENULIST_ANSWERLATTICE_WIDGET_SCRIPT_SRC` can override the script host for a temporary preview.
