...
The protected widget-config GET route uses the shared Answerlattice dashboard `DATA_READ` limiter before permission and store reads. Widget-config load, save, and compiled-context stale-mark failures log fixed runtime diagnostic codes with bounded tenant/store metadata. The widget management persisted scope checks fail closed: widget-config, widget-activity fallback filtering, and hosted-help registry/store checks normalize persisted tenant/store fields with `normalizeAnswerlatticeScopeDocumentId()` before matching the active workspace. The widget-key management route rate-limits before permission/store work, caps action bodies at 4KB, and logs key-management failures with fixed-code bounded tenant/store/user/action/key metadata.
