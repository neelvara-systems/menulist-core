...
The protected widget-config GET route uses the shared Answerlattice dashboard `DATA_READ` limiter before permission and store reads. Widget-config load, save, and compiled-context stale-mark failures log fixed runtime diagnostic codes with bounded tenant/store metadata. The widget management persisted scope checks fail closed: widget-config, widget-key transactions, install packet/kit exports, notification tests, widget-activity fallback filtering, and hosted-help registry/store checks require exact `AL` product plus canonical tenant/store identity through `isAnswerlatticeStoreInScope()` or the equivalent scoped reader. Missing or whitespace-mutated persisted identity is not accepted as legacy fallback. The widget-key management route rate-limits before permission/store work, caps action bodies at 4KB, and logs key-management failures with fixed-code bounded tenant/store/user/action/key metadata.
...
The public iframe `WidgetClient` submits search and feedback requests with same-origin credentials, no-store cache policy, and manual redirect handling. Because those requests originate from the Answerlattice iframe rather than the customer's host page, the host-validated config response includes a short-lived HMAC authorization bound to the widget-key hash and workspace without exposing `tId` or `sId`. The loader passes that token to the iframe and refreshes it before expiry; search and feedback accept either a directly allowed browser origin or a valid token whose signed host origin is still in the current allowlist. `ANSWERLATTICE_WIDGET_RUNTIME_SECRET` is a dedicated server-only secret and missing configuration fails closed for stores with an origin allowlist.

Search success parses through a 256 KB bounded JSON reader and requires the compact widget answer shape (`answer`, optional references, related content, suggestions, procedure, and graph-expansion suggestions) before rendering an assistant message. Arrays and text are capped, links must be HTTP(S) or safe relative paths, and internal scope/key/vector fields are rejected. Malformed, oversized, redirected, rejected, or wrong-shape responses keep fixed public failure copy and log bounded query/session/support metadata only.
...
- Origin allowlist check (v2): configured origins are normalized; direct calls require an allowed `Origin`, while iframe calls require the short-lived host authorization minted by the allowed-origin config request
...
`publicApi` remains reserved for Answerlattice public API credentials. `validatePublicApiKey()` supports both credential sources, but each route explicitly opts into only the sources and scopes it accepts. Answerlattice public API and MCP session admission additionally require the credential's store row to pass exact `AL` product/tenant/store ownership before scoped reads or token issuance. Widget runtime routes only accept `answerlatticeWidgetApi` credentials in separated Firebase mode and fail closed if Answerlattice Admin is unavailable. MenuList public API routes only accept `ml_` keys.
