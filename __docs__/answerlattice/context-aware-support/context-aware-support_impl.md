...
**Answerlattice's approach: Extend existing retrieval pipeline, not build new systems.**
...
### New: AnswerlatticeContextPayload
...
export const AnswerlatticeContextSchema = z
  .object({
...
if (body.context && FEATURE_FLAGS.ENABLE_ANSWERLATTICE_CONTEXT_AWARE) {
  try {
    const { AnswerlatticeContextSchema } =
...
if (candidateProductContext && FEATURE_FLAGS.ENABLE_ANSWERLATTICE_CONTEXT_AWARE) {
  try {
    const { AnswerlatticeContextSchema } =
      await import("@lib/validation/contextSchema");
...
Boundary rule: this mount context is a transient retrieval hint, not Answerlattice tenancy or source identity. It must not carry MenuList tenant/store/menu IDs, customer contact data, or a hardcoded source product. Source identity belongs in CCT/`sourceContext`; MenuList is one client adapter, not an Answerlattice core default.

### 4.6.2 — Server Retrieval Boundary

`coreSearch()` and `attemptCanonicalRetrieval()` run in API routes, so Answerlattice canonical retrieval, instant-cache entity lookup, and server signal emission must use `answerlatticeFirestoreAdmin`. They must not use browser Firebase DAL functions that depend on client auth state.

Current server-side read path:

- `src/lib/search/searchCore.ts` reads entity index and latest release through Answerlattice Admin Firestore for instant-cache lookup.
- `src/lib/answerlattice/canonicalRetrieval.ts` reads entity index, canonical answers, releases, and entity descriptions through Answerlattice Admin Firestore.
- `src/lib/answerlattice/signalEmitter.ts` writes server-side signals through Answerlattice Admin Firestore and skips invalid tenant/store context.
...
```typescript
ENABLE_ANSWERLATTICE_CONTEXT_AWARE: true, // Context-aware retrieval: inject product state into canonical matching
```
...
| ----------------- | ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `entityHints`     | +50 each           | Explicit developer signal. Highest confidence. Developer tells Answerlattice exactly which entity is relevant.        |
...
- Answerlattice already has the right retrieval structure
...
- Answerlattice's ICP is SaaS developers — they can integrate the widget context contract (15-30 min effort)
...
### ADR-5: No Page/Feature Mapping Tables Inside Answerlattice

**Decision:** Answerlattice does NOT maintain mapping tables from page identifiers to entity IDs.

**Rationale:**

- The client product knows its own page→entity mapping better than Answerlattice
...
1. Add `AnswerlatticeContextPayload` type to `src/types/answerlattice/index.ts`
2. Create `src/lib/validation/contextSchema.ts` (Zod schema)
3. Add `ENABLE_ANSWERLATTICE_CONTEXT_AWARE` feature flag
