...
Allow the client product (SaaS using Answerlattice) to pass **structured product context** alongside every support query. This context flows into the existing retrieval pipeline to boost entity matching accuracy.
...
**Then** Answerlattice returns Stripe-specific troubleshooting (not generic connection docs).
...
**Then** Answerlattice resolves "this" to webhook configuration based on page context.
...
**Then** Answerlattice behaves exactly as today (no regression).
...
SaaS developers explicitly send context via the Answerlattice v1 widget browser contract.
...
Answerlattice targets **SaaS developers** (ICP). Explicit browser-contract instrumentation aligns with widget integration expectations without creating a separate public SDK.
