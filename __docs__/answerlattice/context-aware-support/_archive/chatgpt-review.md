...
| 8 separate components needed | ❌ REJECTED | Answerlattice's existing retrieval pipeline needs enhancement, not 8 new systems. |
...
| Page/Feature Mapping System | ❌ REJECTED | Client sends stable identifiers. Answerlattice doesn't need internal mapping tables. Context strings are tokenized against entity search index (same as query tokens). |
...
| ChatGPT Component | Answerlattice Decision | Rationale |
|-------------------|-------------------|-----------|
| Context Payload Schema | **ADOPT** (with Answerlattice types) | Valid design. Add as `AnswerlatticeContextPayload` interface. |
...
| Page/Feature Mapping | **REJECT** | Client sends stable IDs. Answerlattice tokenizes them against search index. No mapping tables needed. |
...
**Answerlattice decision:** 1 enhanced retrieval function + 1 new validation file.

**Why:** Answerlattice's retrieval pipeline (`canonicalRetrieval.ts`) is already well-structured with clear separation:
...
**Answerlattice decision:** Use existing per-request Firestore read.

**Why:** The entity search index read is 1 Firestore query per retrieval attempt. At Answerlattice's expected scale (<100K queries/day in first year), this is:
...
**Answerlattice decision:** SDK-only. No inference.

**Why:** Answerlattice targets SaaS developers (ICP). They can integrate an SDK (15-30 min). URL inference:
...
|---------|--------|-----------|
| SDK-first context collection | Stripe, Segment, Sentry | ✅ Answerlattice follows same pattern |
| Context as request metadata (not stored) | OpenAI Agents SDK, Google context framework | ✅ Answerlattice design matches |
| Deterministic entity resolution with context | Enterprise RAG best practices (2025) | ✅ Answerlattice's rule-based approach is more reliable than ML-only |
| Zendesk `updatePath` + `identify` commands | Zendesk Widget API | ✅ Similar concept — widget accepts runtime context from host app |
| Intercom Fin uses conversation context + customer attributes | Intercom Fin AI Engine | ✅ Answerlattice goes further with product ontology binding |
...
| ML-based context inference | Generic RAG systems | Answerlattice is deterministic-first. ML is fallback only. |
| Full product state dump | Over-engineered enterprise solutions | Answerlattice only needs product state, not user analytics |

---

## §6 — Implementation Impact Assessment

| Dimension | ChatGPT Estimate | Answerlattice Reality |
|-----------|-----------------|-----------------|
