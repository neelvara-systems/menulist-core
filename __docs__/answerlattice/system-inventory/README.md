...
| Native Knowledge Intake connectors | Not implemented; reserved flag only | `src/config/features.ts`, `scripts/verification/verify-answerlattice-native-intake-connectors-boundary.js` | none | No OAuth, credential, provider, sync, UI, route, Functions, rule, index, or public claim exists. Manual GitHub Release export is evidence intake, not a connector. Use export/file/selected-URL/repeated-reply intake; reconsider one read-only provider only after repeated paying-client evidence and complete permission/deletion/cost design. |
