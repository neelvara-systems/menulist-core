...
5. Every answer path writes `aiSearchHistory` for feedback/analytics; widget feedback updates that row. Negative feedback signal-emission failures are logged with bounded metadata and do not fail the feedback response.
...
5. Manual AI governance operations such as draft regeneration, entity extraction, FAQ generation, and translation resolve session scope, check safe mode, rate-limit before permission/request-body/provider work, and log unexpected failures with bounded diagnostics.
