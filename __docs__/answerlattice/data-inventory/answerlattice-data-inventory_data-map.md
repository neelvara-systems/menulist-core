...
| `answerlattice_releases` | Release/version state for governed answer set. Answerlattice App Release ID Boundary normalizes release document IDs before single-release reads, activation/processing writes, drift audit metadata, or compiled-context source invalidation. | Release management/governance. | Retrieval freshness, instant cache versioning, compiled context. | Versioned answer authority. | Durable. Deletes disabled in rules. |
...
| `answerlattice_intakeUsageLedger` | Reserved/finalized/refunded usage units, action, provider/model, byte size, token counts, token count source, metadata, monthly-vs-top-up debit source, and subscription/store balance effects. Answerlattice App Billing Document ID Boundary normalizes ledger IDs before finalize/refund refs. | Media extraction reserve/finalize/refund. | Billing/credit accountability. | Paid intake usage control. | Durable ledger; no TTL found. |
...
Authenticated public-content cache reads (`/api/answerlattice/public-content`) keep tenant/store-scoped cache helpers for KB categories, articles, FAQs, and changelog pages; article detail requests validate KB article IDs through the shared Firestore document-ID boundary before the cached article read, and cache-load failures log fixed runtime codes with bounded content type and tenant/store metadata. Browser callers read the same route through `publicContentClient.ts`, which pins no-store cache, same-origin credentials, and manual redirect handling before the 1 MB bounded JSON parser requires the `{ data }` envelope for Help Center, FAQ, AI Search, and changelog views.
