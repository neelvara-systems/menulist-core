...
- Deferred Owner Support Assistant summary/action/feedback/analytics collections that are not implemented. The live read-only assistant reads five existing compact `platformSummary` documents and creates no collection or write path.
...
| `answerlattice_signalEvents` | Public API signals and internal ticket/chat/feedback/escalation signals: type, normalized entity id or `unresolved`, sanitized metadata, timestamp, actor trace, optional dedupe key, and `expiresAt`. | Signal emitter, public signal ingestion, ticket/feedback/chat flows. Explicit external/request IDs use deterministic server document IDs. | Mutation governance, friction aggregation, support board, predictive help. | Evidence, not automatic authority, without duplicate rows from retried external signals. | New writers set a rule-bounded 365-day `expiresAt`; Firestore TTL owns deletion. Legacy rows use the bounded migration in `firebase-forensic-audit.md`. |

Public/widget/MCP write and AI-triggering routes keep their auth, product, purpose, scope, rate-limit, and origin checks before body parsing. The public answers route caps JSON at 16KB, public signals at 32KB, widget feedback at 2KB, predictive help at 4KB, MCP JSON-RPC at 16KB, and widget search at the existing inline image allowance plus 64KB of JSON overhead. Public answers/entities/signals, widget search/config/feedback, predictive help, MCP failure diagnostics, and protected app-route success breadcrumbs use fixed runtime codes with bounded tenant/store metadata so runtime logs do not expose raw workspace identifiers.
...
| `answerlattice_supportBoardCards` | Internal private support cards, status, priority, notes, linked source/signal/history ids, status history. Nightly recurring-miss and signal-cluster derived cards store source example counts/context only instead of duplicating raw query, prompt, subject, reason, or message text. | Support Board UI and nightly support board sync. | Owner/staff private support workboard. | Convert signals/history into reviewable support tasks without turning derived cards into a second raw source-text store. | Durable; no TTL found. |
...
3. Public widget config authenticates the API key, checks scope/origin/rate limits, reads sanitized config, predictive summary, and public bundle refs. Runtime-status write failures, predictive-summary read failures, and public bundle-manifest read failures are logged with bounded metadata and do not fail the config response.
...
5. Every answer path writes `aiSearchHistory` for feedback/analytics; widget feedback validates search-history document IDs before updating the row. Answerlattice signal entity ID boundary: top-level signal `entityId` values are normalized through the governance document-ID boundary before `answerlattice_signalEvents` writes, with malformed IDs stored as `unresolved`. Negative feedback signal-emission failures are logged with bounded metadata and do not fail the feedback response.
6. Protected Help Center search and article embedding routes rate-limit before parsing bounded JSON bodies, then validate request shape before calling search or embedding providers. Help Center search validation failures use the shared safe Zod detail payload rather than locally mapping issue messages.
7. Self-service onboarding rate-limits before Answerlattice account lookup, uses a 32KB bounded body before schema validation and durable writes, and logs failures with fixed-code bounded metadata.
8. Platform-only Answerlattice intake monitor retries parse a 2KB bounded trigger body, and owner cache revalidation parses an optional 4KB bounded body before revalidating public tags.
