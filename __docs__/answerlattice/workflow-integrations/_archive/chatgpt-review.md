# Answerlattice — External Workflow Integrations — ChatGPT Review
...
|-------|:----------:|-------|
| Answerlattice should be event producer only, NOT workflow orchestrator | ✅ CORRECT | Aligns perfectly with Non-Goals Charter §I (not a helpdesk) and doctrine freeze |
...
| Append-only event log in Firestore | ✅ CORRECT | Matches existing Answerlattice patterns (signalEvents, auditLogs are append-only) |
| onCreate Cloud Function trigger | ✅ CORRECT | Best pattern for Firebase — event-driven, zero cost when idle |
| Per-project integration config | ✅ CORRECT | Adapted to per-tenant in platformSummary (Answerlattice uses tId/sId, not projectId) |
...
|-------|:----------:|---------|
| Event types: generic (ticket_created, ticket_escalated, etc.) | ⚠️ WRONG FOCUS | Answerlattice doesn't send ticket events — it sends **governance events** (drift_detected, mutation_proposed, coverage_drop). ChatGPT proposed helpdesk-style events. |
| `projects/{projectId}/integrations` config path | ❌ WRONG | Answerlattice uses `tId/sId` scoping, not `projectId`. Config stored in `platformSummary/integrationConfig_{tId}_{sId}`. |
| `integration_events/{eventId}` collection name | ⚠️ ADAPTED | Changed to `answerlattice_integrationEvents` to follow Answerlattice naming convention (`answerlattice_` prefix). |
| `integration_logs/{eventId}` for delivery logs | ⚠️ ADAPTED | Changed to `answerlattice_integrationDeliveryLogs` (separate collection, not log per event). |
...
| No mention of existing notification infrastructure | ❌ MISSED | Answerlattice already has `ENABLE_ANSWERLATTICE_NOTIFICATIONS` flag with email notification system for ticket events. The integration email adapter must coexist cleanly with this. |
...
| "processIntegrationEvent()" as generic name | ⚠️ OK BUT ADAPTED | Cloud Function exported as `processAnswerlattice IntegrationEvent` for clarity in multi-product environment. |
...
| Addition | Why |
|----------|-----|
| **Governance-specific event types** | ChatGPT proposed generic ticket events. Answerlattice's value is knowledge governance events (drift, mutations, coverage). Completely different event vocabulary. |
| **platformSummary config storage** | Follows existing Answerlattice pattern for per-tenant config. No new collection for config. |
| **answerlattice_ collection prefix** | All Answerlattice collections use this prefix (existing convention). |
...
| **Coverage drop threshold (60%)** | Specific, meaningful threshold from existing AnswerlatticeCoverageKPI system. |
...
| `projects/{projectId}` scoping | **`tId/sId` scoping** | Answerlattice uses tenant/store isolation, not project-level. |
| Firebase Secret Manager | **Environment variables (v1)** | Simpler for <100 tenants. Migrate to Secret Manager at scale. |
| Generic event types (ticket_created, etc.) | **Governance event types** | Answerlattice's unique value is knowledge governance, not ticket management. |
...
|----------|-------------|
| Bidirectional Slack sync | Violates Answerlattice identity (event producer only) |
...
| Workflow automation builder | Turns Answerlattice into Zapier. Permanent exclusion. |
...
However, ChatGPT had **zero context about Answerlattice's actual architecture**:
...
Cascade's synthesis preserved the valid architectural pattern while adapting it to Answerlattice's real infrastructure, naming conventions, and governance philosophy.
