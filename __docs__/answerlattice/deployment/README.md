...
- No additional Firebase, GCP, Vercel, DNS, key, or provider configuration is
  currently required for Answerlattice QA. The owner-approved account model is
  closed: `admin@neelvara.com` is the only human operator for every product;
  Cloud Identity Free is active at no charge but no additional user was
  created. The only setup-adjacent control still waiting on external state is
...
2. Complete the first isolated restore rehearsal after a ready backup exists.
