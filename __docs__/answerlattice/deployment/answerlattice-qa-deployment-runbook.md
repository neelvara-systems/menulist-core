...
- 2026-08-02: Gemini rolling-spend admission and structured jittered retry changed the shared Answerlattice gateway used by KB generation/embedding and nightly provider health. Local gateway tests, runtime verifier, root TypeScript, lint, Functions build, and deploy preflight passed. The exact affected-subset command `firebase deploy --project answerlattice-qa --config firebase-answerlattice.json --only functions:answerlattice:startGeneration,functions:answerlattice:retryGeneration,functions:answerlattice:answerlatticeNightly,functions:answerlattice:triggerAnswerlatticeNightly,functions:answerlattice:embedArticleWorker,functions:answerlattice:regenerateEmbedding --non-interactive` stopped before predeploy/upload with `Error: Failed to authenticate, have you run firebase login?`. No QA or production Function revision changed; rules, indexes, Storage, hosting, app routes, and unrelated Functions were not targeted.
...
- 2026-07-26: Public compiled-context revocation hardening changed app/Functions object metadata to mandatory revalidation and made dedicated/shared Storage rules explicitly deny direct client reads so all maintained access stays on the rate-limited, size-bounded, existence-revalidating proxy. Both Storage-rule emulators, exact TypeScript, focused lint, runtime source verification, dedicated Functions build, and the 102-object lifecycle emulator pass locally. `npm run verify:functions-deploy-preflight` passed; `firebase deploy --only storage --project answerlattice-qa --config firebase-answerlattice.json --non-interactive`, `firebase deploy --only storage --project menulist-qa --config firebase.json --non-interactive`, and `firebase deploy --only functions:answerlatticeNightly --project answerlattice-qa --config firebase-answerlattice.json --non-interactive` each stopped before upload with `Error: Failed to authenticate, have you run firebase login?`. No QA rule or Function revision changed.
...
### Latest bounded-retention deploy evidence

- 2026-07-26: Compiled-bundle retention now uses deterministic tenant/workspace public-prefix ownership and a durable bounded exact-version cursor. Local root TypeScript, focused ESLint, app/dedicated/shared identity contracts, Firestore/Storage retention emulator, lifecycle Auth/Firestore/Storage emulator, runtime source verification, both Functions builds, deployment preflight, audit tooling, and diff integrity passed. The required scoped QA command `firebase deploy --only functions:answerlatticeNightly --project answerlattice-qa --config firebase-answerlattice.json --non-interactive` failed before upload with `Error: Failed to authenticate, have you run firebase login?`; no remote Function revision changed. Do not treat the local repair as deployed until an authenticated operator reruns the exact command and captures readback/runtime evidence.
- 2026-07-26: The adjacent empty-pointer orphan repair makes retention and lifecycle deletion derive the expected public prefix even when a partial build left no manifest/pointer. Its focused emulators, exact TypeScript, lint, runtime source gate, and dedicated Functions build passed. The same scoped `answerlatticeNightly` deploy was retried after this source change and again failed before upload with the same Firebase CLI authentication error; no remote revision changed.
- 2026-07-26: Failed reserved bundle versions now have bounded nightly recovery through the exact failed lock, including a durable completion marker. The Firestore/Storage emulator, exact TypeScript, focused lint, runtime source verification, and dedicated Functions build passed. The scoped `answerlatticeNightly` deploy again failed before upload because Firebase CLI authentication is unavailable; no remote revision changed.
- 2026-07-26: Empty-pointer retention now persists its derived scan completion instead of repeating empty Storage listings. Focused emulator/type/lint/build/source gates passed; the scoped `answerlatticeNightly` deploy again failed before upload with the same missing Firebase CLI authentication.
- 2026-07-26: The compiled-context salt is now declared and explicitly bound to both scheduled and manual nightly exports. Dedicated Functions build, focused lint, manual scheduler contract, and the aggregate runtime verifier pass locally. Both the nightly-only attempt and `firebase deploy --only functions:answerlatticeNightly,functions:triggerAnswerlatticeNightly --project answerlattice-qa --config firebase-answerlattice.json --non-interactive` stopped before upload with `Error: Failed to authenticate, have you run firebase login?`; no remote Function or secret binding changed. The authenticated retry must confirm an existing QA `ANSWERLATTICE_PUBLIC_BUNDLE_SALT` secret version and use the complete two-export command.
- 2026-07-28: Knowledge Map added `currentVersion`, drift, and review-required counters plus incoming/outgoing relation direction to the existing entity graph summary. The nightly builder now snapshots existing entity/relation/canonical source-version counters before graph queries, writes an empty graph after the final active entity is removed, and lets the owner map report current, stale, or unverified freshness without mutation-time rebuilds. The cost delta is one existing control-document point read per nightly graph rebuild and one additional existing control-document point read per owner map load; no collection, listener, mutation-time map write, AI call, rule, or index was added. Focused map/runtime-summary/scheduler contracts, dedicated and shared platform-summary Firestore-rule emulators, Answerlattice TypeScript, focused lint, dependency freeze, the dedicated Functions build, and the complete `verify:answerlattice-runtime-truth` gate pass locally. `firebase deploy --only functions:answerlatticeNightly,functions:triggerAnswerlatticeNightly --project answerlattice-qa --config firebase-answerlattice.json --non-interactive` was re-attempted after the final gate and stopped before upload with `Error: Failed to authenticate, have you run firebase login?`; no remote Function revision changed.
- 2026-07-28: Knowledge Map persisted-summary recovery now independently hashes the current stored graph payload, validates the exact AL tenant/workspace envelope, and exact-replaces corrupted or stale-shape summaries instead of trusting the stored `sourceHash` alone. The focused Knowledge Map suite, dedicated Functions build, root exact TypeScript, repository lint, audit tooling, and diff hygiene pass locally. The required `firebase deploy --only functions:answerlatticeNightly,functions:triggerAnswerlatticeNightly --project answerlattice-qa --config firebase-answerlattice.json --non-interactive` attempt stopped before predeploy/upload with `Error: Failed to authenticate, have you run firebase login?`; no remote Function revision changed.
