# Answerlattice — Full Forensic System Audit
...
| 🟢 LOW      | 4     | Doc-code drift in nightly step count, coverageKPI reads from wrong Firestore project, functions-answerlattice placeholder, minor scope conflicts                              |
...
| --- | ----------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.1 | Answerlattice Website        | ✅ BUILT     | 18 files in `src/app/sites/answerlattice/`. 6 pages (home, product, pricing, about, contact, get-started). Shared components.                                        |
...
| 4.1 | White-Label Branding       | ✅     | `WhiteLabelBranding.tsx` (10.8KB) + `branding.ts` DAL + `AnswerlatticeBrandingConfig` type       |
| 4.2 | Multi-Language Articles    | ✅     | `MultiLanguageArticles.tsx` (13.4KB) + `/api/answerlattice/translate` route + Gemini integration |
...
#### BUG-1: Answerlattice Nightly Scheduler is DISCONNECTED

**Location:** `functions/src/decisionBlocksScoring.ts` line 1274
**Problem:** The Answerlattice nightly block was REMOVED from the MenuList scheduler (comment says "MOVED TO SEPARATE FIREBASE PROJECT"). But `functions-answerlattice/src/index.ts` is a **placeholder** with TODO comments — no actual Cloud Functions are exported.

**Impact at time of audit:** The entire nightly operational loop (drift detection, signal resolution, mutation clustering, coverage KPI, fallback detection, impact tracking, confidence adjustment, signal TTL) was **dead code**. It existed only in the legacy MenuList-side Answerlattice nightly file and was not called by anything.

**Root cause:** The multi-product separation playbook created the `functions-answerlattice/` directory but the actual function migration was listed as a manual founder action item (doc `10-implementation-action-items.md` §4-6) that has NOT been completed.

**Fix required:** Either:

- (A) Re-wire `answerlatticeNightly` into `decisionBlocksScoring.ts` temporarily (for internal experiment), OR
- (B) Complete the function migration to `functions-answerlattice/` and deploy

#### BUG-2: GovernanceHub Missing tId/sId Props

**Location:** `src/app/(answerlattice)/answerlattice/governance/page.tsx` line 16
...
#### BUG-3: functions-answerlattice is Empty Placeholder

**Location:** `functions-answerlattice/src/index.ts`
**Problem:** The file contains only `export const placeholder = true;` and TODO comments. No actual Cloud Functions are exported. The `ENABLE_ANSWERLATTICE_NIGHTLY` feature flag in `functions/src/constants/features.ts` references code that was removed from the MenuList scheduler but never added to functions-answerlattice.
...
**Location:** `src/app/api/answerlattice/onboard/route.ts` line 40
**Problem:** The onboarding route checks `ENABLE_ANSWERLATTICE_NOTIFICATIONS` as a gate for onboarding — but the check has no consequence (no return statement, just a comment "TODO: Add dedicated ENABLE_ANSWERLATTICE_ONBOARDING flag"). The feature flag is effectively ignored.

```typescript
if (!FEATURE_FLAGS.ENABLE_ANSWERLATTICE_NOTIFICATIONS) {
  // Reuse this flag as onboarding gate — if notifications aren't on, Answerlattice isn't live
  // TODO: Add dedicated ENABLE_ANSWERLATTICE_ONBOARDING flag if needed
}
```

**Impact:** Low (the check is a no-op, so onboarding works). But the comment suggests this was meant to be a gate — currently anyone with auth can create an Answerlattice tenant.
...
**Location:** `src/database/answerlattice/branding.ts` line 24
**Problem:** `getBrandingDocRef(sId)` only uses sId — the tId parameter is passed to the function but never used. The doc key is `answerlattice_branding_${sId}` without tId isolation.

**Impact:** If two tenants share the same sId (unlikely in current MenuList but possible in multi-tenant Answerlattice), their branding configs would collide.

#### BUG-7: Notification Service Not Wired to Ticket DAL

**Location:** `src/database/tickets/index.ts`
**Problem:** The ticket creation signal emitter is wired (`emitAnswerlatticeSignal`), but `sendNotification` from `@lib/notifications` is NOT called anywhere in the ticket DAL or ticket reply flow. The notification service exists but has zero call sites beyond manual testing.
...
**Location at time of audit:** legacy MenuList-side Answerlattice nightly file, line 453
**Problem:** `aggregateCoverageKPI` reads from `DB_COLLECTIONS.AI_SEARCH_HISTORY` using `db` (which is `firestoreAdmin` — the MenuList Firestore). But per the multi-product separation doctrine, Answerlattice should use its own Firestore. Search history is written by the search-kb route which uses `answerlatticeFirestoreAdmin`.

**Impact:** If Answerlattice gets its own Firebase project, the nightly KPI aggregation will read from the wrong database and find zero results.

### 🟢 LOW ISSUES

#### LOW-1: Doc-Code Drift on Nightly Step Count

**Documentation:** answerlattice-activation-clearance.md §4.4 says "7-Step Answerlattice Batch"
**Code:** `answerlatticeNightly.ts` actually runs **8 steps** (step 8 is Signal TTL added in Phase 4)
...
**Problem:** The nightly CF drift detection (`answerlatticeNightly.ts` line 161) checks scope conflict with a simpler algorithm than the client-side version (`driftDetection.ts` line 131-169). The CF version only checks entity + version overlap, while the client version also checks plan/role overlap.
...
→ Feature flag: ENABLE_ANSWERLATTICE_SIGNAL_MUTATION
...
→ emitAnswerlatticeSignal({ type: 'chat_negative', tId, sId, metadata })
...
```
runAnswerlatticeNightly() → discoverActiveTenants()
→ Per tenant:
...
### 5.1 Collections (9 Answerlattice + shared)

All 9 `ANSWERLATTICE_*` constants defined in both:
...
**19 Answerlattice composite indexes** found in `firestore.indexes.json`:

- `answerlattice_entities` (2 indexes)
- `answerlattice_canonicalAnswers` (3 indexes)
- `answerlattice_releases` (2 indexes)
- `answerlattice_signalEvents` (4 indexes)
- `answerlattice_auditLogs` (3 indexes)
- `answerlattice_mutationProposals` (2 indexes)
- `answerlattice_entityCandidates` (2 indexes)
- `answerlattice_entitySearchIndex` (1 index)
- `answerlattice_entityRelations` (1 index — missing from original 17 count in docs)
...
All 9 DAL files in `src/database/answerlattice/`:

- ✅ Use `DB_COLLECTIONS` constants
- ✅ Use `apiCallComposer` wrapper
- ✅ Use `requestBodyComposer` for writes
- ✅ Use `answerlatticeFirebaseClient` (NOT `firebaseClient`)
...
1. **Re-wire nightly scheduler** — Completed via `functions-answerlattice/` migration; do not restore Answerlattice work inside `decisionBlocksScoring.ts`.
...
6. **Add `ENABLE_ANSWERLATTICE_ONBOARDING` feature flag** — Don't reuse the notifications flag.
...
### Answerlattice-Specific Files: 55+ files

| Layer                      | Files                                                      | Total Lines |
| -------------------------- | ---------------------------------------------------------- | ----------- |
| **Types**                  | `src/types/answerlattice/index.ts`                              | 481         |
| **DAL**                    | 9 files in `src/database/answerlattice/`                        | ~2,500      |
| **Lib**                    | 6 files in `src/lib/answerlattice/`                             | ~1,500      |
| **Hooks**                  | 4 files in `src/hooks/answerlattice/`                           | ~500        |
| **Templates (Governance)** | 9 files in `src/components/templates/answerlattice/governance/` | ~3,500      |
| **Templates (Other)**      | 3 files in `src/components/templates/answerlattice/`            | ~500        |
| **Layout**                 | 3 files in `src/components/answerlattice/`                      | ~400        |
| **Dashboard Routes**       | 10 files in `src/app/(answerlattice)/answerlattice/`                 | ~200        |
| **Website**                | 18 files in `src/app/sites/answerlattice/`                      | ~2,000      |
| **API Routes**             | 2 files in `src/app/api/answerlattice/` + 1 widget route        | ~650        |
| **Cloud Functions**        | 1 file `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts` | 857         |
| **Notifications**          | 4 files in `src/lib/notifications/`                        | ~500        |
| **Feature Flags**          | 10 Answerlattice flags in `src/config/features.ts`              | —           |
| **DB Constants**           | 9 collections in both client + server constants            | —           |
| **Firestore Indexes**      | 19 Answerlattice composite indexes                              | —           |

**Total estimated Answerlattice codebase: ~13,000 lines across 55+ files.**
...
After these 3 fixes: Follow the Activation Experiment Framework in `answerlattice-activation-experiment.md`.

### For External SaaS Founders (Production)

**NOT READY** — requires additional work:

- Complete Cloud Functions migration to `functions-answerlattice/`
...
| --- | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **BUG-1/BUG-3: functions-answerlattice was empty placeholder** | Completed full Cloud Function migration: created `constants/database.ts`, `constants/features.ts`, copied `answerlatticeNightly.ts`, wired as `onSchedule` CF (3:00 AM UTC) + `onRequest` manual trigger | `functions-answerlattice/src/constants/database.ts` (NEW), `functions-answerlattice/src/constants/features.ts` (NEW), `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts` (MIGRATED), `functions-answerlattice/src/index.ts` (REWRITTEN) |
| 2   | **BUG-2: GovernanceHub missing tId/sId**                  | Added `useClientAuthSession()` to governance page, passes `tId`/`sId` to GovernanceHub                                                                                                              | `src/app/(answerlattice)/answerlattice/governance/page.tsx`                                                                                                                                                                          |
| 3   | **BUG-4: Onboarding route no-op feature flag**            | Replaced no-op check with actual `ENABLE_ANSWERLATTICE_WIDGET` gate that returns 403                                                                                                                     | `src/app/api/answerlattice/onboard/route.ts`                                                                                                                                                                                    |
...
| 5   | **TS2802: Map iteration in functions-answerlattice**           | Wrapped `Map.entries()` in `Array.from()` for TypeScript compatibility                                                                                                                              | `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts`                                                                                                                                                                       |

### Session 2 Fixes (Remaining Items)

| #   | Bug                                                  | Fix                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Files Changed                                                                                                                                                                                          |
| --- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 6   | **BUG-6: Branding DAL missing tId in doc key**       | Changed doc key from `answerlattice_branding_${sId}` to `answerlattice_branding_${tId}_${sId}`. Updated `getBrandingDocRef` signature to accept `(tId, sId)`.                                                                                                                                                                                                                                                                                                                | `src/database/answerlattice/branding.ts`                                                                                                                                                                    |
...
| 8   | **BUG-8: CoverageKPI Firestore project mismatch**    | **FALSE POSITIVE** — Both write side (`search-kb` → `addAiSearchHistory` via `answerlatticeFirebaseClient`) and read side (`answerlatticeNightly` → `firestoreAdmin` via `ANSWERLATTICE_FIREBASE_*` env) connect to Answerlattice Firestore. **However**, found and fixed a real tId isolation issue: changed doc key from `answerlattice_${sId}` to `answerlattice_${tId}_${sId}` across all files that read/write this doc. Also updated `getAnswerlatticeCoverage()` signature to require `tId`. | `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts`, `src/database/answerlattice/coverageKPI.ts`, `src/components/templates/answerlattice/AnswerlatticeCoverageKPI.tsx` |

### Verification

- `npx tsc --noEmit` (dashboard): **PASS — 0 errors** ✅
- `npx tsc --noEmit` (functions-answerlattice): **PASS — 0 errors** ✅
- `grep answerlattice_${sId}` across src/ + functions/ + functions-answerlattice/: **0 results** (all stale patterns eliminated) ✅
- Notification wiring chain verified: 3 triggerNotification() call sites → API route → sendNotification() → 3 templates ✅
- aiSearchHistory Firestore project verified: both sides use Answerlattice Firestore ✅
