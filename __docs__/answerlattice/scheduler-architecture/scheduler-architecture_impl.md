...
Firestore run-log persistence uses two compatible projections. `tenantRuns` is a bounded flat summary array containing scope, status, duration, counters, `taskCount`, and `errorCount`; it never embeds a second array. `tenantRunsByScope` stores the detailed per-tenant evidence as a map keyed by `{tId}_{sId}`, and every nested sequence is represented as an ordinal map. Owner Operations continues reading the flat summaries, while the platform intake monitor prefers the detailed scope map and retains legacy-array read compatibility. This is required because Firestore rejects arrays nested anywhere inside an array; the former `tenantRuns[].tasks[]` shape could leave a completed run log at `running` even when all tenant tasks succeeded.

The maintained combined gate is `npm run test:answerlattice-master-scheduler:emulator`. It starts Firestore and Storage emulators, uses a disposable bundle salt, validates a real two-tenant governance batch and compiled-context output, excludes an active non-due tenant and an inactive tenant, recovers one stale credit reservation exactly once, confirms repeat-tick idempotency, exercises expired and held task leases, and proves malformed recovery evidence produces a partial scheduler result without minting credits or blocking independent tasks.

Hosted QA adds provider/runtime evidence that emulators cannot supply. It uses four disposable scopes, observes one due-work execution plus the next untouched hourly attempt, validates all 20 per-tenant task outcomes and the concrete compiled-context/graph/trust/knowledge/predictive outputs, proves credit recovery exactly once, and then deletes its exact Firestore and Storage footprint. Production promotion is limited to the same compiled Functions source and required additive indexes; no production fixture or forced scheduler invocation is permitted.

The friction cleanup and chat-analytics cursor queries require ascending composite indexes matching their ascending cursor order. Descending owner-screen indexes are not interchangeable. Managed Functions also derive the Storage bucket from a strictly validated `FIREBASE_CONFIG.storageBucket` fallback when no Answerlattice-specific override is configured. These corrections add no scheduler query, listener, or write: they allow the existing bounded operations to complete instead of failing after already-incurred reads.
