...
1. [What Is Answerlattice](#1-what-is-answerlattice)
...
## 1. What Is Answerlattice

Answerlattice is the **Governed Answer Infrastructure for SaaS Support**. It is NOT a helpdesk, chatbot, or CMS. It is an infrastructure layer that sits behind the existing Help Center and makes support knowledge deterministic, versioned, and governed.
...
| --- | --------------------------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| 1   | **Product Ontology**        | Models product concepts (features, plans, roles, workflows, states, integrations, errors) as first-class entities | `ENABLE_ANSWERLATTICE_ONTOLOGY`          |
| 2   | **Canonical Answer Engine** | Governed, versioned, entity-bound answer assets that replace probabilistic RAG                                    | `ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS` |
| 3   | **Drift Governance**        | Detects when answers become stale through 4 drift classes                                                         | `ENABLE_ANSWERLATTICE_DRIFT_DETECTION`   |
| 4   | **Signal Mutation**         | Converts friction signals (tickets, negative feedback) into mutation proposals for human review                   | `ENABLE_ANSWERLATTICE_SIGNAL_MUTATION`   |
| 5   | **API & Integration**       | Release management, version binding, audit trail                                                                  | `ENABLE_ANSWERLATTICE_PUBLIC_API`        |
...
3. Feature flag `ENABLE_ANSWERLATTICE_ONTOLOGY` is checked — if OFF, returns empty
...
8. Stored in `answerlattice_entityCandidates` with status `pending`
9. Admin reviews and approves/rejects/merges each candidate
10. Approved candidates are promoted to `answerlattice_entities`

**Files:** `src/lib/answerlattice/entityExtraction.ts`, `src/database/answerlattice/entityCandidates.ts`, `src/database/answerlattice/entities.ts`
...
6. Stored in `answerlattice_canonicalAnswers`
...
3. Feature flag `ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS` checked — if OFF, falls through to RAG
4. **Layer 1 — Deterministic Entity Index Lookup:**
   - Query tokenized (lowercase, special chars removed, tokens > 1 char)
   - Tokens matched against `answerlattice_entitySearchIndex` entries
...
**Files:** `src/lib/answerlattice/canonicalRetrieval.ts`, `src/database/answerlattice/canonicalAnswers.ts`, `src/database/answerlattice/entities.ts`, `src/app/api/helpCenter/search-kb/route.ts`
...
3. `emitAnswerlatticeSignal({ type: 'ticket', tId, sId, metadata: { ticketId, subject, category, priority } })` called
4. This is **fire-and-forget** — it never blocks ticket creation
5. Feature flag `ENABLE_ANSWERLATTICE_SIGNAL_MUTATION` checked
6. If enabled: signal stored in `answerlattice_signalEvents`
...
3. If feedback is negative (`isGood === false`):
   - `emitAnswerlatticeSignal({ type: 'chat_negative', tId, sId, metadata: { searchHistoryId, sessionId, messageId, reasons, comments } })` called
...
**Files:** `src/lib/answerlattice/signalEmitter.ts`, `src/database/tickets/index.ts`, `src/components/templates/main-app/helpChat/api.ts`

### 4.4 Nightly Job (Answerlattice Batch)

**When:** Every day at 3:00 AM UTC, exported from `functions-answerlattice/src/index.ts` as `answerlatticeNightly`.

**Feature flag:** `ENABLE_ANSWERLATTICE_NIGHTLY` in `functions-answerlattice/src/constants/features.ts` (ready-to-use default: true)

**What happens:**

1. Feature flag `ENABLE_ANSWERLATTICE_NIGHTLY` checked — if OFF, task is skipped
2. **Tenant Discovery:** Reads `platformSummary/answerlatticeTenantsSummary` first (1 doc). Client onboarding registers tenants as inactive for scheduler purposes until entities exist. Manual entity creation and onboarding bootstrap mark `hasEntities: true`; if the summary is missing/empty, the scheduler falls back to the legacy `answerlattice_entities` scan (max 1000 docs), logs truncation, and backfills the summary for future runs.
3. **For each tenant — governed sequential steps:**
   ...
   **Step 3 — Signal Mutation:**
   ...
   - For clusters meeting threshold (≥3 signals):
     ...
     - Stores as `pending_review` in `answerlattice_mutationProposals`
   ...
   **Step 4B — Founder Trust Metrics:**
   - Feature-flagged by `ENABLE_ANSWERLATTICE_TRUST_METRICS`
...
**Manual trigger:** `triggerAnswerlatticeNightly` is HTTPS-only and requires `Authorization: Bearer ${CRON_SECRET}` outside the Firebase emulator.

**Files:** `functions-answerlattice/src/index.ts`, `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts`, `functions-answerlattice/src/constants/features.ts`, `functions-answerlattice/src/constants/database.ts`
...
**Files:** `src/database/answerlattice/mutationProposals.ts`, `src/database/answerlattice/auditLogs.ts`, `src/components/templates/main-app/helpCenter/MutationProposalReview.tsx`, `src/hooks/answerlattice/useMutationProposals.ts`
...
3. Real entity created in `answerlattice_entities` (status: 'active', version: v1.0.0)
...
**Files:** `src/database/answerlattice/entityCandidates.ts` (`promoteCandidate`), `src/lib/answerlattice/entityExtraction.ts` (`buildSearchIndexEntry`)
...
| Normal usage            | No Answerlattice-specific actions required           |
| No UI changes           | Answerlattice is invisible infrastructure            |
...
**Key insight:** Only the Platform Owner interacts with Answerlattice directly. SMB owners and end customers benefit from it without knowing it exists.
...
| ----------------------------------- | ---------------------------------------------------------------- | -------------------------------------------- |
| `ENABLE_ANSWERLATTICE_ONTOLOGY`          | Entity collections, search index, KB article filtering by tenant | Yes — foundation layer                       |
| `ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS` | Canonical-first retrieval in search-kb                           | Yes, but needs ontology data to be useful    |
| `ENABLE_ANSWERLATTICE_DRIFT_DETECTION`   | 4-class drift evaluation (nightly + on release)                  | Yes, but needs canonical answers to evaluate |
| `ENABLE_ANSWERLATTICE_SIGNAL_MUTATION`   | Signal emitters + nightly clustering + mutation proposals        | Yes — signals accumulate safely              |
| `ENABLE_ANSWERLATTICE_PUBLIC_API`        | External Answerlattice API for answers, entity registry, and signal ingestion | Yes after Pillars 1-4 are live and API keys are issued |
...
14 Answerlattice Firestore collections, all prefixed `answerlattice_` for isolation:

| Collection                         | Purpose                        | Scoping   | Operations               |
| ---------------------------------- | ------------------------------ | --------- | ------------------------ |
| `answerlattice_entities`                | Product entities (ontology)    | tId + sId | CRUD (type immutable)    |
| `answerlattice_entityRelations`         | Entity-to-entity relationships | tId + sId | CRD                      |
| `answerlattice_entitySearchIndex`       | Deterministic retrieval index  | tId + sId | Upsert                   |
| `answerlattice_entityCandidates`        | AI-extracted candidate staging | tId + sId | CRU (status transitions) |
| `answerlattice_canonicalAnswers`        | Governed versioned answers     | tId + sId | CRUD (entityIds >= 1)    |
| `answerlattice_releases`                | Product version releases       | tId + sId | CRU                      |
| `answerlattice_signalEvents`            | Friction signal log            | tId + sId | Append/archive           |
| `answerlattice_mutationProposals`       | Mutation queue                 | tId + sId | CRU (status transitions) |
| `answerlattice_auditLogs`               | Governance audit trail         | tId + sId | Append-only              |
| `answerlattice_frictionDailyStats`      | Daily friction aggregates      | tId + sId | Server write, client read|
| `answerlattice_schedulerRunLogs`        | Nightly scheduler diagnostics  | Platform  | Server write, platform read |

Answerlattice query-backed collections have query/vector indexes mirrored in both `firestore.indexes.json` and `firestore-answerlattice.indexes.json` where the shared and dedicated deployments need the same access path, including the tenant-filtered `kb_articles` vector search index.

All Answerlattice collection constants are mirrored in `src/constants/database.ts`, `functions/src/constants/database.ts`, and `functions-answerlattice/src/constants/database.ts`.

Shared DB testing uses the default Firebase Auth token. Dedicated Answerlattice Firebase mode uses `/api/auth/set-claims` to mint an Answerlattice custom token and `syncAnswerlatticeAuthWithCustomToken()` to sign the browser into the separate Answerlattice Firebase app with the same tenant/store claims.

---

## 8. End-to-End Flows (Happy Path)

### Flow A: First-Time Answerlattice Setup

```
1. Platform Owner enables ENABLE_ANSWERLATTICE_ONTOLOGY
...
7. Enables ENABLE_ANSWERLATTICE_CANONICAL_ANSWERS
8. Customer search queries now check canonical answers first
9. Enables ENABLE_ANSWERLATTICE_SIGNAL_MUTATION
10. Tickets and negative feedback start generating signals
11. Enables ENABLE_ANSWERLATTICE_DRIFT_DETECTION
...
```
3:00 AM UTC — functions-answerlattice `answerlatticeNightly` fires
...
| Feature flag OFF                      | All operations return empty/skip               | Zero — system behaves as if Answerlattice doesn't exist |

**Design principle:** Answerlattice is advisory infrastructure. It NEVER blocks critical user flows (ticket creation, chat, search, releases).
...
**Key cost feature:** Once entities and canonical answers are set up, Answerlattice operates with zero LLM costs. All retrieval and governance is deterministic.
...
| **Write safety**              | Answerlattice writes use `answerlatticeRequestBodyComposer` (`pId='AL'`, source context, trace IDs, auto timestamps) |
...
| DB constants mirrored           | **PASS** — 14 Answerlattice collections mirrored across client, MenuList functions, and Answerlattice functions |
...
| Scheduler wiring                | **PASS** — `answerlatticeNightly` exported from `functions-answerlattice/src/index.ts` at 3:00 AM UTC |
| DAL pattern compliance          | **PASS** — Answerlattice DAL files use DB_COLLECTIONS + apiCallComposer |
...
| Issue                                          | Location                                          | Fix                                                                         |
| ---------------------------------------------- | ------------------------------------------------- | --------------------------------------------------------------------------- |
| Stale doc references (`__docs__/help-center/`) | `.cascade/rules/ANSWERLATTICE_RULES.md` (lines 18, 94) | Updated to `__docs__/answerlattice/`                                             |
...
- No console.log violations in Answerlattice Functions; client-side fire-and-forget warning logs remain non-blocking
...
1. ~~No admin UI for Answerlattice~~ → **MutationProposalReview UI** built (list + approve/reject)
...
6. ~~Nightly job missing operational loop~~ → **Consolidated Answerlattice batch** with drift + resolution + mutation + coverage + trust metrics + fallback + impact + confidence
7. ~~No mobile-safe Answerlattice shell~~ → **Responsive owner shell** with mobile drawer navigation, sticky header, scrollable governance tables, and viewport-width modals
...
| File                    | Lines | Contents                                         |
| ----------------------- | ----- | ------------------------------------------------ |
| `src/types/answerlattice.ts` | 410   | All interfaces, enums, and helpers for 5 pillars |

### DAL Layer (7 files, ~1,280 lines total)

| File                                         | Lines | Functions | Collection(s)                                |
| -------------------------------------------- | ----- | --------- | -------------------------------------------- |
| `src/database/answerlattice/entities.ts`          | 299   | 10        | entities, entityRelations, entitySearchIndex |
| `src/database/answerlattice/canonicalAnswers.ts`  | 185   | 7         | canonicalAnswers                             |
| `src/database/answerlattice/mutationProposals.ts` | 177   | 6         | mutationProposals                            |
| `src/database/answerlattice/releases.ts`          | 190   | 5         | releases                                     |
| `src/database/answerlattice/entityCandidates.ts`  | 215   | 6         | entityCandidates (+promoteCandidate)         |
| `src/database/answerlattice/signalEvents.ts`      | 132   | 4         | signalEvents                                 |
| `src/database/answerlattice/auditLogs.ts`         | 90    | 3         | auditLogs                                    |

### Library Layer (6 files, ~1,380 lines total)

| File                                     | Lines | Purpose                                             |
| ---------------------------------------- | ----- | --------------------------------------------------- |
| `src/lib/answerlattice/canonicalRetrieval.ts` | 358   | 3-layer retrieval stack (parallel entity reads)     |
| `src/lib/answerlattice/driftDetection.ts`     | 342   | 4-class drift evaluation engine                     |
| `src/lib/answerlattice/signalMutation.ts`     | 289   | Signal clustering + mutation proposals              |
| `src/lib/answerlattice/entityExtraction.ts`   | 245   | AI entity extraction + buildSearchIndexEntry        |
| `src/lib/answerlattice/signalEmitter.ts`      | 80    | Fire-and-forget + deduplication                     |
| `src/lib/answerlattice/tokenizer.ts`          | 35    | Shared deterministic tokenizer (index + query time) |

### Cloud Functions (1 file, ~810 lines)

| File                                        | Lines | Schedule                                          |
| ------------------------------------------- | ----- | ------------------------------------------------- |
| `functions-answerlattice/src/answerlattice/answerlatticeNightly.ts` | ~810  | Exported by `functions-answerlattice/src/index.ts` at 3:00 AM UTC |
...
| `src/hooks/answerlattice/useMutationProposals.ts`                              | ~90   | Data fetching + actions hook   |
...
| File                                     | Purpose                                      |
| ---------------------------------------- | -------------------------------------------- |
| `src/config/features.ts`                 | Answerlattice client feature flags                |
| `functions-answerlattice/src/constants/features.ts` | ENABLE_ANSWERLATTICE_NIGHTLY flag (CF)      |
| `src/constants/database.ts`              | 14 ANSWERLATTICE\_\* collection constants (client) |
| `functions/src/constants/database.ts`    | Answerlattice constants mirrored for shared functions |
| `functions-answerlattice/src/constants/database.ts` | Answerlattice constants for dedicated functions |
| `functions-answerlattice/src/index.ts`        | Answerlattice scheduled + manual function exports |
| `firestore.rules`                        | Shared-DB Answerlattice tenant rules              |
| `firestore-answerlattice.rules`               | Dedicated Answerlattice Firebase tenant rules     |
| `firestore.indexes.json`                 | 33 Answerlattice query/vector indexes for shared mode |
| `firestore-answerlattice.indexes.json`        | 33 Answerlattice query/vector indexes for dedicated mode |
| `IDE_PROMPTS/MASTER-EXECUTION-PROMPT.md` | STEP 9B: Answerlattice completion rules           |
| `.cascade/rules/ANSWERLATTICE_RULES.md`       | Development rules for Answerlattice               |

### Documentation (in `__docs__/answerlattice/`)

| Document                                                | Purpose                                       |
| ------------------------------------------------------- | --------------------------------------------- |
| `doctrine/01-core-doctrine.md`                          | Product identity, pillars, retrieval doctrine |
| `doctrine/02-non-goals-charter.md`                      | What Answerlattice will NOT become                 |
...
| 3   | All 14 Answerlattice DB collections defined in all database constant mirrors    | ✅     |
...
| 5   | All 33 Answerlattice Firestore query/vector indexes mirrored in shared and dedicated index files | ✅     |
| 6   | Nightly job exported from functions-answerlattice/src/index.ts                 | ✅     |
...
| 19  | Nightly job exported from `functions-answerlattice` (ENABLE_ANSWERLATTICE_NIGHTLY) | ✅     |
