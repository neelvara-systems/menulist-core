...
> **Audience:** Developers
> **Firestore Project:** Answerlattice (separate from menulist-qa)
...
| `answerlattice_signalEvents` | Write (1 per escalation) | Emit ESCALATION signal for mutation engine |
| `answerlattice_auditLogs` | Write (0 — audit is via signal) | No direct audit write needed |
...
| 3 | Emit ESCALATION signal | `answerlattice_signalEvents` | 1 write |
...
| Step | Operation | Collection | Cost |
|------|-----------|-----------|------|
| 1 | Canonical retrieval (existing) | `answerlattice_entitySearchIndex` + `answerlattice_canonicalAnswers` | 2-4 reads (already happening) |
...
|-----------|-------|--------|
| Daily conversations per tenant | 50-200 | Answerlattice ICP (SaaS founders) |
...
| `emitAnswerlatticeSignal()` | `src/lib/answerlattice/signalEmitter.ts` | Emits ESCALATION signal |
