...
> **Reviewer:** Cascade (with full Answerlattice codebase access)

---

## §1 — Conversation Summary

ChatGPT proposed "Product Friction Intelligence" as System 5 of 12 in the Answerlattice ICP Coverage Index. The conversation covered 6 components (Systems 26-31) with 68 total capability blocks across all 12 systems.
...
|---|-----------------|-----------------|--------------|
| 1 | Signal aggregation is foundation | ✅ Agree | BUT: collection already exists as `answerlattice_signalEvents`. No new collection needed. |
...
| 3 | Daily aggregation of stats | ✅ Agree | New `answerlattice_frictionDailyStats` collection. 1 doc per entity per day. |
...
|---|-----------------|------------------|
| 1 | New `supportSignals` collection | **Already exists** as `answerlattice_signalEvents` with 4 DAL functions, fire-and-forget emitter, deduplication, entity resolution, and 12-month TTL. |
| 2 | Embedding-based query clustering + Vector DB (Qdrant/pgvector) | **Violates Answerlattice doctrine.** Deterministic > LLM. Entity graph already provides topic taxonomy via 7 entity types. No external Vector DB needed. |
...
| 4 | `workspaceId` model | **Wrong model.** Answerlattice uses `tId/sId` (tenant + store). ChatGPT unaware of multi-tenant architecture. |
...
| 7 | 6 new Firestore collections (supportSignals, topicClusters, featureFriction, workflowFailures, topicDailyStats, featureDailyStats) | **Excessive.** Only 1 new collection needed (`answerlattice_frictionDailyStats`). Insights stored in existing `platformSummary`. |
| 8 | Separate embedding storage | **Not applicable.** Answerlattice uses deterministic token matching, not embeddings. |
...
| 2 | Workflow step-level failure detection | Needs `ENABLE_ANSWERLATTICE_CONTEXT_AWARE` with sufficient data. |
...
2. **Proposed 6+ new collections when 1 suffices:** Over-engineered data model. Answerlattice's `platformSummary` pattern handles insights.
...
5. **`workspaceId` instead of `tId/sId`:** Wrong tenant model. Shows ChatGPT doesn't know Answerlattice internals.
...
**Overall: ~45%** — Strategic direction was right, but implementation was largely unaware of existing Answerlattice infrastructure.

---

## §6 — Key Takeaway

ChatGPT provided valuable **conceptual framing** — the idea that support signals should surface product friction is correct and important. But the **implementation proposal was 55% redundant** with existing Answerlattice infrastructure. The final architecture reuses 13 existing systems and adds only 1 new collection + 2 platformSummary docs + 2 new Cloud Function files + 3 new frontend files.
