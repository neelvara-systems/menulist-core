...
```
┌──────────────────────────────────────────────────────────────────────┐
│                    EXISTING ANSWERLATTICE PIPELINE                        │
│                                                                      │
│  signalEmitter ──→ answerlattice_signalEvents ──→ signalMutation          │
│                          │                        │                  │
│                          │                        ▼                  │
│                          │               mutationProposals            │
│                          │                                           │
...
│  answerlattice_signalEvents + aiSearchHistory                             │
│       ──→ answerlattice_frictionDailyStats (per entity, per day)         │
│       ──→ platformSummary/frictionSnapshot_{tId}_{sId}              │
│                                                                      │
│  STEP 10 — Weekly Friction Insight (Sundays only)                   │
│  answerlattice_frictionDailyStats (14 days)                              │
│       ──→ Gemini summary                                            │
...
### 2.1 — New Collection: `answerlattice_frictionDailyStats`
...
```typescript
interface AnswerlatticeFrictionDailyStat {
    tId: number;
    sId: number;
    entityId: string;
...
```
answerlattice_frictionDailyStats: tId ASC, sId ASC, date DESC
answerlattice_frictionDailyStats: tId ASC, sId ASC, entityId ASC, date DESC
```
...
```typescript
interface AnswerlatticeFrictionSnapshot {
...
```typescript
interface AnswerlatticeFrictionInsight {
    lastUpdated: Timestamp;
...
### 3.1 — Nightly Friction Aggregation (Step 9 of answerlatticeNightly)

**Trigger:** Part of existing nightly scheduler (3:00 AM UTC).
**Runs:** Every night for every active Answerlattice tenant.

```
Input:
  - answerlattice_signalEvents (last 24h, per tenant)
...
Output:
  - N documents in answerlattice_frictionDailyStats (1 per active entity)
...
### 3.2 — Weekly Friction Insight (Step 10 of answerlatticeNightly, Sundays only)

**Trigger:** Sunday night in nightly scheduler (after Step 9).
**Runs:** Once per week for every active Answerlattice tenant.
...
```
functions-answerlattice/src/answerlattice/frictionAggregation.ts    — Nightly aggregation logic
functions-answerlattice/src/answerlattice/frictionInsight.ts         — Weekly Gemini insight generation
...
```
src/config/features.ts                                     — Add ENABLE_ANSWERLATTICE_FRICTION_INTELLIGENCE flag
src/constants/database.ts                                  — Add ANSWERLATTICE_FRICTION_DAILY_STATS constant
functions-answerlattice/src/constants/database.ts               — Mirror collection constant
functions-answerlattice/src/answerlattice/answerlatticeNightly.ts          — Add Step 9 + Step 10
...
2. Add types to `src/types/answerlattice/index.ts`
3. Implement `frictionAggregation.ts` (nightly Step 9)
4. Wire into `answerlatticeNightly.ts`
...
2. Add Gemini prompt
3. Wire Sunday-only logic into answerlatticeNightly
4. Test with mock friction data

### Phase 3: Frontend (Day 4-5)
1. Add DAL `src/database/answerlattice/frictionStats.ts`
2. Add hook `src/hooks/answerlattice/useFrictionInsights.ts`
...
**Decision:** Use Answerlattice's entity graph as the topic taxonomy instead of embedding-based clustering.
**Rationale:** Answerlattice doctrine mandates deterministic over LLM. Entity graph already has 7 types with names, aliases, and relations. Adding separate ML-based topic clustering duplicates the ontology and introduces non-deterministic behavior.
...
**Decision:** Only `answerlattice_frictionDailyStats`. Insights go in existing `platformSummary`.
**Rationale:** Minimizes Firestore footprint. platformSummary is the established pattern for Answerlattice KPI docs (coverage, branding already use it). No need for separate `frictionInsights` collection when data is 1 doc per tenant.
...
**Rationale:** Answerlattice already has a deterministic entity search index with tokenized lookup, aliases, and synonyms. The entity graph IS the topic taxonomy. Adding vector similarity would introduce non-deterministic topic grouping that conflicts with Answerlattice's governance philosophy.

### ADR-5: Workflow Step Failure Deferred
**Decision:** Defer workflow step-level failure detection to v2.
**Rationale:** While procedure steps exist on canonical answers, detecting which step fails requires page context + user session sequencing. This needs `ENABLE_ANSWERLATTICE_CONTEXT_AWARE` to be active with sufficient data. Insufficient ROI for v1.
...
**Safe to deploy with flag OFF.** Zero impact on existing Answerlattice behavior.
...
### Logs
- `[Answerlattice Friction] Aggregation complete for {tId}/{sId}: {entityCount} entities, {totalSignals} signals`
- `[Answerlattice Friction] Insight generated for {tId}/{sId}: health={health}`
- `[Answerlattice Friction] Skipped insight for {tId}/{sId}: insufficient data ({signalCount} < 5)`
- `[Answerlattice Friction] Stats cleanup: {deleted} docs older than 90 days`

### Nightly Result Extension
```typescript
interface AnswerlatticeNightlyResult {
    // ... existing fields ...
