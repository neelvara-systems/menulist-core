# Answerlattice — Guided Workflows: Technical Implementation
...
Convert Answerlattice canonical answers from text-only to optionally structured procedures. Enable deterministic, step-based answers for procedural ("how to") queries while maintaining backward compatibility, zero new collections, and 2-read retrieval cost.

---

## §2 — Position Inside Answerlattice Architecture
...
Add to `src/types/answerlattice/index.ts`:
...
export type AnswerlatticeAnswerType =
  (typeof ANSWERLATTICE_ANSWER_TYPES)[keyof typeof ANSWERLATTICE_ANSWER_TYPES];
...
export type AnswerlatticeProcedureAction =
  (typeof ANSWERLATTICE_PROCEDURE_ACTIONS)[keyof typeof ANSWERLATTICE_PROCEDURE_ACTIONS];
...
export type AnswerlatticeWarningSeverity =
  (typeof ANSWERLATTICE_WARNING_SEVERITY)[keyof typeof ANSWERLATTICE_WARNING_SEVERITY];
...
export type AnswerlatticePrerequisiteType =
  (typeof ANSWERLATTICE_PREREQUISITE_TYPE)[keyof typeof ANSWERLATTICE_PREREQUISITE_TYPE];
...
### §3.2 — Modified Type: AnswerlatticeCanonicalAnswer

Add two new fields (additive, freeze-compliant):

```typescript
export interface AnswerlatticeCanonicalAnswer {
  // ... all existing fields unchanged ...

  answerType?: AnswerlatticeAnswerType; // NEW — defaults to 'explanation' if undefined
  ...
  content: {
    ...
    edgeCases?: string;
    constraints?: string;
    procedure?: AnswerlatticeProcedure; // NEW — required when answerType === 'procedure'
  };
}
```
...
### §3.3 — Modified Type: AnswerlatticeMutationProposal

Extend `suggestedChange` to support procedure structure:

```typescript
export interface AnswerlatticeMutationProposal {
  // ... all existing fields unchanged ...
  ...
  suggestedChange: {
    ...
    edgeCases?: string;
    constraints?: string;
    procedure?: AnswerlatticeProcedure; // NEW — for procedure refinement proposals
  };
}
```
...
**Zero new collections.** All data lives on existing `answerlattice_canonical_answers` documents.
...
| ----------------------------------------- | --------------------------------------------- | ------------ |
| `src/lib/answerlattice/procedureValidation.ts` | Write-time validation for procedure structure | ~80          |

### §5.2 — Modified Files

| File                                                                     | Change                                                                       | Impact                      |
| ------------------------------------------------------------------------ | ---------------------------------------------------------------------------- | --------------------------- |
| `src/types/answerlattice/index.ts`                                            | Add procedure types, answer type enum, constraints const                     | Additive only               |
| `src/config/features.ts`                                                 | Add `ENABLE_ANSWERLATTICE_GUIDED_WORKFLOWS` flag                                  | Additive only               |
| `src/database/answerlattice/canonicalAnswers.ts`                              | Add procedure validation on `addCanonicalAnswer` and `updateCanonicalAnswer` | Write-time guard            |
| `src/lib/answerlattice/canonicalRetrieval.ts`                                 | Return `answerType` in retrieval result                                      | Minor enhancement           |
...
| `src/components/templates/answerlattice/governance/CanonicalAnswerEditor.tsx` | Add step editor UI when `answerType === 'procedure'`                         | UI enhancement              |

---

## §6 — Procedure Validation Logic

New file: `src/lib/answerlattice/procedureValidation.ts`
...
5. `action` MUST be a value from `ANSWERLATTICE_PROCEDURE_ACTIONS`
6. Warnings limited to 5, each message ≤200 chars, severity from `ANSWERLATTICE_WARNING_SEVERITY`
7. Prerequisites limited to 5, each description ≤200 chars, type from `ANSWERLATTICE_PREREQUISITE_TYPE`
...
export interface CanonicalRetrievalResult {
  ...
  answerType?: AnswerlatticeAnswerType; // NEW — exposed for widget/API response
}
```
...
```tsx
{
  answerType === "procedure" && (
    <ProcedureStepEditor
      steps={steps}
      onChange={setSteps}
      maxSteps={ANSWERLATTICE_PROCEDURE_CONSTRAINTS.MAX_STEPS}
    />
  );
}
```
...
- Per step: Action dropdown (from `ANSWERLATTICE_PROCEDURE_ACTIONS`) + Instruction input
...
The `suggestedChange` field on `AnswerlatticeMutationProposal` already mirrors the answer `content` structure. Adding `procedure?: AnswerlatticeProcedure` to `suggestedChange` allows the mutation engine to propose procedure refinements.
...
| ---- | ----------------------------------------------------------------------- | ---------------- |
| 1    | Add types to `src/types/answerlattice/index.ts`                              | Small            |
| 2    | Add feature flag to `src/config/features.ts`                            | Small            |
| 3    | Create `src/lib/answerlattice/procedureValidation.ts`                        | Small            |
...
**Decision:** Embed procedure data in the existing `AnswerlatticeCanonicalAnswer.content` field.

**Rationale:**

- Canonical answer is the atomic knowledge unit per Answerlattice doctrine
...
**Decision:** Store approved actions as `ANSWERLATTICE_PROCEDURE_ACTIONS` const, not a Firestore collection.
