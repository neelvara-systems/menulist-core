# Answerlattice — Guided Workflows: ChatGPT Feedback Review
...
| 12 | ChatGPT proposes `entityId` + `intent` on answer | ❌ REJECTED | Answerlattice uses `scope.entityIds[]` (multiple entities). ChatGPT doesn't know Answerlattice's actual schema. |
...
**~55% on architecture decisions** — Core concept (structured steps) was correct, but almost all collection/storage proposals were wrong for Answerlattice's embedded model. ChatGPT designed for a generic system; Answerlattice's approach is superior for Firebase cost discipline.
