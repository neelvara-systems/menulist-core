...
> **Version:** 1.0.0
> **Created:** 2026-03-09
> **Source:** ChatGPT conversation about System D (Automatic Knowledge Creation) from Answerlattice ICP expansion
...
|---|-------------------|----------|-----------|
| 1 | Semantic embedding clustering with Qdrant | **REJECTED** | Answerlattice uses entity-based clustering (deterministic, $0 cost, explainable). Doctrine: "Entities define structure." |
| 2 | New `supportSignals` collection | **REJECTED** | `answerlattice_signal_events` already exists with 4 DAL functions |
| 3 | New `clusters` collection | **REJECTED** | Entity-based clustering is in-memory (Map), no persistence needed |
| 4 | New `knowledgeProposals` collection | **REJECTED** | `answerlattice_mutation_proposals` already handles this exact use case |
...
| 7 | 30-day signal retention | **REJECTED** | Answerlattice doctrine mandates 12-month TTL (already implemented) |
...
- Human review must be mandatory (correct — matches Answerlattice doctrine)
- Batch processing over real-time (correct approach)
- Gap detection from signal patterns (correct — already exists in Answerlattice)
- The system should NOT become a ticket analytics dashboard (correct — matches doctrine)

**What ChatGPT Got WRONG:**
- Assumed Answerlattice needs semantic embedding clustering → entity-based already exists
...
ChatGPT was unaware of Answerlattice's entity-based architecture. It designed a generic AI support system from scratch, missing that ~70% of the infrastructure already exists. The entity-first approach is fundamentally different from the embedding-first approach ChatGPT assumed.
...
This is strategically correct. Answerlattice's signal pipeline + entity ontology + gap detection is genuinely a product intelligence layer, not just a help center feature. The automatic knowledge creation enhancement makes this intelligence actionable.
