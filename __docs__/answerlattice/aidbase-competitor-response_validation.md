...
| Prevent old fallback cache from bypassing truth | `instantCache.ts`, `cacheFreshness.ts`, `searchCore.ts` | `canon:v5` hashes query, complete context, entity/plan/role/state key segments, validates cached payloads, and retains evaluated confidence. Graph-aware selection bypasses Redis until graph state has an authoritative cache version; `rag-v4` includes canonical source version; non-canonical rows require references and yield when canonical truth changes. |
