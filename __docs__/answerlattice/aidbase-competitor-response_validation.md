...
| Prevent old fallback cache from bypassing truth | `instantCache.ts`, `cacheFreshness.ts`, `searchCore.ts` | `canon:v4` hashes entity/plan/role/state key segments, validates cached payloads, and retains evaluated confidence; `rag-v4` includes canonical source version; non-canonical rows require references and yield when canonical truth changes. |
