...
2. Audited entire Answerlattice codebase for existing relevant infrastructure
...
|---|--------------|---------|--------|
| 1 | Need client SDK with `canon.page()`, `canon.feature()`, `canon.workflow()` | **MODIFIED** | AnswerlatticeContextPayload ALREADY has page/feature/workflow fields. Only need to extend widget to send on page entry (not just on search). |
| 2 | Events must use stable page IDs, not URLs | **ACCEPTED** | Correct. AnswerlatticeContextPayload already uses identifiers, not URLs. |
| 3 | Event payload must be <500 bytes | **ACCEPTED** | Correct constraint. |
| 4 | Need Edge Collector (Cloud Run) + Pub/Sub + Message Queue | **REJECTED** | Massive over-engineering. Answerlattice doesn't have 100K users with 20 events/session. A simple API call per page entry is sufficient. Stateless evaluation means the API route IS the "trigger worker." |
...
| 3 | Only allow: pageId, featureId, workflowId, role, plan, eventType as conditions | **ACCEPTED** | Adopted. Maps to AnswerlatticeContextPayload fields. |
...
| # | ChatGPT Claim | Verdict | Reason |
|---|--------------|---------|--------|
| 1 | Need frictionPatterns collection with entityType, topic, signalCount, confidence | **REJECTED** | This ALREADY EXISTS as AnswerlatticeFrictionSnapshot (topFrictionEntities + emergingTopics) computed nightly by frictionAggregation.ts. No new collection needed. |
...
| 3 | Daily pattern detection job | **MODIFIED** | Already runs nightly in answerlatticeNightly.ts. |
| 4 | 30-day signal retention | **MODIFIED** | Answerlattice uses 12-month TTL on signals. 14-day rolling window for clustering. |
...
| 6 | Topic extraction via lightweight NLP | **REJECTED** | Answerlattice uses entity-based classification, not topic extraction. Entity binding > NLP. |
| 7 | supportSignals collection | **REJECTED** | Already exists as answerlattice_signalEvents. |
...
| 7 | POST /predictive/suggestions endpoint | **MODIFIED** | Route: POST /api/answerlattice/predictive-help (consistent naming). |
...
| 5 | suggestionSignals collection, 14-30 day retention | **REJECTED** | Uses existing answerlattice_signalEvents with new signal types. 12-month TTL. |
...
4. ❌ **Topic extraction via NLP** — Answerlattice uses entity-based classification.
...
7. ❌ **2M events/day scenario** — Irrelevant for Answerlattice's B2B SaaS scale.
8. ❌ **Separate frictionPatterns collection** — Already exists as AnswerlatticeFrictionSnapshot in platformSummary.

---

## §5 — What ChatGPT Missed (Answerlattice-Specific)

1. AnswerlatticeContextPayload already provides page/feature/workflow context
2. AnswerlatticeFrictionSnapshot already computes friction patterns nightly
...
| Architecture | 20% | Pub/Sub + Cloud Run is wrong for Answerlattice's scale |
...
| **Overall** | **~55%** | Good concepts, wrong implementation for Answerlattice |
...
**The primary error:** Designing for 100K-user scale with event streaming when Answerlattice needs a lightweight rule evaluation API that leverages existing infrastructure.
