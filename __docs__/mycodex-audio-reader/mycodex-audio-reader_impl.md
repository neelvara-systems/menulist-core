...
| File | Change |
| --- | --- |
| `src/app/sites/mycodex/components/MyCodexClientContainer.tsx` | Adds speech queue state, header page-read controls, settings controls, India voice filtering, full-page reading, favorites and read-later queue playback, pause/resume/stop, local preference persistence, screen wake lock, and active mini-player. |
...
11. On `/favorites` and `/queue`, Play all loads saved Markdown files from the protected same-origin MyCodex document route and speaks them as one browser speech queue.
...
- Favorites playback uses only the protected MyCodex static document route to read Markdown already available under `__docs__`; it does not generate audio server-side or persist spoken content.
