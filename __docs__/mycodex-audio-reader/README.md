...
- Cost model: no Firebase, no OpenAI, no Google Cloud, no paid TTS; favorites use a protected same-origin Markdown route with bounded response parsing
...
| File | Responsibility |
| --- | --- |
| `src/app/sites/mycodex/components/MyCodexClientContainer.tsx` | Header page-read control, settings drawer controls, favorites/read-later queue playback, bounded document response validation, India voice filter, speech queue, mini-player, persistence |
