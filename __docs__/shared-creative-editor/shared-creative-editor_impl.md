...
- Arbitrary owner-provided SVG files or pasted SVG markup are not imported as runtime image layers. SVG-looking URLs, extensionless owner-entered URLs, arbitrary data URLs, JavaScript URLs, and direct selected-image source edits are rejected. File imports are limited to bounded PNG, JPEG, WebP, and GIF data URLs whose declared type matches their magic bytes; product adapters can still provide curated/internal approved assets.
...
- Every document mutation passes `creativeEditorDocumentSchema` before authoritative state, history, autosave, callbacks, or Fabric reload changes. Selected-layer patches validate before live Fabric mutation, and invalid Fabric serialization restores the last valid document.
- Native JSON imports are capped at 5 MiB. Raster file imports are capped at 1.4 MB so their base64 data URLs remain within the persisted 2,000,000-character image-source contract, and their magic bytes must match PNG, JPEG, WebP, or GIF metadata.
- Legacy Fabric JSON is inspected for bounded object/node counts and safe image sources, fully awaited through Fabric 7's Promise API in an isolated off-screen canvas, serialized into the neutral document, validated, and disposed before the live editor changes.
- AI, Design Cue, export, template-save, clipboard, and JSON-import operations use immediate single-flight ownership plus document revision checks where late results could overwrite newer design truth.
