...
| Local autosave | Browser-local recovery draft keyed to the document id/product context. It restores only after the owner accepts the newer local draft and does not replace product-owned persistence. |
...
| MyCodex deployed preview | Temporary private deployed route at `https://www.menulist.digital/creative-editor-test`. It is reachable only through the existing MyCodex auth gate and only when `FEATURE_FLAGS.ENABLE_CAMPAIGNCUE_EDITOR_TEST_ROUTE` is enabled. It reuses the CampaignCue editor test fixture and performs browser-local edits only. |
