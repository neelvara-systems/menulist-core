...
- **Paid-cycle plan entitlement:** Current `active` subscriptions carry the purchased plan mirror only through a valid `cycleEndDate`; `cancelled` and `paused` subscriptions retain the purchased plan mirror only through a valid `cycleEndDate`, matching the owner-visible paid-access promise and the transaction query. Missing, malformed, or elapsed cycle evidence and `past_due`, `expired`, or `completed` status fail closed. Browser/Admin timestamp shapes are read without method invocation or scalar coercion, and plan IDs must be bounded strings. Legacy/provider rows missing cycle evidence require provider reconciliation rather than implicit entitlement. The existing maintenance scheduler queries only exact-dual-`ML` rows, checks at most 500 due cancelled/paused rows each hour, revalidates product identity before mutation, transitions them to `expired`, synchronizes store/platform mirrors, and leaves a durable retry marker if the post-transition entitlement repair fails.
