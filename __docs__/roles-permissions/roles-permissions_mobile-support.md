...
- The optional staff email uses one shared client/server schema. Mobile rejects malformed non-empty email input before any API request and keeps the server schema authoritative for bypass-resistant validation.
