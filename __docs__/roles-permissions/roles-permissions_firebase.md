...
| Role save validation | `stores`, `users` | Role create/update/deactivate | Rare | 1 store + current-store users when deactivating | Uses the target store read to reject inactive, soft-deleted, or platform-blocked stores, validate the role, and prevent deactivating a role assigned to active staff in the affected store. |
