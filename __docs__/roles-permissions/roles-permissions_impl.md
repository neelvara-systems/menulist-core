...
Private concurrency state lives at `staffStoreAccessState/{tenantId}_{storeId}`. Its `assignments[]` projection admits at most 500 unique, valid Firestore user document IDs with bounded canonical role IDs; the document must also contain the exact tenant/store scope, a nonnegative safe-integer revision, and an Admin Firestore `updatedAt` timestamp. Malformed or oversized state fails closed before any staff or role mutation. The projection contains only active, not-deleted, not-explicitly-unverified, unblocked user IDs and their role IDs. Staff creation, mapping/active changes, role mutations, and platform user block/unblock serialize through that document together with the authoritative `users` or `stores` write. The state document is lazy-initialized from two bounded numeric/string tenant scans and filters the authoritative persisted `stores[]` mappings instead of depending on possibly missing or stale `storeId`/`storeIds` aliases. It is not client-readable or client-writable under the default-deny Firestore rules boundary. Legacy rows with missing `isVerified` remain compatible; explicit `isVerified: false` rows do not count as active owners.
...
Server-side permission guards in `src/lib/permissions/server.ts` fail closed when the session store is missing, belongs to another tenant, is inactive, is soft-deleted, or is platform-blocked. `normalizeStorePermissionScopeDocumentId()` validates session and explicit tenant/store scope before `stores/{storeId}` permission reads, so malformed, reserved, whitespace-mutated, path-shaped, decimal, zero, negative, unsafe, or nonnumeric scope IDs fail before route-specific writes or provider calls. Every supplied root/nested session tenant/store alias must agree, every present persisted `tenantId`/`tId` alias must match the admitted tenant, and optional embedded `storeId`/`sId` aliases must agree with the selected store document. This applies to `requireAnyStorePermission()`, `requireAnyStorePermissionForStore()`, and `requireAnyStorePermissionForStoreData()` for non-platform sessions, so owner public-truth writers inherit the same exact alias, blocked-store, and document-ID boundary. A signed `PLATFORM` role is not sufficient for the platform override: every shared guard resolves the current `users/{sessionUserId}` record and requires exact current identity, email, lifecycle, revocation and platform-role authority before bypassing store-role permissions. Staff authority uses the exact authenticated projector before store/role reads.
...
- **One-time login sharing:** desktop and mobile login-detail popups let owners copy Staff ID, copy passcode, copy both details, use the native browser share sheet when `navigator.share` exists, or open WhatsApp Web with a prefilled login message. If the staff phone number is saved, WhatsApp Web opens with that number in the URL. Desktop create and reset render these one-time credentials through component-bound Ant modal context holders, so a contextual/theme provider cannot drop the only passcode presentation after the server has already rotated access. Desktop failures log `desktop_staff_login_details_copy_failed`, `desktop_staff_login_details_whatsapp_open_failed`, or `desktop_staff_login_details_native_share_failed`; mobile failures log the matching `mobile_staff_login_details_*` codes. Both surfaces record bounded presence/length metadata only. This is client-only and does not create extra Firebase reads or writes.
...
| Mobile staff | `src/components/mobile/screens/MobileUsersScreen.tsx` | Uses the same staff client helpers as desktop. Exact tenant/store keyed mounts mask the shared list until its latest scoped request succeeds; one synchronous mutation boundary and source-user ownership prevent delayed create/update/role/remove/reset/sign-out settlement or one-time credentials from crossing stores or replacing newer truth. The mounted guard is re-armed during every React Strict Mode effect setup. Add Staff, Staff details, and one-time login-detail popups are programmatically named; selected role controls expose pressed state. The active-state handler supports both deactivate and reactivate while retaining scope, permission, liveness, and duplicate-mutation guards. Managers can create ordinary Staff assignments; Owner-protected rows show read-only explanation and disabled reset/sign-out/activate/remove/edit actions unless the actor also has `canAssignRoles`. The global context contract is the runtime-validated public-safe `StaffUserSummary[]`, not full authenticated `UserDataType[]`. |
| Mobile roles | `src/components/mobile/screens/MobileRolesScreen.tsx` | Uses the shared role-save client helper; exact tenant/store keyed mounts, synchronous single-mutation admission, component liveness and roles-leaf ownership prevent delayed save settlement from crossing stores or overwriting newer same-store truth. The editor's Active switch is the one reversible mobile deactivation/reactivation path; the former misleading soft-delete duplicate is not shipped. |
