...
| Mobile staff | `src/components/mobile/screens/MobileUsersScreen.tsx` | Uses the same staff client helpers as desktop. Exact tenant/store keyed mounts mask the shared list until its latest scoped request succeeds; one synchronous mutation boundary and source-user ownership prevent delayed create/update/role/remove/reset/sign-out settlement or one-time credentials from crossing stores or replacing newer truth. Managers can create ordinary Staff assignments; Owner-protected rows show read-only explanation and disabled reset/sign-out/activate/remove/edit actions unless the actor also has `canAssignRoles`. The global context contract is the runtime-validated public-safe `StaffUserSummary[]`, not full authenticated `UserDataType[]`. |
| Mobile roles | `src/components/mobile/screens/MobileRolesScreen.tsx` | Uses the same role client helpers as desktop; exact tenant/store keyed mounts, synchronous single-mutation admission, component liveness and roles-leaf ownership prevent delayed save/delete settlement from crossing stores or overwriting newer same-store truth |
