...
| MLD-A026 | Pilot proof and owner learning operating pack | Codex | Done | Created `menulist-marketing-distribution_pilot-proof-and-owner-learning-pack.md` with post-activation proof states, item-level permission, interviews, clinic agenda, neutral review request, AssetOS/SignalDesk handoff, and revocation flow |
| MLD-A027 | First proof distribution run operating pack | Codex | Done | Created `menulist-marketing-distribution_first-proof-distribution-run-operating-pack.md`; mapped the first permissioned customer proof to existing SignalDesk source, asset, draft, approval, manual publication, performance, decision, and revocation contracts without adding runtime scope |
...
| MLD-R016 | Publisher and AI distribution readiness dossier | Codex | Done | Created `menulist-marketing-distribution_publisher-ai-distribution-readiness.md`; documented Google/Apple/Bing/read-only AI gates, consent, ownership, authority, reporting, support, and provider application inputs without enabling integration |
| MLD-R017 | Cross-platform distribution workflow research | Codex | Done | Created Markdown and self-contained HTML briefs from current official X, Reddit, Meta, YouTube, Google, Product Hunt, Notion, Canva, Stripe, HubSpot, and Buffer guidance plus bounded practitioner signals; converted the findings into one proof-led MenuList operating loop |
...
| MLD-C003 | Supply permissioned businesses and sender identity | Founder | Blocked | Needs at least one real owner-approved business or approved partner introduction to start; the board may hold up to ten, with no scraping or bulk contact |
...
| MLD-I006 | Prepare publisher and AI distribution readiness dossier | Codex | Done | Dossier is complete in `menulist-marketing-distribution_publisher-ai-distribution-readiness.md`; current owner-confirmed placement remains unchanged |
| MLD-I007 | Apply for publisher/API or direct AI-provider access | Founder + Codex | Deferred | Requires five permissioned activations, three day-30 active businesses, repeated owner demand, legal/security/cost approval, and founder-owned provider/support identity |
...
| MLD-V029 | Distribution workflow research verification | Codex | Done | Passed `npm run docs:check-links` with 0 broken links and 0 naming violations, `git diff --check`, required HTML section/source checks, and Chrome desktop/long-page render review; no runtime or deploy gate was required for this docs-only pass |
