...
| `ownerControlUsage/{tId}_{sId}` | Exact owner-control aggregate: allowed positive lifetime counts, matching last-used timestamps, UTC month counts, first/last timestamps and canonical string tenant/store scope. | Confirmed owner/manager control changes; client transaction serializes first-write/update and rules admit only one-control `+1`. | Strictly projected, 500-row-paged Authority Maturation input and internal Business Health source evidence; malformed rows are counted/skipped. |
