...
The canonical owner boundary is every top-level MenuList message namespace except the separately governed public `Website` and adjacent-product `CampaignCue` trees. It currently contains 4,025 strings across 50 namespaces and is key-exact across all 52 registered locale files: missing and obsolete locale-only owner keys both fail verification. The source gate also checks non-empty values, ICU syntax and variables, locale-file/registry parity, runtime dashboard fallback copy, canonical desktop/mobile settings and shell copy, mixed-script artifacts, and regression in the established 1,020-string translated dashboard/settings contract.
...
All 50 non-English packs are semantic-evidence gated across the full owner boundary. The August dashboard audit projected Business Health, owner actions, public-truth modules, source labels, and dynamic statuses from stable IDs; localized date/number formatting uses the active next-intl locale; and the non-localized Business Health assistant is explicitly unavailable outside English rather than returning an English answer inside localized chrome. Protected product/platform tokens are checked inside complete messages, not only as standalone labels. The evidence currently records 7,030 bounded repairs. Explicit per-key English source fallback is retained only where local model output mixed scripts, changed meaning, or could not support a defensible localized business claim.
