...
August 23 billing-document delivery reuses the existing deterministic event, per-channel delivery, preference, consent, rate-limit, and recovery records. One issued invoice or credit note can therefore create one event plus up to two channel-delivery rows when both email and WhatsApp are selected. If at least one selected channel is eligible, the processor adds one private direct billing-document read and renders one bounded PDF in memory for reuse across both channels; no eligible channel means no attachment read. PDF bytes, Firebase Storage objects, public document URLs, and Meta media IDs are not persisted by MenuList. The billing document keeps only a best-effort aggregate delivery summary; the NotificationOS event and delivery rows remain the channel-level authority. No new collection or index is introduced.
...
August 1 ops manual-send convergence adds no normal-path Firestore operation: a newly created manual event is still one transactional event creation followed by the existing processing path. A replay of an existing deterministic manual action adds one direct event read so product/scope/dedupe/timestamps, requested channel, normalized recipient hint, source event and immutable action fingerprint can be re-proved before processing. A collision performs no provider call or write. Manual send/handoff source events now use the shared persisted-event projector. Protected response headers and limiter-outage diagnostic separation add no Firestore, provider, cache, rule, index or Function operation.
...
August 23 billing-attachment deployment attempt: `npm run verify:functions-deploy-preflight` passed. The scoped command `firebase deploy --project menulist-qa --config firebase.json --only functions:triggerStoreNightlyScheduler --non-interactive` then passed its Functions lint/build predeploy steps but stopped before upload because the Firebase CLI credentials were expired and require `firebase login --reauth`. No QA Function changed, so production was not attempted.
...
The consolidated MenuList maintenance cleanup constrains owner-notification events, deliveries, and rate-limit queries to exact `productId: ML` before the `expiresAt` cutoff and 50-row cap. The shared composite indexes are `productId + expiresAt` for each family. The real maintenance emulator proves colliding expired `AL` rows remain untouched; legacy `messageLogs` keeps its separate MenuList-only cleanup path.
