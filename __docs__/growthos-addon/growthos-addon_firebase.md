...
GrowthOS project/kit/scope ID admission is cost-neutral: refresh/generate request schemas and `readGrowthOSProjectDataServer()` validate and normalize `projectId` through the shared Firestore document-ID guard before scoped or legacy project reads. Export request schema and `readGrowthOSKitServer()` validate and normalize `kitId` before kit reads. Server helpers also validate session-derived tenant/store scope IDs before store reads, summary reads/writes, scoped project reads, kit reads/writes, or export status writes. This adds no Firestore reads/writes/deletes for valid requests, Storage operations, provider calls, cache invalidations, rules, indexes, Cloud Function logic, Firebase deploy requirement, or Vercel deploy action; malformed project, kit, or scope IDs now stop on the existing request/server admission path before Firestore refs.
