...
**Public-facing representation of Tenant is the master store.** Store fields remain the rendered business truth. The public lookup may fetch the referenced canonical `Tenant` document on a cold cache miss solely to prove tenant existence, exact identity, and lifecycle/platform-block eligibility; tenant fields are not rendered, serialized, or merged into the public store payload. "Brand" = master store. When this doctrine says "tenant URL" in user-facing language, it technically means "master-store-rooted subdomain or custom domain."
...
### D-10. Customer App install identity — store-level app

**Supersedes prior position:** The older lock preserved install context, creating distinct installed apps for OBP, Store, and Project surfaces. That was intentional at the time, but it is now superseded by the Customer App surface policy.

**Current lock:** Customer App is **one installed customer app per tenant store origin**. Installing from OBP, `/menu`, or any project slug must resolve to the same store-branded app identity.

**Why:** Owners and customers expect one restaurant app, not multiple apps with the same name/logo depending on where installation started. OBP, `/menu`, project URLs, and outlet/project URLs remain in-app navigation/source attribution. They are not separate installed app identities.

**Lock — store-level PWA identity:**
...
1. **Store-level manifest** — the manifest route must produce the same `id` for every install source on the same tenant store origin.
2. **Shortcuts inside manifest** stay store-level (View Menu, phone, directions, WhatsApp, reservation, order). These are actions inside the app, not separate app surfaces.
3. **Install prompt eligibility** (`CustomerAppController`) may appear across OBP, `/menu`, and project surfaces, but all prompts install the same store app.
4. **Analytics on install** records which route triggered it as entry/source context (`obp`, `menu-alias`, `project`, `unknown`) so we can measure conversion by source without fragmenting app identity.
...
| **G-03** | Customer App manifest identity was previously per-surface (D-10 superseded).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Emit one store-level manifest identity (`id: /?store={storeId}`), ignore legacy `?start=`, and use `/menu` as the store-level launch entry when a customer menu exists.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | P0       |
...
| G-03 | ✅ done | `@/src/app/manifest.webmanifest/route.ts` ignores legacy `?start=` for identity and chooses store-level `/menu` or `/`; `@/src/lib/pwa/manifestGenerator.ts` emits stable store-level `id` |
...
| G-11 | ✅ superseded | Manifest start-url degradation removed. Deleted project paths now use render-side public fallback only; Customer App identity stays store-level.                                                                                                                                             |
...
- `@/src/database/campaigns/index.ts` writes `menuQrUrl` using the canonical default slug (audited during link-emitter sweep). Legacy flat-dotted `projects.${id}` and `stores.${id}` shapes use the shared null-prototype `summaryMapParser`, which rejects magic path segments. Project/store summary map keys are authoritative over embedded `projectId`/`storeId`; canonical project writers reject dotted/magic project IDs and field paths. Public menu, OBP, pull API, sitemap, screens, campaigns, Public Truth Monitor, platform store selectors, and Functions scoring consumers must use that same authority rule.
...
### A-06. Customer App identity collision policy

**Superseded:** Older doctrine treated Chrome coalescing as a bug and required distinct manifest IDs per install surface.

**Current policy:** Coalescing is the desired behavior. OBP, `/menu`, and project pages on the same tenant store origin must all resolve to the same installed Customer App identity. Different tenant stores must still produce different manifest IDs.
...
`storeDetails.pwaSettings.promoteInstallation` already exists (`@/src/components/templates/website/clientWebsite/index.tsx:216`). Owner can already disable Customer App install prompts. This opt-out globally suppresses the prompt across all public routes while the manifest identity remains one store-level app.
...
Wave 1 (P0 — parallel, no inter-deps):
  ...
  G-03  Customer App store-level manifest id + launch entry
...
| G-03 PWA manifest             | Existing installs with per-surface IDs may remain as old OS-level entries until the customer reinstalls. New installs converge to one store-level app identity (`/?store=X`).                                                                                                                       | None                                                                                                                               | Document as one-time identity consolidation; new installs from all surfaces resolve to the same app.                                                           |
...
| T2-N-03 | §6.4 entry/source attribution ✅ done     | `@/src/lib/pwa/surfaceDetection.ts` classifies the current pathname into `obp`, `menu-alias`, `project`, or `unknown`. Wired into `fireInstalledEventOnce` and `detectAndTrackAppOpen`. Event payload keeps `pwaInstallSurface`; aggregation keeps `installsBySurface.*` and `appOpensBySurface.*` as source attribution only, not app identity. |
...
| T2-N-06 | A-06 manifest-id regression guard ✅ done | `@/src/__tests__/manifestStoreIdentity.ts` verifies all source paths for the same store produce the same manifest `id`, different stores produce different IDs, and project source paths do not echo into `start_url`. |
...
| T3-N-07 | Dead CustomDomainTab export ✅ done       | Removed the unused `CustomDomainTab.tsx` re-export from `tabs/index.ts`. The guarded legacy file remains available to contract verifiers, while `DomainSettingsTab` is the sole rendered desktop domain surface.                                                                                                                                                                                                                                                                                                                                |
...
| T5-N-02 | Subdomain-guard diagnostics ✅ done      | `@/src/database/stores/index.tsx` records the blocked G-08 mutation through `store_subdomain_change_blocked_after_publish` with bounded identifier/subdomain presence-length context before returning the owner-safe error. The retired `SUBDOMAIN_MUTATION_BLOCKED` call was browser-only GA4 code invoked from a server DAL, so it had no valid delivery path and was removed. No public analytics or Firestore event is added for this operational guard. |
| T5-N-03 | G-11 manifest-degradation events superseded | Removed from Customer App identity. Deleted project paths are normal public-routing fallback/not-found behavior; they no longer affect manifest `start_url` or emit manifest degradation analytics. |
