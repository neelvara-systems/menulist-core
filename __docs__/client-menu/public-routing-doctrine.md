...
| T5-N-02 | Subdomain-guard block events ✅ done      | New `TrackingEvent.SUBDOMAIN_MUTATION_BLOCKED` at `unified.ts:140-144` with Firebase aggregation at `unified.ts:473-484`. Wired into `@/src/database/stores/index.tsx` — fires when G-08 guard blocks subdomain mutation on published store. Tracks `attemptedSubdomain`, `currentSubdomain`, `storeId`, `tenantId` for security/support signal analysis. The event remains fire-and-forget so the guard is not delayed by analytics, but rejected tracking logs `store_subdomain_block_analytics_signal_failed` with bounded store metadata.                                                                            |
