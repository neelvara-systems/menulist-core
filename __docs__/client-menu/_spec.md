...
Internal history generated by owner menu work must remain append-only and store-scoped. A pending event must never move to a newly selected tenant/store during page-hide flushing; completed publish and revision-summary events must never replace one another inside a debounce window; malformed scope, identity, project, event, or snapshot payloads must fail before persistence; and authenticated users may read or create only for stores in their current claims and permitted role.
...
Customer scans QR code
    ↓
Browser navigates to joespizza.menulist.online
    ↓
...
| ------------- | ------------------------------ | ----------------------------------------------- |
| Subdomain     | `joespizza.menulist.online`        | Middleware reads subdomain, looks up store      |
| Custom Domain | `joespizza.com`                | DNS points to us, middleware looks up by domain |
| Slug Path     | `joespizza.menulist.online/drinks` | Multiple projects per store                     |

Custom-domain ownership is serialized before provider work. The same store cannot start overlapping add/remove operations, another store cannot acquire a hostname during cleanup, and a provider conflict is not treated as ownership proof. Duplicate legacy rows or claim mismatches fail closed instead of selecting a winner. Public resolution requires one unique active/verified store plus an existing, eligible canonical tenant; tenant data is checked for admission only and is not shown to customers.
...
│                     CUSTOMER REQUEST                             │
│                                                                  │
│   joespizza.menulist.online  →  joespizza.menulist.online/drinks        │
...
│                                                                  │
│   1. Resolve tenant from validated Host                          │
...
Not technology — **installed presence**. When restaurants use `restaurant.menulist.online` everywhere (QR codes, Google, Instagram, website), switching requires replacing all links. This creates natural lock-in without contracts.
