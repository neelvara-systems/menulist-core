...
|-----------|-----------|---------|-----------|-----------|-------|
| Read owner dashboard summary | `analytics/{tId}_{sId}_{projectId}_dashboard_summary` | Owner opens Dashboard | Per scheduler cache window | 1 | Settled Overview/Daily/Weekly/Monthly/All-time read from one nightly read-model doc. Cached until after the next store-local business-day settlement window. |
