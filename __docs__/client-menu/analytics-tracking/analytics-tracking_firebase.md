...
|-----------|-----------|---------|-----------|-----------|-------|
| Read owner dashboard summary | `analytics/{tId}_{sId}_{projectId}_dashboard_summary` | Owner opens Overview, Daily, Weekly, Monthly, or Overall tab | Per scheduler cache window | 1 | Settled Overview/Daily/Weekly/Monthly/All-time read from one nightly read-model doc. Cached on the device until after the next store-local business-day settlement window. |
...
- **Dashboard tab rule:** owner Dashboard uses `Today`, `Overview`, `Daily`, `Weekly`, `Monthly`, and `Overall` tabs. `Today` reads only current partial Menu + OBP daily docs; opening any settled tab triggers the cached Menu + OBP read-model reads for that scheduler cycle.
