...
- **OBP language-usage rule:** OBP uses the same multi-language-only gate. `obpViewsByLanguage` and `obpSessionsByLanguage` are attached to the existing OBP view write; `obpLanguageAdoptions` writes only after the switched OBP language stays active for the dwell window.
