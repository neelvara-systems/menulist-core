...
- `sanitizeForClient()` allowlists the public project payload before the React client boundary; `projectPublicClientStore()` independently allowlists and runtime-projects the already-loaded store values needed by the customer renderer. It admits descriptor-safe plain records, exact scalar/list/map/localized-text shapes and exact custom-attribute fields. Denormalized `activePlanType` is projected only as a bounded normalized string, so malformed nested data cannot cross under an allowlisted scalar key. Canonical store credentials, billing, roles, owner contacts, POS/integration secrets, notification state, and future unknown fields stay server-side.
...
- **Item-symbol guide is payload-only**: The public menu resolves and deduplicates its applicable item symbols in memory from the already-loaded currently available items. It adds no Firestore read/write/delete, listener, Storage operation, Cloud Function, API route, provider call, cache invalidation, rule, index, or deploy requirement.
...
- **Footer freshness diagnostics:** Public menu footer freshness parsing is browser-local render logic. Date, ISO, callable Firestore Timestamp, and serialized `seconds`/`_seconds` wire shapes use the existing guarded freshness normalizer. Malformed `lastPublishedAt` values omit unverified freshness text and log bounded `public_menu_footer_freshness_relative_failed` / `public_menu_footer_freshness_iso_failed` diagnostics only; this adds no Firestore read/write/delete, analytics write, Storage operation, Cloud Function, API route, cache invalidation, rule, index, or deploy requirement.
- **Feedback nudge storage diagnostics:** Public menu feedback nudge de-dupe is browser-local and tab-scoped. Its visible, dismissed and triggered state is bound to the exact validated project ID, including across client-side project transitions. Failed sessionStorage guard read/write paths log bounded `public_menu_feedback_nudge_storage_read_failed` / `public_menu_feedback_nudge_storage_write_failed` diagnostics only and add no Firestore read/write/delete, analytics write, Storage operation, Cloud Function, API route, cache invalidation, rule, index, or deploy requirement.
- **Language preference storage diagnostics:** Public menu language preference storage is browser-local. The destination project preference is restored before that project's key can be written, and invalid saved languages are evicted. Failed project-scoped localStorage read/remove/write paths log bounded `public_menu_language_storage_read_failed` / `public_menu_language_storage_remove_failed` / `public_menu_language_storage_write_failed` diagnostics only and add no Firestore read/write/delete, analytics write, Storage operation, Cloud Function, API route, cache invalidation, rule, index, or deploy requirement.
- **Public menu session-state diagnostics:** Language/page/scroll session state uses exact tenant/store/project keys. Language restoration completes before the destination key can be written; scroll admits only canonical bounded integers and evicts malformed values. Failed sessionStorage reads/removals/writes log bounded `public_menu_session_state_read_failed` / `public_menu_session_state_remove_failed` / `public_menu_session_state_write_failed` diagnostics only and add no Firestore read/write/delete, analytics write, Storage operation, Cloud Function, API route, cache invalidation, rule, index, or deploy requirement.
- **Breadcrumb language preservation diagnostics:** Public menu, feedback and
  outlet-OBP callers project their server-resolved content language into the
  initial breadcrumb Link destinations. Browser URL parsing remains a
  compatibility fallback only. Failed fallback parsing logs bounded
  `public_menu_breadcrumb_language_preserve_failed` diagnostics and adds no
  Firestore read/write/delete, analytics write, Storage operation, Cloud
