...
| Footer timestamp boundary | Relative freshness and ISO metadata share the guarded public freshness normalizer, admitting Date, ISO, callable Firestore Timestamp, and serialized `seconds`/`_seconds` wire shapes while omitting malformed values through bounded diagnostics. | `src/lib/obp/freshnessTimestamp.ts`, `src/components/templates/main-app/projects/b2cView/output/MenuFooter.tsx` |
