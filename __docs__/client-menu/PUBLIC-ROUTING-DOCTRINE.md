...
### Tier 1 — Material (explicit doctrine items not yet shipped)

| ID      | Item                                  | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T1-N-01 | A-08 sitemap R5-violating             | `@/src/app/client/sitemap.ts:63-76` hard-codes `/` and `/menu` for every tenant. Under R5 (§8, A-08) `/menu` belongs in the sitemap **only** when an owner has claimed slug `menu`; otherwise sitemap must enumerate canonical project slug URLs. Multi-outlet tenants additionally need `/{outletSlug}` entries per active outlet. `previousSlugs[]` must remain excluded (they 301 to canonical).                                                       |
| T1-N-02 | A-09 footer brand not a link          | `@/src/components/templates/main-app/projects/b2cView/output/MenuFooter.tsx:96-135` renders `businessName` as text. A-09 locks the footer brand as the "back to OBP" affordance — after G-02 made the header logo decorative, the footer brand is the only visible way up. Should become `<a href="/">`.                                                                                                                                                  |
| T1-N-03 | A-12 render-side fallback ladder      | `@/src/app/client/[[...slug]]/page.tsx:1003-1013` still shows a terminal "Menu Not Found" `<h1>` with static copy. G-11 shipped the manifest-side degradation (online PWA relaunch recovers), but **offline PWA launches hit this render branch first**. Needs the A-12 ladder: Project → `/menu` alias → Store OBP ("this menu is no longer available" notice) → Brand OBP, with standalone-mode detection to auto-redirect up after a visible 2 s hint. |
| T1-N-04 | A-12 slug reservation window          | Deleted / inactive project's `slug` + `previousSlugs[]` must stay reserved for 90 days to prevent a same-slug replacement from hijacking incoming QR scans. No mechanism exists today — recycle-bin flow does not block slug re-use.                                                                                                                                                                                                                      |
| T1-N-05 | A-03 subdomain admin-tier rename path | G-08 locks the owner-facing subdomain after first publish. A-03 specifies a support-tier escape hatch with 12-month redirect window + written acknowledgment. No infrastructure for admin-tier rename exists — support cannot currently unblock a trademark-dispute rename without code changes.                                                                                                                                                          |

### Tier 2 — Doctrine shipped but thin UX

| ID      | Item                                  | Evidence                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| T2-N-01 | G-07 rename API has no dashboard UI   | `@/src/app/api/outlets/rename/route.ts` ships a validated rename endpoint with `previousOutletSlugs[]` chain support. No owner-facing button calls it — the capability is not yet accessible from the settings UI. The rename flow should live alongside the outlet identity settings and must surface the doctrinal warning: "Old URLs keep working via redirect for 12 months."                        |
| T2-N-02 | A-07 OBP click surface attribution    | `@/src/lib/analytics/unified.ts:881-884` — `trackOBPMenuClick(storeId, additionalData)` accepts no `surface` discriminator. With outlet OBPs live (G-01), measurement can no longer distinguish brand-OBP → menu vs outlet-OBP → menu conversion. Extend the signature to require `surface: 'brand'                                                                                                      | 'outlet'`and pipe outlet context in from`OBPContent`. |
| T2-N-03 | Per-surface PWA install analytics     | §6 rule 4 mandates that install events record `install_surface: 'obp'                                                                                                                                                                                                                                                                                                                                    | 'store'                                               | 'project'`. The install controller currently fires a generic install event. With G-03 wiring surfaces via the manifest `start_url`, the same identification should flow to the controller. |
| T2-N-04 | G-04 outlet-scope QR generation       | `@/src/components/templates/main-app/useMenuList/index.tsx` is master-tenant scoped. An outlet owner logged into their outlet store has no UI to download their outlet's Store Menu QR (`/{outletSlug}/menu`). The three-QR row ships today against the master tenant only. Decide whether UseMenuList should switch per selected store, or whether a dedicated outlet settings QR section should exist. |
| T2-N-05 | G-09 breadcrumb on OBP pages          | `@/src/app/client/[[...slug]]/MenuBreadcrumb.tsx` renders only inside MenuContent. OBP and outlet OBP pages have no visible breadcrumb — the brand-identity block there already implies "you are at the top", but consistency arguments may warrant adding a 1-node breadcrumb on outlet OBP ("Business → Store").                                                                                       |
| T2-N-06 | A-06 manifest-id differentiation test | G-03 encodes `start_url` into the manifest `id` so Chrome treats each install surface as distinct. No automated test verifies this holds across OBP, outlet OBP, and project URLs simultaneously. A regression here silently re-introduces the install-collision bug without surfacing.                                                                                                                  |
| T2-N-07 | A-10 `promoteInstallation` parity     | `storeDetails.pwaSettings.promoteInstallation` is consumed at the install-prompt controller. After G-03 made manifests per-surface, verify the opt-out still suppresses prompts across **all three surfaces** (not only the surface where it was originally checked). Currently unverified.                                                                                                              |

### Tier 3 — Latent tech debt discovered

| ID      | Item                                  | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T3-N-01 | `slugify.ts` no-tenant branch         | `@/src/lib/utils/slugify.ts:87` — when neither subdomain nor custom domain is present, the helper emits `/menu/{slug}`. Under R5 no URL in the product should ever take the `/menu/{slug}` shape. Branch is unreachable today because every call site provides tenant context, but it is a time bomb if a marketing / dev URL code path ever hits it.                                                                           |
| T3-N-02 | Dead `SubdomainTab.tsx`               | `@/src/components/templates/main-app/businessSettings/tabs/SubdomainTab.tsx` is exported from `tabs/index.ts` but has zero import sites. Superseded by `DomainSettingsTab.tsx`. Remove to avoid future edits hitting the wrong file.                                                                                                                                                                                            |
| T3-N-03 | Flat-dotted Firestore writer pattern  | Writers in `@/src/app/api/public/create-menu/claim/route.ts`, `@/src/app/api/outlets/create/route.ts`, `@/src/app/api/msg-preview/[sessionId]/approve/route.ts`, `@/src/database/multiOutlet/propagation.ts`, and `@/src/database/projects/index.ts` use `{[`projects.${id}`]: val}` merge pattern which produces flat dot-notation keys rather than nested objects. Safe today thanks to `parseSummaryProjects` but confusing. |
| T3-N-04 | Invalid Firestore collection path     | `@/src/database/campaigns/index.ts:688` uses `projects/{tId}/{sId}/metadata` (four segments — only valid for `doc()`, not `collection()`). Same latent bug as the one fixed in `@/src/app/client/[[...slug]]/page.tsx`. Should be changed to `projects/{tId}/{sId}` with `where('isDefault','==',true)` + `where('active','==',true)`.                                                                                          |
| T3-N-05 | `outletSlug` backfill migration       | G-12 removed the `store-${storeId}` fallback and filtered slug-less outlets from `BrandOBPContent`. No migration script exists for outlets created before `outletSlug` was enforced at creation. User directive is "no backward compat", so no production impact today — but once a live tenant creates outlets at volume, a one-time backfill script should run to assign slugs to any legacy outlets missing the field.       |
| T3-N-06 | Import auto-organizer strips new deps | Multiple files (`OBPMenuCTA.tsx`, `useMenuList/index.tsx`, `[[...slug]]/page.tsx`, `projects/index.tsx`) had newly-added imports silently removed by the workspace's auto-organizer during the session. Not a code issue per se, but the workflow friction is worth noting — either lock the auto-organizer off new files or accept that some refactors will need two save passes.                                              |

### Tier 4 — i18n + copy surfaces

| ID      | Item                            | Evidence                                                                                                                                                                                                                                                                                                |
| ------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T4-N-01 | G-08 locked-state alert strings | `@/src/components/templates/main-app/businessSettings/tabs/DomainSettingsTab.tsx:264-265` — "Your web address is set." / "This is your permanent link…" are hard-coded English. Add to `BusinessSettings` i18n namespace.                                                                               |
| T4-N-02 | G-09 breadcrumb separator       | `@/src/app/client/[[...slug]]/MenuBreadcrumb.tsx` uses a literal `/` as separator. Works for LTR; in RTL (Arabic / Hebrew stores) the rendering is still left-to-right by default. Either use a locale-aware separator or flip via `dir`.                                                               |
| T4-N-03 | G-04 QR card labels             | `@/src/components/templates/main-app/useMenuList/index.tsx:529-564` — "Store Menu QR", "Business Profile QR", "Project Menu QR (advanced)" and their descriptions are hard-coded English. Route through i18n before a non-English tenant ships.                                                         |
| T4-N-04 | G-13 advisory modal copy        | `@/src/components/templates/main-app/projects/index.tsx:456-483` — the divergence advisory text is entirely English literal (including the `Set "..." as default` button label). Move to i18n; the business-type label `labels.offeringLower` is already localized but wraps around hard-coded English. |

### Tier 5 — Observability gaps

| ID      | Item                              | Evidence                                                                                                                                                                                                                                                                                                                                                        |
| ------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T5-N-01 | Layer 2 alias-hit analytics       | `isMenuAliasFallback` in `@/src/app/client/[[...slug]]/page.tsx` drives canonical emission but fires no analytics event. We cannot measure the share of `/menu` visits resolved via Layer 1 (claimed project) vs Layer 2 (universal alias) — which is the core metric for deciding whether R5 is working post-launch.                                           |
| T5-N-02 | Subdomain-guard block events      | `@/src/database/stores/index.tsx:232-238` — the G-08 server guard `console.warn`s when a subdomain mutation is blocked, but does not emit an analytics / security event. An owner repeatedly attempting to rename their subdomain post-publish is a support-ticket precursor; we should capture it.                                                             |
| T5-N-03 | G-11 manifest-degradation events  | `@/src/app/manifest.webmanifest/route.ts` — `resolveStartUrlWithFallback` silently downgrades `start_url` when the original target is gone. Worth emitting an analytics event (`pwa_start_url_degraded`, tagged with surface + degraded-to) so we can measure how often the A-12 ladder is actually saving installs.                                            |
| T5-N-04 | G-10 alias-fallback switch source | `PROJECT_SWITCH` fires with `source: 'obp_secondary_card'` and `source: 'in_menu'`. No source enum value covers "customer hit `/menu` and was served the default project" (Layer 2) — which is effectively a "latent switch" from the URL the customer typed to what we rendered. Either extend the source enum or accept this as outside PROJECT_SWITCH scope. |
