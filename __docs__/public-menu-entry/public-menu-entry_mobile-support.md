...
Test iOS Safari, installed iOS PWA, Android Chrome, and installed Android PWA for camera and saved-photo choice, same-photo retry, 10MB rejection, link keyboard/paste, auth return, background/foreground polling, timeout retry, claim, copy/share, delayed session refresh, and dashboard handoff. These checks are external evidence and do not reopen Local source complete status.
