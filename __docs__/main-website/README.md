...
Version 3.6.118 closes the locally actionable findings from the July website audit without changing domain, DNS, canonical-host, Search Console, or email-domain configuration. The public header lazy-loads the authenticated sign-out chain only when an authenticated user requests logout, and closed desktop Features/Resources panels are inert and absent from the keyboard order. The website product-path provider now derives the `/ml` alias from the browser pathname instead of reading a request header in the shared website layout; locale/session rendering remains server-aware, so this is not a claim that every website route is statically cached. The contact write boundary is unchanged, while a new feature-flagged `/ops/website-enquiries` view gives verified platform operators a bounded, manual-refresh, read-only inbox for accepted MenuList contact rows with no realtime listener. The homepage customer-link grid now holds three, two, then one column at practical breakpoints; the floating CTA requires both desktop width and usable viewport height; desktop mega-menu descriptions are more readable; anonymous pages do not statically import the Firebase auth client through Header; unsupported X navigation is removed; and public trust/contact/create-preview wording is more accurate and owner-friendly. No public form, extraction, publishing, pricing/payment, owner data, Firebase rule/index, Cloud Function, dependency, production build, Vercel deployment, or domain contract changed.

Version 3.6.117 remains Homepage Private Preview Surface Polish and is preserved below as the previous website version note.
...
QA certification note (August 25, 2026): locale-prefixed resource routes now apply their route-owned locale to the document root after the shared cookie-locale provider mounts. The existing localized wrapper remains in place, while assistive technology also receives the correct root `lang` and `dir` values; leaving the localized route safely restores the prior document locale.
...
Version 3.6.31 originally added a calm full-width homepage Business Health section after the owner outcome/capability proof and before Resources. The later compressed homepage replaced that mount with the compact Business Health card inside `OwnerProofSection`; the deeper component remains available as an unmounted reference. Public website copy describes Business Health as an AI health check for latest menu state, public surfaces, customer attention, locations, freshness, and safe handoff paths. It does not position the feature as an AI assistant, chatbot, revenue optimizer, prediction system, or autonomous mutation surface. This is public website component/CSS/locale/docs only; owner dashboard runtime, Business Health APIs, scheduler read models, Firebase rules, Cloud Functions, pricing, payment, auth, extraction, customer menu runtime, and Vercel deployment were not changed.
