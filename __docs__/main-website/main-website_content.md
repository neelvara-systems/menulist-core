...
> Owner workflow gallery note (August 31, 2026): `OwnerWorkflowGallerySection` appears directly after the hero and demonstrates the complete owner-controlled path: photograph or upload an existing menu/service list, review the private preview, then publish the approved customer link. The intake photograph is generated illustrative material and the section is explicitly labelled as a demonstration with sample business content. Do not describe it as customer UGC, a testimonial, or evidence of automatic publishing. Permissioned customer footage may replace the intake image only after consent and usage-rights review, while the review-before-publish boundary remains unchanged.

> Create-menu preview bridge note (July 7, 2026; runtime truth corrected July 18, 2026; placement updated August 31, 2026): the homepage includes `CreateMenuPreviewSection` immediately after the owner workflow gallery and uses a browser-local selector for the two public inputs supported by `/create-menu`: a photo/image or an owned public link. An owned public link may point to a menu, service list, image, or PDF. It is a conversion bridge into `/create-menu`, not a second intake implementation. Keep the current guardrails explicit: sign in before processing, private owner-bound preview, five preview attempts per day, 24-hour draft expiry, and review before publishing.
