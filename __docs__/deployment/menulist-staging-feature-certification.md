...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one application commit | Latest application source `738f8a2374ffa28bde28ab045ad50bfe84033872` reached hosted Preview with env `preview` at deployment `menulist-core-ikeeb6vsa-neelvara-systems.vercel.app`; staging alias `/api/version` reported the same full commit before final hosted Transactions certification | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; immutable Preview and `GET https://app.menulist.digital/api/version` on 2026-08-15 | None | None | `738f8a2374ffa28bde28ab045ad50bfe84033872` | `738f8a2374ffa28bde28ab045ad50bfe84033872` | The following evidence-only ledger commit does not change application runtime; future application edits require a new exact-build gate |
...
| ML-QA-060-SEO | Business Settings / SEO and AEO | Distinguishable section actions plus text-field persistence and exact cleanup | Desktop Chrome plus source-gated mobile parity | QA owner `2/2`; Tagline, Meta Title, Meta Description, and Keywords blank; canonical URL `https://menulist.digital` | Repeated page actions are section-named; one bounded SEO text save survives an independent app load; one reverse save restores the exact baseline | The preceding hosted build exposed two generic Save and four generic Reset controls, with the SEO card contributing no section-specific action names. Exact fixed build exposed one `Save SEO and AEO` and one `Reset SEO and AEO`, reducing generic counts to one Save and three Reset. Temporary Tagline, Meta Title, and Meta Description values saved, disabled the clean-state action, and survived an independent Dashboard-to-Business Settings document transition. A reverse save cleared all three, and a second independent document restored zero-length fields with the canonical URL unchanged. | FIXED | Exact `/api/version` build `c54662feab8617ad2a5b6466b9c87935bd5ba396`; immutable Preview `menulist-core-dvufpb0s8-neelvara-systems.vercel.app` and staging alias; controlled hosted DOM across independent app documents; `verify:public-business-truth`; `verify:mobile-shell-route-map`; focused lint; TypeScript | Exactly two existing store-document updates: blank SEO text fields to bounded temporary QA values, then back to blank; no keyword, canonical URL, domain, provider, AI generation, analytics, public-form, public-route, dependency, or production operation | Tagline, Meta Title, Meta Description, and Keywords blank; canonical URL `https://menulist.digital`; temporary QA strings absent; named Save disabled after the final independent load | `c54662feab8617ad2a5b6466b9c87935bd5ba396` | `c54662feab8617ad2a5b6466b9c87935bd5ba396` | Keyword/tag entry, intentional canonical URL changes, authorized public projection, and true handheld MobileShell interaction remain pending |
