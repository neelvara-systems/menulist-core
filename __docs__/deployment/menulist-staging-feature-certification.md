...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one application commit | Latest application source `c53fc7463faeeebe6fb89fade584bf76b7eb26a5` reached hosted Preview with env `preview` at deployment `menulist-core-mfnsaqt3b-neelvara-systems.vercel.app`; staging alias `/api/version` reported the same full commit before final hosted Help feedback-recovery certification | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; immutable Preview and `GET https://app.menulist.digital/api/version` on 2026-08-15 | None | None | `c53fc7463faeeebe6fb89fade584bf76b7eb26a5` | `c53fc7463faeeebe6fb89fade584bf76b7eb26a5` | The following evidence-only ledger commit does not change application runtime; future application edits require a new exact-build gate |
