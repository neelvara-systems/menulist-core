...
- `2026-08-14` - Exact-build desktop `/projects` under retained ordinary-flow
  fixture `2/2` stopped at the truthful `No Active Subscription` guard and
  offered `View Plans`; no project list, editor, or mutation controls were
  exposed and console retained only the known QA App Check diagnostic. The
  retained fixture contract reserves `1/1` for bounded Razorpay Test Mode, so
  it cannot be repurposed for CRUD certification and no entitlement was
  fabricated. Plan-boundary presentation passes; create/edit/duplicate/
  deactivate/delete/restore/reload remains fixture-blocked.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one application commit | Latest application source `0bf5e4d7c71f4c1ebee31dc053b0969235e12078` reached hosted Preview with env `preview`; the following evidence-only ledger commit does not change runtime files | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; public GitHub Vercel commit status; `GET https://app.menulist.digital/api/version` on 2026-08-14 | None | None | `0bf5e4d7c71f4c1ebee31dc053b0969235e12078` | `0bf5e4d7c71f4c1ebee31dc053b0969235e12078` | Future application edits require a new exact-build gate |
