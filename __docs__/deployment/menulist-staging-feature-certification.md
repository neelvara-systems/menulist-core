...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one application commit | Latest application source `ed3d3336658df30c380ddecb0f717ed47086a1c5` reached hosted Preview with env `preview` at deployment `menulist-core-uu0ox2bcr-neelvara-systems.vercel.app`; staging alias `/api/version` reported the same full commit before final hosted Help request-recovery certification | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; immutable Preview and `GET https://app.menulist.digital/api/version` on 2026-08-15 | None | None | `ed3d3336658df30c380ddecb0f717ed47086a1c5` | `ed3d3336658df30c380ddecb0f717ed47086a1c5` | The following evidence-only ledger commit does not change application runtime; future application edits require a new exact-build gate |
...
- ML-QA-101/103 successful ticket-summary, search-answer, and ticket-create data
  still require authorized Answerlattice QA backing data. Rejection recovery is
