...
- `2026-08-14` - Hosted `/menu-manager` on the prior build reproduced a false
  ...
  noindex. The Mac is locked, so the required authenticated visual rerun is not
  yet certified.
- `2026-08-14` - Help Center auth tracing found that the shared provider keyed
  ...
  expectation. Authenticated no-regression visual proof is also pending unlock.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one application commit | Latest application source `597af4a9a5a63186bf544e4d074e319c1394b24f` reached hosted Preview with env `preview`; the following evidence-only ledger commit does not change runtime files | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; public GitHub Vercel commit status; `GET https://app.menulist.digital/api/version` on 2026-08-14 | None | None | `597af4a9a5a63186bf544e4d074e319c1394b24f` | `597af4a9a5a63186bf544e4d074e319c1394b24f` | Future application edits require a new exact-build gate |
...
| ML-QA-030 | Dashboard/Today | Dashboard, Today, Business Health, and Feedback entry | Desktop and mobile | Auth and owner shell pass | Each visible entry loads correct scoped state and honest empty/no-plan behavior | Dashboard settled to the honest no-plan Billing state. Today rendered its complete action list and stable no-action state. Feedback now renders its truthful retryable failure on exact hosted QA instead of empty inbox truth. | IN PROGRESS | Controlled Chrome DOM/console on exact hosted `923713c6d`; focused Feedback verifier, lint, TypeScript, contextual-state gate | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb` | `923713c6d84ee189b6619b3498ad572a26b11a74` | Business Health detail, mobile, and a successful Feedback data read remain pending |
...
| ML-QA-060 | Business Settings | All owner-controlled public business facts | Desktop and mobile | Auth and test project | Valid changes persist and invalid/cancelled changes do not mutate truth | Desktop loaded the scoped business profile, discovery-gap count, locale/hours/analytics/feedback/external-sync sections, and Digital Screens. The initial screen-settings diagnostic recovered to both QA TV modes without mutation. | IN PROGRESS | Controlled Chrome DOM/console on hosted `97a328f19` | None | None | None | `97a328f19160dcc37eb68691a9fe6a712a68ccea` | Persist/cancel/invalid edits, public projection, mobile, and physical screen opens remain pending |
...
| ML-QA-100 | Settings/Support/PWA | Help, app settings, locale, theme, offline/reload, accessibility | Desktop and mobile | Core owner flows available | State inherits correctly and recovery paths are usable | Help shell, search, support entries, recent-viewed state, footer, and navigation rendered. Category/article/changelog reads settled to honest empty content, and failed ticket summary remains retryable. App Settings rendered every maintained desktop appearance/locale/layout option and closed without mutation. | IN PROGRESS | Controlled Chrome DOM/console through hosted `0bc70c9e`; Help Center and auth-scope verifiers, lint, TypeScript, contextual-state gate | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb`, `206c9621b5df5b24ec8d1c070241142bbfb1ce16`, `597af4a9a5a63186bf544e4d074e319c1394b24f` | `597af4a9a5a63186bf544e4d074e319c1394b24f` | Search/ticket mutations, settings persistence, mobile, offline and physical-device PWA remain pending |
| ML-QA-101 | Help Center | Failed landing requests versus confirmed empty/absent content | Desktop Chrome | QA owner `2/2`; protected public-content and ticket transports | A failed landing request stays visibly unavailable and retryable; confirmed empty content remains distinct | Exact hosted QA confirmed honest category/article/changelog empty states and a persistent open-ticket-summary failure. Retrying produced the same bounded DAL failure while the error remained visible and `No tickets yet` stayed absent. | FIXED | Exact hosted Chrome DOM/console; `verify:help-center-boundary`; contextual-state gate; focused lint; TypeScript | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb`, `206c9621b5df5b24ec8d1c070241142bbfb1ce16` | `aa3333a25014001fce86ebaa4632d50510e3b611` | Underlying QA ticket read remains unavailable and requires environment/auth investigation; category/article/changelog currently have confirmed empty data |
| ML-QA-102 | App Settings | Desktop configuration presentation and cancel | Desktop Chrome | Retained QA owner `2/2`; settings dialog available | Every maintained appearance, locale, direction, layout, date/time and display control renders; closing without save does not mutate state | Complete control matrix rendered and dialog closed cleanly without save or new diagnostic | PASS | Controlled Chrome visual/console inspection on 2026-08-14 | None | Closed without saving | None | `0bc70c9eab8207dcad21fbc928196c3ce356ab17` | Persistence/reload, cross-tab propagation, keyboard/screen-reader interaction and MobileShell inheritance remain pending under ML-QA-100 |
...
| ML-QA-120 | AI Menu Manager | Zero-menu, load failure, composer/context and recovery truth | Desktop and mobile | Retained QA owner `2/2` has no selectable menu | Loading, failure and confirmed zero-menu states stay distinct; no fabricated menu, suggestions, context, composer, pending cards or receipts | Prior hosted build reproduced fabricated `Untitled` and suggestion/context affordances. Source fix and exact Preview transport are live, but authenticated visual rerun is blocked by the locked Mac. | IN PROGRESS | Prior controlled Chrome reproduction; `verify-ai-menu-manager`; contextual-state gate; focused lint; TypeScript; exact `/api/version` and route headers on `597af4a9a` | None | None | `adec881498ce8fd311c34de3055ba59f661992a8`, `d1876c5f155c4d7758939eed74656ac2117082c6` | `597af4a9a5a63186bf544e4d074e319c1394b24f` | Unlock Mac and rerun desktop zero-menu state/console; true handheld MobileShell evidence remains separately blocked |
...
Unlock the Mac and rerun exact hosted `597af4a9a` on `/menu-manager` and
`/help-center`, retaining the QA owner tab afterward. Continue the read-only
owner-route matrix after the auth-sync limiter cools;
