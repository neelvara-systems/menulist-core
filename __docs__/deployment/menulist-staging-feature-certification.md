...
- `2026-08-15` - App Settings accessibility and browser-local persistence were
  ...
  provider, entitlement, or public data operation occurred. Physical
  fullscreen, true handheld inheritance, offline/device PWA, and live cross-tab
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one application commit | Latest application source `981e91a0dbc6244612015d9a975cc1794ece133d` reached hosted Preview with env `preview` at deployment `menulist-core-qoea3bm12-neelvara-systems.vercel.app`; staging alias `/api/version` reported the same full commit before final hosted App Settings certification | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; visible Vercel Ready state; immutable Preview and `GET https://app.menulist.digital/api/version` on 2026-08-15 | None | None | `d3d995b88fd1a78b186860f1b0d6f0028a24500b`, `981e91a0dbc6244612015d9a975cc1794ece133d` | `981e91a0dbc6244612015d9a975cc1794ece133d` | The following evidence-only ledger commit does not change application runtime; future application edits require a new exact-build gate |
...
| ML-QA-100 | Settings/Support/PWA | Help, app settings, locale, theme, offline/reload, accessibility | Desktop and mobile | Core owner flows available | State inherits correctly and recovery paths are usable | Help shell, search, support entries, recent-viewed state, footer, and navigation rendered. Category/article/changelog reads settled to honest empty content, and failed ticket summary remains retryable on the latest exact build. App Settings now has named selectors/colour dialog, keyboard favourite controls, explicit colour/direction state, and exact reload/restore proof across appearance, colour, layout, sidebar, direction, language, timezone, date/time, and user-name display preferences. | IN PROGRESS | Controlled Chrome DOM/console through exact hosted `981e91a0d`; Help Center/auth-scope, browser-runtime, Ant theme, owner-dashboard locale, lint, TypeScript, and contextual-state gates | Browser-local preferences only; no server/Firebase/provider mutation | Dark, blue, Vertical, Expanded, LTR, British English, UTC, long date, padded 12-hour time, all display switches off, and no recent/favourite colours restored | `12376cf3f1957c6d8df388bb701619f8c4564adb`, `206c9621b5df5b24ec8d1c070241142bbfb1ce16`, `597af4a9a5a63186bf544e4d074e319c1394b24f`, `d3d995b88fd1a78b186860f1b0d6f0028a24500b`, `981e91a0dbc6244612015d9a975cc1794ece133d` | `981e91a0dbc6244612015d9a975cc1794ece133d` | Help search/ticket mutations, live cross-tab settings propagation, true handheld inheritance, offline/device PWA, and physical fullscreen remain pending |
...
| ML-QA-102 | App Settings | Desktop configuration presentation, accessibility, and local persistence | Desktop Chrome | Retained QA owner `2/2`; settings dialog available | Every maintained control renders with a usable name/state; reversible preference changes survive independent loads and restore exactly | Preceding hosted controls reproduced unnamed language/timezone/date/time selectors, an unnamed colour trigger, pointer-only favourites, and missing selected state. Exact fixed build exposes named selectors and colour dialog, `F` add/remove favourite behavior, and pressed colour/direction state. Light, purple, Horizontal, Collapsed, RTL, American English, Pacific/Midway, short date, 24-hour time, and Show user name on each survived an independent load; every value was reversed. The prior Show today's date lifecycle remains valid. | FIXED | Exact `/api/version` build `981e91a0dbc6244612015d9a975cc1794ece133d`; immutable Preview `menulist-core-qoea3bm12-neelvara-systems.vercel.app`; controlled hosted DOM across independent documents; `test:browser-runtime-boundaries`; `test:antd-theme-boundary`; `verify:owner-dashboard-locales`; focused lint; TypeScript | Browser-local preferences only; no Firebase, server, tenant, store, project, provider, entitlement, or public-data mutation | Final independent load restored Dark, `#3B82F6`, Vertical, Expanded, LTR, British English, UTC, `14 Aug 2026`, padded 12-hour time, Show date/user/fullscreen off, no recent/favourite colours; panel closed | `d3d995b88fd1a78b186860f1b0d6f0028a24500b`, `981e91a0dbc6244612015d9a975cc1794ece133d` | `981e91a0dbc6244612015d9a975cc1794ece133d` | Live cross-tab propagation, physical fullscreen, true handheld MobileShell inheritance, and offline/device PWA remain pending under ML-QA-100 |
