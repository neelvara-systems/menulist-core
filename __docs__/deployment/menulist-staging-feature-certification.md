...
- `2026-08-14` - A client-side owner-navigation sweep avoided repeated full
  ...
  corrects both false-empty UI boundaries locally; exact hosted rerun is
  pending.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ML-QA-000 | Deployment | Exact source/host alignment | HTTP | `staging` checkout | Runtime source, remote, and hosted Preview report one commit | Runtime fix commit, `origin/staging`, and hosted Preview reported `97a328f19160dcc37eb68691a9fe6a712a68ccea`; hosted env is `preview` | PASS | `git rev-parse HEAD`; `git rev-parse origin/staging`; branch-triggered Vercel Preview; `GET https://app.menulist.digital/api/version` on 2026-08-14 | None | None | `97a328f19160dcc37eb68691a9fe6a712a68ccea` | `97a328f19160dcc37eb68691a9fe6a712a68ccea` | A later evidence-only ledger commit does not change the certified runtime files |
...
| ML-QA-030 | Dashboard/Today | Dashboard, Today, Business Health, and Feedback entry | Desktop and mobile | Auth and owner shell pass | Each visible entry loads correct scoped state and honest empty/no-plan behavior | Dashboard settled to the honest no-plan Billing state. Today rendered its complete action list and stable no-action state. Feedback entry rendered but its failed read was falsely shown as empty; local correction is pending hosted proof. | IN PROGRESS | Controlled Chrome DOM/console on hosted `97a328f19`; focused Feedback verifier, lint, TypeScript, contextual-state gate | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb` | `97a328f19160dcc37eb68691a9fe6a712a68ccea` | Exact hosted Feedback recovery-state rerun and mobile remain pending |
| ML-QA-031 | Feedback | Failed inbox read versus confirmed empty state | Desktop Chrome | QA owner `2/2`; deployed indexes match source | Failed list/count read stays visibly unavailable and retryable; it never becomes empty/zero truth | Hosted build reproduced the false-empty state. Source now keeps a persistent retryable error, suppresses empty copy, and marks the attention total unavailable; hosted correction is pending. | IN PROGRESS | Hosted Chrome reproduction; read-only QA index inventory; `verify:guest-feedback-boundary`; focused lint; TypeScript | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb` | Pending | Underlying QA read failure remains to be classified after truthful UI reaches the host; App Check skip diagnostic is unchanged |
...
| ML-QA-100 | Settings/Support/PWA | Help, app settings, locale, theme, offline/reload, accessibility | Desktop and mobile | Core owner flows available | State inherits correctly and recovery paths are usable | Help shell, search, support entries, recent-viewed state, footer, and navigation rendered. Failed approved-content reads were falsely presented as no articles/categories; local correction is pending hosted proof. | IN PROGRESS | Controlled Chrome DOM/console on hosted `97a328f19`; Help Center verifier, lint, TypeScript, contextual-state gate | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb` | `97a328f19160dcc37eb68691a9fe6a712a68ccea` | Exact hosted Help recovery rerun, content-service availability, search/ticket mutations, mobile, offline and physical-device PWA remain pending |
| ML-QA-101 | Help Center | Failed landing-content reads versus confirmed empty content | Desktop Chrome | QA owner `2/2`; protected public-content transport | Failed category/article/changelog loads stay visibly unavailable and retryable | Hosted build reproduced false empty and blank panels. Source now renders persistent translated error alerts with retry controls before healthy empty states; hosted correction is pending. | IN PROGRESS | Hosted Chrome reproduction; `verify:help-center-boundary`; focused lint; TypeScript | None | None | `12376cf3f1957c6d8df388bb701619f8c4564adb` | Pending | Root QA public-content service failure remains to be classified after truthful UI reaches the host |
