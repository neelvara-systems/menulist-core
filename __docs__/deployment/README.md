...
| --- | --- | --- |
| QA infrastructure and keyless runtime | Configuration is prepared under the recorded one-maintainer exception. The custom Vercel `qa` environment tracks only branch `staging`; commit `5161d998e0ff4dcc0358cda02ec0e942856a4550` is Ready and proves canonical-host isolation, authenticated keyless runtime, and zero exact-deployment warning/error/fatal logs. App Check monitoring records verified Authentication and Firestore traffic; the dedicated Maps Embed-only key/host restrictions are prepared. The isolated company-team QA Redis database is Pay as You Go with a provider-confirmed USD 20 hard cap and no Prod Pack. | Hosted feature/provider certification is deliberately deferred until both MenuList QA and production configuration are complete. Later, produce one bounded verified Storage request and certify the map embed on an authorized location-enabled QA fixture before App Check enforcement. Do not manufacture fixture traffic or enable enforcement early. |
