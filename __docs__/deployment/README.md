...
| Answerlattice QA and production | Company-owned QA and production projects, domains, Firebase foundations, keyless Vercel identities, budgets, rules, Storage rules, and indexes are prepared. QA also has 11 approved Functions, Scheduler, task queue, App Check monitoring registration, and rotated AI Studio credentials. Production Functions and App Check remain intentionally absent. | Use the [Answerlattice live setup ledger](../answerlattice/deployment/answerlattice-environment-setup-checklist.md). QA still needs a fresh custom-`qa` deployment from the approved staging revision so Canonica receives the rotated server-side Gemini value, plus the first restore drill after a backup reaches READY. Production then needs owner-created Gemini and legacy reCAPTCHA v3 keys, approved Functions/App Check activation, source promotion, an explicitly authorized Vercel Production redeploy, hosted identity proof, and recovery evidence. |
