...
| Answerlattice QA and production | Company-owned QA and production projects, domains, Firebase foundations, keyless Vercel identities, budgets, rules, Storage rules, indexes, App Check monitoring registrations, and approved core Functions are prepared. QA and production use independent AI Studio authorization keys. Production Secret Manager version 2 is enabled and bound to all eight Gemini Functions; version 1 is destroyed after an HTTP 200 `OK` provider proof. | Use the [Answerlattice live setup ledger](../answerlattice/deployment/answerlattice-environment-setup-checklist.md). Remaining work is certification and release governance: authenticated fixture/data-path proof, recovery fixture and TTL reapplication evidence, physical/browser/provider checks, and an explicitly authorized Vercel Production redeployment so the rotated Gemini value and corrected build provenance replace the prior deployment snapshot. Retain the previous production AI Studio credential until that redeployment drains, then revoke it. |
