...
| --- | --- | --- |
| QA infrastructure and keyless runtime | Complete under the recorded one-maintainer exception. The custom Vercel `qa` environment tracks only branch `staging`; hosted Auth, Firestore, Storage, Cloud Tasks, OIDC exchange, static-key removal, zero-user-managed-key readback, the two-key Gemini reduction, QA-only App Check registration in monitoring mode, and the dedicated Maps Embed-only key/host restrictions are prepared. | Release the current source to the custom `qa` environment, verify valid App Check token traffic and the QA map embed without enabling App Check enforcement, and close only the remaining hosted proof/feature-certification rows in `menulist-staging-feature-certification.md`. |
| Production Google/Firebase foundation | Project `menulist-prod`, billing, budget, Blaze, `us-central1` Firestore and Storage, empty Authentication, Web app, required APIs, Workload Identity Federation, zero-key service account, Vercel Production selectors, Firestore PITR, App Check monitoring-mode registration, canonical Neelvara aliases, production OAuth/NextAuth, Gemini Paid Tier import, INR 750 cap, two service-account-bound Gemini credentials, the isolated `menulist-prod` Sentry project, the frozen 13-event Razorpay production webhook contract, the Maps Embed-only production key/host restrictions, and the MenuList-only managed-env/domain/Firebase Web/admitted-provider inventory are prepared. Optional provider paths are reviewed fail-closed, the non-secret Functions production env is prepared, the production revalidation secret is rotated consistently across Vercel and Secret Manager, and the maintained non-deploy source/configuration plus metadata-only separation gates pass; GA4, Telegram, external uptime monitoring, and EmailOS are intentionally omitted or parked for the initial production setup. | `PROD-D01` remains blocked on the Upstash billing card, `PROD-D04` Resend onboarding is deliberately parked, `PROD-D05`-`PROD-D07` remain owner-blocked behind Razorpay Live KYC, `PROD-D09`-`PROD-D10` remain blocked because Meta has only QA/test assets, and `PROD-D14` awaits complete provider ownership/MFA evidence. `PROD-E04` is partial only because real provider webhook secrets remain unavailable; its revalidation-secret rotation is complete. `PROD-E07` is blocked because current Functions option groups bind unavailable provider secrets; do not create dummy versions. No Phase F release/deploy step is authorized. Resume with the real provider prerequisites or a separately approved and validated conditional-binding source change; OAuth publishing, deployments, provider calls, and traffic remain release-gated. |
