...
| --- | --- | --- |
| QA infrastructure and keyless runtime | Configuration is prepared under the recorded one-maintainer exception. The custom Vercel `qa` environment tracks only branch `staging`; commit `0ad6bc7ba0950f0d69375c9eef2df5ef8fbdfb0f` is Ready on the exact Vercel deployment `dpl_395c2D5ZJfybiDgLrPrjVv5GBkZK`. The deployment and both canonical QA hosts return the same build identity, and the exact-deployment runtime log inventory reports zero warnings, errors, or fatals. Earlier evidence proves canonical-host isolation and authenticated keyless runtime. App Check monitoring records verified Authentication and Firestore traffic; the dedicated Maps Embed-only key/host restrictions are prepared. The isolated company-team QA Redis database is Pay as You Go with a provider-confirmed USD 20 hard cap and no Prod Pack. | Hosted feature/provider certification is deliberately deferred until both MenuList QA and production configuration are complete. Later, produce one bounded verified Storage request and certify the map embed on an authorized location-enabled QA fixture before App Check enforcement. Do not manufacture fixture traffic or enable enforcement early. |
