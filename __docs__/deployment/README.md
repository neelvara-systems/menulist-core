...
| [Answerlattice Deployment Hub](../answerlattice/deployment/README.md) | Canonical entry point for the now-reopened Answerlattice QA-first setup, production setup, historical QA deployment evidence, backup/recovery, and release certification |
| [Answerlattice Environment Setup Checklist](../answerlattice/deployment/answerlattice-environment-setup-checklist.md) | Live owner/operator ledger for the independent `neelvara-answerlattice-qa` and `neelvara-answerlattice-prod` Firebase projects, shared-Vercel OIDC/WIF setup, secrets/providers, scoped deploys, and setup closure |
...
> **State recorded:** August 20, 2026. The rows below are the durable MenuList
> QA and production setup snapshot at the point Answerlattice setup preparation
> was reopened. Later Answerlattice work must not rewrite or reuse MenuList
> project credentials, service accounts, Firebase resources, provider secrets,
> or environment selectors.
...
| Answerlattice | The former `answerlattice-qa` and `answerlattice` project IDs are unowned/unreadable from the company account and retired. On August 20, 2026, `admin@neelvara.com` created company-owned Google Cloud project `neelvara-answerlattice-qa` (project number `216985843437`) under `neelvara.com`. Source/config contracts now use the fresh company namespace; Firebase enablement and product resources remain pending. | Enable and inventory Firebase in `neelvara-answerlattice-qa`, then execute the [Answerlattice Environment Setup Checklist](../answerlattice/deployment/answerlattice-environment-setup-checklist.md) QA first. Create `neelvara-answerlattice-prod` only after QA setup closes. MenuList resources remain isolated and unchanged. |
