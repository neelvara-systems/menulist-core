...
- Item decision symbols come from the same canonical structured facts as the public menu. Vegetarian and non-vegetarian retain their accepted square-dot marks; vegan uses the green Lucide `Vegan` pictogram; gluten-free uses the neutral/theme-coloured Lucide `Wheat` pictogram with an explicit footer definition; every spice intensity alone uses one to four red Game Icons chilli peppers; and owner-confirmed audiences use conventional gender/age pictograms. Marks render inline beside item names and each content-page footer explains only the marks used in the current menu. Options remain text, allergens never become safety icons, and the generic pictograms never claim third-party certification.
- Category headings use the same canonical `category.icon` value as the public live menu. Lucide icons render in the selected print theme accent, while emoji retain their native appearance; both stay inline with the category name. The shared `showCategoryIcons` design switch hides them on both surfaces. Category icon values are part of the print source hash, so changing an icon invalidates stale local exports.
- Premium and service/wellness PDFs now use original transparent botanical ornaments in deterministic cover/content-page compositions. Full-page themes with a contrast veil place that veil 14 mm from the top, left, and right page edges, then retain 10 mm of internal breathing room before menu content. This expands the usable text width without crowding names, descriptions, icons, or prices; each theme's existing bottom/footer clearance remains unchanged. Staff-reference output remains plain.
- All 47 governed parent themes use the owner-approved logo-or-initials identity stack, optional real localized tagline, business-name mastheads on content pages, and a dedicated branded closing page. A valid client logo fills the centred identity box; without one, the same box renders a full-size theme-accent badge with high-contrast two-letter initials instead of a tiny dot-like mark. Each theme keeps its own artwork, palette, contrast, and typography. Printed tagline, phone, and address participate in artifact invalidation; no public claim or contact fact is invented.
- Rosewater Editorial and Mineral Sanctuary content pages retain their governed contrast field as a translucent paper surface rather than removing it. The field extends beyond the text column so category headings, descriptions, and prices do not sit on its edge while the parent-theme artwork remains visible.
