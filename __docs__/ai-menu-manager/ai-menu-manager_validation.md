...
- Direct compact-session executed/failed/manual-task completion now replays the transaction-current matching receipt after a lost acknowledgement. Grouped completion requires the complete matching receipt set, and cancellation persists/replays a compact `cancelled` receipt instead of treating an absent pending card as proof. Contradictory, duplicate, or partial terminal evidence fails closed with no counter or timeline mutation.
