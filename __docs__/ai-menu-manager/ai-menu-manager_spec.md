...
**Feature flag:** `ENABLE_AI_MENU_MANAGER` planned
...
- Direct external publishing without preview and approval.
...
| Human approval for risky actions | Agree | Prices, deletes, external publish, multi-outlet scope, rollback, and generated media publish require approval. |
...
| External publishing everywhere | Partial | Direct publish only where an adapter exists. Otherwise create manual task/export cards. |
...
- quick action chips for common work.
...
11. Commands default to the selected store and selected project shown on the AMM screen.
12. AMM must not silently apply project-level work to all projects; cross-project or all-store scope requires an explicit scope card and owner approval.
...
| Heavy | Price, delete/archive, all-outlet change, external publish, rollback. | Before/after, scope, customer preview, approver, rollback note. |
...
- External publish always requires preview and approval.
...
| `system_manual_task_create` | "Update Zomato image" | Manual checklist/export | Light |
...
- store proposal docs only for actionable cards.
- update proposal docs for lifecycle/status instead of writing every event.
...
| Firebase cost grows with chat history | Compact sessions, proposal docs only for cards, Storage for heavy artifacts. |
...
| External publish confusion | Direct publish only where adapter exists; otherwise manual task/export card. |
...
| External publishing adapters | Unsupported destinations become manual tasks until direct adapters exist. |
