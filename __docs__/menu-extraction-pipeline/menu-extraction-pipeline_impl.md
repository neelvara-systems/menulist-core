...
`src/lib/firebase/menuProcessing.ts` still creates jobs through `POST /api/menu-extraction/jobs`, keeps development callable trigger support, cancels pending/processing jobs through the existing Firestore status fields, and returns the latest active job from `checkExistingActiveJob()`. It now applies the existing exact project/job projectors before the request project field, active-job query, returned job acknowledgement, development callable, cancellation document ref, ignore set, and active-query result handoff. Malformed identities fail before client Firestore access or downstream state. `src/hooks/useMenuProcessingJob.ts` still subscribes to the job document and exposes the same derived states, but listener and cancel failures use the bounded diagnostic helper instead of direct console logging.
...
August 26 hosted-QA follow-up: authenticated menu-link import route failures also carry one fixed `failureStage` value: `project_lookup`, `active_job_lookup`, `source_acquisition`, `storage_write`, or `job_persistence`. This distinguishes a failed public-source acquisition from Firebase/Storage persistence infrastructure without logging the submitted URL, resolved host, acquired bytes, Storage path, project/job/artifact identity, tenant/store/user identity, provider response, or raw exception text. Owner copy and cleanup/persistence behavior remain unchanged.
...
The short-lived browser dismissal map is also tenant/store scoped. Desktop and mobile pass their exact active scope into every read, mark, expiry cleanup, and rollback. The session-storage boundary admits only bounded document IDs, safe integer timestamps, a maximum one-hour interval, and at most 100 records; malformed, expired, future, coercible, or oversized data is removed. Sign-out enumerates and removes the scoped processing/dismissal prefixes so another account in the same tab cannot inherit them.
