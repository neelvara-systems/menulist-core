...
This feature adds only static files (`llms.txt`, `llms-full.txt`) served by Vercel CDN and a feature flag placeholder. No Firestore operations, no Storage operations, no Cloud Functions.
...
- **Future consideration:** If `ENABLE_AGENT_DISCOVERY` is activated with a dynamic API endpoint, cost tracking will need to be added at that time
