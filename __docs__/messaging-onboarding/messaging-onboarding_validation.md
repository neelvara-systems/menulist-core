...
**Date:** July 13, 2026
**Implementation Version:** v3.13 delivery, bounded replacement staging/cleanup, terminal retention, and hard-expiry convergence
**Status:** HISTORICAL SOURCE VALIDATION — not current target, provider, launch, or deploy certification

> **Launch boundary:** Not current launch certification or deploy approval. This report preserves May 17 source and historical smoke evidence only. Current source registers WhatsApp only, while checked-in Functions environments keep provider processing disabled and `/whatsapp` routes to `/create-menu`. Current launch evidence still requires the final owned account, real Meta secrets, webhook registration, explicit target enablement and scoped deploy evidence, provider smoke, browser/device QA, production-host smoke, and the External Certification Runbook.
...
| 1.3  | IMessagingProvider interface   | ✅     | `functions/src/messagingOnboarding/providers/IMessagingProvider.ts` — verifyWebhook, parseIncomingMessages, downloadMedia, sendTextMessage, sendLinkMessage                   |
...
| 1.12 | Durable inbound queue          | ✅     | `functions/src/messagingOnboarding/inboundQueue.ts` — batched BulkWriter dedup, token claim, handler/reply checkpoint, bounded retry, stale-claim recovery |
...
| 4.3  | Rate limiting logic           | ✅     | `sessionEngine.ts:checkRateLimit, createSessionWithId, recordInvalidUploadAttempt` — read-only preflight plus transactional per-user admission/cooldown |
| 4.4  | Post-publish messages (INV-7) | ✅     | `sessionEngine.ts:handleMessage` — Step 4: LIVE session check, returns dashboard link                         |
| 4.5a | Unsupported message types     | ✅     | `WhatsAppAdapter.ts:parseIncomingMessages` — traverses every batched entry/change; video/audio/sticker → "unsupported" |
...
**Initial implementation review found five defects; the maintained ledger below records later runtime and production-hardening findings. Current commands, not this historical count, are the verification authority.**
...
| 30  | **Asset Intelligence trusted a public token URL and did not re-prove stored bytes** | HIGH | Server-owned files were fetched through persisted URLs; path checks did not prove the downloaded bytes still matched the admitted upload | Asset Intelligence now reads only the exact Admin Storage path and rejects bucket/path, length, MIME signature, or SHA-256 mismatch before Gemini |
| 31  | **Valid sessions could exceed Gemini inline request limits** | HIGH | Up to fifteen 10 MB sources were base64-encoded into one `generateContent` request even though the provider limits that inline request to 20 MB | Requests estimated above 18 MiB use bounded Gemini Files API uploads; known provider files and local temp files are cleaned on success and partial failure |
| 32  | **Re-upload after `FAILED` was non-atomic and retained the failed job binding** | HIGH | Upload append and state transition were separate writes; `extractionJobId` remained set and caused the next enqueue to skip | Valid re-upload now appends, clears stale extraction/preview state and the failed job binding, resets intake, and reopens `COLLECTING_INPUT` in one transaction |
| 33  | **Active sessions bypassed invalid-upload cooldown** | HIGH | The third rejection updated only the rate document; active-session routing ran before the rate check, and processing/post-preview invalid files were not counted | Every active media path uses the same transactional rejection counter; the third invalid upload writes session `COOLDOWN` and the per-user expiry atomically |
| 34  | **Poison pending outbound messages retried forever and starved `.limit(1)` delivery scans** | HIGH | Preview, confirmation, and fix pending flags had no token claim or terminal attempt ceiling | Added token-bound leases, a five-claim ceiling, expired preview/fix discard, stale-token protection, discard observability, and producer-side attempt resets |
| 35  | **Queue replay after a committed session mutation could suppress the owner reply** | MEDIUM | Initial-session and invalid-upload idempotency markers caused `handleMessage()` to return `null` if the worker stopped before persisting `handlerCompletedAt` | Reconstructs the deterministic first-upload or invalid-upload reply on replay while leaving the already-committed session mutation unchanged |
| 36  | **Full resend mixed the previous menu into the replacement cycle and retained obsolete delivery leases** | HIGH | Restart cleared extracted output but persisted the full cumulative upload array and did not invalidate preview/fix/reminder delivery state | Restart now persists only post-preview replacement uploads, clears all stale extraction/delivery fields, and best-effort deletes superseded Storage objects; extraction success and publish also invalidate obsolete delivery kinds |
| 37  | **The documented 24-hour hard expiry relied on the daily cleanup sweep on active runtime paths** | HIGH | Active-session lookup, upload/rejection commits, intake claim, validation, enqueue, and terminal extraction handlers did not transactionally compare the session `expiresAt`, so a stale row could accept uploads or consume model/job work before cleanup | Every active lookup/upload/intake/validation/extraction transaction now validates `expiresAt`, records `FAILED` → `EXPIRED` for interrupted work, clears pending delivery/job state, and stops later work; emulator regressions cover each race boundary |
| 38  | **Cumulative post-preview staging made full resend unreachable for 13-15-source previews and leaked partial replacements on old-preview publish** | HIGH | Replacement files consumed the same 15-entry `uploads` cap as authoritative sources; one or two staged files were not project sources but could remain in a LIVE session indefinitely | Added a separate bounded replacement set, legacy timestamp split, atomic promotion at the three-file threshold, and a 45-path durable cleanup queue drained immediately and by the scheduler. Emulator coverage proves 15 old + 3 replacement files and old-preview publish cleanup without authoritative deletion |
| 39  | **Malformed orphan-cleanup rows could permanently occupy the bounded daily retry query** | MEDIUM | Invalid cleanup contracts returned `invalid` while retaining `uploadCleanupPending: true`; twenty corrupt rows could therefore starve all later valid Storage deletions | Invalid rows now transactionally clear only the retry selector while preserving raw pointers and emitting a bounded failure log. Emulator coverage proves a cross-session path is not deleted, remains inspectable, and no longer matches the retry query |
| 40  | **Cleanup Firestore failures could escape after publish, fix, or full-resend state was already committed** | HIGH | The worker caught Storage deletion failures but not its source read or completion transaction; an operational error could report a committed mutation as failed, suppress a deterministic reply, and abort later scheduler rows | Source-read, quarantine, and completion failures now emit bounded diagnostics and return durable retry outcomes without clearing pointers or escaping into the committed owner flow. Emulator coverage forces a completion failure and proves the selector/pointer remain retryable |
| 41  | **Concurrent cleanup workers could double-count the same drained pointers** | LOW | Each worker returned the size of its initial snapshot even when another completion transaction had already removed those entries | Completion returns only the number of matching pointers removed from its current transactional snapshot; a barrier-based emulator race proves two workers count two queued entries once, not four |
| 42  | **Terminal cooldown sessions were never eligible for retention cleanup** | MEDIUM | `COOLDOWN` was intentionally immutable and excluded from active expiry transitions, but the storage/session cleanup query selected only `EXPIRED` | The 48-hour terminal cleanup query now safely includes both `EXPIRED` and `COOLDOWN`; emulator coverage proves a cooled-down session is deleted without an invalid state transition |
| 43  | **Invalid terminal rows could recycle through cooldown retention and poison the bounded query** | MEDIUM | Invalid expiry/reminder rows are safely quarantined into `COOLDOWN`; once terminal cooldown became retention-eligible, an invalid document could be selected and re-quarantined forever | Beyond the retention threshold, an invalid terminal row is precondition-deleted only by its queried document reference. No unvalidated embedded session ID or Storage path is followed; bounded logging exposes the deliberately untouched unsafe orphan. Emulator coverage proves the forged target remains unchanged and the poison row is removed |
| 44  | **Hourly health paid an idle control read on every two-minute intake run** | LOW | The expensive snapshot had an hourly lease, but every enabled poll still transaction-read the lease document | Intake checks the existing lease only in the first four UTC minutes of each hour. The lease still prevents duplicate snapshots; the idle control path drops from about 720 to at most 48 reads/day. |
| 45  | **Pending outbound retry failures were invisible to scheduler health** | MEDIUM | Preview, confirmation, and fix helpers caught provider/query failures internally and returned no result, so the task could report zero errors and no activity | Each helper now returns bounded sent/error counts; the scheduler marks successful outbound work as activity and includes failures in the existing health error metric. |

---

## HISTORICAL VERDICT: SOURCE IMPLEMENTATION COMPLETE
