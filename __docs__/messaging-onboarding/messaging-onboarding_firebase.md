...
Deployment of the June 28, 2026 session-cleanup diagnostic change was attempted with `firebase deploy --only functions:menulistMaintenanceScheduler --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.
...
Deployment of the June 29, 2026 WhatsApp media lookup parse diagnostic was attempted with `PATH="$HOME/.nvm/versions/node/v20.20.2/bin:$PATH" firebase deploy --only functions:messagingOnboarding,functions:menulistMaintenanceScheduler,functions:verifyMenuPublish,functions:computeDecisionBlocksScores,functions:triggerStoreNightlyScheduler,functions:triggerDecisionBlocksScoring --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.

Deployment of the June 29, 2026 messaging response-size hardening change was attempted with `firebase deploy --only functions:messagingOnboarding,functions:menulistMaintenanceScheduler --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.

After the response-size guard was moved into the shared bounded response-body helper, deployment was attempted again with `firebase deploy --only functions:processMenuImagesJob,functions:messagingOnboarding,functions:menulistMaintenanceScheduler --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.

After the Meta media lookup JSON parse moved to the shared bounded JSON response helper, deployment was attempted with `firebase deploy --only functions:messagingOnboarding,functions:menulistMaintenanceScheduler,functions:verifyMenuPublish,functions:computeDecisionBlocksScores,functions:triggerStoreNightlyScheduler,functions:triggerDecisionBlocksScoring --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.
...
Deployment of the June 28, 2026 shared lifecycle-event metadata sanitization change was attempted with `firebase deploy --only functions:messagingOnboarding,functions:processMenuImages,functions:processMenuImagesJob,functions:msgExtractionWatcher,functions:computeDecisionBlocksScores,functions:triggerDecisionBlocksScoring,functions:triggerStoreNightlyScheduler,functions:triggerSchedulerManually,functions:menulistMaintenanceScheduler,functions:verifyMenuPublish,functions:gcpBudgetAlertWebhook --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.

Deployment of the June 28, 2026 asset-validation upload URL and WhatsApp media URL hardening was attempted with `firebase deploy --only functions:messagingOnboarding,functions:menulistMaintenanceScheduler --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.

Deployment of the June 28, 2026 shared Functions network-target helper and public-output/WhatsApp target hardening was attempted with `firebase deploy --only functions:messagingOnboarding,functions:menulistMaintenanceScheduler,functions:processMenuImagesJob,functions:computeDecisionBlocksScores,functions:triggerDecisionBlocksScoring,functions:triggerStoreNightlyScheduler,functions:verifyMenuPublish --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.

Deployment of the June 28, 2026 asset-intelligence upload target-validation change was attempted with `firebase deploy --only functions:messagingOnboarding --project menulist-qa --non-interactive`. The predeploy lint/build completed, but Firebase failed to read `menulist-qa` project metadata through Cloud Resource Manager with HTTP 403: caller does not have permission.
...
| `/api/msg-preview/[sessionId]`         | GET       | 1R                               | Token access + hashed session/IP read limiter | Reads session doc for preview rendering. Rate-limit provider key stores hashed session/IP segments, not raw preview identifiers. |
| `/api/msg-preview/[sessionId]/approve` | POST      | 3-5R + 8-10W                     | Idempotent per attempt + hashed IP publish limiter | Rejects bodies above 4KB before JSON parsing. Reads session, checks existing phone user and subdomain, writes tenant+store+user+project+summaries, finalizes session to LIVE in the same transaction, deletes session `extractedProjectFiles`, then revalidates `menu-store-{storeId}`, `store-{storeId}`, and `client-stores`. May retry if publish fails before commit (recovery to AWAITING_APPROVAL). |
| `/api/msg-preview/[sessionId]/fix`     | POST      | 1R + 1W                          | 3/session max + hashed session/IP action limiter | Rejects bodies above 4KB and limits issue choices to 5 before Firestore work. Reads session, writes fix request to session. Rate-limit provider key stores hashed session/IP segments. |
