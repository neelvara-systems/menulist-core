...
**August 27, 2026 invalid-link recovery:** an invalid, missing, expired, or unavailable preview now tells the owner to use the newest WhatsApp preview link or resend the menu photos, and provides a 44px MenuList recovery action. This is browser-only recovery copy: it performs no provider request, session mutation, Firestore operation, or payment action. The state remains intentionally plain because the channel-specific instruction and action are the complete message; no new illustration or dependency is introduced.
