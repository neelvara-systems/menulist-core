...
- `detected_business_type` — AI-detected from menu content (e.g., "Restaurant", "Salon", "Cafe"). Uses existing `BUSINESS_TYPES` from `src/constants/common.ts` (60+ types, 7 categories). Pre-fills editable dropdown on preview page. Drives: schema.org mapping, OBP structure, time slot defaults, UI labels, menu filters. Confidence-based: HIGH/MEDIUM → use directly, LOW → fallback to "Restaurant"/"food".
...
- Preview URL: `preview.menulist.ai/{sessionId}?token={previewToken}`
...
| 3   | User account creation method — phone-based auth or magic link?    | Affects dashboard login flow | **Decision: Magic link via WhatsApp. Phone number linked to user.**                                                                                                                                   |
