...
**Sources:** ChatGPT conversation (Feb 16, 2026), Cascade architecture review, codebase cross-check, multi-provider architecture design
...
| ---- | ---------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | -------- |
| A-01 | **Single image, complete menu**    | Owner sends 1 clear photo of complete menu → waits → preview → approves               | Session: LIVE. Store, tenant, project, OBP, QR created. Final message with live link sent.          | P0       |
...
| ID   | Scenario                                      | Steps                                                                         | Expected Outcome                                                                                                                                                 | Priority |
| ---- | --------------------------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| B-01 | **Blurry/unreadable photo**                   | Owner sends 2 blurry images                                                   | Asset Intelligence: no valid menu files. Reply: "Please send clearer menu photos or a menu PDF." Session stays in COLLECTING_INPUT.                              | P0       |
| B-02 | **Cropped photo (half menu)**                 | Owner sends photo of only half a menu page                                    | AI detects partial menu. Reply: "Please share full menu for best result." Wait for more.                                                                         | P0       |
| B-03 | **Too many images (>15)**                     | Owner sends 16+ images in one session                                         | After 15th image, reply: "Please combine remaining pages into a PDF or send fewer clearer photos." Additional images not stored.                                 | P0       |
...
| B-07 | **Password-protected PDF**                    | Owner sends encrypted/locked PDF                                              | Detection during download/parse. Reply: "This PDF is locked. Please send an unlocked PDF or photos."                                                             | P0       |
...
| B-09 | **Non-image file (Excel, Word, etc.)**        | Owner sends .xlsx or .docx file                                               | MIME type check rejects. Reply: "Please send menu photos or a menu PDF."                                                                                         | P0       |
...
| B-16 | **Menu image is actually a visiting card**    | Owner sends business card instead of menu                                     | Asset Intelligence classifies as invalid (no menu items/prices). Reply: "Please send menu photos or a menu PDF."                                                 | P0       |
...
| D-03 | **No valid menu files at all**                   | Owner sends 3 photos: all logos/interiors/people                               | `valid_menu_files: []`. Reply: "Please send clear menu photos or menu PDF." Session stays active. `invalidUploadAttempts` incremented. | P0       |
| D-04 | **Partial menu detected**                        | Owner sends 2 photos covering only appetizers section                          | `menu_completeness: "partial"`. Reply: "Please share full menu for best result." State → AWAITING_MORE_UPLOADS.                        | P0       |
...
| E-09 | **Extraction produces 0 items (blank result)** | Gemini AI returns empty categories/items despite valid-looking images                 | **BLANK PREVENTION GATE:** Never generate preview. Session → FAILED. Reply: "Please send clearer menu photos." Owner can retry.             | P0       |
...
| ID   | Scenario                                       | Steps                                                                                                                          | Expected Outcome                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Priority |
| ---- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| G-01 | **Successful publish (full atomicity)**        | Owner approves preview                                                                                                         | Transaction creates: tenant (with incremented tId), store (with sId, default roles, timeSlotPresets, businessCategory), user (linked to phone), platformSummary counts updated, storesSummary synced. Post-transaction: project created, OBP created (if flag on), QR generated. Final WA message sent.                                                                                                                                                                                                                                     | P0       |
| G-02 | **Publish fails mid-transaction**              | Database error during Firestore transaction                                                                                    | Transaction rolls back. No partial tenant/store/user. Retry attempted once. If retry fails: session returns to AWAITING_APPROVAL (not FAILED). Preview + extraction data preserved. Error WA message: "Something went wrong. Please try again." Owner can re-open preview and retry approve.                                                                                                                                                                                                                                                | P0       |
...
| G-05 | **Publish creates correct store data**         | Verify store document after publish                                                                                            | Store has: name, businessType (AI-detected actual type e.g. "Restaurant" — NOT "B2C"), businessCategory (derived via `getBusinessCategory()`), email (from phone mapping), phoneNumber (from WhatsApp), defaultLanguage (from extraction), country (from phone code), currencyCode/currencySymbol (from country), active: true, roles (Owner/Manager/Staff), timeSlotPresets (based on detected businessType), tenantId, storeId, storeKey, isMaster: true, onboardingSource: 'messaging', activationDeadline (24h from publish), logo: ''. | P0       |
...
| G-07 | **Publish idempotency**                        | Session already in LIVE state, approve endpoint called again                                                                   | Returns "Already published" response. No duplicate tenant/store/project created.                                                                                                                                                                                                                                                                                                                                                                                                                                                            | P0       |
| G-08 | **Publish with OBP flag OFF**                  | `ENABLE_OBP: false` in features.ts                                                                                             | Store and project created. OBP step skipped. Final message uses direct menu link (not OBP link).                                                                                                                                                                                                                                                                                                                                                                                                                                            | P1       |
...
| ID   | Scenario                                  | Steps                                                                | Expected Outcome                                                                                                                    | Priority |
| ---- | ----------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | -------- |
| H-01 | **Owner sends "Hi" only (no menu)**       | Owner sends text "Hello" or "Hi" as first message                    | Reply: "Please send your menu photos or menu PDF." No session created (session only starts on first valid media).                   | P0       |
...
| H-03 | **Owner sends "change price to 199"**     | Owner sends text command to edit menu via WhatsApp                   | If preview ready: "Your preview is ready: {link}." Otherwise: "Please send menu photos." Never process text as edit command.        | P0       |
| H-04 | **Owner sends video**                     | Owner records and sends a video of their menu                        | Reply: "Please send menu photos or a menu PDF." Video not stored.                                                                   | P0       |
| H-05 | **Owner sends voice note**                | Owner sends audio message describing their menu                      | Reply: "Please send menu photos or a menu PDF." Audio not stored.                                                                   | P0       |
...
| H-11 | **Duplicate webhook from provider**       | Provider sends same message ID twice (retry behavior)                | `providerMessageIds` array checked. If message ID already processed, skip silently. No duplicate upload or reply.                   | P0       |
...
| ID   | Scenario                                     | Steps                                                               | Expected Outcome                                                                                                                                                   | Priority |
| ---- | -------------------------------------------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| I-01 | **Rate limit: 3rd session same day**         | Same phone creates 2 sessions (both expired), tries 3rd             | 3rd session blocked. Reply: "Please try again later." Phone enters 24h cooldown.                                                                                   | P0       |
| I-02 | **Rate limit: 6th session same week**        | Same phone creates 5 sessions in a week, tries 6th                  | Blocked. Reply: "Please try again later."                                                                                                                          | P0       |
| I-03 | **Invalid uploads exceed limit (3)**         | Same session: owner sends junk 3 times, each time told to send menu | After 3rd invalid attempt, session → EXPIRED. Reply: "Please try again later." Cooldown applied.                                                                   | P0       |
...
| I-10 | **Personal document accidentally uploaded**  | Owner sends government ID or bank statement by mistake              | Asset Intelligence: "invalid file" (not a menu). Reply: "Please send menu photos or a menu PDF." File stored temporarily, auto-deleted on expiry. Never published. | P1       |
...
| L-04 | **Master flag OFF → all webhooks ignored** | Set `ENABLE_MESSAGING_ONBOARDING: false` → send any webhook                      | 200 response. Zero processing. Zero Firestore reads/writes. Zero cost.                                                          | P0       |
...
| L-08 | **Provider-agnostic publish pipeline**     | Publish from WhatsApp session vs hypothetical Telegram session                   | Same atomic transaction: tenant, store, user, project, OBP, QR. Only outbound confirmation message uses provider adapter.       | P0       |
...
| L-11 | **Zero existing code impact**              | Run full test suite with `ENABLE_MESSAGING_ONBOARDING: true`                     | All existing tests pass. No regressions in dashboard onboarding, extraction, store creation, auth, billing.                     | P0       |
| L-12 | **Clean teardown: disable**                | Set `ENABLE_MESSAGING_ONBOARDING: false`                                         | All webhooks return 200. Existing sessions expire in 24h. Published stores unaffected.                                          | P0       |
...
| M-04 | **Dashboard login via magic link**               | Owner clicks dashboard link from WhatsApp after publish                                | Dashboard login page accepts phone number. Magic link sent via WhatsApp. First login creates NextAuth session. Owner linked to existing user doc (by phone).                                                 | P0       |
...
| N-03 | **Tracking flag OFF → zero events written**               | Set `ENABLE_MESSAGING_ONBOARDING_TRACKING: false` → complete full onboarding | `messagingOnboardingEvents` collection has 0 new docs. Onboarding works normally. Zero tracking cost.                                                                                              | P0       |
...
| N-05 | **Error events include error details**                    | Trigger `ASSET_VALIDATION_FAILED` or `PUBLISH_FAILED`                        | Event has `error` object with `code`, `message`, `retryable`. Message contains no PII. Code is machine-readable (e.g., `'GEMINI_API_ERROR'`).                                                      | P0       |
...
T+0:07  Owner clicks "Approve & Publish"
        → State: PUBLISHING
T+0:08  Transaction completes: tenant, store, user, project, OBP, QR.
        → State: LIVE
...
T+0:10  WhatsApp: "Please share full menu for best result."
...
T+0:10  Processing → AI validation: 0 valid menu files
        ...
        → System: "Please send clear menu photos or menu PDF."
...
**Total P0 tests: 116. ALL must pass before controlled testing.**

---

_Document Status: Implementation-Complete (v4.0 — 159 total test cases (116 P0). Includes ChatGPT chaos-engineering review (Feb 18, 2026): +20 cases covering Firestore failures, AI cascade, multi-device chaos, WhatsApp delivery reality, extreme abuse, session corruption. Last updated: Feb 18, 2026.)_
