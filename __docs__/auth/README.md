...
- **Response storage:** Every rate, invalid, expired, success, and failure response uses `private, no-store, max-age=0`, `Pragma: no-cache`, and `nosniff`; claim-token preview state must not be retained by browser or intermediary caches.
...
- **Auth:** Requires active NextAuth session
- **Admission:** All supplied root/nested session user-ID aliases must resolve to one exact actor and pass the shared Firestore document-ID/160-character guard, then `AUTH_SENSITIVE` rate limit by HMAC-hashed normalized session user ID before a 2KB bounded JSON body and Firebase Auth verification
...
- **Commit and session truth:** Firebase Auth is authoritative for the password and automatically invalidates its prior refresh tokens. The user metadata write records `passwordChangedAt`, `authTokensRevokedAt`, and `sessionRevokedAt`, causing the existing access-status monitor to reject every older MenuList server session. A failed metadata write is recorded through bounded diagnostics but does not return a false failure for a password that is already active.
- **Client request/response boundary:** Every route response uses protected private/no-store headers. Desktop account modal and mobile More account access send the request with same-origin credentials, no browser cache, and manual redirect handling. They cap JSON at 16KB, require `success: true` plus `reauthenticationRequired: true`, show the confirmed result, and immediately end the current Firebase/NextAuth browser session so the owner signs in with the new password.
