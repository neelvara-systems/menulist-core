...
Persisted-auth bootstrap boundary: before deciding that a Google/NextAuth session needs a new custom token, the client awaits Firebase Auth's `authStateReady()` settlement and then inspects the restored browser actor and canonical tenant/store claims. A hard reload with a still-valid matching Firebase session therefore performs zero `/api/auth/set-claims` calls and avoids the user-profile query plus canonical-store read that fresh token minting requires. A new OAuth actor, changed store, mismatched claims, or missing persisted actor still enters the same protected set-claims path. Initial-state failure is coded generically and uses the owner-visible Retry/Sign out recovery; it never renders protected data or bypasses the rate limit.
