...
- **Access-status entity reference boundary:** `/api/auth/access-status` normalizes session/user document IDs with the shared Firestore document ID guard before user, tenant, or store direct reads. Path-shaped user IDs fail as `USER_NOT_FOUND`; path-shaped tenant/store references fail as `TENANT_REFERENCE_INVALID` or `STORE_REFERENCE_INVALID` before entity-block checks. String revocation timestamps must be canonical ISO `...Z` values that round-trip before participating in session-revocation comparison.
