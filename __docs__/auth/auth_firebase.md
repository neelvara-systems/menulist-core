...
- **Credentials whitespace boundary:** `src/lib/auth/loginIdentifiers.ts` trims the submitted login identifier and only leading/trailing password or passcode whitespace. The login page reuses those normalized values for NextAuth and its Firebase client sync, while the credentials provider repeats normalization server-side. Internal password spaces remain intact. This changes no account data and adds no Firebase operations.
...
- **Authenticated API response storage:** `withAuth()` applies the shared `private, no-store, max-age=0`, `Pragma: no-cache`, and `nosniff` response policy to CORS rejection, authentication failure, authorization failure, unexpected failure, and admitted handler responses. Protected session or tenant truth must not become shared-cache state; OPTIONS preflight keeps the dedicated CORS policy.
