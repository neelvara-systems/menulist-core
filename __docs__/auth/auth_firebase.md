...
Commit boundary: the Firebase Auth password update is authoritative. The subsequent `modifiedOn` / `passwordChangedAt` write is metadata, so its failure emits bounded `change_password_metadata_sync_failed` diagnostics and still acknowledges the already-completed password change instead of instructing the owner to retry an effect that happened. Audit marker: change-password current-authority hardening.
