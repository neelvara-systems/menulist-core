...
- QR: near-black modules on white panel with four-module quiet zone generated by the QR renderer, not only by surrounding whitespace.
...
Trust cue boundary: the printed surface may show business identity, one current action, a short link, and MenuList attribution. It must not add unsupported "official", "verified", "secure", "no spam", WhatsApp badge, or consent copy to a normal MenuList menu/page QR.
