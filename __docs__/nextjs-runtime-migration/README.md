...
**Current frozen runtime security-patched on September 4, 2026:** Next.js 16.3.4, React 19.2.8
...
The shared root application now runs on the exact stable Next.js 16.3.4 and React 19.2.8 runtime. The patch retains the prior Next 16 application contracts and default 16.3 Turbopack build cache and memory eviction, native-stream SSR, prefetch inlining, immutable static assets, and version-matched bundled documentation while closing the August 2026 image-optimization RCE advisory. Opt-in Cache Components, Partial Prefetching, TypeScript 7, and experimental flags remain outside this dependency migration. Both Webpack and the default Turbopack production paths remain mandatory release-verification paths.
...
|---|---:|---:|---|
| `next` | 14.2.35 | 16.3.4 | Exact stable runtime; closes the August image-optimization RCE and retains bundled PostCSS 8.5.23 |
...
| `eslint-config-next` | 14.2.33 | 16.3.4 | Matches the framework |
| `@next/bundle-analyzer` | 16.2.1 | 16.3.4 | Matches the framework |
