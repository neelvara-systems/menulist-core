...
- `src/lib/ownerBusinessAssistant/server/sessionScope.ts`; every supplied compact and nested tenant, store, and actor session alias must normalize exactly and agree before Business Health route scope is admitted. Thread reads and appends also require the stored `userId` to equal the current actor, not only the tenant/store. The thread response projects an explicit metadata/message allowlist rather than spreading persisted fields.
