...
**AI Model**: `GEMINI_MODELS.IMAGE_GEN` (`gemini-2.5-flash-image`)
...
**Current method: Gemini image generation**
...
The active runtime no longer keeps an Imagen fallback path. Model changes must go
through `src/constants/AI/models.ts` and the image-generation regression checks.
