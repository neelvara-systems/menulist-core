...
- `src/components/mobile/screens/MobileFeedbackScreen.tsx` uses the shared `getFeedbackList()` DAL and requires `assertFeedbackListLoadSucceeded()` with the captured tenant/store before rendering. Filter changes are effect-driven to avoid duplicate reads, and the screen preserves `lastDocId`/`hasMore` so feedback after the first 50 records remains reachable.
