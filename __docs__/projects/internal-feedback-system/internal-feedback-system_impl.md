...
**Last Runtime Audit:** July 6, 2026
...
- Guest Feedback target document-ID boundary: `src/lib/feedback/guestFeedbackProjectIdBoundary.ts` also exports `normalizeGuestFeedbackNumericDocumentId()` for public feedback tenant/store IDs. The public feedback page uses `normalizeGuestFeedbackNumericDocumentId()` for the project-derived tenant/store path segments before `projects/{tId}/{sId}/{projectId}` and `stores/{sId}` reads, so malformed prefix-parsed values cannot reach page lookup. `POST /api/public/feedback/submit` normalizes `data.tId` and `data.sId` into exact positive numeric IDs plus Firestore-safe document IDs before honeypot, Turnstile, `projects/{tId}/{sId}/{projectId}` reads, `stores/{sId}` reads, `tenants/{tId}` reads, `guestFeedback` writes, MOL event logging, or bounded diagnostics. Whitespace-mutated, path-shaped, reserved, zero, negative, decimal, unsafe, or nonnumeric tenant/store IDs fail as validation errors before Firestore work. Valid public feedback links, valid submissions, store/tenant eligibility checks, field defaults, review URL behavior, write shape, and response shape are unchanged.
- Guest Feedback project ID boundary: `src/lib/feedback/guestFeedbackProjectIdBoundary.ts` validates public feedback project IDs with the supported project-ID character rule plus the shared Firestore document-ID boundary before the public QR page reads `projects/{tId}/{sId}/{projectId}` and before `POST /api/public/feedback/submit` reaches honeypot, Turnstile verification, project/store/tenant reads, `guestFeedback` writes, or MOL event logging. The submit schema no longer trims project IDs before validation. The submit route also re-normalizes `data.projectId` into local `projectId` before the project document ref, write payload, MOL event, or bounded diagnostics can use it, and excludes `.doc(data.projectId)`. Malformed, whitespace-mutated, path-shaped, or reserved Firestore document IDs fail quietly at page lookup, request validation, or route-level admission.
...
- Review URL parser failures use `guest_feedback_review_url_parse_failed` with fixed `omit_review_url` fallback policy, source label, value type, value presence/length, and max-length metadata only. Raw review URLs, query strings, store IDs, tenant IDs, project IDs, guest details, and parser exception text are not logged.
...
- Feedback QR download filename boundary: desktop Use MenuList and mobile Share downloads must call `getQrCodeFilename(data.storeName)` from `src/lib/utils/feedbackQrCode.ts` before passing filenames to `downloadQrCode()`. This keeps browser-visible filenames lowercase, ASCII-safe, and suffix-stable instead of deriving them through raw store-name whitespace replacement.
...
The form itself must not render a separate `Powered by MenuList` line or its own duplicate business header. The public feedback route owns the shared temporary-status banner, public business identity header, and public menu footer. The shared identity header falls back to the business initial when a configured logo fails during cold load or after hydration, preserving the same 44px home link without exposing a broken image. The footer keeps the same Share Feedback, contact, policy, social, theme toggle, and MenuList attribution treatment as the menu and OBP surfaces so customer-facing pages stay visually consistent.
...
Guest feedback form accessibility and client-length hardening is also cost-neutral. The visible note prompt is the textarea's associated label; its guidance, privacy/character-count text, and validation error are connected through stable IDs and `aria-describedby`. Optional name, phone, and email controls keep their existing placeholders and validation while also exposing explicit accessible labels, stable field names, autocomplete hints, invalid state, and linked error text. The shared 300-character note limit now guards both the native textarea and controlled React state, so synthetic/programmatic input cannot produce an over-limit counter or oversized client payload before the server schema check. `npm run verify:guest-feedback-boundary` source-gates these semantics. This adds no Firestore reads/writes/deletes, Storage operations, API routes, provider calls, cache invalidations, feature flags, owner settings, or deploy requirement.
