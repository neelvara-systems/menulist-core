# Owner Dashboard v2.2 - Archived Overview-Era Reference
...
- Desktop graph mode is gated by `ENABLE_OWNER_DASHBOARD_GRAPH_MODE`. It is presentation-only: Recharts renders the existing settled `daily30d`, cached `trendSummary`, source, action, item, search, and OBP dashboard-summary data. It includes small comparison chart cards for menu activity, customer actions, search demand, item interest, unavailable demand, and missing-search trends so owners can see week/month movement visually. It adds no analytics events, Firestore writes outside the existing summary write, collections, scheduled jobs, or hot-path tracking.
- Mobile does not expose graph mode in the PWA shell. Mobile keeps the Today-first collapsed activity detail model.
...
- Owner UX now opens with the official customer source before activity: desktop and mobile show menu/hours/customer-link readiness, owner-confirmed Google/Instagram/WhatsApp placement count, private feedback state, and existing Menu setup / Menu trust check primitives before period analytics.
- Owner Action Layer now sits below the official customer source on desktop and mobile. It uses `buildOwnerActionLayer()` to choose one next owner action across menu publish state, hours, customer link, owner-confirmed placement, private feedback, daily menu changes, today status, staff handoff, and price updates, then routes owners to existing MenuList screens.
...
- Owner navigation is grouped by owner verbs on desktop and mobile: Check feedback and activity, Update what customers see, Share and place the link, Account and team, and Advanced setup. The grouping keeps existing routes and permissions; it does not add backend contracts.
- Official-link placement is a presentation layer over existing fields and routes. Desktop keeps placement, private feedback, and share/QR actions inside the official customer source first screen; mobile routes through `MobileShell` to Presence Monitor, Share, and Feedback. No Google/social sync, external scan, crawler, new report storage, new reads, new writes, new API route, or new Cloud Function is added.
...
│  │                     UI Components                              │  │
│  │  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────┐  │  │
│  │  │ Overview │  │ Graphs   │  │  Daily   │  │ Weekly/Month │  │  │
│  │  │ (PRIMARY)│  │  View    │  │  View    │  │    Views     │  │  │
│  │  └──────────┘  └──────────┘  └──────────┘  └──────────────┘  │  │
│  │                      ┌──────────────┐                         │  │
│  │                      │ Overall Footer│                        │  │
│  │                      └──────────────┘                         │  │
...
```
User clicks "Graphs" tab
         │
         ▼
┌─────────────────────────────┐
│ showHistorical = true       │
│ settled summary SWR active  │
└────────────┬────────────────┘
             │
             ▼
┌─────────────────────────────┐
│ dashboard_summary docs      │
│ - menu daily30d             │
│ - cached trendSummary       │
│ - OBP daily30d              │
│ - source/action/item/search │
└────────────┬────────────────┘
             │
             ▼
┌─────────────────────────────┐
│ OwnerDashboardGraphMode     │
│ Recharts presentation only  │
└─────────────────────────────┘
```

Graph mode must stay read-only. It visualizes already-settled dashboard rows, cached weekly/monthly movement labels, small comparison chart cards, and existing rollups; it must not add customer tracking, new Firestore write paths, scheduled aggregation, export controls, branch filters, date-range report builders, or owner-facing BI-style controls. Week checks compare the last settled 7 days with the previous 7 days. Month checks compare this month so far with the same part of last month only when the full comparison window exists inside the compact summary cache; otherwise the owner sees a neutral not-enough-history state. Unavailable demand and missing-search trends are problem signals: increases are shown as attention, while decreases are treated as calmer movement.
...
│       ├── OwnerDashboardGraphMode (desktop, feature-flagged)
│       ├── DailyView
│       ├── WeeklyView
│       └── MonthlyView
└── OverallFooter
...
1. Today (default live view)
2. Overview
3. Graphs (desktop only, behind `ENABLE_OWNER_DASHBOARD_GRAPH_MODE`)
4. Yesterday
5. Last 7 Days
6. This Month
7. Overall
