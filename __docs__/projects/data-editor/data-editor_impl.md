...
The desktop menu selector exposes each menu and the Add card as Enter/Space-operable controls, gives every per-menu action trigger a project-specific accessible name, reveals that trigger on keyboard focus, and provides a visible 44px close action. Pointer selection and the existing edit/duplicate/delete confirmations remain unchanged.
