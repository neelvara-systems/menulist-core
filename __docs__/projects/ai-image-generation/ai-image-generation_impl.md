...
July 28 overlapping-listener follow-up: The current-key query now reads at most five recent project jobs and selects the newest owner-visible row after runtime projection. A newer finished, discarded, or cancelled job can no longer conceal an older queued/processing/completed/failed job that was started concurrently from another browser tab. The legacy fallback remains capped at five and is still subscribed only when no current-key row exists. This adds at most four bounded listener reads for an overlapping project and adds no write, collection, index, cache, Storage, provider, accounting, rules, Function, or deployment change.
