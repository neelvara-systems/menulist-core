...
July 5 local route-log and accounting-input boundary: `/api/image-generation`, `/api/image-editing`, `/api/image-generation/batch-trigger`, and the `/api/image-generation/batch-generation` worker now write bounded local request, response, and transaction summaries plus validation attempted-data, task-start, and batch-start summaries. Local logs record prompt/config shape, item field presence/length or counts, provider response counts, MIME types, token/cost totals, transaction IDs, and invalid-request attempted-data shape only. AI operation rows use `itemSummary` and `generationConfigSummary` instead of raw item details or generation config payloads. Raw prompt text, raw item names/descriptions/categories, reference image URLs, generated base64 images, full provider responses, full transaction objects, raw batch-trigger item ID arrays, raw image accounting input payloads, raw project IDs, raw file IDs, and raw job IDs are not written by these routes. The single-image and edit-image routes avoid redundant normal-path `logger.debug()` breadcrumbs for prompt counts, transaction IDs, provider start/finish counts, and generated edit-prompt length. The batch worker also avoids redundant normal-path `logger.debug()` breadcrumbs for fetched job data, uploaded images, transaction IDs, remaining capacity balance, and job updates; durable local summaries and bounded failure/security diagnostics remain the observability path. Valid provider calls, base64 preview responses, image editing output, owner acceptance/upload flow, batch worker behavior, accounting write count, credit consumption, and Storage writes are unchanged.
