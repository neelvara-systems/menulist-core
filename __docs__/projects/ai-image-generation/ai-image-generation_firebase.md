...
| Batch job status listener | `imageBatchProcessingJobs/{tId}/{sId}` | Batch generation started | Real-time (`onSnapshot`) | 1 current row; up to 5 only on legacy fallback | Current `projectJobKey` range/order query, limit 1; legacy project/status query, limit 5 | The listener validates exact active tenant/store/project and projects an allow-listed owner DTO. The legacy query is subscribed only when the current-key query is empty. |
