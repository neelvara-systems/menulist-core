...
- **Preference diagnostics bounded and browser-local**: Image-generation preference save/load/clear failures log stable `image_generation_preferences_*_failed` diagnostics with tenant/store/storage-key presence-length metadata, preference counts, serialized/stored payload presence-length metadata, and source error metadata only. The zero-Firestore local DTO requires exact numeric tenant/store key scope, API-compatible bounded arrays/strings/aspect ratios, booleans, and canonical non-future save time; corrupt entries are evicted.
