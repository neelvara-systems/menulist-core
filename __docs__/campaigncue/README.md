...
| [campaign-inbox](./campaign-inbox/README.md) | Deterministic, review-first owner intake for several current campaign details in one batch. Draft parsing is browser-local; canonical contacts route to Business Details; confirmed source facts reuse the existing source collection and current snapshot. |
| [photo-clip-missions](./photo-clip-missions/README.md) | Recipe-linked real photo and clip tasks using the existing private Asset Library uploader, explicit rights review, and durable visual readiness without a new collection or listener. |
| [campaign-memory](./campaign-memory/README.md) | Bounded owner-reported recipe/channel evidence inside the existing dashboard summary. It adds no collection, raw-event scan, listener, provider call, or overview read. |
| [read-only-result-evidence](./read-only-result-evidence/README.md) | Owner-copied compact provider-report totals on the existing campaign record, labelled as directional evidence and isolated from Campaign Memory. Live provider API connections remain disabled. |
| [durable-cloud-export-archive](./durable-cloud-export-archive/README.md) | Bounded private retention of the current Campaign Pack ZIP through two rotating Storage slots, one deterministic Asset Library record, checksum-validated signed upload, and generation-pinned signed re-download. |
| [winning-pack-refresh](./winning-pack-refresh/README.md) | Rebuilds a useful owner-reported campaign from current facts and trust checks, with current-fit context and bounded refresh ancestry instead of stale cloning. |
| [vertical-campaign-playbooks](./vertical-campaign-playbooks/README.md) | Curated business-action playbooks and twenty deterministic recipes for current CampaignCue verticals, with protected evidence and claim boundaries and no added Firebase operations. |
| [hosted-offer-page](./hosted-offer-page/README.md) | Explicit owner-published, noindex, expiring Campaign Pack destination with one checked CTA, cached server reads, local QR download, and no visitor tracking or direct posting. |
| [campaign-experiment-coach](./campaign-experiment-coach/README.md) | Deterministic one-change guidance with explicit owner acceptance, recipe-bound measurement, no inferred tested variable, and no performance prediction or provider experiment. |
| [local-visibility-action-center](./local-visibility-action-center/README.md) | Prioritized evidence-backed local actions derived from the existing overview, with freshness/trust checks, manual handoff, and no external-profile inspection or added Firebase operation. |
| [approval-comment-inbox](./approval-comment-inbox/README.md) | Bounded campaign review comments, role-scoped resolution, output/location checks, and approval blocking without a comment collection or added overview read. |
...
| [campaign-pack-template-registry](./campaign-pack-template-registry/README.md) | Implemented category-aware pack-template registry for curated CampaignCue platform templates and owner-saved reusable packs, using shared business category truth, one default category catalog read, lazy overflow reads, and purpose-scoped private Firebase sessions. |
...
| [video-reel-studio](./video-reel-studio/README.md) | In-house browser video studio: source-linked projects, bounded variants, scene timeline, captions, owner media/audio, approval, 9:16/1:1/16:9 rendering, download, and render receipts without an external video provider. |
...
| [analytics-learning](./analytics-learning/README.md) | Observed actions, owner-reported result receipts, bounded Campaign Memory summaries, confidence labels, and deterministic one-variable learning. Provider outcome imports remain disabled. |
| [agency-workspace](./agency-workspace/README.md) | Current campaign approval/comment inbox plus the longer-term client-workspace, report, and external share boundary. |
| [multi-location-center](./multi-location-center/README.md) | Bounded branch-pack creation from one current workspace campaign, with location-specific protected truth, trust, approval, export, result memory, and assigned-manager scope on existing records. |
...
| Public feature pages | Added for Daily Campaign Desk, Campaign Pack Studio, Creative Studio, CueLayers, Video Reel Studio, Creative Trust Center, Brand Playbook and Proof Deck, and Reusable Pack Templates. Sitemap and homepage/footer links use product-scoped feature-page constants. |
...
| Runtime modules | App shell, deterministic Campaign Decision Engine, Daily Campaign Desk, review-first Campaign Inbox, AI Assistance Plan, Owner Pulse, commercial safety, twenty bounded vertical/action recipes, source freshness receipts, structured Campaign Pack Output ZIPs, in-house browser Video Studio, local presence passport, protected-language handoff, manual staff/schedule tasks, trust gates, asset rights metadata, approval logging, owner-reported result receipts, owner-copied directional provider-report snapshots, one-variable learning, launch-readiness checks, and analytics summaries are enabled. Manual handoff copy feedback waits for Clipboard API success or acknowledged textarea fallback success. |
...
| Firebase | Dedicated CampaignCue Admin client plus an auth-isolated named browser app, purpose-scoped direct upload/template sessions, Firestore rules, Storage rules, indexes, and deploy config are implemented and emulator-verified; QA deploy still requires an authenticated Firebase CLI session. |
...
| Disabled modules | Social account connections, direct provider publishing, WhatsApp direct send, ad spend mutation, billing checkout, paid AI generation, external rendered-video provider calls, synthetic avatars/customers, and server/cloud video rendering. |
