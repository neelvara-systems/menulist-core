...
Current CampaignCue output generates WhatsApp-ready copy inside the campaign pack and exposes manual export/copy actions. There is no contact import, audience send, template submission, webhook processing, delivery/reply capture, or opt-out processing in the active runtime; direct send returns the manual fallback path.
