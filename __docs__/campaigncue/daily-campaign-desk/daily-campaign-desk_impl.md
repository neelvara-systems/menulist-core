...
`CampaignCueManualDeliveryCard` is the structured manual-use contract. It groups `handoffFields` into copyable channel cards for WhatsApp, Google/local, social creative, ads handoff, video brief, creator script, and calendar/manual reminders. Copying values and ZIP generation are browser-local only; they do not write Firestore and do not call providers. Copy success is shown only after the Clipboard API acknowledges the handoff; unavailable or rejected copy attempts stay on fixed product copy and log bounded `campaigncue_handoff_copy_failed` diagnostics without storing copied field text.
