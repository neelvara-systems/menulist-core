...
| --- | --- | --- | --- |
| `campaigncueWorkspaces/{workspaceId}/campaigns/{campaignId}` | Bounded direct campaign list and workspace overview | Deterministic export/download-first campaign pack and action state | No realtime listeners; action writes are scoped and rate-limited. |
...
Current runtime creates deterministic export/download-first campaign packs:
