...
This validation record covers the CampaignCue public foundation and the protected manual/export-first runtime: product routing, deployment matrix, static public shell, workspace app, owner setup screens, protected APIs, dedicated CampaignCue Firebase Admin boundary, Firestore rules, Storage rules, source inputs, integration setup records, location records, asset metadata, campaign pack generation, trust gates, schedule/manual actions, analytics summary, docs sync, and Firebase cost posture.

Direct provider publishing, WhatsApp direct send, ad spend mutation, paid generation, billing checkout, rendered video provider calls, and MenuList write-back are intentionally not activated.
...
| Feature flags | Public site, app shell, source integrations, deterministic generation, and analytics enabled; publishing and billing disabled. |
...
| Protected APIs | `/api/campaigncue/workspace`, `/api/campaigncue/campaigns`, `/api/campaigncue/campaigns/[campaignId]/actions`, `/api/campaigncue/assets`, `/api/campaigncue/analytics`, `/api/campaigncue/sources`, `/api/campaigncue/integrations`, and `/api/campaigncue/locations` added with auth, scope, validation, and rate limits. |
| Owner screens | Workspace app includes Home first-use checklist, Business Details, Inputs, Posting Connections, Settings, Campaign Ideas, Campaign Packs, Creative/Social, Video/Reel, UGC, WhatsApp, Google Local, Ads, Checks, Calendar, Assets, Results, Agency, Locations, and Plan/Access posture screens. |
...
| Owner-usability and route-boundary smoke | Built-server HTTP smoke with `X-Forwarded-Proto: https` returned `200` for `/__campaigncue`, `/__campaigncue/app`, `/campaigncue/app`, `campaigncue.ai/app`, and `campaigncue.menulist.online/app`, set `x-product-id: campaigncue` on product-routed requests, loaded the app bundle, preserved workspace `noindex`, and did not include the old Firebase setup instruction. Direct unauthenticated API smoke returned `401`. In-app Browser plugin attempts to open `localhost` and `127.0.0.1` were blocked by the client before the app loaded, so this pass used HTTP route evidence instead of a fresh visual screenshot. |
