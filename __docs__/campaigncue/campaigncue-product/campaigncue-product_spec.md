...
The implemented runtime is manual/export-first. It creates source-backed campaign packs, trust reports, schedule/manual-task records, approval requests, asset metadata, provider setup records, location records, and dashboard summaries. Direct provider publishing, WhatsApp direct send, paid generation, rendered video, billing checkout, provider metric import, and MenuList write-back remain disabled until external setup and feature flags allow them.
...
| Credits, jobs, trust checks, manual fallback | Hidden generation cost |
...
1. Add or connect business data.
...
6. User exports, copies, schedules a manual task, hands off, publishes only where connected, or sends for approval.
...
| Trust checks | Export/publish/direct send blocked when critical issues exist. |
| Manual fallback | Copy/download/export remains available when direct integration fails or is unavailable. |
...
CampaignCue should not compete on raw generation volume or generic scheduling. It competes on local-business source truth, campaign cues, source snapshots, trust checks, WhatsApp/Google/local channel fit, owner approval, agency/multi-location boundaries, credit visibility, manual fallback, and outcome learning.
...
| Google/WhatsApp/Meta direct tools | Native account actions and platform-owned insights. | Multi-channel campaign pack, manual fallback, and cross-channel learning. |
...
| Export/manual fallback is a first-class path. | Every channel output must be useful without direct publishing credentials. |
| WhatsApp is consent-led. | Message generation, copy/share, template/direct-send, opt-out, and pricing posture must be separate. |
| Google Local is useful but constrained. | Prepare posts/offers/events/media and support connected publish only where eligible. |
