...
| Campaign create | existing bounded authority reads, including `sourceSnapshots/current` | existing create batch | New metadata is nested in the existing campaign document. The durable source snapshot, not only the recent source-input query, supplies the freshness hash. |
| Download/export/cloud archive/mark used/schedule for a new pack | workspace guard + idempotency claim; one transactional campaign read, one current-workspace read for public-use actions, and one conditional source-snapshot read only when the current pack carries a freshness hash | idempotency claim, then campaign + event + summary + optional schedule + idempotency completion in one transaction | The transaction derives updates from current campaign truth and rechecks public-use gates immediately before writes. |
| Record result | workspace guard + idempotency claim + one transactional campaign read + one current-workspace read | idempotency claim, then campaign + event + summary + idempotency completion in one transaction | The current workspace read closes role/location revocation races; metrics and experiment variable remain in the campaign and current event, concurrent results cannot lose counters, and no result collection is created. |
...
- Campaign creation reads the compact durable source snapshot once so retained facts older than the recent source-input window cannot make a newly created pack immediately appear stale.
