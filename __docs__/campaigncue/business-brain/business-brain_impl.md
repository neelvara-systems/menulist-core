...
`BusinessBrainService` owns profile, catalog, brand kit, CTA, locale, source confidence, and readiness summaries. Other services read Business Brain through typed service methods, not raw collection access.
...
Business Brain is ready when signed-in store bootstrap or owner-edited details can produce a source-backed campaign cue without duplicate setup. Agency and multi-location posture plus location records exist today; location-specific cue automation requires provider/location setup before activation.
