...
The access-denied surface identifies the current session with a masked account label, uses the canonical client sign-out boundary to clear Firebase, NextAuth, and authenticated browser state before routing **Try another account** back to the preserved sign-in callback, and sends **Get help** to the public contact route. A failed account switch remains on the page with an announced retry message. The sign-in identifier prompt explicitly supports email, phone number, or staff ID and uses **Continue** until the credential-specific step is known.
...
`src/lib/onboarding/ownerAccessRecovery.ts` is the presentation-only recovery classifier shared by desktop and mobile access gates. It distinguishes pending payment, expired Starter access, ended subscription, ready workspace without an active plan, and missing workspace without granting access. Subscription loading failures remain handled by the provider-level retry screen. The pricing onboarding modal reveals business identity first and billing/tax fields second while preserving the same validated purchase-intent payload. Every owner-app plan-selection fallback resolves the current public MenuList origin before opening `/pricing`; local testing therefore remains local, while QA and production leave the canonical app host for their matching website host.
...
After the Firestore transaction, the route creates the provider plan/subscription using server plan data and an `onboardingAttemptId`. Direct and recovered responses must match provider plan, quantity, total count, source, attempt, plan, user, tenant and store. If provider create throws ambiguously, a bounded list scan accepts only that exact full identity. An exact owned attempt whose commercial fields are wrong remains safe to cancel; an unrelated or incompletely bound provider ID is never cancelled. The route then requires an allowlisted HTTPS `rzp.io` checkout URL and persists a pending subscription. An exact owned effect with an invalid URL is cancelled before local compensation. If the local write throws after a possible commit, exact-document recovery requires both ML product aliases, both user aliases, both numeric tenant aliases, both numeric store aliases, and exact provider/plan identity before acknowledging success; incomplete or conflicting rows enter the provider/local compensation path.
...
Every onboarding response, including authentication, limiting, validation and failure branches, is `private, no-store, max-age=0` and `nosniff`. Limiter-provider outages return 503 without quota/reset diagnostics; actual quota exhaustion remains 429 with its bounded retry evidence. The same outage/quota diagnostic separation is enforced on subscription creation, top-up creation and both payment-verification routes.
