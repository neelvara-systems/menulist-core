...
> **Version:** 3.4 (Slug + Canonical + Product-Domain Guardrails + Source Gate)
...
The July 14 post-commit repair changes no Firestore operation count: existing cache tags, Digital Screens touch, and assistant-packet invalidation are attempted after commit with all-settled isolation. Their failure cannot roll back or falsely fail the canonical claim/store/summary/audit transaction and adds no Firebase deployment target.
...
The July 16 custom-domain end-to-end correction moves the advisory availability read from the browser to `GET /api/domain?candidate=`. A valid check performs the same four canonical reads as one claim admission set: tenant, store, deterministic claim, and a `stores.customDomain` query capped at two rows. It uses `DATA_READ`, performs no provider call or write, and POST remains authoritative. This removes a browser query that current tenant-scoped Firestore rules cannot authorize as cross-store uniqueness truth. The advisory and provider-management limiters now fail closed with retryable `503` responses when Upstash is unavailable, before these reads, claim writes, or Vercel calls; normal quota exhaustion remains `429`. Status-precedence and pending-copy fixes are browser-local and add no reads, writes, provider calls, cache invalidations, rules, indexes, Functions, or Firebase deployment target.

The same pass replaces the hard-coded DNS fallback with a pure mapper over the already-returned Vercel `recommendedIPv4`, `recommendedCNAME`, `apexName`, and verification challenge fields. This adds response fields only, not an extra provider or Firestore request. Missing provider guidance now renders a retry state rather than a fabricated DNS value.

The July 14 product-namespace refusal is Firebase- and provider-cost neutral. `isReservedCustomDomainClaimCandidate()` derives reserved roots from the complete deployment-stage product matrix plus declared product-site domains and rejects both exact roots and descendant hosts before the claim transaction or Vercel call. Valid unrelated owner domains keep the same reads, writes and provider flow. No rules, indexes, Cloud Functions, Firebase deploy, or Vercel deployment action is added.
