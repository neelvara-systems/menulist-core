...
| --- | ------------------------------------------------ | ------------------------------- | ------------------------------------------- | ------------------------------------ | --------------------------------------------- | ---------------------------------------------------- | ---------------- |
| 1   | `/api/image-generation`                          | `generateGeminiImageViaFlash`   | `gemini-2.5-flash-preview-05-20`            | Single image generation              | Text prompt + optional ref image              | Base64 image(s)                                      | ⚠️ COMMENTED OUT |
...
| 3   | `/api/image-generation/batch-generation`         | `generateGeminiImageViaFlash`   | `gemini-2.5-flash-preview-05-20`            | Batch image generation               | Text prompt + optional ref image              | Base64 image(s) + upload to Storage                  | ⚠️ COMMENTED OUT |
...
| 5   | `/api/image-editing`                             | `editImageViaFlash`             | `gemini-2.0-flash-preview-image-generation` | Image editing                        | Reference image + edit prompt                 | Base64 edited image                                  | ⚠️ COMMENTED OUT |
...
| 10  | `/api/analytics/weekly-narrative/generate-local` | `POST handler`                  | `gemini-2.0-flash-exp` (OLD SDK)            | Weekly narrative (local fallback)    | Aggregated analytics metrics                  | JSON (narrative, highlights, recommendations)        | ❌ ZERO TRACKING |
...
| 22  | `feedbackAnalysis.ts`      | `generateFeedbackAnalysis`           | `gemini-2.0-flash-exp` (OLD SDK) | Feedback intelligence              | Negative feedback messages                          | JSON (themes, severity, recommendations)      | ❌ ZERO TRACKING                               |
| 23  | `weeklyNarrative.ts`       | `generateWeeklyNarrative`            | `gemini-2.0-flash-exp` (OLD SDK) | Weekly narrative generation        | Aggregated weekly metrics                           | JSON (narrative, highlights, recommendations) | ❌ ZERO TRACKING                               |
| 24  | `kbQuality.ts`             | `analyzeKBArticleQuality`            | `gemini-2.0-flash-exp` (OLD SDK) | KB quality analysis                | Article + low-confidence queries + feedback         | JSON (qualityScore, issues, suggestions)      | ❌ ZERO TRACKING                               |
| 25  | `ownerDashboardSummary.ts` | `generateOwnerDashboardSummary`      | `gemini-2.0-flash-exp` (OLD SDK) | Weekly owner summary               | Weekly menu analytics metrics                       | JSON (bulletPoints)                           | ❌ ZERO TRACKING                               |
| 26  | `ownerDashboardSummary.ts` | `generateDailyAISummary`             | `gemini-2.0-flash-exp` (OLD SDK) | Daily owner summary                | Daily menu analytics metrics                        | JSON (bulletPoints)                           | ❌ ZERO TRACKING                               |
| 27  | `ownerDashboardSummary.ts` | `generateMonthlyAISummary`           | `gemini-2.0-flash-exp` (OLD SDK) | Monthly owner summary              | Monthly menu analytics metrics                      | JSON (bulletPoints)                           | ❌ ZERO TRACKING                               |
...
| **Model**              | `gemini-2.5-flash-preview-05-20` (Gemini) or `imagen-3.0-generate-002` (Imagen)                  |
...
| **Models**             | `gemini-2.0-flash-exp` (OLD SDK — `@google/generative-ai`)                                                 |
...
| **Note**               | Uses OLD SDK (`@google/generative-ai`) instead of new SDK (`@google/genai`). No `usageMetadata` available. |
...
| 3   | **OLD SDK in Cloud Functions**              | `feedbackAnalysis.ts`, `weeklyNarrative.ts`, `kbQuality.ts`, `ownerDashboardSummary.ts` all use `@google/generative-ai` (old SDK). New SDK `@google/genai` is already used in `genAiClient.ts` and `aiUtils.ts` | No `usageMetadata` in old SDK responses = can't track tokens even if we wanted to | Migrate all 4 files to new SDK                                                                        |
...
| 6   | **Weekly narrative local route**            | `/api/analytics/weekly-narrative/generate-local/route.ts` uses `@google/generative-ai` (old SDK) with `NEXT_PUBLIC_GEMINI_AI_KEY` fallback (exposes key to client)                                              | Security risk + redundant with Cloud Function version                             | Remove or secure                                                                                      |
