...
>
> **Note (August 27, 2026 — first-source project propagation):** Hosted provider-free QA reproduced that immediate propagation of a newly created empty master menu is denied by the correct Firestore rule requiring a single-source master. `addProject()` now keeps the empty draft local; the first successful empty-to-one-source master save invokes the existing deterministic propagation boundary once. Linked outlet saves and ordinary later master saves do not invoke it. `npm run test:project-propagation-boundary`, `npm run verify:multi-location-boundary`, `npm run verify:public-business-truth`, focused ESLint, and strict TypeScript pass locally. Exact-current hosted retest remains pending the later explicitly authorized consolidated staging release.
