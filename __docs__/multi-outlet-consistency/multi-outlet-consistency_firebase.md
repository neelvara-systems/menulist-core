...
| Read master job status     | `menuImageProcessingJobs` + linked outlet project guard | Desktop outlet project editor | Every 15s while outlet editor is open | 0-2 | Query capped at 1 | `/api/projects/master-job-status` applies the shared `DATA_READ` cheap-fail gate with HMAC-hashed owner/tenant/store limiter key material, then validates session, tenant, store permission, and linked outlet project before querying active master extraction jobs. Rate-limited requests perform no Firestore reads. Browser polling uses same-origin credentials, no-store cache policy, and manual redirect handling before parsing the response through an 8KB bounded JSON guard and updating outlet blocking state. |
