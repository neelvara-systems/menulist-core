...
Brand propagation keeps its bounded tenant query and initial master permission read, but the final master re-read plus up-to-201 tenant-store documents now occur inside the write transaction. Successful writes remain one master, up to 200 eligible outlets, and one nested summary merge; override-enabled targets still receive no propagated writes. Transaction retries add reads only under contention and prevent stale override/status/block/role decisions. No new collection, index, rule, Function, or Firebase deploy is required.
