...
**Current source gate:** ON (`ENABLE_MULTI_OUTLET: true`); release certification remains separate
...
> **July 16, 2026 billing acknowledgement checkpoint:** Outlet deactivation is idempotent for store truth and retries an unfinished provider quantity reduction even when the outlet is already inactive. The response uses `billingReductionPending` and `billingActionRequired: "CONTACT_SUPPORT"` when owner follow-up is required; desktop and mobile Locations show that state. Manual/offline capacity remains prepaid and is not automatically reduced.
...
> - **July 14, 2026 master-only atomic project propagation:** propagation derives a bounded unique candidate plan from current tenant truth and proves master authority/status from canonical `stores/{source}`, preserving legacy compact rows that omit `isMaster`. The current master project must match tenant/store scope and cannot be linked, deleted, or `localOnly`; each target store is revalidated inside its transaction. Every target uses a deterministic retry ID (or one compatible legacy summary-linked ID), then creates the inherited project and its `projects_{outlet}` summary atomically. An outlet can no longer propagate its local project to siblings, and a summary failure cannot strand a newly committed inherited project.
...
> - **July 26, 2026 master job status response hardening:** `useMasterJobStatus()` calls `/api/projects/master-job-status` with same-origin credentials, no-store cache policy, and manual redirect handling, then parses the response through an 8KB bounded JSON parser and requires the inactive/active response envelope before updating outlet editor blocking state. A rejected, malformed, oversized or unavailable response keeps outlet editing blocked until authoritative master status can be verified.
...
> - **July 6, 2026 master job status session store ID boundary:** `/api/projects/master-job-status` validates authenticated session store scope as an exact positive numeric Firestore document ID before the caller `stores/{sId}` read, tenant access check, store-link comparison, or linked outlet project validation. Whitespace-mutated session store IDs fail closed instead of being numeric-coerced.
> - **July 25, 2026 master job status session alias agreement:** `/api/projects/master-job-status` resolves compact and nested authenticated tenant/store aliases through the shared exact permission-scope resolver. Every supplied alias must be canonical and agree before the caller-store Admin read; contradictory or stale dual-alias sessions fail closed.
...
> - **July 6, 2026 shared project ID boundary:** `src/lib/multiOutlet/projectIdBoundary.ts` now backs the shared resolver and `/api/projects/outlet-save`, requiring `projectId` / `masterProjectId` to be exact Firestore document IDs with exact positive numeric tenant/store path segments before master reads, linked outlet writes, cache invalidation, or acknowledgement.
> - **July 6, 2026 linked outlet save session store ID boundary:** `/api/projects/outlet-save` now validates authenticated session store scope with the same exact positive numeric Firestore document-ID guard before tenant access, caller-store reads, store permission checks, outlet/master store reads, OutletPolicy enforcement, linked outlet writes, cache invalidation, or acknowledgement. Valid outlet and master-store saves keep the same behavior.
...
> - **July 11, 2026 tenant-scoped outlet slug claim boundary:** `src/lib/routing/outletSlugClaim.ts` serializes `outletSlug` ownership through deterministic `platformSummary/outletSlugClaim_{tenantId}_{slug}` documents. Create and rename read the claim plus bounded canonical/history compatibility queries inside the same transaction that writes the outlet, tenant `storesList`, summary, and current/redirect claim. Deactivation releases the current claim with the canonical inactive transition. Different tenants may reuse the same path segment; two outlets in one tenant cannot commit it concurrently. Outlet creation now re-reads the tenant inside its creation transaction instead of appending to a pre-transaction `storesList` snapshot.
> - **July 11, 2026 outlet policy fresh-scope transaction:** `/api/outlets/policy` re-reads the current store and tenant inside its mutation transaction, revalidates exact tenant/active/block/master-or-safe-legacy-promotion state, derives `storesList` and merged policy from those snapshots, and then writes store, tenant list, and summary. Stale pre-transaction data can no longer promote an outlet or overwrite concurrent location-list changes.
> - **July 11, 2026 brand propagation read/write transaction:** `/api/outlets/brand-propagation` now reads current master permission/policy and the bounded tenant outlet set inside the same Firestore transaction that writes master, eligible outlets, and summary rows. Concurrent override, status, block, role, or outlet-set changes conflict and retry instead of being overwritten after an out-of-transaction eligibility check. Cache, screen, and Owner Business Assistant effects use only the committed target outlet IDs after success.
> - **July 14, 2026 brand propagation post-commit isolation:** committed master/outlet/summary truth is now the acknowledgement boundary. Menu/store tags, conditional screen version/global tag work, `client-stores`, and Owner Business Assistant packets run for the master plus every committed target through the shared 20-store bounded all-settled helper. A failed derived effect is logged once and returned as `effectsPending`/`failedEffectCount` without a false HTTP 500 or suppressing later outlet effects.
> - **July 14, 2026 outlet-policy post-commit isolation:** once the current master policy and any legacy master/list/summary repair commit, cache, screen, and Owner Business Assistant effects run through the same shared all-settled boundary. Derived failure returns pending/count metadata and one bounded diagnostic instead of a false mutation failure.
> - **July 14, 2026 outlet lifecycle commit boundary:** create, rename, and deactivate also use the shared post-commit runner. Most critically, create-derived effect failure can no longer enter billing compensation after the outlet transaction committed, so Razorpay/internal quantity is not rolled back beneath existing store truth. Lifecycle responses expose pending/count metadata while keeping their existing success DTO fields.
> - **July 14, 2026 linked-outlet save effect separation:** standard and image-selection saves use the shared runner after transaction success. Each cache tag, screen touch, and project-scoped assistant invalidation is independently settled; one cache-provider exception cannot suppress later global tags, and both save envelopes expose pending/count metadata.
> - **July 12, 2026 linked outlet active projection:** `/api/projects/outlet-save` validates `active` as a boolean and, when present, commits the outlet project write together with `platformSummary/projects_{outletSId}.projects.{projectId}.active` in one batch. Customer menu rendering also fails closed if the fetched project doc is inactive/deleted, so stale legacy summary rows cannot publish a disabled inherited menu.
> - **July 27, 2026 linked-write and project-deletion authority:** already-linked project documents are server-write-only; `setProjectActive()` uses `/api/projects/outlet-save`. Project deletion uses `/api/projects/delete`, which verifies current store permission and tenant membership, checks every active tenant store for a linked outlet inside the same Admin transaction, preserves special-menu/default-summary behavior, writes the soft-delete tombstone and summary update atomically, and runs public-truth post-commit effects. Firestore rules reject direct linked updates, direct soft-delete transitions, and project hard deletes.
> - **July 11, 2026 tenant-name atomic propagation:** `updateTenant()` delegates changed tenant names to authenticated `/api/tenants/name`. The route transaction reads the current tenant and a capped canonical tenant-store query, rechecks current-store permission for non-platform callers, updates tenant name/list plus every store `tenantName` and summary row atomically, then refreshes menu/store/client-store/screen and Owner Business Assistant state in bounded chunks. Optional caller store names are projected to store ID/name only and reconciled against the current canonical tenant list.
...
> - **July 28, 2026 persisted context and permission transition hardening:** `src/lib/multiOutlet/activeStoreContext.ts` now accepts only structured owner-scoped context and evicts legacy scalar, partial, malformed, or unreadable browser values before `SessionProvider` can consume them. `src/providers/sessionProvider.tsx` clears permission state before target/login-store changes and projects missing or unmatched role data to an all-denied permission object, preventing previous-store permissions from surviving a transition.
...
Resetting one item or category override uses a nested `deleteField()` update plus the existing `outletLocalState` version/timestamp fields in the same write. It performs no pre-read and never rewrites sibling override maps, so concurrent resets cannot reintroduce an override deleted by another acknowledged action.
