...
| D14 | **`verifyAllOutletsHaveAllMasterProjects()` safety job**                                  | **DESIGN ONLY — NOT ACTIVE**                  | No runtime Function or scheduler owns this proposed check. Current creation-time propagation instead uses deterministic per-outlet identity and an atomic project/summary transaction; legacy live-data reconciliation remains an explicit external audit task. See §15. |
| D15 | **Idempotent propagation** — Re-running propagation for same master+outlet is safe        | **IMPLEMENTED FOR CURRENT WRITES**             | `propagateNewProjectToOutlets()` derives a deterministic outlet project ID and reuses one compatible existing summary link. Conflicting or duplicate identity fails closed. |
...
## 15. Proposed Safety Job — `verifyAllOutletsHaveAllMasterProjects()` (Not Active)

> Repository truth, July 14, 2026: this section is a retained design proposal, not deployed or scheduled runtime. Do not rely on it for recovery. Current propagation prevents new project/summary partial commits and supports deterministic retry, but existing live orphan/missing rows require an authorized inventory and reconciliation decision.
