...
> **May 20, 2026 UPI recovery note:** Razorpay test-mode production QA on account `danny.tools.4884@gmail.com` showed that an active UPI-backed subscription can be charged normally but Razorpay rejects `subscriptions.update({ quantity })` with `subscriptions cannot be updated when payment mode is upi`. MenuList now treats that as a payment-action case, not a generic outlet failure: `/api/outlets/create` returns `402` with `code: "OUTLET_LOCATION_PAYMENT_REQUIRED"`, desktop/mobile Locations point the owner to Billing, and desktop/mobile Billing can create a replacement same-plan subscription with the next `quantity`. Store creation stays blocked until the new subscription is active and paid capacity exists.

> **July 14, 2026 replacement recovery note:** The replacement checkout carries `replacementForSubscriptionId`. Browser verification and the signed activation/charge webhook both invoke the shared finalizer: cancel the old provider subscription unless terminal, atomically expire the old local row, carry transaction-current credits into the new row exactly once, and synchronize both entitlement states. A matching pending provider `created` checkout is reused on retry; conflicting pending/current rows are blocked. The owner recurring controls are available only while signed in to the directly billed master/HQ store. Switched-store and inherited-outlet views are read-only for recurring actions.
...
| B6  | **Auto-debit via mandate (no payment popup)**                 | **PARTIAL**     | Card/mandate subscriptions can update quantity in place. UPI-backed subscriptions can be active but cannot be quantity-updated by Razorpay; those require a fresh replacement-subscription checkout before outlet creation.        |
...
| B14b | **UPI-backed Razorpay outlet creation**                     | **AGREE**       | If Razorpay rejects provider quantity update because the payment mode is UPI, the API returns `OUTLET_LOCATION_PAYMENT_REQUIRED`; Billing creates a new same-plan subscription with larger `quantity`, then the owner creates the outlet after activation. |
...
| B20 | **Never delete store fully — always archive**                                    | **AGREE**       | Aligns with the current atomic store/summary soft-deactivation (`active = false`). Store doc and summary identity are preserved for audit trail and potential reactivation. |
...
| BE5  | Manual Razorpay dashboard quantity edit     | Webhook `subscription.updated` syncs to Firestore                             | **AGREE**                              | ✅ Webhook validates and transactionally applies positive provider quantity |
...
| BE7  | Provider/local quantity mismatch            | Periodic provider fetch repairs a missed quantity webhook                     | **AGREE**                              | ✅ Leased subscription reconciliation validates and syncs provider quantity |
...
HQ clicks "Add Outlet"
    ...
    │       → If Razorpay rejects UPI quantity update → return 402 OUTLET_LOCATION_PAYMENT_REQUIRED
    │          and route owner to Billing for a replacement same-plan checkout with larger quantity
...
- `subscription.updated` — validates and applies provider quantity with the webhook event replay guard

Quantity updates do not depend on activation/charge payloads. The explicit `subscription.updated` branch requires a safe positive integer, writes through the event-keyed subscription transaction, records `lastWebhook`, and updates Founder Monitor MRR only on a new application.

### 8.2 Reconciliation Safety Net

**File:** `functions/src/billing/reconcileSubscriptions.ts`

The leased reconciliation job also compares provider and local quantity after fetching current provider truth. It accepts only a safe positive quantity capped at 10,000 and writes the mismatch in the same transaction as other provider-state repairs. This recovers a missed `subscription.updated` webhook without changing active-store truth or making a second provider mutation.
...
| BT7  | Sync provider `quantity` in webhook handler — implemented  | `src/app/api/razorpay/webhook/route.ts`                                | `subscription.updated` event-keyed transaction                              |
| BT8  | Add provider/local quantity repair — implemented Jul 14   | `functions/src/billing/reconcileSubscriptions.ts`                      | Positive bounded quantity comparison in the leased reconciliation loop      |
