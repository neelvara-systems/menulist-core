...
| Direct linked-project update, direct soft delete, or hard delete | Firestore rules reject the client write | `test:tenant-store-scoped-rules` |
| Read a project in an unassigned store within the caller's tenant, or link to a master in that store | Firestore rules and the browser DAL reject it; an exact mapped multi-store claim or platform role succeeds | `test:tenant-store-scoped-rules`; `verify:menu-project-editor-boundary` |
| Delete a master while an outlet link exists or races | `/api/projects/delete` checks all active tenant stores in the deletion transaction and rejects without a tombstone/summary mutation | Multi-location source gate plus Firestore master-reference rules regression |
