# Neelvara Main Website - Firebase And Cost Tracking

**Status:** Implemented  
**Scope:** Neelvara Systems company website v1
...
- Neelvara `pId`
- Neelvara Firebase project
- Neelvara Firestore collections
- Neelvara Storage bucket
- Neelvara billing plans
- Neelvara owner notifications
- Neelvara Cloud Functions
- Neelvara feature flags

Allowed and implemented:

- Neelvara route/domain slug in `ProductSiteId`
- Neelvara deployment target with empty `firebaseProjectId`
- Neelvara static public website under `src/app/sites/neelvara/`

Source evidence:

- Product codes do not include Neelvara: `src/constants/product.ts:13`.
- Neelvara deployment targets use an empty Firebase project id: `src/constants/deploymentTargets.ts`.
- Environment validation maps Neelvara to no Firebase env requirements: `src/lib/env/validateEnv.ts`.
