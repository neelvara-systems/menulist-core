...
Prepared `digitalScreenSlide` uploads keep a variant URL ledger, and deterministic sibling paths remain immutable content-addressed objects. Storage rules deny overwrite and duplicate attempts reuse the existing object. After an add-slide request fails, the client performs one owner-state read before compensation: a matching committed slide converts the lost response into success; confirmed absence permits bounded Storage cleanup; and an unavailable verification read retains the object so an ambiguous server commit can never produce a broken public reference. This adds one protected read only on the exceptional failed-add path. A successful first upload retains its existing Storage-create count, and successful deletion commits the canonical/mirror mutation before cleaning the referenced object.

The management API scopes both GET and POST to the active store ID while deriving tenant identity only from the authenticated session. Mapped-store admission and target-store permission/store-document validation happen before the canonical transaction. This adds no normal-path Firestore operation beyond the existing permission/store read and screen transaction; it prevents a branch owner surface from reading or mutating the login store by mistake.
