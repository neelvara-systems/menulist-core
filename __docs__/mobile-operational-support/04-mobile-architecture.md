...
- A More sub-screen transition must update the screen's local state and the controlled MobileShell state in the same callback. Do not mirror local state back to the parent from a second effect: Back transitions can otherwise cross the incoming `initialScreen` value and recurse. Role-gated platform and Answerlattice hubs remain valid return states. Components rendered through a More sub-screen must honor their embedded-mobile mode and must not normalize routes or tabs by replacing the MenuList shell URL.
