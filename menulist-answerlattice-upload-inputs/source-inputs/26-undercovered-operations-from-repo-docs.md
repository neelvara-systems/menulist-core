...
This source captures operational features that were underrepresented in the earlier Answerlattice inputs but are present in MenuList docs and public website content.
...
- Answerlattice should explain that the check prevents the wrong menu from being added to the wrong place; it should not tell owners to ignore mismatch warnings without review.
...
- Holiday closures are documented as coming soon in the repo helpdoc, so Answerlattice must not claim holiday exceptions are fully available unless the live UI proves they are enabled.
...
- Product domains such as Answerlattice and MyCodex are separate from MenuList tenant/customer routing.
...
Answerlattice can explain visible editor states in plain language, but it should not invent hidden settings or claim that dismissed onboarding guidance changes menu data.
...
These features are real support topics, but several are account-state or flag-sensitive. Answerlattice should answer with "where to check" and "what the feature does", then escalate when:
