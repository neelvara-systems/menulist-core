...
} from '@lib/canonica/publicContentCache';
import { renderPublicTiptapHtml } from '@lib/canonica/publicRichText';
import { resolveHostedHelpSiteByDomain } from '@lib/canonica/hostedHelpServer';
...
} from '@template/canonica/hostedHelp/HostedHelpClient';
...
const getRequestDomain = (searchParams?: { domain?: string }) => {
    ...
    const routedHost = headerList.get('x-canonica-hosted-help-domain');
    if (routedHost) return routedHost;

    const isDevRewrite = headerList.get('x-canonica-hosted-help-dev') === '1';
...
export default async function CanonicaHostedHelpPage({ params, searchParams }: PageProps) {
    const site = await resolvePage(searchParams);
    if (!site) notFound();

    const rateLimitConfig = getRateLimitForFeature('CANONICA_HOSTED_HELP');
    const rateLimit = await checkRateLimit({
        key: `canonica-hosted-help:${site.domain}:${getRequestIp()}`,
