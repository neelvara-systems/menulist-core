...
const TENANT_SUMMARY_DOC_ID = 'canonicaTenantsSummary';
...
function getManualTriggerUrl() {
    const explicitUrl = process.env.CANONICA_TRIGGER_NIGHTLY_URL || process.env.CANONICA_NIGHTLY_TRIGGER_URL;
    if (explicitUrl) return explicitUrl;
    const projectId = process.env.CANONICA_FIREBASE_PROJECT_ID || process.env.NEXT_PUBLIC_CANONICA_FIREBASE_PROJECT_ID;
    return projectId ? `https://us-central1-${projectId}.cloudfunctions.net/triggerCanonicaNightly` : '';
}

export const GET = withPlatformAuth(async (request: NextRequest) => {
    if (!FEATURE_FLAGS.ENABLE_CANONICA_INTAKE_PLATFORM_MONITOR) {
        return NextResponse.json({ error: 'Canonica intake monitor is disabled.' }, { status: 404 });
    }
    ...
    try {
        const [tenantSummary, schedulerSnap] = await Promise.all([
            ...
            db.collection(DB_COLLECTIONS.CANONICA_SCHEDULER_RUN_LOGS)
        ...
        if (selectedScope && !selectedTenant) {
            return NextResponse.json({ error: 'Selected Canonica workspace is not present in canonicaTenantsSummary.' }, { status: 404 });
        }
        ...
        if (selectedScope) {
            const [jobsSnap, ledgerSnap] = await Promise.all([
                db.collection(DB_COLLECTIONS.CANONICA_KNOWLEDGE_INTAKE_JOBS)
                    .where('tId', '==', selectedScope.tId)
                    .where('sId', '==', selectedScope.sId)
                    .orderBy('modifiedOn', 'desc')
                    .limit(limit)
                    .get(),
                db.collection(DB_COLLECTIONS.CANONICA_INTAKE_USAGE_LEDGER)
                    .where('tId', '==', selectedScope.tId)
                    .where('sId', '==', selectedScope.sId)
        ...
        const warnings = [
            ...(!selectedScope ? ['Select a Canonica workspace to load job and usage-ledger details.'] : []),
            ...
            ...(!selectedScope && latestSchedulerRun && latestSchedulerRun.errorCount > 0 ? ['Latest Canonica scheduler run recorded errors. Select a workspace before retrying.'] : []),
        ...
        secureError('[Canonica Intake Monitor] Failed to load snapshot', error as Error, {
            endpoint: request.nextUrl.pathname,
        });
        return NextResponse.json({ error: 'Failed to load Canonica intake monitor.' }, { status: 500 });
    }
});
...
export const POST = withPlatformAuth(async (request: NextRequest) => {
    ...
    const secret = process.env.CANONICA_CRON_SECRET || process.env.CRON_SECRET;
    const triggerUrl = getManualTriggerUrl();
    if (!secret || !triggerUrl) {
        return NextResponse.json({ error: 'Canonica manual scheduler trigger is not configured.' }, { status: 503 });
    }
    ...
    try {
        ...
        if (!response.ok) {
            return NextResponse.json({
                error: result?.error || 'Canonica nightly retry failed.',
                result,
        ...
        return NextResponse.json({
            ...
            result,
            costModel: {
                readPattern: 'Manual retry first reads canonicaTenantsSummary once, then runs Canonica nightly for the selected workspace only.',
                realtime: false,
                writes: true,
            },
        });
    } catch (error) {
        secureError('[Canonica Intake Monitor] Manual nightly retry failed', error as Error, {
            tId,
            sId,
        });
        return NextResponse.json({ error: 'Failed to trigger selected Canonica nightly retry.' }, { status: 500 });
    }
});
