    ...
    try {
        ...
        if (!rateLimit.allowed) {
            return NextResponse.json(
                { error: 'Too many feedback changes. Please wait before trying again.' },
        ...
        if (!input) return NextResponse.json({ error: 'Invalid content feedback request.' }, { status: 400, headers: NO_STORE_HEADERS });
