...
import { CANONICA_KNOWLEDGE_INTAKE_CONSTRAINTS } from '@type/canonica';
...
export const POST = withAuth(async (request: NextRequest, session, params: { jobId: string }) => {
    ...
    const access = await requireCanonicaKnowledgeIntakeContext(request, session, {
        rateLimitKey: 'canonica-intake:media-source',
        rateLimit: 8,
        rateWindow: 60,
    ...
    try {
        ...
        secureLog('[Canonica Intake] Media source extracted', {
            jobId: params.jobId,
        ...
        if (status >= 500) {
            secureError('[Canonica Intake] Failed to extract media source', error as Error, {
