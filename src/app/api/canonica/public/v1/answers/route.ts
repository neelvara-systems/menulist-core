...
/**
 * Canonica Public Answers API
 *
 * Public, read-only canonical answer retrieval for external SaaS systems.
 * Authenticated with hash-only `cn_*` API keys and scoped to the key's store.
 */

import { FEATURE_FLAGS } from '@config/features';
import { attemptCanonicalRetrieval } from '@lib/canonica/canonicalRetrieval';
...
function serializeAnswer(answer: any) {
    ...
    return {
        ...
        content: {
            ...
            procedure: FEATURE_FLAGS.ENABLE_CANONICA_GUIDED_WORKFLOWS ? answer.content?.procedure || null : null,
        },
        scope: {
...
export async function POST(request: NextRequest) {
    const auth = await authenticateCanonicaPublicApi(request, 'POST /api/canonica/public/v1/answers');
    ...
    try {
        ...
        const productContext = body.context && FEATURE_FLAGS.ENABLE_CANONICA_CONTEXT_AWARE
            ? CanonicaContextSchema.safeParse(body.context)
            : null;
        ...
        secureError('[Canonica Public API] Answer retrieval failed', error as Error, {
            tId: auth.context.tId,
            sId: auth.context.sId,
        });
