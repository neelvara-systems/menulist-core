...
import { CANONICA_FAQ_SOURCE, CANONICA_FAQ_STATUS } from '@type/canonica';
...
Rules:
- Generate at most ${CANONICA_FAQ_GENERATED_PER_ARTICLE_LIMIT} FAQs.
...
export const POST = withAuth(async (request: NextRequest, session) => {
    try {
        if (!FEATURE_FLAGS.ENABLE_CANONICA_FAQ_MANAGEMENT) {
        ...
        const rateLimitResult = await checkRateLimit({
            key: `canonica-faq-generation:${tenantId}:${storeId}:${session.user.id}`,
            limit: 8,
            window: 3600,
        });
        ...
        if (!db || typeof (db as any).collection !== 'function') {
            return NextResponse.json({ error: 'Canonica database is not configured.' }, { status: 500 });
        }
        ...
        if (existingSnapshot.size >= CANONICA_FAQ_ARTICLE_LINK_LIMIT) {
        ...
        const uniqueFaqs = normalizedFaqs.filter((faq) => {
            ...
            return true;
        }).slice(0, Math.max(0, CANONICA_FAQ_ARTICLE_LINK_LIMIT - existingSnapshot.size));

        const now = Timestamp.now();
        const createdFaqs: Array<Record<string, any>> = uniqueFaqs.map((faq, index) => {
            const ref = db.collection(DB_COLLECTIONS.CANONICA_FAQS).doc();
            return {
                id: ref.id,
                ...
                source: CANONICA_FAQ_SOURCE.ARTICLE,
                active: true,
                articleId,
        ...
        if (createdFaqs.length > 0) {
            ...
            createdFaqs.forEach((faq) => {
                batch.set(db.collection(DB_COLLECTIONS.CANONICA_FAQS).doc(faq.id), faq, { merge: true });
            });
        ...
        recordAiOperationForSession(session, {
            ...
            tId: tenantId,
            sId: storeId,
            action: AI_ACTIONS_TYPES.CANONICA_FAQ_GENERATION,
            articleId,
            ...
            source: 'canonica_article_faq_generation',
        }).catch((logError) => {
            secureError('[Canonica FAQ] Operation log failed', logError as Error, { articleId, tenantId, storeId });
        });

        secureLog('[Canonica FAQ] Article suggestions generated', {
            articleId,
            tenantId,
            storeId,
            createdCount: createdFaqs.length,
        ...
        secureError('[Canonica FAQ] Article suggestion generation failed', error as Error, {
