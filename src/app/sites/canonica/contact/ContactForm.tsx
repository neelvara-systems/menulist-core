...
const getSourcePath = () => {
    ...
    return window.location.pathname.replace(/^\/__canonica/, '') || '/contact';
};

export default function CanonicaContactForm({ basePath = '' }: { basePath?: string }) {
    ...
    const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch('/api/canonica/public/contact', {
                method: 'POST',
            ...
            if (!response.ok || !result?.accepted) {
                throw new Error(result?.error || 'Could not send right now. Please email hello@canonica.app.');
            }

            setSubmitted(true);
            setForm(initialForm);
        } catch (submitError) {
            setError(submitError instanceof Error ? submitError.message : 'Could not send right now. Please email hello@canonica.app.');
        } finally {
            setSubmitting(false);
        }
    };
        ...
        return (
            <div className="rounded-2xl border border-teal-300/20 bg-teal-400/[0.04] p-8 text-center shadow-2xl shadow-black/30">
                ...
                <p className="mt-3 text-sm leading-relaxed text-[#a0a0c0]">
                    We will review the details and reply from hello@canonica.app. Include your product URL in the next message if you missed it.
                </p>
                <button
                    type="button"
    ...
    return (
        <form onSubmit={onSubmit} className="rounded-2xl border border-white/[0.08] bg-white/[0.035] p-5 shadow-2xl shadow-black/30 sm:p-7">
            <div className="grid gap-5">
                <div>
                    <label htmlFor="canonica-contact-name" className={labelClass}>Name</label>
                    <input
                        id="canonica-contact-name"
                        value={form.name}
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label htmlFor="canonica-contact-email" className={labelClass}>Work email</label>
                        <input
                            id="canonica-contact-email"
                            type="email"
                    ...
                    <div>
                        <label htmlFor="canonica-contact-phone" className={labelClass}>Phone</label>
                        <input
                            id="canonica-contact-phone"
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label htmlFor="canonica-contact-url" className={labelClass}>Product URL</label>
                        <input
                            id="canonica-contact-url"
                    ...
                    <div>
                        <label htmlFor="canonica-contact-topic" className={labelClass}>What do you need?</label>
                        <select
                            id="canonica-contact-topic"
                ...
                <div>
                    <label htmlFor="canonica-contact-message" className={labelClass}>Message</label>
                    <textarea
                        id="canonica-contact-message"
                        value={form.message}
                ...
                <div className="hidden" aria-hidden>
                    <label htmlFor="canonica-contact-website">Website</label>
                    <input
                        id="canonica-contact-website"
                        tabIndex={-1}
                        autoComplete="off"
                        value={form.website}
                ...
                <label className="flex items-start gap-3 rounded-xl border border-white/[0.06] bg-[#0f1023] p-4 text-sm leading-relaxed text-[#a0a0c0]">
                    ...
                    />
                    <span>
                        Canonica can contact me about this request. I agree to the{' '}
