...
const getSubscriptionCollectionRef = () => collection(canonicaFirebaseClient, DB_COLLECTIONS.SUBSCRIPTIONS);
const getPaymentTransactionCollectionRef = () => collection(canonicaFirebaseClient, DB_COLLECTIONS.PAYMENT_TRANSACTIONS);
const getStoreDocumentRef = (storeId: number) => doc(canonicaFirebaseClient, DB_COLLECTIONS.STORES, String(storeId));
const getSubscriptionDocumentRef = (subscriptionId: string) => doc(canonicaFirebaseClient, DB_COLLECTIONS.SUBSCRIPTIONS, subscriptionId);
...
const fetchSubscriptionFromStoreSummary = async (
    ...
    const summary = storeSnapshot.exists() ? storeSnapshot.data()?.canonicaSubscription : null;
    if (!summary || typeof summary !== 'object') return null;

    const subscriptionId = String(summary.id || summary.providerSubscriptionId || '').trim();
    if (!subscriptionId) {
        return normalizeSubscription(summary, `canonica_summary_${tenantId}_${storeId}`, tenantId, storeId);
    }
...
export const getCanonicaActiveSubscriptionForStore = async (
    tenantId: number,
    storeId: number,
): Promise<FirestoreSubscriptionDoc | null> => {
    ...
    const request = fetchCanonicaSubscriptionRaw(tenantId, storeId)
        .catch((error) => {
            console.error('[Canonica Billing] Failed to load active subscription', {
                tenantId,
                storeId,
...
export const getCanonicaBillingHistoryForStore = async (
    tenantId: number,
    storeId: number,
): Promise<any[]> => {
    return await apiCallComposer(
        ...
        `getCanonicaBillingHistoryForStore: ${storeId}`,
    );
};
