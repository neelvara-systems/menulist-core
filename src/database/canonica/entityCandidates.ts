/**
 * Canonica — Entity Candidates DAL (Ontology Bootstrap)
 * 
...
export const getEntityCandidates = async (tId: number, sId: number) => {
    return await apiCallComposer(
        async () => {
            ...
            snapshot.forEach((d) => {
                list.push({ ...d.data(), id: d.id } as CanonicaEntityCandidate);
            });
            return list;
        },
        "getEntityCandidates"
...
export const addEntityCandidate = async (data: Omit<CanonicaEntityCandidate, 'id'>) => {
    return await apiCallComposer(
        async () => {
            ...
            return { ...submitData, id: docRef.id } as CanonicaEntityCandidate;
        },
        data,
        "addEntityCandidate"
    );
...
export const promoteCandidate = async (candidateId: string, tId: number, sId: number) => {
    return await apiCallComposer(
        async () => {
            ...
            const candidate = candidateSnap.data() as CanonicaEntityCandidate;
            ...
            const { addEntity } = await import('@database/canonica/entities');
