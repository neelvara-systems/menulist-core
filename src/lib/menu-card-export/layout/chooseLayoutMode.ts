...
export function chooseLayoutMode(settings: MenuCardExportSettings): MenuCardLayoutMode {
    if (settings.preset === 'qr_insert') return 'qr_insert';
    if (settings.styleId === 'premium') return 'single_column';
    if (settings.styleId === 'compact' && settings.paperSize === 'a4') return 'three_column_compact';
