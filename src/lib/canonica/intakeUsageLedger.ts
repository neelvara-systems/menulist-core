...
async function resolveSubscriptionRef(scope: CanonicaScope) {
    ...
    const summary = storeData.canonicaSubscription || {};
    ...
    if (!match?.id) {
        throw new Error('An active Canonica subscription is required before running paid intake processing.');
    }
...
export async function reserveCanonicaIntakeUsage(scope: CanonicaScope, input: ReserveUsageInput) {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (Number(subscription.tId || subscription.tenantId) !== Number(scope.tId) || Number(subscription.sId || subscription.storeId) !== Number(scope.sId)) {
            throw new Error('Canonica subscription scope does not match this workspace.');
        }
        ...
        if (remaining < unitsRequired) {
            throw new Error('Not enough Canonica support credits for this intake processing step.');
        }
        ...
        transaction.set(storeRef, {
            'canonicaSubscription.monthlyCredits': nextMonthlyCredits,
            'canonicaSubscription.topUpCredits': nextTopUpCredits,
            'canonicaSubscription.creditsLastResetMonth': billingPeriod,
            'canonicaSubscription.updatedAt': timestamp,
        }, { merge: true });
        ...
        transaction.set(ledgerRef, {
            ...
            aiOperationId: null,
            errorMessage: null,
            createdBy: cleanText(input.actor?.email || input.actor?.name || input.actor?.id, 160) || 'canonica',
            modifiedBy: cleanText(input.actor?.email || input.actor?.name || input.actor?.id, 160) || 'canonica',
...
export async function finalizeCanonicaIntakeUsage(_scope: CanonicaScope, ledgerId: string, input: FinalizeUsageInput = {}) {
    if (!ledgerId) return;
    await db.collection(DB_COLLECTIONS.CANONICA_INTAKE_USAGE_LEDGER).doc(ledgerId).set({
        status: 'succeeded',
...
export async function refundCanonicaIntakeUsage(scope: CanonicaScope, ledgerId: string, reason: string) {
    if (!ledgerId) return;
    const { storeRef, subscriptionRef } = await resolveSubscriptionRef(scope);
    const ledgerRef = db.collection(DB_COLLECTIONS.CANONICA_INTAKE_USAGE_LEDGER).doc(ledgerId);
    const timestamp = now();
