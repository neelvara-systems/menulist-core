...
export function sanitizeWidgetRuntimeTelemetry(request: NextRequest): Omit<CanonicaWidgetRuntimeStatus, 'lastSeenAt' | 'seenCount'> {
...
export function getWidgetRuntimeStatusFromStoreData(storeData: Record<string, any>): CanonicaWidgetRuntimeStatus | null {
...
export function shouldUpdateWidgetRuntimeStatus(
    existing: CanonicaWidgetRuntimeStatus | null,
    next: Omit<CanonicaWidgetRuntimeStatus, 'lastSeenAt' | 'seenCount'>,
): boolean {
