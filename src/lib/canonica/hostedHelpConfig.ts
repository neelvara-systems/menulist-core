...
export const CanonicaHostedHelpConfigSchema = z.object({
...
export type CanonicaHostedHelpConfig = z.infer<typeof CanonicaHostedHelpConfigSchema>;

export const DEFAULT_CANONICA_HOSTED_HELP_CONFIG: CanonicaHostedHelpConfig = CanonicaHostedHelpConfigSchema.parse({});
...
export function normalizeHostedHelpConfig(value: unknown): CanonicaHostedHelpConfig {
    const parsed = CanonicaHostedHelpConfigSchema.safeParse(value || {});
    const config = CanonicaHostedHelpConfigSchema.parse(parsed.success ? parsed.data : {});
...
export function parseHostedHelpConfigSaveInput(value: unknown): CanonicaHostedHelpConfig {
