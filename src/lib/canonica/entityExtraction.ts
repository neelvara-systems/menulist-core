/**
 * Canonica — Entity Extraction Pipeline (Ontology Bootstrap)
 * 
...
import { addEntityCandidate } from "@database/canonica/entityCandidates";
import { canonicaTokenize } from "@lib/canonica/tokenizer";
import { CANONICA_ENTITY_TYPES, CanonicaEntityCandidate, CanonicaEntityType } from "@type/canonica";
...
function isValidEntity(entity: ExtractedEntityRaw): boolean {
    ...
    if (!entity.type || !Object.values(CANONICA_ENTITY_TYPES).includes(entity.type)) return false;
 ...
 * This is called from the platform admin UI (KB Generation or Canonica dashboard).
 * 
    ...
    for (const entity of newCandidates) {
        try {
            await addEntityCandidate({
                ...
                status: 'pending',
            } as Omit<CanonicaEntityCandidate, 'id'>);
        } catch (error) {
...
export function buildSearchIndexEntry(entity: { name: string; slug: string; description: string; aliases?: string[] }) {
    ...
    const nameTokens = canonicaTokenize(entity.name);
    const descTokens = canonicaTokenize(entity.description, 4).slice(0, 10);
...
export async function extractEntitiesForArticle(
    ...
    if (!FEATURE_FLAGS.ENABLE_CANONICA_ONTOLOGY) return null;
