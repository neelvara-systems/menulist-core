...
import { CANONICA_FAQ_STATUS, type CanonicaFaq, type CanonicaRelatedFaqRef, type CanonicaSurfaceContentItem } from '@type/canonica';
import type { KnowledgeBaseArticleType } from '@type/knowledgeBase';
import { canonicaFirestoreAdmin } from '@lib/firebase/canonicaFirebaseAdmin';
import { CANONICA_FAQ_PUBLIC_LIMIT } from './faqContent';
import { normalizeContextKeys, normalizeSurfaceList } from './productSurfaceContent';
import { canonicaTokenize } from './tokenizer';
...
const rememberFaqs = (cacheKey: string, faqs: CanonicaFaq[]) => {
...
const readFaqsFromCache = (cacheKey: string): CanonicaFaq[] | undefined => {
...
const tokenizeMeaningful = (value: unknown): string[] => Array.from(new Set(
    canonicaTokenize(String(value || ''), 2)
...
const buildFaqFromRelatedRef = (faq: CanonicaRelatedFaqRef, scope: { tId: number; sId: number }): FaqCandidate => ({
    id: faq.id,
    pId: 'CN',
    tId: scope.tId,
    sId: scope.sId,
    ...
    answer: faq.answer || '',
    status: CANONICA_FAQ_STATUS.PUBLISHED,
    source: 'manual',
    active: true,
...
const loadPublishedFaqs = async (tId: number, sId: number, sourceVersion?: number): Promise<CanonicaFaq[]> => {
    const cacheKey = `${Number(tId)}:${Number(sId)}:${sourceVersion || 'unversioned'}`;
    const cached = readFaqsFromCache(cacheKey);
    if (cached) return cached;

    const snapshot = await canonicaFirestoreAdmin
        ...
        .limit(CANONICA_FAQ_PUBLIC_LIMIT)
        .get();

    const faqs = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as CanonicaFaq));
...
export const attemptFaqAnswerRetrieval = async (
    ...
    if (relatedMatch) {
        const faq = relatedMatch.faq as CanonicaFaq;
        return {
            found: true,
            faq,
    ...
    return {
        found: true,
        faq: match.faq as CanonicaFaq,
        score: match.score,
        confidence: match.score >= 110 ? 'high' : 'medium',
        references: await loadLinkedArticleReference(match.faq as CanonicaFaq, options.includeFullArticleReference !== false),
