...
import {
    ensureCanonicaRolesForStore,
    findCanonicaRole,
    getCanonicaDb,
    normalizeCanonicaRolesForStore,
    requireCanonicaPermission,
    requireCanonicaTeamPermission,
} from '@lib/canonica/accessControl';
import { canonicaAuthAdmin } from '@lib/firebase/canonicaFirebaseAdmin';
...
const CreateCanonicaStaffSchema = z.object({
...
const UpdateCanonicaStaffSchema = z.object({
...
const SaveCanonicaRoleSchema = z.object({
    role: z.object({
        active: z.boolean().optional(),
...
const DeleteCanonicaRoleSchema = z.object({
    roleId: z.string().trim().min(1).max(120),
});

type CreateCanonicaStaffInput = {
    countryCode?: string;
    dialCode?: string;
    email: string;
    name?: string;
...
const buildManagedStaffEmail = (tenantId: number, loginId: string) => (
    `canonica-staff-${tenantId}-${loginId}@${STAFF_EMAIL_DOMAIN}`.toLowerCase()
);
...
const generateUniqueCanonicaStaffLoginId = async () => {
    for (let attempt = 0; attempt < 10; attempt += 1) {
        ...
        const canonicaSnapshot = db
            ? await db.collection(DB_COLLECTIONS.USERS)
                .where('loginUsername', '==', loginId)
                .limit(1)
                .get()
            : null;
        if (!canonicaSnapshot || canonicaSnapshot.empty) return loginId;
    }

    throw new Error('CANONICA_STAFF_LOGIN_ID_GENERATION_FAILED');
};
...
const sanitizeCanonicaStaffUser = (id: string, data: any, roles: CanonicaRoleDefinition[]) => {
...
const getCanonicaUserByEmail = async (email: string) => {
    const db = getCanonicaDb();
...
const getCanonicaUserById = async (userId: string) => {
    const db = getCanonicaDb();
    if (!db) return null;
...
const syncDefaultAuthProductAccount = async (params: {
    ...
    const shouldSetRootCanonicaScope = !existingDefaultUser?.tenantId || !existingDefaultUser?.storeId || existingDefaultUser?.pId === PRODUCT_IDS.CANONICA;
    ...
    const defaultUserUpdate = sanitizeFirestoreValue({
        ...
        productAccounts: {
            [PRODUCT_IDS.CANONICA]: productAccount,
        },
        modifiedOn: now,
        ...(shouldSetRootCanonicaScope ? {
...
const updateDefaultAuthProductAccountStatus = async (params: {
    ...
    const account = productAccounts[PRODUCT_IDS.CANONICA] || {};
    await userRef.set(sanitizeFirestoreValue({
        ...
        ...((userData.pId === PRODUCT_IDS.CANONICA || userData.productId === PRODUCT_IDS.CANONICA) ? {
            active: params.active,
...
const ensureCanonicaAuthUserDisabledState = async (email: string, disabled: boolean) => {
    try {
        const canonicaUser = await canonicaAuthAdmin.getUserByEmail(email);
        if (canonicaUser.disabled !== disabled) {
            await canonicaAuthAdmin.updateUser(canonicaUser.uid, { disabled });
        }
    } catch (error: any) {
...
const revokeDefaultFirebaseRefreshTokens = async (data: any, fallbackEmail: string, context: Record<string, unknown>) => {
    ...
    try {
        ...
        if (error?.code !== 'auth/user-not-found') throw error;
        logger.warn('[Canonica Staff] Default Firebase Auth user missing during session revoke', {
            ...context,
            email,
        });
        return null;
...
const buildCanonicaPermissionClaims = (
    roles: CanonicaRoleDefinition[],
    roleId: string,
    platformRole?: string,
) => {
    const normalizedRoleId = String(roleId || DEFAULT_CANONICA_ROLE_IDS.STAFF).trim();
    const permissions = isPlatformRole(platformRole) || normalizedRoleId === DEFAULT_CANONICA_ROLE_IDS.OWNER
        ? normalizeCanonicaRolePermissions(DEFAULT_CANONICA_ROLE_METADATA[DEFAULT_CANONICA_ROLE_IDS.OWNER].permissions)
        : normalizeCanonicaRolePermissions(findCanonicaRole(roles, normalizedRoleId)?.permissions);
...
const syncCanonicaAuthClaimsForStaffUser = async (params: {
    active: boolean;
    email: string;
    platformRole?: string;
    roleId: string;
    ...
    try {
        ...
        const adminClaim = isPlatformRole(platformRole) || params.roleId === DEFAULT_CANONICA_ROLE_IDS.OWNER;
        await canonicaAuthAdmin.setCustomUserClaims(canonicaUser.uid, {
            pId: PRODUCT_IDS.CANONICA,
            role: params.roleId,
            platformRole,
            ...
            ...buildCanonicaPermissionClaims(params.roles, params.roleId, platformRole),
        });
        if (canonicaUser.disabled !== !params.active) {
            await canonicaAuthAdmin.updateUser(canonicaUser.uid, { disabled: !params.active });
        }
        await canonicaAuthAdmin.revokeRefreshTokens(canonicaUser.uid);
    } catch (error: any) {
        if (error?.code !== 'auth/user-not-found') throw error;
    }
};
...
const ensureAnotherActiveOwner = async (
    ...
    const db = getCanonicaDb();
    if (!db) throw new Error('CANONICA_FIREBASE_NOT_CONFIGURED');
    ...
    const hasOtherOwner = snapshot.docs.some((doc) => {
        ...
        return (Array.isArray(data?.stores) ? data.stores : []).some((store: any) => (
            Number(store?.storeId) === storeId && store?.role === DEFAULT_CANONICA_ROLE_IDS.OWNER
        ));
    });
...
const verifyStaffFeature = () => FEATURE_FLAGS.ENABLE_CANONICA_STAFF_ACCESS;

export const listCanonicaStaffUsers = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'DATA_READ', 'canonica-staff-read');
    if (rateLimit) return rateLimit;

    const { access, response } = await requireCanonicaTeamPermission(request, session);
    if (response) return response;
    if (!access) return jsonError('Forbidden', 403, 'FORBIDDEN');

    const db = getCanonicaDb();
    if (!db) return jsonError('Canonica Firebase is not configured', 503, 'CANONICA_FIREBASE_NOT_CONFIGURED');

    const snapshot = await db.collection(DB_COLLECTIONS.USERS)
        .where('tenantId', '==', access.scope.tenantId)
        .get();
    const users = snapshot.docs
        .map((doc) => sanitizeCanonicaStaffUser(doc.id, doc.data(), access.roles))
...
export const createCanonicaStaffUser = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'AUTH_SENSITIVE', 'canonica-staff-create');
    if (rateLimit) return rateLimit;
    ...
    const validation = validateAPIInput(CreateCanonicaStaffSchema, await request.json().catch(() => ({})));
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Staff Create', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }

    const input = validation.data as CreateCanonicaStaffInput;
    const requestedRoleId = input.roleId || DEFAULT_CANONICA_ROLE_IDS.STAFF;
    if (requestedRoleId !== DEFAULT_CANONICA_ROLE_IDS.STAFF && !access.permissions[CANONICA_PERMISSION_KEYS.ASSIGN_ROLES]) {
    ...
    const staffLoginId = await generateUniqueCanonicaStaffLoginId();
    const loginEmail = hasEmail ? String(input.email) : buildManagedStaffEmail(access.scope.tenantId, staffLoginId);
    const existingCanonicaUser = await getCanonicaUserByEmail(loginEmail);
    if (existingCanonicaUser && Number(existingCanonicaUser.data.tenantId) !== access.scope.tenantId) {
        return jsonError('This email is registered with another Canonica workspace.', 409, 'EMAIL_OTHER_TENANT');
    }
    ...
    const userId = existingCanonicaUser?.id || existingDefaultUser?.id || defaultFirebaseUser.uid;
    const now = admin.firestore.Timestamp.now();
    ...
    const userDoc = sanitizeFirestoreValue({
        ...
        createdOn: existingCanonicaUser?.data?.createdOn || now,
        createdVia: hasEmail ? 'canonica-staff-invite' : 'canonica-owner-passcode',
        deleted: false,
        dialCode: input.dialCode,
        email: loginEmail,
        firebaseUid: defaultFirebaseUser.uid,
        isVerified: true,
        loginUsername: existingCanonicaUser?.data?.loginUsername || staffLoginId,
        ...
        staffAuthMode,
        staffLoginId: existingCanonicaUser?.data?.staffLoginId || staffLoginId,
        storeId: access.scope.storeId,
    ...
    await syncCanonicaAuthClaimsForStaffUser({
        active: true,
        email: loginEmail,
    ...
    logger.info('[Canonica Staff] Staff user created', {
    ...
    return NextResponse.json({
        ...
        user: sanitizeCanonicaStaffUser(userId, userDoc, access.roles),
        userId,
    });
};

export const updateCanonicaStaffUser = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'DATA_WRITE', 'canonica-staff-update');
    if (rateLimit) return rateLimit;
    ...
    const validation = validateAPIInput(UpdateCanonicaStaffSchema, await request.json().catch(() => ({})));
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Staff Update', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }

    const input = validation.data as UpdateCanonicaStaffInput;
    const target = await getCanonicaUserById(input.userId);
    ...
    if (Number(existingData.tenantId || existingData.tId) !== access.scope.tenantId) {
        logSecurity('Authorization Failed - Canonica Staff Tenant Mismatch', session, request, {
    ...
    const nextRoleId = input.roleId || currentStore.role || existingData.role || DEFAULT_CANONICA_ROLE_IDS.STAFF;
    const roleChanged = nextRoleId !== currentStore.role;
    if (roleChanged && !access.permissions[CANONICA_PERMISSION_KEYS.ASSIGN_ROLES]) {
    ...
    try {
        ...
        if ((input.active === false || roleChanged) && currentStore.role === DEFAULT_CANONICA_ROLE_IDS.OWNER && nextRoleId !== DEFAULT_CANONICA_ROLE_IDS.OWNER) {
    ...
    const updateData = sanitizeFirestoreValue({
        ...
        sessionRevokedReason: active === false ? 'canonica_staff_deactivated' : undefined,
    });
    ...
    if (active !== undefined) {
        if (active === false) {
            await revokeDefaultFirebaseRefreshTokens(existingData, existingData.email, {
                action: 'canonica-staff-deactivate',
                tenantId: access.scope.tenantId,
                storeId: access.scope.storeId,
                userId: input.userId,
            });
        ...
        await ensureCanonicaAuthUserDisabledState(String(existingData.email || ''), active === false);
    } else if (roleChanged) {
        await updateDefaultAuthProductAccountStatus({
    ...
    return NextResponse.json({
        success: true,
        user: sanitizeCanonicaStaffUser(input.userId, updated.data(), access.roles),
        userId: input.userId,
    });
};

export const removeCanonicaStaffUser = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'DATA_WRITE', 'canonica-staff-remove');
    if (rateLimit) return rateLimit;
    ...
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Staff Remove', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }
    ...
    const currentStore = currentStores.find((store: any) => Number(store.storeId) === access.scope.storeId);
    if (!currentStore) return jsonError('Team member is not assigned to this workspace', 404, 'STORE_MAPPING_NOT_FOUND');
    if (currentStore.role === DEFAULT_CANONICA_ROLE_IDS.OWNER) {
        try {
            await ensureAnotherActiveOwner(access.scope.tenantId, access.scope.storeId, input.userId);
        } catch (error: any) {
            if (error?.message === 'LAST_OWNER') return jsonError('Add another Owner before removing this access.', 409, 'LAST_OWNER');
    ...
    await target.ref.update(sanitizeFirestoreValue({
        ...
        sessionRevokedReason: shouldDeactivate ? 'canonica_staff_removed' : undefined,
    }));
    ...
    await syncCanonicaAuthClaimsForStaffUser({
        ...
        email: existingData.email,
        platformRole: existingData.platformRole,
        roleId: nextPrimaryStore?.role || currentStore.role || existingData.role || DEFAULT_CANONICA_ROLE_IDS.STAFF,
        roles: access.roles,
    ...
    if (shouldDeactivate) {
        await revokeDefaultFirebaseRefreshTokens(existingData, existingData.email, {
            action: 'canonica-staff-remove-last-workspace',
            tenantId: access.scope.tenantId,
            storeId: access.scope.storeId,
            userId: input.userId,
        });
        await ensureCanonicaAuthUserDisabledState(String(existingData.email || ''), true);
    }
...
export const requestCanonicaStaffPasswordReset = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'AUTH_SENSITIVE', 'canonica-staff-password-reset');
    if (rateLimit) return rateLimit;
    ...
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Staff Password Reset', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }
    ...
    const currentStores = Array.isArray(existingData.stores) ? existingData.stores : [];
    const currentStore = currentStores.find((store: any) => Number(store.storeId) === access.scope.storeId);
    const roleId = currentStore?.role || existingData.role || DEFAULT_CANONICA_ROLE_IDS.STAFF;
    const storeIds = (Array.isArray(existingData.storeIds) ? existingData.storeIds : currentStores.map((store: any) => store.storeId))
        .map(Number)
        .filter(isPositiveId);

    const loginId = String(existingData.staffLoginId || existingData.loginUsername || '').trim() || await generateUniqueCanonicaStaffLoginId();
    ...
    await target.ref.update(sanitizeFirestoreValue({
        ...
        sessionRevokedReason: 'canonica_staff_passcode_reset',
...
export const forceSignOutCanonicaStaffUser = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'AUTH_SENSITIVE', 'canonica-staff-force-signout');
    if (rateLimit) return rateLimit;
    ...
    const validation = validateAPIInput(UserIdSchema, await request.json().catch(() => ({})));
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Staff Force Signout', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }
    ...
    const existingData = target.data || {};
    if (Number(existingData.tenantId || existingData.tId) !== access.scope.tenantId) {
        logSecurity('Authorization Failed - Canonica Staff Force Signout Tenant Mismatch', session, request, {
            requestedTenantId: access.scope.tenantId,
            targetTenantId: existingData.tenantId || existingData.tId,
            userId: input.userId,
        }, 'critical');
    ...
    await revokeDefaultFirebaseRefreshTokens(existingData, email, {
        action: 'canonica-staff-force-signout',
        tenantId: access.scope.tenantId,
        storeId: access.scope.storeId,
        userId: input.userId,
    });
    ...
    logger.info('[Canonica Staff] Owner forced team member session signout', {
        tenantId: access.scope.tenantId,
        storeId: access.scope.storeId,
        userId: input.userId,
    });
...
export const saveCanonicaRoleDefinition = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'DATA_WRITE', 'canonica-role-save');
    if (rateLimit) return rateLimit;

    const { access, response } = await requireCanonicaPermission(request, session, CANONICA_PERMISSION_KEYS.ASSIGN_ROLES);
    if (response) return response;
    if (!access) return jsonError('Forbidden', 403, 'FORBIDDEN');

    const validation = validateAPIInput(SaveCanonicaRoleSchema, await request.json().catch(() => ({})));
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Role Save', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }

    const input = validation.data as SaveCanonicaRoleInput;
    const db = getCanonicaDb();
    if (!db) return jsonError('Canonica Firebase is not configured', 503, 'CANONICA_FIREBASE_NOT_CONFIGURED');
    ...
    const existingRoles = await ensureCanonicaRolesForStore(storeRef, storeData, session?.user?.email);
    ...
    if (roleId === DEFAULT_CANONICA_ROLE_IDS.OWNER) {
    ...
    const nextRole: CanonicaRoleDefinition = {
        ...
        permissions: normalizeCanonicaRolePermissions(input.role.permissions as Record<CanonicaPermissionKey, boolean>),
        pId: PRODUCT_IDS.CANONICA,
        tId: access.scope.tenantId,
        sId: access.scope.storeId,
    };
...
export const deleteCanonicaRoleDefinition = async (request: NextRequest, session: any) => {
    if (!verifyStaffFeature()) return jsonError('Canonica staff access is not enabled.', 403, 'FEATURE_DISABLED');
    const rateLimit = await applyRateLimit(request, session, 'DATA_WRITE', 'canonica-role-delete');
    if (rateLimit) return rateLimit;
    ...
    const validation = validateAPIInput(DeleteCanonicaRoleSchema, {
        roleId: request.nextUrl.searchParams.get('roleId'),
    });
    if (!validation.success) {
        logSecurity('Input Validation Failed - Canonica Role Delete', session, request, { error: getValidationLogError(validation) }, 'medium');
        return jsonError('Invalid input', 400, 'INVALID_INPUT');
    }

    const { roleId } = validation.data as DeleteCanonicaRoleInput;
    if (roleId === DEFAULT_CANONICA_ROLE_IDS.OWNER) {
        return jsonError('Owner role is locked', 409, 'OWNER_ROLE_LOCKED');
    }
    ...
    if (!storeSnap.exists) return jsonError('Workspace not found', 404, 'STORE_NOT_FOUND');
    const storeData = storeSnap.data() || {};
    const current = normalizeCanonicaRolesForStore(storeData.canonicaRoles, access.scope.tenantId, access.scope.storeId, session?.user?.email);
