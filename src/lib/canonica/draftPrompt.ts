/**
 * Canonica — Draft Prompt Template & Response Parser
 * 
 ...
 * - Server-side functions-canonica draftGenerator.ts (nightly CF batch)
 * 
...
import { CanonicaProcedure, CanonicaProcedureStep, CanonicaProcedureWarning, CanonicaProcedurePrerequisite } from '@type/canonica';
...
export const DRAFT_SYSTEM_PROMPT = `You are Canonica's Knowledge Draft Generator. You create structured canonical answer skeletons from support signal evidence.
...
export function parseDraftResponse(rawResponse: string | null): ParsedDraftResponse | null {
    ...
    try {
        ...
        let procedure: CanonicaProcedure | null = null;
        if (parsed.procedure && parsed.procedure.steps && Array.isArray(parsed.procedure.steps)) {
            const steps: CanonicaProcedureStep[] = parsed.procedure.steps
            ...
            const warnings: CanonicaProcedureWarning[] = (parsed.procedure.warnings || [])
                .slice(0, 5)
                .map((w: any) => ({
            ...
            const prerequisites: CanonicaProcedurePrerequisite[] = (parsed.procedure.prerequisites || [])
                .slice(0, 5)
                .map((p: any) => ({
