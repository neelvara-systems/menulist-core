...
import type {
  ...
  MenuReadabilitySourceKind,
} from '@/lib/public-truth-tools/menuReadabilityTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const SOURCE_KIND_OPTIONS: MenuReadabilitySourceKind[] = [
  'menu',
  'service_list',
  'catalog',
  'rate_card',
...
const MENU_READABILITY_CONTACT_MESSAGE_MAX_LENGTH = 1900;

const FACT_FIELDS: Array<{
  key: 'categoriesClear' | 'pricesShown' | 'pricesNotNeeded' | 'descriptionsHelpful' | 'notesShown' | 'customerActionShown';
  icon: typeof LuCheck;
}> = [
  { key: 'categoriesClear', icon: LuFileText },
  { key: 'pricesShown', icon: LuTag },
  { key: 'pricesNotNeeded', icon: LuCircleDashed },
  { key: 'descriptionsHelpful', icon: LuInfo },
  { key: 'notesShown', icon: LuFileText },
  { key: 'customerActionShown', icon: LuMessageCircle },
];

const CHECK_ICONS: Record<MenuReadabilityCheckId, typeof LuCheck> = {
  source_material: LuFileText,
  categories_or_sections: LuFileText,
  items_or_services: LuFileText,
  prices_or_rates: LuTag,
  descriptions_or_details: LuInfo,
  customer_action: LuMessageCircle,
  current_customer_link: LuLink,
};

const RESULT_ICONS: Record<MenuReadabilityResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: MenuReadabilityInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  sourceKind: 'menu',
  sourceText: '',
  publicUrl: '',
  categoriesClear: false,
  pricesShown: false,
  pricesNotNeeded: false,
  descriptionsHelpful: false,
  notesShown: false,
...
type MenuReadabilityHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: MenuReadabilityResult) {
...
function buildReportEventContext(report: MenuReadabilityReport) {
  return {
    missing_count: report.summary.missing,
    menu_readability_status: report.status,
...
function getSafeReportFilename(report: MenuReadabilityReport): string {
  const baseName = report.businessName || 'menu-readability-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'menu-readability-check';
  return `${safeName}-menu-readability-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('menu_readability_check_download_unavailable');
  }
...
function MenuReadabilityReportCard({ report }: { report: MenuReadabilityReport }) {
  const t = useTranslations('Website.MenuReadabilityCheckPage');
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('menu_readability_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('menu_readability_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('menu_readability_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('menu_readability_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('menu_readability_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('menu_readability_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('menu_readability_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('menu_readability_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('menu_readability_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof MenuReadabilityHandoffForm>(key: K, value: MenuReadabilityHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/menu-readability-check' : window.location.pathname;
    const message = [
      'Menu Readability Check follow-up request',
      '',
      reportText.length > MENU_READABILITY_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, MENU_READABILITY_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'menu_readability_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('menu_readability_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('menu_readability_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('menu_readability_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <TurnstileWidget
          action="menulist_menu_readability_check"
...
export default function MenuReadabilityCheckPage() {
  const t = useTranslations('Website.MenuReadabilityCheckPage');
  const [form, setForm] = useState<MenuReadabilityInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildMenuReadabilityReport(form), [form]);
  ...
  function updateBooleanField(
    ...
    setForm((current) => {
      if (key === 'pricesNotNeeded' && checked) {
        return { ...current, pricesNotNeeded: true, pricesShown: false };
      }
      if (key === 'pricesShown' && checked) {
        return { ...current, pricesShown: true, pricesNotNeeded: false };
      }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('menu_readability_check_completed', {
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => setForm((current) => ({
                    ...current,
                    sourceKind: event.target.value as MenuReadabilitySourceKind,
                  }))}
                >
