...
import type {
  ...
  PriceAvailabilityMode,
  PriceAvailabilityPricingMode,
  PriceAvailabilitySourceKind,
} from '@/lib/public-truth-tools/priceAvailabilityGapTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const SOURCE_KIND_OPTIONS: PriceAvailabilitySourceKind[] = [
  'menu',
  'service_list',
  'catalog',
  'rate_card',
...
const PRICING_MODE_OPTIONS: PriceAvailabilityPricingMode[] = [
  'fixed_prices',
  'starting_prices',
  'quote_based',
  'not_needed',
...
const AVAILABILITY_MODE_OPTIONS: PriceAvailabilityMode[] = [
...
const PRICE_AVAILABILITY_CONTACT_MESSAGE_MAX_LENGTH = 1900;

type GapField =
  | 'currencyOrUnitShown'
  | 'variantOrPackagePricesShown'
  | 'quoteOrContactShown';
...
const GAP_FIELDS: Array<{
  ...
  { key: 'currencyOrUnitShown', icon: LuTag },
  { key: 'variantOrPackagePricesShown', icon: LuFileText },
  { key: 'quoteOrContactShown', icon: LuRefreshCw },
];

const CHECK_ICONS: Record<PriceAvailabilityGapCheckId, typeof LuCheck> = {
  source_material: LuFileText,
  price_clarity: LuTag,
  currency_or_unit_context: LuTag,
  variant_or_package_prices: LuFileText,
  availability_clarity: LuInfo,
  unavailable_items_marked: LuAlertTriangle,
  quote_or_contact_path: LuRefreshCw,
  current_customer_link: LuLink,
  external_price_availability_inspection: LuShieldCheck,
};

const RESULT_ICONS: Record<PriceAvailabilityGapResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: PriceAvailabilityGapInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  sourceKind: 'menu',
  ...
  currencyOrUnitShown: false,
  variantOrPackagePricesShown: false,
  quoteOrContactShown: false,
};
...
type PriceAvailabilityHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: PriceAvailabilityGapResult) {
...
function buildReportEventContext(report: PriceAvailabilityGapReport) {
  return {
    availability_mode: report.availabilityMode,
    missing_count: report.summary.missing,
    not_checked_count: report.summary.notChecked,
    price_availability_gap_status: report.status,
    pricing_mode: report.pricingMode,
    source_kind: report.sourceKind,
    unclear_count: report.summary.unclear,
  };
}

function getSafeReportFilename(report: PriceAvailabilityGapReport): string {
  const baseName = report.businessName || 'price-availability-gap-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'price-availability-gap-check';
  return `${safeName}-price-availability-gap-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('price_availability_gap_check_download_unavailable');
  }
...
function PriceAvailabilityGapReportCard({ report }: { report: PriceAvailabilityGapReport }) {
  const t = useTranslations('Website.PriceAvailabilityGapCheckPage');
  ...
  const shareableReportPayload = useMemo(() => buildShareablePublicTruthToolReportPayload({
    ...
    t,
    toolId: 'price-availability-gap-check',
  ...
  const reportText = useMemo(() => {
    const lines = [
      ...
      `${t('export.pricingMode')}: ${t(`pricingModes.${report.pricingMode}`)}`,
      `${t('export.availabilityMode')}: ${t(`availabilityModes.${report.availabilityMode}`)}`,
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('price_availability_gap_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('price_availability_gap_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('price_availability_gap_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('price_availability_gap_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('price_availability_gap_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('price_availability_gap_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('price_availability_gap_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('price_availability_gap_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('price_availability_gap_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof PriceAvailabilityHandoffForm>(key: K, value: PriceAvailabilityHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/price-availability-gap-check' : window.location.pathname;
    const message = [
      'Price & Availability Gap Check follow-up request',
      '',
      reportText.length > PRICE_AVAILABILITY_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, PRICE_AVAILABILITY_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'price_availability_gap_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('price_availability_gap_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('price_availability_gap_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('price_availability_gap_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <TurnstileWidget
          action="menulist_price_availability_gap_check"
...
export default function PriceAvailabilityGapCheckPage() {
  const t = useTranslations('Website.PriceAvailabilityGapCheckPage');
  const [form, setForm] = useState<PriceAvailabilityGapInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildPriceAvailabilityGapReport(form), [form]);

  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('price_availability_gap_check_completed', {
          ...
          <AnimateOnScroll preset="media" delay={0.08}>
            <form className="ws-public-truth-check-form" onSubmit={handleSubmit}>
              <div className="ws-public-truth-check-form__header">
                <LuTag size={22} aria-hidden="true" />
                <div>
                  <h2>{t('formTitle')}</h2>
                  <p>{t('formSubtitle')}</p>
                </div>
              ...
              <div className="ws-public-truth-check-form__grid">
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) => setForm((current) => ({
                      ...current,
                      sourceKind: event.target.value as PriceAvailabilitySourceKind,
                    }))}
                  >
                ...
                <label>
                  <span>{t('fields.pricingMode')}</span>
                  <select
                    value={form.pricingMode}
                    onChange={(event) => setForm((current) => ({
                      ...current,
                      pricingMode: event.target.value as PriceAvailabilityPricingMode,
                    }))}
                  >
                    {PRICING_MODE_OPTIONS.map((option) => (
                      <option key={option} value={option}>
                        {t(`pricingModes.${option}`)}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <div className="ws-public-truth-check-form__grid">
                <label>
                  <span>{t('fields.availabilityMode')}</span>
                  <select
                    value={form.availabilityMode}
                    onChange={(event) => setForm((current) => ({
                      ...current,
                      availabilityMode: event.target.value as PriceAvailabilityMode,
                    }))}
                  >
                    {AVAILABILITY_MODE_OPTIONS.map((option) => (
                      <option key={option} value={option}>
                        {t(`availabilityModes.${option}`)}
                      </option>
                    ))}
                  </select>
                </label>
