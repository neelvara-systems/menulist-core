...
import type {
  ...
  MenuPdfCleanupInput,
  MenuPdfCleanupLastUpdated,
  MenuPdfCleanupPdfLocation,
  MenuPdfCleanupReport,
  MenuPdfCleanupResult,
  MenuPdfCleanupSourceKind,
} from '@/lib/public-truth-tools/menuPdfCleanupTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const SOURCE_KIND_OPTIONS: MenuPdfCleanupSourceKind[] = [
  'menu_pdf',
  'service_pdf',
  'catalog_pdf',
  'rate_card_pdf',
...
const PDF_LOCATION_OPTIONS: MenuPdfCleanupPdfLocation[] = [
  'website',
  'google_profile',
  'whatsapp',
  'instagram',
...
const LAST_UPDATED_OPTIONS: MenuPdfCleanupLastUpdated[] = [
  'this_month',
  'last_3_months',
  'older_than_3_months',
  'unknown',
...
const MENU_PDF_CLEANUP_CONTACT_MESSAGE_MAX_LENGTH = 1900;
...
type CleanupField =
  ...
  | 'actionClear'
  | 'oldVersionsRemoved'
  | 'qrOrPrintStillUsesPdf';

const CLEANUP_FIELDS: Array<{
  key: CleanupField;
  icon: typeof LuCheck;
}> = [
  { key: 'mobileReadable', icon: LuInfo },
  { key: 'copyableText', icon: LuFileText },
  { key: 'pricesClear', icon: LuCheck },
  { key: 'actionClear', icon: LuLink },
  { key: 'oldVersionsRemoved', icon: LuRefreshCw },
  { key: 'qrOrPrintStillUsesPdf', icon: LuAlertTriangle },
];

const CHECK_ICONS: Record<MenuPdfCleanupCheckId, typeof LuCheck> = {
  pdf_source_present: LuFileText,
  pdf_source_current: LuRefreshCw,
  mobile_readability: LuInfo,
  text_copyability: LuFileText,
  price_and_item_clarity: LuCheck,
  action_path: LuLink,
  qr_or_print_dependency: LuAlertTriangle,
  current_customer_link: LuLink,
  external_pdf_inspection: LuShieldCheck,
};

const RESULT_ICONS: Record<MenuPdfCleanupResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: MenuPdfCleanupInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  sourceKind: 'menu_pdf',
  ...
  pricesClear: false,
  actionClear: false,
  oldVersionsRemoved: false,
  qrOrPrintStillUsesPdf: false,
};
...
type MenuPdfCleanupHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: MenuPdfCleanupResult) {
...
function buildReportEventContext(report: MenuPdfCleanupReport) {
  return {
    last_updated: report.lastUpdated,
    menu_pdf_cleanup_status: report.status,
    missing_count: report.summary.missing,
    not_checked_count: report.summary.notChecked,
    pdf_location: report.pdfLocation,
    source_kind: report.sourceKind,
    unclear_count: report.summary.unclear,
  };
}

function getSafeReportFilename(report: MenuPdfCleanupReport): string {
  const baseName = report.businessName || 'menu-pdf-cleanup-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'menu-pdf-cleanup-check';
  return `${safeName}-menu-pdf-cleanup-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('menu_pdf_cleanup_check_download_unavailable');
  }
...
function MenuPdfCleanupReportCard({ report }: { report: MenuPdfCleanupReport }) {
  const t = useTranslations('Website.MenuPdfCleanupCheckPage');
  ...
  const reportText = useMemo(() => {
    const lines = [
      ...
      `${t('export.pdfLocation')}: ${t(`pdfLocations.${report.pdfLocation}`)}`,
      `${t('export.lastUpdated')}: ${t(`lastUpdated.${report.lastUpdated}`)}`,
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('menu_pdf_cleanup_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('menu_pdf_cleanup_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('menu_pdf_cleanup_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('menu_pdf_cleanup_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('menu_pdf_cleanup_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('menu_pdf_cleanup_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('menu_pdf_cleanup_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('menu_pdf_cleanup_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('menu_pdf_cleanup_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof MenuPdfCleanupHandoffForm>(key: K, value: MenuPdfCleanupHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/menu-pdf-cleanup-check' : window.location.pathname;
    const message = [
      'Menu PDF Cleanup Check follow-up request',
      '',
      reportText.length > MENU_PDF_CLEANUP_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, MENU_PDF_CLEANUP_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'menu_pdf_cleanup_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('menu_pdf_cleanup_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('menu_pdf_cleanup_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('menu_pdf_cleanup_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <TurnstileWidget
          action="menulist_menu_pdf_cleanup_check"
...
export default function MenuPdfCleanupCheckPage() {
  const t = useTranslations('Website.MenuPdfCleanupCheckPage');
  const [form, setForm] = useState<MenuPdfCleanupInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildMenuPdfCleanupReport(form), [form]);

  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('menu_pdf_cleanup_check_completed', {
      has_current_customer_link: Boolean(form.currentCustomerLink.trim()),
      has_pdf_reference: Boolean(form.pdfReference.trim()),
      last_updated: form.lastUpdated,
      menu_pdf_cleanup_status: report.status,
      pdf_location: form.pdfLocation,
            ...
            <form className="ws-public-truth-check-form" onSubmit={handleSubmit}>
              ...
              <div className="ws-public-truth-check-form__grid">
                <label>
                  ...
                  <select value={form.sourceKind} onChange={(event) => setForm((current) => ({ ...current, sourceKind: event.target.value as MenuPdfCleanupSourceKind }))}>
                    {SOURCE_KIND_OPTIONS.map((option) => (
                      <option key={option} value={option}>{t(`sourceKinds.${option}`)}</option>
                    ))}
                  </select>
                </label>
                <label>
                  <span>{t('fields.pdfLocation')}</span>
                  <select value={form.pdfLocation} onChange={(event) => setForm((current) => ({ ...current, pdfLocation: event.target.value as MenuPdfCleanupPdfLocation }))}>
                    {PDF_LOCATION_OPTIONS.map((option) => (
                      <option key={option} value={option}>{t(`pdfLocations.${option}`)}</option>
                    ))}
                  </select>
                </label>
              </div>
              ...
              <div className="ws-public-truth-check-form__grid">
                <label>
                  <span>{t('fields.lastUpdated')}</span>
                  <select value={form.lastUpdated} onChange={(event) => setForm((current) => ({ ...current, lastUpdated: event.target.value as MenuPdfCleanupLastUpdated }))}>
                    {LAST_UPDATED_OPTIONS.map((option) => (
                      <option key={option} value={option}>{t(`lastUpdated.${option}`)}</option>
                    ))}
                  </select>
                </label>
                <label>
              ...
              <fieldset className="ws-public-truth-check-facts">
                ...
                <div>
                  {CLEANUP_FIELDS.map(({ key, icon: Icon }) => (
                    <label key={key}>
                      <input
                        type="checkbox"
                        checked={form[key]}
      ...
      <section className="ws-section ws-public-truth-check-results">
        <div className="ws-container ws-public-truth-check-results__inner">
          <AnimateOnScroll preset="card" className="ws-public-truth-check-results__copy">
            ...
            <WebsiteLink href="/features/menu-import" className="ws-public-truth-check-results__link">
              {t('learnMore')}
