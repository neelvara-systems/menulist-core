...
import type {
  ...
  PhotoGapCheckReport,
  PhotoGapCheckResult,
} from '@/lib/public-truth-tools/photoGapCheckTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const BUSINESS_TYPE_OPTIONS: PhotoGapBusinessType[] = [
  'restaurant',
  'salon',
  'clinic',
  'shop',
...
const PHOTO_GAP_CONTACT_MESSAGE_MAX_LENGTH = 1900;

const FACT_FIELDS: Array<{
  key:
    | 'logoPresent'
    | 'coverImagePresent'
    | 'locationOrTeamPhotoPresent'
    | 'productOrServicePhotosPresent'
    | 'photosLookCurrent'
    | 'publicPageHasImages';
  icon: typeof LuCheck;
}> = [
  { key: 'logoPresent', icon: LuBadgeCheck },
  { key: 'coverImagePresent', icon: LuImage },
  { key: 'locationOrTeamPhotoPresent', icon: LuStore },
  { key: 'productOrServicePhotosPresent', icon: LuPackage },
  { key: 'photosLookCurrent', icon: LuCamera },
  { key: 'publicPageHasImages', icon: LuUserCheck },
];

const CHECK_ICONS: Record<PhotoGapCheckId, typeof LuCheck> = {
  logo: LuBadgeCheck,
  cover_image: LuImage,
  location_or_team_photo: LuStore,
  product_or_service_photos: LuPackage,
  photo_context: LuCamera,
  public_page_images: LuUserCheck,
  current_customer_link: LuLink,
  external_photo_verification: LuShieldCheck,
};

const RESULT_ICONS: Record<PhotoGapCheckResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: PhotoGapCheckInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  ...
  locationOrTeamPhotoPresent: false,
  productOrServicePhotosPresent: false,
  photosLookCurrent: false,
  publicPageHasImages: false,
};
...
type PhotoGapHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: PhotoGapCheckResult) {
...
function buildReportEventContext(report: PhotoGapCheckReport) {
  return {
    missing_count: report.summary.missing,
    not_checked_count: report.summary.notChecked,
    photo_gap_status: report.status,
    unclear_count: report.summary.unclear,
  };
}

function getSafeReportFilename(report: PhotoGapCheckReport): string {
  const baseName = report.businessName || 'photo-gap-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'photo-gap-check';
  return `${safeName}-photo-gap-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('photo_gap_check_download_unavailable');
  }
...
function PhotoGapReportCard({ report }: { report: PhotoGapCheckReport }) {
  const t = useTranslations('Website.PhotoGapCheckPage');
  ...
  const reportText = useMemo(() => {
    const lines = [
      ...
      `${t('export.businessType')}: ${t(`businessTypes.${report.businessType}`)}`,
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('photo_gap_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('photo_gap_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('photo_gap_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('photo_gap_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('photo_gap_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('photo_gap_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('photo_gap_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('photo_gap_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('photo_gap_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof PhotoGapHandoffForm>(key: K, value: PhotoGapHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/photo-gap-check' : window.location.pathname;
    const message = [
      'Photo Gap Check follow-up request',
      '',
      reportText.length > PHOTO_GAP_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, PHOTO_GAP_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'photo_gap_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('photo_gap_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('photo_gap_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('photo_gap_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <TurnstileWidget
          action="menulist_photo_gap_check"
...
export default function PhotoGapCheckPage() {
  const t = useTranslations('Website.PhotoGapCheckPage');
  const [form, setForm] = useState<PhotoGapCheckInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildPhotoGapCheckReport(form), [form]);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('photo_gap_check_completed', {
      business_type: form.businessType,
      has_cover_image: form.coverImagePresent,
      has_current_customer_link: Boolean(form.currentCustomerLink.trim()),
      has_logo: form.logoPresent,
      has_product_or_service_photos: form.productOrServicePhotosPresent,
      photo_gap_status: report.status,
    });
  }
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => setForm((current) => ({
                    ...current,
                    businessType: event.target.value as PhotoGapBusinessType,
                  }))}
                >
                  {BUSINESS_TYPE_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {t(`businessTypes.${option}`)}
                    </option>
                  ))}
                </select>
              </label>
