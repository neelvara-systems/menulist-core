...
import type {
  ...
  CustomerQuestionCoverageSourceKind,
} from '@/lib/public-truth-tools/customerQuestionCoverageTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const SOURCE_KIND_OPTIONS: CustomerQuestionCoverageSourceKind[] = [
  'menu',
  'service_list',
  'catalog',
  'rate_card',
...
const CUSTOMER_QUESTION_COVERAGE_CONTACT_MESSAGE_MAX_LENGTH = 1900;

type AnswerCoverageField =
  | 'menuOrServicesAnswered'
  | 'hoursAnswered'
  | 'pricesAnswered'
  | 'pricesNotNeeded'
  | 'locationOrContactAnswered'
  | 'actionAnswered'
  | 'notesOrAvailabilityAnswered';

const ANSWER_FIELDS: Array<{
  key: AnswerCoverageField;
  icon: typeof LuCheck;
}> = [
  { key: 'menuOrServicesAnswered', icon: LuFileText },
  { key: 'hoursAnswered', icon: LuInfo },
  { key: 'pricesAnswered', icon: LuTag },
  { key: 'pricesNotNeeded', icon: LuCircleDashed },
  { key: 'locationOrContactAnswered', icon: LuMapPin },
  { key: 'actionAnswered', icon: LuMessageCircle },
  { key: 'notesOrAvailabilityAnswered', icon: LuFileQuestion },
];

const CHECK_ICONS: Record<CustomerQuestionCoverageCheckId, typeof LuCheck> = {
  customer_questions: LuFileQuestion,
  source_material: LuFileText,
  menu_or_services_answers: LuFileText,
  hours_answers: LuInfo,
  prices_answers: LuTag,
  location_contact_answers: LuMapPin,
  action_answers: LuMessageCircle,
  notes_availability_answers: LuFileQuestion,
  current_customer_link: LuLink,
};

const RESULT_ICONS: Record<CustomerQuestionCoverageResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: CustomerQuestionCoverageInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  sourceKind: 'menu',
  ...
  commonQuestions: '',
  publicUrl: '',
  menuOrServicesAnswered: false,
  hoursAnswered: false,
  pricesAnswered: false,
  pricesNotNeeded: false,
  locationOrContactAnswered: false,
  actionAnswered: false,
  notesOrAvailabilityAnswered: false,
};
...
type CustomerQuestionCoverageHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: CustomerQuestionCoverageResult) {
...
function buildReportEventContext(report: CustomerQuestionCoverageReport) {
  return {
    customer_question_coverage_status: report.status,
...
function getSafeReportFilename(report: CustomerQuestionCoverageReport): string {
  const baseName = report.businessName || 'customer-question-coverage-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'customer-question-coverage-check';
  return `${safeName}-customer-question-coverage-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('customer_question_coverage_check_download_unavailable');
  }
...
function CustomerQuestionCoverageReportCard({ report }: { report: CustomerQuestionCoverageReport }) {
  const t = useTranslations('Website.CustomerQuestionCoverageCheckPage');
  ...
  const shareableReportPayload = useMemo(() => buildShareablePublicTruthToolReportPayload({
    ...
    t,
    toolId: 'customer-question-coverage-check',
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('customer_question_coverage_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('customer_question_coverage_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('customer_question_coverage_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('customer_question_coverage_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('customer_question_coverage_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('customer_question_coverage_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('customer_question_coverage_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('customer_question_coverage_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('customer_question_coverage_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof CustomerQuestionCoverageHandoffForm>(key: K, value: CustomerQuestionCoverageHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/customer-question-coverage-check' : window.location.pathname;
    const message = [
      'Customer Question Coverage Check follow-up request',
      '',
      reportText.length > CUSTOMER_QUESTION_COVERAGE_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, CUSTOMER_QUESTION_COVERAGE_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'customer_question_coverage_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('customer_question_coverage_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('customer_question_coverage_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('customer_question_coverage_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <TurnstileWidget
          action="menulist_customer_question_coverage_check"
...
export default function CustomerQuestionCoverageCheckPage() {
  const t = useTranslations('Website.CustomerQuestionCoverageCheckPage');
  const [form, setForm] = useState<CustomerQuestionCoverageInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildCustomerQuestionCoverageReport(form), [form]);

  function updateBooleanField(key: AnswerCoverageField, checked: boolean) {
    setForm((current) => {
      if (key === 'pricesNotNeeded' && checked) {
        return { ...current, pricesNotNeeded: true, pricesAnswered: false };
      }
      if (key === 'pricesAnswered' && checked) {
        return { ...current, pricesAnswered: true, pricesNotNeeded: false };
      }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('customer_question_coverage_check_completed', {
      customer_question_coverage_status: report.status,
      has_common_questions: Boolean(form.commonQuestions.trim()),
      has_public_url: Boolean(form.publicUrl.trim()),
      has_source_text: Boolean(form.sourceText.trim()),
      source_kind: report.sourceKind,
    });
  }
          ...
          <AnimateOnScroll preset="media" delay={0.08}>
            <form className="ws-public-truth-check-form" onSubmit={handleSubmit}>
              <div className="ws-public-truth-check-form__header">
                <LuFileQuestion size={22} aria-hidden="true" />
                <div>
                  <h2>{t('formTitle')}</h2>
                  <p>{t('formSubtitle')}</p>
                </div>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(event) => setForm((current) => ({
                    ...current,
                    sourceKind: event.target.value as CustomerQuestionCoverageSourceKind,
                  }))}
                >
              ...
              <fieldset className="ws-public-truth-check-facts">
                <legend>{t('answersLegend')}</legend>
                <div>
                  {ANSWER_FIELDS.map(({ key, icon: Icon }) => (
                    <label key={key}>
                      <input
                        type="checkbox"
                        checked={form[key]}
      ...
      <section className="ws-section ws-public-truth-check-results">
        <div className="ws-container ws-public-truth-check-results__inner">
          <AnimateOnScroll preset="card" className="ws-public-truth-check-results__copy">
            ...
            <WebsiteLink href="/features/business-health" className="ws-public-truth-check-results__link">
              {t('learnMore')}
