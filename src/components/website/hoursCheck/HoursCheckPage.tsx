...
import type {
  ...
  LateNightClarity,
  SpecialHoursStatus,
} from '@/lib/public-truth-tools/hoursCheckTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const LATE_NIGHT_OPTIONS: LateNightClarity[] = [
  'not_applicable',
  'same_day_clear',
  'past_midnight_clear',
  'unclear',
...
const SPECIAL_HOURS_OPTIONS: SpecialHoursStatus[] = [
  'missing',
  'listed',
  'not_applicable',
];

const HOURS_CHECK_CONTACT_MESSAGE_MAX_LENGTH = 1900;

const CHECK_ICONS: Record<HoursCheckId, typeof LuCheck> = {
  regular_hours: LuClock,
  closed_days: LuCalendarClock,
  late_night_hours: LuTimerReset,
  holiday_hours: LuCalendarClock,
  timezone_context: LuMapPin,
  contact_fallback: LuRefreshCw,
  current_customer_link: LuLink,
  external_hours_verification: LuShieldCheck,
};

const RESULT_ICONS: Record<HoursCheckResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: HoursCheckInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  timeZone: '',
  ...
  specialHoursText: '',
  currentCustomerLink: '',
  lateNightClarity: 'not_applicable',
  specialHoursStatus: 'missing',
...
type HoursCheckHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: HoursCheckResult) {
...
function buildReportEventContext(report: HoursCheckReport) {
  return {
    hours_check_status: report.status,
...
function getSafeReportFilename(report: HoursCheckReport): string {
  const baseName = report.businessName || 'hours-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'hours-check';
  return `${safeName}-hours-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('hours_check_download_unavailable');
  }
...
function HoursCheckReportCard({ report }: { report: HoursCheckReport }) {
  const t = useTranslations('Website.HoursCheckPage');
  ...
  const reportText = useMemo(() => {
    const lines = [
      ...
      `${t('export.timeZone')}: ${report.timeZone || t('export.notProvided')}`,
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('hours_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('hours_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('hours_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('hours_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('hours_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('hours_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('hours_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('hours_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('hours_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof HoursCheckHandoffForm>(key: K, value: HoursCheckHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/hours-check' : window.location.pathname;
    const message = [
      'Hours Check follow-up request',
      '',
      reportText.length > HOURS_CHECK_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, HOURS_CHECK_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'hours_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('hours_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('hours_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('hours_check_create_link_clicked', eventContext)}
        >
...
export default function HoursCheckPage() {
  const t = useTranslations('Website.HoursCheckPage');
  const [form, setForm] = useState<HoursCheckInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildHoursCheckReport(form), [form]);

  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('hours_check_completed', {
      has_closed_days_text: Boolean(form.closedDaysText.trim()),
      has_current_customer_link: Boolean(form.currentCustomerLink.trim()),
      has_regular_hours_text: Boolean(form.regularHoursText.trim()),
      has_special_hours_text: Boolean(form.specialHoursText.trim()),
      hours_check_status: report.status,
      late_night_clarity: form.lateNightClarity,
      special_hours_status: form.specialHoursStatus,
    });
  }
          ...
          <AnimateOnScroll preset="media" delay={0.08}>
            <form className="ws-public-truth-check-form" onSubmit={handleSubmit}>
              <div className="ws-public-truth-check-form__header">
                <LuClock size={22} aria-hidden="true" />
                <div>
                  <h2>{t('formTitle')}</h2>
                  <p>{t('formSubtitle')}</p>
                </div>
              ...
              <div className="ws-public-truth-check-form__grid">
                <label>
                  <span>{t('fields.lateNightClarity')}</span>
                  <select
                    value={form.lateNightClarity}
                    onChange={(event) => setForm((current) => ({
                      ...current,
                      lateNightClarity: event.target.value as LateNightClarity,
                    }))}
                  >
                    {LATE_NIGHT_OPTIONS.map((option) => (
                      <option key={option} value={option}>
                        {t(`lateNightOptions.${option}`)}
                      </option>
                    ))}
                  </select>
                </label>
                <label>
                  <span>{t('fields.specialHoursStatus')}</span>
                  <select
                    value={form.specialHoursStatus}
                    onChange={(event) => setForm((current) => ({
                      ...current,
                      specialHoursStatus: event.target.value as SpecialHoursStatus,
                    }))}
                  >
                    {SPECIAL_HOURS_OPTIONS.map((option) => (
                      <option key={option} value={option}>
                        {t(`specialHoursOptions.${option}`)}
                      </option>
                    ))}
                  </select>
                </label>
              ...
              <fieldset className="ws-public-truth-check-facts">
                ...
                <div>
                  <label>
                    <input
                      type="checkbox"
                      checked={form.contactFallbackShown}
                      onChange={(event) => setForm((current) => ({ ...current, contactFallbackShown: event.target.checked }))}
                    />
                    <span>
                      <LuRefreshCw size={16} aria-hidden="true" />
                      {t('facts.contactFallbackShown')}
                    </span>
                  </label>
                </div>
              </fieldset>
