...
import { buildWhatsAppActionLinkReport } from '@/lib/public-truth-tools/whatsappActionLinkReport';
...
import type {
  ...
  WhatsAppActionMessageIntent,
} from '@/lib/public-truth-tools/whatsappActionLinkTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const MESSAGE_INTENT_OPTIONS: WhatsAppActionMessageIntent[] = [
  'ask_question',
  'order',
  'book',
  'quote',
...
const WHATSAPP_ACTION_LINK_CONTACT_MESSAGE_MAX_LENGTH = 1900;

const FACT_FIELDS: Array<{
  key: 'menuOrServiceLinkAttached' | 'hoursExpectationSet' | 'fallbackActionShown';
  icon: typeof LuCheck;
}> = [
  { key: 'menuOrServiceLinkAttached', icon: LuLink },
  { key: 'hoursExpectationSet', icon: LuClock },
  { key: 'fallbackActionShown', icon: LuMousePointerClick },
];

const CHECK_ICONS: Record<WhatsAppActionLinkCheckId, typeof LuCheck> = {
  whatsapp_number: LuPhone,
  click_to_chat_format: LuMessageCircle,
  message_intent: LuMousePointerClick,
  suggested_message: LuSend,
  menu_or_service_link: LuLink,
  hours_expectation: LuClock,
  fallback_action: LuRefreshCw,
  message_delivery: LuShieldCheck,
};

const RESULT_ICONS: Record<WhatsAppActionLinkResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: WhatsAppActionLinkInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  whatsappNumber: '',
...
type WhatsAppActionLinkHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: WhatsAppActionLinkResult) {
...
function buildReportEventContext(report: WhatsAppActionLinkReport) {
  return {
    missing_count: report.summary.missing,
    not_checked_count: report.summary.notChecked,
    unclear_count: report.summary.unclear,
    whatsapp_action_link_status: report.status,
    message_intent: report.messageIntent,
    has_preview_link: Boolean(report.previewLink),
  };
}

function getSafeReportFilename(report: WhatsAppActionLinkReport): string {
  const baseName = report.businessName || 'whatsapp-action-link-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'whatsapp-action-link-check';
  return `${safeName}-whatsapp-action-link-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('whatsapp_action_link_check_download_unavailable');
  }
...
function WhatsAppActionLinkReportCard({ report }: { report: WhatsAppActionLinkReport }) {
  const t = useTranslations('Website.WhatsAppActionLinkCheckPage');
  ...
  const [handoff, setHandoff] = useState<WhatsAppActionLinkHandoffForm>(INITIAL_HANDOFF_FORM);
  ...
  const shareableReportPayload = useMemo(() => buildShareablePublicTruthToolReportPayload({
    ...
    t,
    toolId: 'whatsapp-action-link-check',
  ...
  const reportText = useMemo(() => {
    const lines = [
      ...
      `${t('export.messageIntent')}: ${t(`messageIntents.${report.messageIntent}`)}`,
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('whatsapp_action_link_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('whatsapp_action_link_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('whatsapp_action_link_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('whatsapp_action_link_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('whatsapp_action_link_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('whatsapp_action_link_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('whatsapp_action_link_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('whatsapp_action_link_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('whatsapp_action_link_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof WhatsAppActionLinkHandoffForm>(key: K, value: WhatsAppActionLinkHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/whatsapp-action-link-check' : window.location.pathname;
    const message = [
      'WhatsApp Action Link Check follow-up request',
      '',
      reportText.length > WHATSAPP_ACTION_LINK_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, WHATSAPP_ACTION_LINK_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    trackWebsiteMarketingEvent('whatsapp_action_link_check_handoff_submitted', eventContext);
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'whatsapp_action_link_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('whatsapp_action_link_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('whatsapp_action_link_check_contact_failed');
      }

      setHandoff(INITIAL_HANDOFF_FORM);
      setHandoffStatus('submitted');
      trackWebsiteMarketingEvent('whatsapp_action_link_check_handoff_accepted', eventContext);
    } catch (error) {
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('whatsapp_action_link_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <div style={{ display: 'none' }} aria-hidden>
          <label htmlFor="whatsapp-action-link-check-website">{t('handoff.website')}</label>
          <input
            id="whatsapp-action-link-check-website"
            maxLength={500}
        ...
        <TurnstileWidget
          action="menulist_whatsapp_action_link_check"
...
export default function WhatsAppActionLinkCheckPage() {
  const t = useTranslations('Website.WhatsAppActionLinkCheckPage');
  const [form, setForm] = useState<WhatsAppActionLinkInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildWhatsAppActionLinkReport(form), [form]);
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('whatsapp_action_link_check_completed', {
      has_current_customer_link: Boolean(form.currentCustomerLink.trim()),
      has_existing_whatsapp_link: Boolean(form.existingWhatsappLink.trim()),
      has_suggested_message: Boolean(form.suggestedMessage.trim()),
      has_whatsapp_number: Boolean(form.whatsappNumber.trim()),
      message_intent: form.messageIntent,
      whatsapp_action_link_status: report.status,
    });
  }
          ...
          <AnimateOnScroll preset="media" delay={0.08}>
            <form className="ws-public-truth-check-form" onSubmit={handleSubmit}>
              ...
              <div className="ws-public-truth-check-form__grid">
                ...
                <label>
                  <span>{t('fields.existingWhatsappLink')}</span>
                  <input
                    maxLength={PUBLIC_TRUTH_TOOL_INPUT_LIMITS.url}
                    value={form.existingWhatsappLink}
                    onChange={(event) => setForm((current) => ({ ...current, existingWhatsappLink: event.target.value }))}
                    autoComplete="url"
                    inputMode="url"
                  />
                </label>
              ...
              <label>
                <span>{t('fields.messageIntent')}</span>
                <select
                  value={form.messageIntent}
                  onChange={(event) => setForm((current) => ({
                    ...current,
                    messageIntent: event.target.value as WhatsAppActionMessageIntent,
                  }))}
                >
                  {MESSAGE_INTENT_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {t(`messageIntents.${option}`)}
                    </option>
                  ))}
                </select>
              </label>
              ...
              <label>
                <span>{t('fields.suggestedMessage')}</span>
                <textarea
                  maxLength={PUBLIC_TRUTH_TOOL_INPUT_LIMITS.longText}
                  value={form.suggestedMessage}
                  onChange={(event) => setForm((current) => ({ ...current, suggestedMessage: event.target.value }))}
                  rows={5}
                />
              </label>
      ...
      <section className="ws-section ws-public-truth-check-results">
        <div className="ws-container ws-public-truth-check-results__inner">
          ...
          <AnimateOnScroll preset="card">
            {hasChecked ? <WhatsAppActionLinkReportCard key={report.generatedAt} report={report} /> : <EmptyReport />}
          </AnimateOnScroll>
        </div>
      </section>
    </main>
