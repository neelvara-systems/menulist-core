...
import type {
  ...
  QrLinkHealthReport,
  QrLinkHealthResult,
} from '@/lib/public-truth-tools/qrLinkHealthTypes';
import AnimateOnScroll, { AnimateStaggerChild } from '../shared/AnimateOnScroll';
import WebsiteButton from '../shared/WebsiteButton';
import WebsiteHeadline from '../shared/WebsiteHeadline';
import WebsiteLink from '../shared/WebsiteLink';

const DESTINATION_OPTIONS: QrLinkExpectedDestination[] = [
...
const QR_LINK_HEALTH_CONTACT_MESSAGE_MAX_LENGTH = 1900;

const FACT_FIELDS: Array<{
  key: 'targetLooksCurrent' | 'customerActionVisible' | 'printedContextClear' | 'replacementNeeded';
  icon: typeof LuCheck;
}> = [
  { key: 'targetLooksCurrent', icon: LuRefreshCw },
  { key: 'customerActionVisible', icon: LuMousePointerClick },
  { key: 'printedContextClear', icon: LuMapPin },
  { key: 'replacementNeeded', icon: LuAlertTriangle },
];

const CHECK_ICONS: Record<QrLinkHealthCheckId, typeof LuCheck> = {
  qr_target: LuQrCode,
  url_format: LuLink,
  menulist_customer_link: LuBadgeCheck,
  current_link: LuRefreshCw,
  customer_action: LuMousePointerClick,
  printed_context: LuMapPin,
  target_page_inspection: LuScanLine,
};

const RESULT_ICONS: Record<QrLinkHealthResult, typeof LuCheck> = {
  present: LuCheck,
...
const INITIAL_FORM: QrLinkHealthInput = {
  mode: 'self_report',
  businessName: '',
  cityOrArea: '',
  qrTargetUrl: '',
  expectedDestination: 'menulist_customer_link',
  targetLooksCurrent: false,
  customerActionVisible: false,
  printedContextClear: false,
...
type QrLinkHealthHandoffForm = typeof INITIAL_HANDOFF_FORM;
...
function getResultTone(result: QrLinkHealthResult) {
...
function buildReportEventContext(report: QrLinkHealthReport) {
  return {
    missing_count: report.summary.missing,
    not_checked_count: report.summary.notChecked,
    qr_link_health_status: report.status,
    expected_destination: report.expectedDestination,
    unclear_count: report.summary.unclear,
  };
}

function getSafeReportFilename(report: QrLinkHealthReport): string {
  const baseName = report.businessName || 'qr-link-health-check';
  const safeName = baseName
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'qr-link-health-check';
  return `${safeName}-qr-link-health-check.txt`;
}

function downloadTextFile(filename: string, value: string): void {
  if (typeof window === 'undefined' || typeof document === 'undefined') {
    throw new Error('qr_link_health_check_download_unavailable');
  }
...
function QrLinkHealthReportCard({ report }: { report: QrLinkHealthReport }) {
  const t = useTranslations('Website.QrLinkHealthCheckPage');
  ...
  const reportText = useMemo(() => {
    const lines = [
      ...
      `${t('export.expectedDestination')}: ${t(`destinationKinds.${report.expectedDestination}`)}`,
  ...
  async function handleCopyReport() {
    trackWebsiteMarketingEvent('qr_link_health_check_report_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(reportText);
      setReportActionStatus('copied');
      trackWebsiteMarketingEvent('qr_link_health_check_report_copied', eventContext);
    } catch (error) {
      setReportActionStatus('copy_failed');
      logRuntimeFailure('qr_link_health_check_report_copy_failed', error, {
        ...eventContext,
  ...
  async function handleCopyShareLink() {
    trackWebsiteMarketingEvent('qr_link_health_check_share_link_copy_clicked', eventContext);

    try {
      await copyRuntimeTextToClipboard(shareableReportUrl);
      setReportActionStatus('share_copied');
      trackWebsiteMarketingEvent('qr_link_health_check_share_link_copied', eventContext);
    } catch (error) {
      setReportActionStatus('share_copy_failed');
      logRuntimeFailure('qr_link_health_check_share_link_copy_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function handleDownloadReport() {
    trackWebsiteMarketingEvent('qr_link_health_check_report_download_clicked', eventContext);

    try {
      downloadTextFile(getSafeReportFilename(report), reportText);
      setReportActionStatus('downloaded');
      trackWebsiteMarketingEvent('qr_link_health_check_report_downloaded', eventContext);
    } catch (error) {
      setReportActionStatus('download_failed');
      logRuntimeFailure('qr_link_health_check_report_download_failed', error, {
        ...eventContext,
        ...getBoundedRuntimeStringContext('businessName', report.businessName),
      });
    }
  ...
  function updateHandoff<K extends keyof QrLinkHealthHandoffForm>(key: K, value: QrLinkHealthHandoffForm[K]) {
  ...
  async function handleHandoffSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const sourcePath = typeof window === 'undefined' ? '/tools/qr-link-health-check' : window.location.pathname;
    const message = [
      'QR Link Health Check follow-up request',
      '',
      reportText.length > QR_LINK_HEALTH_CONTACT_MESSAGE_MAX_LENGTH
        ? `${reportText.slice(0, QR_LINK_HEALTH_CONTACT_MESSAGE_MAX_LENGTH)}\n[Report trimmed for contact message]`
        : reportText,
    ].join('\n');
    ...
    try {
      ...
      const result = await readMenulistPublicContactResponseJson(
        response,
        'qr_link_health_check_contact_response_parse_failed',
        responseLogContext,
      );
      resetCaptcha();
      ...
      if (!response.ok || !isAcceptedMenulistPublicContactResponse(result, 'general')) {
        if (response.ok) {
          logInvalidMenulistPublicContactResponse('qr_link_health_check_contact_response_invalid', result, 'general', {
            ...responseLogContext,
            responseStatus: response.status,
          });
        }
        throw new Error('qr_link_health_check_contact_failed');
      }
    ...
    <div className={`ws-public-truth-check-report-card ws-public-truth-check-report-card--${report.status}`} aria-live="polite">
      ...
      <div className="ws-public-truth-check-next">
        ...
        <WebsiteButton
          ...
          onClick={() => trackWebsiteMarketingEvent('qr_link_health_check_create_link_clicked', eventContext)}
        >
      ...
      <form className="ws-public-truth-check-handoff" onSubmit={handleHandoffSubmit}>
        ...
        <TurnstileWidget
          action="menulist_qr_link_health_check"
...
export default function QrLinkHealthCheckPage() {
  const t = useTranslations('Website.QrLinkHealthCheckPage');
  const [form, setForm] = useState<QrLinkHealthInput>(INITIAL_FORM);
  const [hasChecked, setHasChecked] = useState(false);
  const report = useMemo(() => buildQrLinkHealthReport(form), [form]);
  ...
  function updateBooleanField(
    ...
    setForm((current) => {
      if (key === 'replacementNeeded' && checked) {
        return { ...current, replacementNeeded: true, targetLooksCurrent: false };
      }
      if (key === 'targetLooksCurrent' && checked) {
        return { ...current, targetLooksCurrent: true, replacementNeeded: false };
      }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setHasChecked(true);
    trackWebsiteMarketingEvent('qr_link_health_check_completed', {
      expected_destination: form.expectedDestination,
      has_qr_target_url: Boolean(form.qrTargetUrl.trim()),
      qr_link_health_status: report.status,
      replacement_needed: form.replacementNeeded,
    });
  }
          ...
          <AnimateOnScroll preset="media" delay={0.08}>
            <form className="ws-public-truth-check-form" onSubmit={handleSubmit}>
              <div className="ws-public-truth-check-form__header">
                <LuQrCode size={22} aria-hidden="true" />
                <div>
                  <h2>{t('formTitle')}</h2>
                  <p>{t('formSubtitle')}</p>
                </div>
              ...
              <label>
                <span>{t('fields.expectedDestination')}</span>
                <select
                  value={form.expectedDestination}
                  onChange={(event) => setForm((current) => ({
                    ...current,
                    expectedDestination: event.target.value as QrLinkExpectedDestination,
                  }))}
                >
                  {DESTINATION_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {t(`destinationKinds.${option}`)}
                    </option>
                  ))}
                </select>
              </label>
