...
export default function AnswerlatticeFeedbackReview() {
    const session = useClientAuthSession();
    const tId = Number(session?.tId || 0);
    const sId = Number(session?.sId || 0);

    if (!FEATURE_FLAGS.ENABLE_ANSWERLATTICE_FEEDBACK_REVIEW) {
        return (
            <Alert
                showIcon
                type="info"
                message="Feedback review is disabled"
                description="Enable ENABLE_ANSWERLATTICE_FEEDBACK_REVIEW to review Help Center feedback inside Answerlattice."
            />
        );
    }
    ...
    if (!tId || !sId) {
        return (
            <Alert
                ...
                description="Open Answerlattice from a workspace account before reviewing feedback."
            />
        );
    }
