...
const DEFAULT_FAQ_VALUES = {
    status: CANONICA_FAQ_STATUS.DRAFT,
    source: CANONICA_FAQ_SOURCE.MANUAL,
    active: true,
    sortOrder: 100,
    tags: [],
    contextKeys: [],
...
const STATUS_COLORS: Record<string, string> = {
    [CANONICA_FAQ_STATUS.DRAFT]: 'default',
    [CANONICA_FAQ_STATUS.NEEDS_REVIEW]: 'orange',
    [CANONICA_FAQ_STATUS.PUBLISHED]: 'green',
    [CANONICA_FAQ_STATUS.ARCHIVED]: 'red',
};
...
const sortFaqList = (items: CanonicaFaq[]) => [...items].sort((left, right) => (
...
export default function CanonicaFaqManagement() {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            const wasPublished = selectedFaq?.status === CANONICA_FAQ_STATUS.PUBLISHED && selectedFaq.active !== false;
            const willBePublished = values.status === CANONICA_FAQ_STATUS.PUBLISHED;
            const saved = await saveFaq({
                ...selectedFaq,
                ...values,
                id: selectedFaq?.id,
                active: values.status !== CANONICA_FAQ_STATUS.ARCHIVED,
                articleTitle: linkedArticle?.title || values.articleTitle || null,
            });
            const nextFaq = { ...(selectedFaq || {}), ...saved } as CanonicaFaq;
            ...
            if (wasPublished || willBePublished) {
                try {
                    ...
                    console.error('[Canonica FAQ] Product surface summary refresh failed after FAQ save', summaryError);
                }
            }
    ...
    const handleArchive = useCallback(async () => {
        ...
        try {
            ...
            const nextFaq = { ...selectedFaq, status: CANONICA_FAQ_STATUS.ARCHIVED, active: false } as CanonicaFaq;
            ...
            if (wasPublished) {
                try {
                    ...
                    summaryRefreshFailed = true;
                    console.error('[Canonica FAQ] Product surface summary refresh failed after FAQ archive', summaryError);
                }
            }
            if (summaryRefreshFailed) {
                                                ...
                                                <Row gutter={12}>
                                                    <Col xs={24} md={8}>
                                                        <Form.Item name="status" label="Status">
                                                            <Select
                                                                options={Object.values(CANONICA_FAQ_STATUS).map(status => ({
                                                    ...
                                                    <Col xs={24} md={8}>
                                                        <Form.Item name="source" label="Source">
                                                            <Select
                                                                options={Object.values(CANONICA_FAQ_SOURCE).map(source => ({
                                        ...
                                        children: (
                                            <Card>
                                                ...
                                                <Paragraph type="secondary" style={{ marginTop: 16 }}>
                                                    When a linked article changes, Canonica marks related FAQs as needs review so owners can confirm the short answer is still correct.
                                                </Paragraph>
                                            </Card>
                                        ),
                                    },
