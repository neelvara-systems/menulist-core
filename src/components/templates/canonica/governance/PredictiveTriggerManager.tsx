...
/**
 * Canonica — Predictive Trigger Manager (Expansion Item #12)
 * 
...
import { usePredictiveTriggers } from '@hook/canonica/usePredictiveTriggers';
import {
    CANONICA_PREDICTIVE_CONSTRAINTS,
    CANONICA_TRIGGER_ACTION_TYPES,
    CANONICA_TRIGGER_SOURCE,
...
export default function PredictiveTriggerManager({ tId, sId }: PredictiveTriggerManagerProps) {
    ...
    const handleCreate = useCallback(async () => {
        try {
            ...
            await create({
                ...
                status: 'active',
                source: CANONICA_TRIGGER_SOURCE.MANUAL,
            });
    ...
    const openEdit = useCallback((trigger: CanonicaPredictiveTrigger) => {
    ...
    const columns = [
        {
            ...
            dataIndex: 'name',
            key: 'name',
            render: (name: string, record: CanonicaPredictiveTrigger) => (
        ...
        },
        {
            ...
            onFilter: (value: any, record: CanonicaPredictiveTrigger) => record.status === value,
        },
        {
            title: 'Source',
            dataIndex: 'source',
        ...
        },
        {
            ...
            key: 'priority',
            width: 80,
            sorter: (a: CanonicaPredictiveTrigger, b: CanonicaPredictiveTrigger) => a.priority - b.priority,
        },
        {
            title: 'Cooldown',
        ...
        },
        {
            ...
            key: 'effectiveness',
            width: 120,
            render: (_: any, record: CanonicaPredictiveTrigger) => {
    ...
    const TriggerForm = ({ form }: { form: any }) => (
        <>
            <Form.Item name="name" label="Name" rules={[{ required: true, max: CANONICA_PREDICTIVE_CONSTRAINTS.MAX_NAME_LENGTH }]}>
                <Input placeholder="e.g., Webhook Setup Help" />
            </Form.Item>
            <Form.Item name="description" label="Description">
                <Input.TextArea placeholder="Optional description" maxLength={CANONICA_PREDICTIVE_CONSTRAINTS.MAX_DESCRIPTION_LENGTH} rows={2} />
            </Form.Item>
            ...
            <Flex gap={12}>
                <Form.Item name="entityId" label="Entity ID" style={{ flex: 1 }}>
                    <Input placeholder="Canonica entity ID" />
                </Form.Item>
            ...
            <Form.Item name="customSummary" label="Custom Summary">
                <Input.TextArea placeholder="Override answer summary (optional)" maxLength={CANONICA_PREDICTIVE_CONSTRAINTS.MAX_CUSTOM_SUMMARY_LENGTH} rows={2} />
            </Form.Item>
            ...
            <Flex gap={12}>
                <Form.Item name="priority" label="Priority (0-100)" initialValue={50} style={{ flex: 1 }}>
                    <InputNumber min={CANONICA_PREDICTIVE_CONSTRAINTS.MIN_PRIORITY} max={CANONICA_PREDICTIVE_CONSTRAINTS.MAX_PRIORITY} style={{ width: '100%' }} />
                </Form.Item>
                <Form.Item name="cooldownHours" label="Cooldown (hours)" initialValue={24} style={{ flex: 1 }}>
                    <InputNumber min={CANONICA_PREDICTIVE_CONSTRAINTS.MIN_COOLDOWN_HOURS} max={CANONICA_PREDICTIVE_CONSTRAINTS.MAX_COOLDOWN_HOURS} style={{ width: '100%' }} />
                </Form.Item>
            </Flex>
        </>
    );
