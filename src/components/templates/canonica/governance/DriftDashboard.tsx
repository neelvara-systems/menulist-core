 ...
 * 
 * @see __docs__/canonica/doctrine/01-core-doctrine.md (Pillar 3)
 */
...
import { updateCanonicalAnswer, updateAnswerGovernance } from '@database/canonica/canonicalAnswers';
import { addAuditLog } from '@database/canonica/auditLogs';
import { evaluateDriftForTenant } from '@lib/canonica/driftDetection';
import {
...
export default function DriftDashboard() {
    ...
    const [selectedDrifted, setSelectedDrifted] = useState<CanonicaCanonicalAnswer | null>(null);
    ...
    const driftBreakdown = useMemo((): DriftClassBreakdown[] => {
        ...
        driftedAnswers.forEach(answer => {
            ...
            Object.keys(CANONICA_DRIFT_CLASS).forEach(key => {
                const classValue = (CANONICA_DRIFT_CLASS as Record<string, string>)[key];
    ...
    const handleResolve = useCallback(async (answer: CanonicaCanonicalAnswer) => {
        try {
        ...
        <>
            <Card>
                ...
                <List
                    ...
                    renderItem={(answer: CanonicaCanonicalAnswer) => (
                        <List.Item
                            actions={[
                                <Button
                                    key="resolve"
