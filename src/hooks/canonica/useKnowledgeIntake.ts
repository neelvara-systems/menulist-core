...
import type {
    ...
    CanonicaKnowledgeIntakeJob,
    CanonicaKnowledgeSource,
} from '@type/canonica';
...
const apiJson = async <T>(url: string, init?: RequestInit): Promise<T> => {
    ...
    if (!response.ok) {
        throw new Error(data.error || 'Canonica request failed.');
    }
    return data as T;
};

export function useKnowledgeIntake() {
    const enabled = FEATURE_FLAGS.ENABLE_CANONICA_KNOWLEDGE_INTAKE === true;
    const [jobs, setJobs] = useState<CanonicaKnowledgeIntakeJob[]>([]);
    ...
    const refreshJobs = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await apiJson<{ jobs: CanonicaKnowledgeIntakeJob[] }>('/api/canonica/knowledge-intake/jobs');
            setJobs(data.jobs || []);
    ...
    const refreshBundle = useCallback(async (jobId?: string | null) => {
        ...
        setError(null);
        try {
            const data = await apiJson<{ bundle: IntakeBundle }>(`/api/canonica/knowledge-intake/jobs/${encodeURIComponent(id)}`);
    ...
    const createJob = useCallback(async (input: Record<string, any>) => {
        setSaving(true);
        try {
            const data = await apiJson<{ job: CanonicaKnowledgeIntakeJob }>('/api/canonica/knowledge-intake/jobs', {
                method: 'POST',
    ...
    const addSource = useCallback(async (jobId: string, input: Record<string, any>) => {
        setSaving(true);
        try {
            const data = await apiJson<{ source: CanonicaKnowledgeSource }>(`/api/canonica/knowledge-intake/jobs/${encodeURIComponent(jobId)}/sources`, {
                method: 'POST',
                body: JSON.stringify(input),
            });
    ...
    const addMediaSource = useCallback(async (jobId: string, file: File, input: Record<string, any> = {}) => {
        ...
        try {
            ...
            const response = await fetch(`/api/canonica/knowledge-intake/jobs/${encodeURIComponent(jobId)}/media`, {
                method: 'POST',
                body: formData,
                cache: 'no-store',
            });
            ...
            message.success(units > 0
                ...
                : 'Media extracted');
            return data as { source: CanonicaKnowledgeSource; usage?: Record<string, any> };
        } catch (err) {
    ...
    const discoverLinks = useCallback(async (url: string) => {
        const data = await apiJson<{ links: Array<{ url: string; title: string; role: string; reason: string }> }>('/api/canonica/knowledge-intake/discover', {
            method: 'POST',
    ...
    const analyzeJob = useCallback(async (jobId: string) => {
        setSaving(true);
        try {
            const data = await apiJson<{ result: { created: number } }>(`/api/canonica/knowledge-intake/jobs/${encodeURIComponent(jobId)}/analyze`, {
                method: 'POST',
                body: JSON.stringify({}),
            });
            await refreshBundle(jobId);
    ...
    const updateReviewItem = useCallback(async (jobId: string, itemId: string, patch: Record<string, any>) => {
        setSaving(true);
        try {
            await apiJson(`/api/canonica/knowledge-intake/jobs/${encodeURIComponent(jobId)}/review-items/${encodeURIComponent(itemId)}`, {
                method: 'PATCH',
    ...
    const publishJob = useCallback(async (jobId: string, itemIds?: string[]) => {
        setSaving(true);
        try {
            const data = await apiJson<{ result: { published: Array<{ itemId: string; target: string; id: string }> } }>(`/api/canonica/knowledge-intake/jobs/${encodeURIComponent(jobId)}/publish`, {
                method: 'POST',
