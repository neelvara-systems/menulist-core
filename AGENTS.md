...
- **MyCodex remains static/no DB and no active domain dependency**: Do not add MyCodex Firestore, Storage, Cloud Functions, billing plans, owner notifications, product `pId` writes, Firebase env keys, or a `menulist.digital` dependency. MyCodex Vercel env is limited to `MYCODEX_BASIC_AUTH_USER`, `MYCODEX_BASIC_AUTH_PASSWORD`, and `MYCODEX_SESSION_SECRET` unless the static-reader architecture is explicitly changed first.
