...
- **Current product matrix is fixed unless deployment sources change**: MenuList uses `ML` with Firebase `menulist-qa` staging/local and `menulist-prod` production; Answerlattice uses `AL` with Firebase `neelvara-answerlattice-qa` and `neelvara-answerlattice-prod`; CampaignCue uses `CC` with `campaigncue-qa` and `campaigncue`; SignalDesk uses `SD` with Firebase `menulist-signaldesk-qa` staging/local and `menulist-signaldesk` production, but its env keys use `SIGNALDESK_*`; MyCodex uses reserved code `MC`, slug `mycodex`, no active public domain, and no Firebase project.
