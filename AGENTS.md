...
- **Promotion policy**: when a push is explicitly authorized, ordinary pushes target `staging` only and must follow the consolidated Vercel build-cost control below. Never infer staging-push authorization from this default. Do not move or push `main` unless Danny explicitly requests it in the current instruction. An explicit main promotion must preserve all remote history without force and finish with local/remote `main` and `staging` on the same verified SHA.
...
- **Vercel build-cost control is a release gate**: because every push to the Vercel-connected `staging` branch can trigger a billable remote build, do not commit-and-push incremental fixes, routine QA checkpoints, or per-worktree batches to `staging`. Keep implementation, fix/retest loops, and local/emulator certification local until the complete coordinated release candidate is ready.
- **Explicit staging-push permission is mandatory**: never push `staging` merely because ordinary pushes traditionally target `staging`, a prior turn authorized a push, or a workflow has reached a deployment checkpoint. Ask for and receive Danny's explicit current-turn authorization for the one consolidated staging push immediately before performing it.
- **Coordinate all worktrees before the single push**: before requesting staging-push permission, inventory every registered worktree and all local changes, preserve and attribute concurrent work, reconcile the intended release candidate without loss, run the required local gates once against that coordinated snapshot, and present the exact commit/deployment scope. Do not trigger separate Vercel builds for individual worktrees or small fixes.
- After an authorized consolidated staging push, wait for that one automatic Vercel QA deployment and complete hosted verification against it. If further defects are found, return to local fix/retest and accumulate them for a later explicitly authorized consolidated push; do not immediately push each correction.
