...
- **Before/after evidence**: every ledger entry must contain an ISO-8601 timestamp with timezone, operation ID, actor/session ID, worktree path/ID, source and destination refs, before/after full SHAs, command class, validation run, dirty/untracked state, remote readback, and whether the operation was performed here or merely observed from reflog/provider evidence.
- **Infrastructure delta is mandatory**: before saying Firebase deployment is current or unnecessary, inspect every changed path since the last verified infrastructure release, including merge parents and post-release commits. Classify each product/target as exactly one of: `NO_INFRA_CHANGE`, `DEPLOY_REQUIRED`, `SOURCE_RESTORED_TO_DEPLOYED_BYTES`, or `DEPLOYED_AND_READ_BACK`. The restored-to-deployed classification requires authenticated deployed-source/hash evidence; a matching filename or commit message is insufficient.
...
- For a non-destructive MenuList Firebase change intended for the shared product runtime, promote the same locally validated source to `menulist-qa` first and then `menulist-prod` in the same session, and verify both targets before closing the task. Do not defer production parity to a later cleanup pass. Stop and report the exact blocker when QA fails, production has an explicit hold, a real required production secret is missing, or the change needs migration/backfill/destructive approval; never create placeholder credentials to force parity.
