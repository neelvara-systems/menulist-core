...
function deliveryMatchesProductAndProvider(
    ...
    if (!delivery.exists || delivery.get('productCode') !== 'AL') return false;
...
export async function handleAnswerlatticeEmailOsWebhook(request: RawBodyRequest, response: Response): Promise<void> {
    ...
    if (event.productCode !== null && event.productCode !== 'AL') {
        logger.info('[EmailOS] Answerlattice provider event ignored', {
    ...
    await firestoreAdmin.runTransaction(async (transaction) => {
        ...
        if (!isEmailOsProviderEventBoundToProduct(event, 'AL', Boolean(delivery))) {
            ignored = true;
            return;
        }
