...
const EMBED_ARTICLE_WORKER_SCOPE_MISSING_CODE = 'ANSWERLATTICE_EMBED_ARTICLE_WORKER_SCOPE_MISSING';
...
export const embedArticleWorkerLogic = async (articleData: EmbedArticleType, jobId: string) => {
    ...
    try {
        ...
        if (!Number.isFinite(tenantId) || !Number.isFinite(storeId)) {
            logger.error('[embedArticleWorkerLogic] Article is missing tenant/store scope.', {
                failureCode: EMBED_ARTICLE_WORKER_SCOPE_MISSING_CODE,
                ...getEmbedArticleWorkerContext(articleData, jobId),
            });
            await jobRef.update({ articlesEmbeddedCount: FieldValue.increment(1) });
            return;
