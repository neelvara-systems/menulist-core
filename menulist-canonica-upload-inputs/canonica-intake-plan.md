# Canonica Intake Plan For MenuList

## Goal

Prepare MenuList as the production client and example product Canonica can understand deeply:

- what MenuList does;
- which surfaces customers and owners use;
- which knowledge is authoritative;
- what questions Canonica should answer;
- what topics require owner/admin review;
- which product screenshots and generated assets can support Canonica website and marketing work;
...
- which live MenuList owner support questions Canonica must answer safely.

## Canonica Intake Shape

Use one Canonica job:

```text
MenuList production client onboarding and asset proof
```

Recommended Canonica job metadata:

- `title`: `MenuList production client onboarding and asset proof`
- `description`: `Canonica intake package for MenuList production-client onboarding, product knowledge, support examples, public-surface proof, dashboard demo state, and website asset planning.`
- `productWebsiteUrl`: `https://www.menulist.online`
- `appUrl`: `https://menulist.ai`
- `targetAudience`: `SaaS founders, product owners, and support operators evaluating Canonica through the MenuList example.`
...
| Group | File | Canonica source type | Primary tags |
| --- | --- | --- | --- |
...
| Canonica example | `source-inputs/06-canonica-example-story.md` | `product_note` | `canonica_example`, `asset_story`, `support_story` |
...
| File | Use |
| --- | --- |
| `menulist-client-profile.json` | Canonica client profile and launch constraints for MenuList. |
| `onboarding-runbook.md` | Approval, production workspace, intake, widget, signal loop, and screenshot sequence. |
| `dashboard-demo-data-requirements.md` | Minimum meaningful Canonica dashboard state before website captures. |
| `product-surface-map.csv` | Product Surface records to create in Canonica. |
...
| `live-owner-support-test-questions.csv` | Thirty pre-live owner-style questions for Canonica answer review. |

## Review Strategy

Ask Canonica to create review items in this order:
...
8. Website-vs-doc parity: keep public website claims, help docs, and Canonica answers aligned.

## Publish Targets

Suitable Canonica destinations:

- KB article drafts for internal/external explanation.
- FAQ drafts for common buyer and support questions.
- Product-surface suggestions for Canonica website proof sections.
- Canonical answer mutation proposals for stable product facts.
- Activation/dashboard readiness state for MenuList as a production Canonica client.
...
3. Current Canonica intake and doctrine docs.
