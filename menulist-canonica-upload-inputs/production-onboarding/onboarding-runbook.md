# MenuList On Canonica Production Onboarding Runbook

## Operating Principle

MenuList should be onboarded into Canonica as a real production client, not as fake seed data. The resulting dashboard state can then be used for Canonica website and marketing assets only after approval and privacy review.

## Phase 0: Approval And Scope Lock

- Confirm MenuList founder approval to onboard MenuList as a Canonica production client.
- Confirm the legal basis for using MenuList product data inside Canonica.
- Confirm whether Canonica marketing can publicly show MenuList name, screenshots, and dashboard state.
...
## Phase 1: Canonica Production Workspace

- Create or select the Canonica production workspace for MenuList.
- Confirm production Canonica environment points to `https://canonica.app` and the `canonica` Firebase project.
- Confirm MenuList production remains `https://menulist.ai` and the `menulist` Firebase project.
- Confirm whether the public website content source remains `https://www.menulist.online` or has moved to the canonical production host before final Canonica source review.
- Activate license/subscription for the MenuList Canonica workspace.
- Add required admin users only.
- Generate a Canonica widget key for MenuList production.
...
- Add selected public MenuList website URLs through Canonica discovery after the prepared source files are imported.
- Keep each source under Canonica's runtime source-size constraints.
...
- Create Canonica Product Surfaces from `product-surface-map.csv`.
- Map owner routes first because the MenuList Canonica widget embed is mounted on owner routes.
...
- `/canonica/product-surfaces` shows MenuList route coverage.
- Product surfaces link to related KB, FAQ, and canonical answers.

## Phase 5: MenuList Production Widget Connection

- Set `NEXT_PUBLIC_MENULIST_CANONICA_WIDGET_KEY` in the MenuList production environment to the Canonica-issued `cn_` key.
- Do not hardcode the key in source.
- Use `NEXT_PUBLIC_MENULIST_CANONICA_WIDGET_SCRIPT_SRC` only for an approved temporary override.
- Verify the default production script host resolves to `https://canonica.app/widget/v1/canonica-widget.js`.
...
- `/canonica/widget` shows a production runtime last-seen event from MenuList.
- `/canonica/activation` marks widget seen in product and page context received.
...
- Confirm the signal reaches Canonica feedback/support surfaces.
- Create or verify support board cards for missing or weak knowledge areas.

Exit criteria:

- Support signal loop tested.
- `/canonica/support-board`, `/canonica/feedback`, and governance queues show meaningful MenuList-derived activity.

## Phase 7: Dashboard And Website Asset Capture

- Confirm dashboard demo requirements in `dashboard-demo-data-requirements.md`.
- Capture the Canonica dashboard only after activation, knowledge, surfaces, widget runtime, and support signals are present.
...
- Run a final privacy scrub before using screenshots in Canonica website or marketing assets.
