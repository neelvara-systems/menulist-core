...
export async function syncKnowledgeIntakeSummary(tId: number, sId: number): Promise<IntakeSummaryResult> {
    ...
    const jobsSnap = await db.collection(DB_COLLECTIONS.CANONICA_KNOWLEDGE_INTAKE_JOBS)
        .where('tId', '==', tId)
        .where('sId', '==', sId)
    ...
    await summaryRef.set({
        ...
        summarySource: 'canonica_nightly',
    }, { merge: true });

    result.summaryWritten = true;
    logger.info('[Canonica Intake] Summary synced', {
        tId,
        sId,
