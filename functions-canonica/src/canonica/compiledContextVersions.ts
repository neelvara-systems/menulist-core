...
export type CanonicaContextSourceKey = typeof CANONICA_CONTEXT_SOURCE_KEYS[number];
...
export const compiledSourceVersionsEqual = (
    ...
    return CANONICA_CONTEXT_SOURCE_KEYS.every(key => Number(a[key] || 0) === Number(b[key] || 0));
};
...
const assertScope = (tId: number, sId: number) => {
    ...
    if (!Number.isFinite(tenantId) || tenantId <= 0 || !Number.isFinite(storeId) || storeId <= 0) {
        throw new Error('Cannot update Canonica compiled context versions without valid tenant and store scope.');
    }
    return { tenantId, storeId };
};

const assertSource = (source: CanonicaContextSourceKey) => {
    if (!CANONICA_CONTEXT_SOURCE_KEYS.includes(source)) {
        throw new Error(`Unsupported Canonica compiled context source: ${source}`);
    }
};
