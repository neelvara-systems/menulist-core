...
const BUNDLE_ROOT = 'canonica-context';
...
const getPublicBundleId = (existing: any, tId: number, sId: number): string => {
    ...
    const salt = process.env.CANONICA_PUBLIC_BUNDLE_SALT || process.env.CANONICA_MCP_SESSION_SECRET;
...
const loadChangelogEntries = async (tId: number, sId: number) => {
    try {
        ...
        logger.warn('[Canonica Context Bundle] Changelog load skipped', {
            tId,
            sId,
...
const loadSourceData = async (tId: number, sId: number) => {
    const [
        ...
        loadDocs(db.collection(DB_COLLECTIONS.CANONICA_ENTITIES).where('tId', '==', tId).where('sId', '==', sId).limit(MAX_ENTITIES)),
        loadDocs(db.collection(DB_COLLECTIONS.CANONICA_ENTITY_RELATIONS).where('tId', '==', tId).where('sId', '==', sId).limit(MAX_RELATIONS)),
        loadDocs(db.collection(DB_COLLECTIONS.CANONICA_CANONICAL_ANSWERS).where('tId', '==', tId).where('sId', '==', sId).where('status', '==', 'active').limit(MAX_CANONICAL)),
        loadDocs(db.collection(DB_COLLECTIONS.CANONICA_PRODUCT_SURFACES).where('tId', '==', tId).where('sId', '==', sId).limit(MAX_SURFACES)),
        loadDocs(db.collection(DB_COLLECTIONS.KB_ARTICLES).where('tId', '==', tId).where('sId', '==', sId).where('status', '==', 'published').limit(MAX_ARTICLES)),
        loadDocs(db.collection(DB_COLLECTIONS.CANONICA_FAQS).where('tId', '==', tId).where('sId', '==', sId).where('status', '==', 'published').where('active', '==', true).limit(MAX_FAQS)),
        loadDocs(db.collection(DB_COLLECTIONS.CANONICA_RELEASES).where('tId', '==', tId).where('sId', '==', sId).limit(MAX_RELEASES)),
...
const buildObjects = (params: {
    ...
    const publicObjects: Record<string, any> = {
        'manifest.json': { schemaVersion: SCHEMA_VERSION, pId: 'CN', publicBundleId, bundleVersion, generatedAt, sourceVersions },
    ...
    const privateObjects: Record<string, any> = {
        'manifest.json': { schemaVersion: SCHEMA_VERSION, pId: 'CN', tId, sId, publicBundleId, bundleVersion, generatedAt, sourceVersions },
...
export const repairCompiledContextBundle = async (tId: number, sId: number): Promise<ContextBundleRepairResult> => {
    ...
    await lockRef.set({
        ...
        requestedBy: 'system:canonica_nightly',
