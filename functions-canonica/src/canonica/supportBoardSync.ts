...
function buildSystemStatusEntry(status: string, now: Timestamp, remark: string) {
    return {
        ...
        createdBy: {
            ...
            name: 'Canonica nightly',
            email: 'system@canonica.internal',
        },
        remark,
    };
}
...
async function upsertSupportBoardCards(tId: number, sId: number, candidates: SupportBoardCandidate[]): Promise<{
    ...
    const collection = db.collection(DB_COLLECTIONS.CANONICA_SUPPORT_BOARD_CARDS);
...
async function writeSupportBoardSummary(tId: number, sId: number, syncStats: {
    ...
    const cardsSnap = await db
        .collection(DB_COLLECTIONS.CANONICA_SUPPORT_BOARD_CARDS)
        .where('tId', '==', tId)
        .where('sId', '==', sId)
...
export async function syncSupportBoardNightly(tId: number, sId: number): Promise<SupportBoardSyncResult> {
    const result: SupportBoardSyncResult = {
        enabled: FUNCTION_FLAGS.ENABLE_CANONICA_SUPPORT_BOARD_SYNC,
    ...
    if (!FUNCTION_FLAGS.ENABLE_CANONICA_SUPPORT_BOARD_SYNC) return result;
    ...
    try {
        ...
        if (result.cardsCreated > 0 || result.cardsUpdated > 0 || result.summaryWritten) {
            logger.info('[Canonica SupportBoard] Nightly sync complete', {
                tId,
                sId,
                cardsCreated: result.cardsCreated,
                cardsUpdated: result.cardsUpdated,
        ...
        logger.error('[Canonica SupportBoard] Nightly sync failed', {
            tId,
            sId,
            error: message,
        });
